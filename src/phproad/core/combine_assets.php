...
require_once PATH_APP . '/config/config.php'; // we need to load the config when it has access to PATH_APP for $allowed_paths
...
function phpr_get_resource_type()
{
    ...
    if (preg_match(
        '#ls_(.+)_combine#simU',
        htmlentities($_GET[ $request_param_name ], ENT_COMPAT, 'UTF-8'),
        $match
    ) 
...
$allowed_types  = isset($CONFIG['ALLOWED_RESOURCE_EXTENSIONS']) ? $CONFIG['ALLOWED_RESOURCE_EXTENSIONS'] : array(
    'js',
    'css',
);
...
if ($type == 'css' ) {
    include PATH_APP . '/phproad/thirdpart/csscompressor/UriRewriter.php';
}
