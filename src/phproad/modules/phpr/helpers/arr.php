...
class Arr
{
     ...
     * $result = Phpr\Array::getKeyValue('a', array('a' => 1, 'b' => 2));
     * The result will be 1, the value of the 'a' key.
     */
    public static function getKeyValue($key, $list)
    {
        return $list[$key];
    }
     ...
     * Phpr\Array::filterByKeys(Phpr\Array::getKeyValue('fields', $model->serialize()), array('id', 'name', 'url_name'));
     */
    public static function filterByKeys($list, $keys = array())
    {
     ...
     * @example 1
     * You can sanitized an array like this:
     * $result = Phpr\Array::sanitizeValueTypes(array('1', '1.0'));
     * The result will be an array of 1 and 1.0, an int and a float.
     */
    public static function sanitizeValueTypes($list)
    {
        $good_list = array();

        foreach ($list as $key => $item) {
            if (is_array($item)) {
                $new_item = self::sanitizeValueTypes($item);
            } else {
                if (is_object($item)) { // unhandled
                    $new_item = $item;
                } else {
                    if ((string)intval($item) === (string)$item) { // we found an int
    ...
    public static function injectAtKey(&$array, $key, $value, $position = false)
    {
