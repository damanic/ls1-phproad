...
* #@+
    ...
    function __construct()
    {
        ...
        if (!defined('CRYPT_RSA_MODE')) {
            ...
            case function_exists('phpinfo') && extension_loaded('openssl') && file_exists($this->configFile):
                ...
                case version_compare($versions['Header'], '1.0.0') >= 0 && version_compare(
                    $versions['Library'],
                    '1.0.0'
                ) >= 0:
    ...
    function createKey($bits = 1024, $timeout = false, $partial = array())
    {
        ...
        if (CRYPT_RSA_MODE == self::MODE_OPENSSL && $bits >= 384 && CRYPT_RSA_EXPONENT == 65537) {
            ...
            $privatekey = call_user_func_array(
                array($this, '_convertPrivateKey'),
                array_values($this->_parseKey($privatekey, self::PRIVATE_FORMAT_PKCS1))
            );
            $publickey = call_user_func_array(
                array($this, '_convertPublicKey'),
    ...
    function _convertPrivateKey($n, $e, $d, $primes, $exponents, $coefficients)
    {
        ...
        case self::PRIVATE_FORMAT_OPENSSH:
            ...
            $publicKey = pack(
                'Na*Na*Na*', strlen('ssh-rsa'), 'ssh-rsa', strlen($raw['publicExponent']),
                $raw['publicExponent'], strlen($raw['modulus']), $raw['modulus']
            );
            $privateKey = pack(
        ...
        default: // eg. self::PRIVATE_FORMAT_PKCS1
            ...
            foreach ($raw as $name => $value) {
                $components[$name] = pack(
                    'Ca*a*', self::ASN1_INTEGER, $this->_encodeLength(strlen($value)),
                    $value
                );
            }
            ...
            if ($num_primes > 2) {
                ...
                for ($i = 3; $i <= $num_primes; $i++) {
                    ...
                    // }
                    $OtherPrimeInfo = pack(
                        'Ca*a*', self::ASN1_INTEGER,
                        $this->_encodeLength(strlen($primes[$i]->toBytes(true))), $primes[$i]->toBytes(true)
                    );
                    $OtherPrimeInfo .= pack(
                        'Ca*a*', self::ASN1_INTEGER,
                        $this->_encodeLength(strlen($exponents[$i]->toBytes(true))), $exponents[$i]->toBytes(true)
                    );
                    $OtherPrimeInfo .= pack(
                        'Ca*a*', self::ASN1_INTEGER,
                    ...
                    $OtherPrimeInfos .= pack(
                        'Ca*a*', self::ASN1_SEQUENCE,
                        $this->_encodeLength(strlen($OtherPrimeInfo)), $OtherPrimeInfo
                    );
                }
                $RSAPrivateKey .= pack(
                    'Ca*a*', self::ASN1_SEQUENCE, $this->_encodeLength(strlen($OtherPrimeInfos)),
                    $OtherPrimeInfos
                );
            }

            $RSAPrivateKey = pack(
                'Ca*a*', self::ASN1_SEQUENCE, $this->_encodeLength(strlen($RSAPrivateKey)),
                $RSAPrivateKey
            );
            ...
            if ($this->privateKeyFormat == self::PRIVATE_FORMAT_PKCS8) {
                ...
                if (!empty($this->password) || is_string($this->password)) {
                    ...
                    $RSAPrivateKey = pack(
                        'Ca*a*', self::ASN1_SEQUENCE,
                        $this->_encodeLength(strlen($RSAPrivateKey)), $RSAPrivateKey
                    );
    ...
    function _convertPublicKey($n, $e)
    {
        ...
        case self::PUBLIC_FORMAT_OPENSSH:
            ...
            // mpint     n
            $RSAPublicKey = pack(
                'Na*Na*Na*', strlen('ssh-rsa'), 'ssh-rsa', strlen($publicExponent),
        ...
        default: // eg. self::PUBLIC_FORMAT_PKCS1_RAW or self::PUBLIC_FORMAT_PKCS1
            ...
            $components = array(
                ...
                'publicExponent' => pack(
                    'Ca*a*', self::ASN1_INTEGER, $this->_encodeLength(strlen($publicExponent)),
                    $publicExponent
                )
            );
    ...
    function _parseKey($key, $type)
    {
        ...
        case self::PUBLIC_FORMAT_PKCS1:
            ...
            if (strlen($temp) != 1 || ord($temp) > 2) {
                ...
                $length = $this->_decodeLength($key);
                $components[$type == self::PUBLIC_FORMAT_PKCS1 ? 'publicExponent' : 'privateExponent'] = new BigInteger(
                    $this->_string_shift(
                        $key,
                        $length
                    ), 256
        ...
        case self::PRIVATE_FORMAT_PUTTY:
            ...
            $private = base64_decode(
                implode(
                    '',
                    array_map('trim', array_slice($key, $publicLength + 5, $privateLength))
                )
            );
    ...
    function getPublicKeyFingerprint($algorithm = 'md5')
    {
        ...
        $RSAPublicKey = pack(
            'Na*Na*Na*', strlen('ssh-rsa'), 'ssh-rsa', strlen($publicExponent), $publicExponent,
    ...
    function getPrivateKey($type = self::PUBLIC_FORMAT_PKCS1)
    {
        ...
        $temp = $this->_convertPrivateKey(
            $this->modulus, $this->publicExponent, $this->exponent, $this->primes,
    ...
    function decrypt($ciphertext)
    {
        ...
        $ciphertext[count($ciphertext) - 1] = str_pad(
            $ciphertext[count($ciphertext) - 1], $this->k, chr(0),
            STR_PAD_LEFT
        );
