    ...
    function _decode_ber($encoded, $start = 0, $encoded_pos = 0)
    {
        ...
        case self::TYPE_BIT_STRING:
            ...
            if (!$constructed) {
                ...
                $current['content'] = $temp[$last]['content'][0] . $current['content'] . substr(
                    $temp[$i]['content'],
                    1
                );
            }
                ...
                if ($maymatch) {
                    ...
                    if ($child['type'] != self::TYPE_CHOICE) {
                        ...
                        if (isset($constant) && isset($temp['constant'])) {
                            ...
                            $maymatch = !isset($child['constant']) && array_search(
                                $child['type'],
                                array($temp['type'], self::TYPE_ANY, self::TYPE_CHOICE)
                            ) !== false;
                        }
                    }
                }
            ...
            if (isset($mapping['mapping'])) {
                ...
                /*
                   ...
                   "When a "NamedBitList" is used in defining a bitstring type ASN.1 encoding rules are free to add (or remove)
                    ...
                    0 bits."
                */
                $bits = count($mapping['mapping']) == $size ? array() : array_fill(
                    0,
                    count($mapping['mapping']) - $size, false
                );
        ...
        case self::TYPE_ANY:
            ...
            case $source instanceof BigInteger:
                return $this->_encode_der(
                    $source, array('type' => self::TYPE_INTEGER) + $mapping, null,
                    $special
                );
            ...
            case is_bool($source):
                return $this->_encode_der(
                    $source, array('type' => self::TYPE_BOOLEAN) + $mapping, null,
                    $special
                );
            case is_array($source) && count($source) == 1:
                ...
                if ($outtype !== false) {
                    return $this->_encode_der(
                        $source[$typename], array('type' => $outtype) + $mapping, null,
                        $special
                    );
                }
            }
