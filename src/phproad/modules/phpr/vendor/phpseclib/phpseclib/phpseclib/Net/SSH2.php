...
* #@+
    ...
    function _key_exchange($kexinit_payload_server = false)
    {
        ...
        $server_host_key_algorithm = $this->_array_intersect_first(
        ...
        $compression_algorithm_out = $this->_array_intersect_first(
        ...
        $compression_algorithm_in = $this->_array_intersect_first(
    ...
    function read($expect = '', $mode = self::READ_SIMPLE)
    {
        ...
        while (true) {
            ...
            if (is_bool($response)) {
                ...
                return $response ? $this->_string_shift(
    ...
    function _get_binary_packet($skip_channel_filter = false)
    {
        ...
        if ($this->hmac_check !== false) {
            ...
            if ($hmac === false || strlen($hmac) != $this->hmac_size) {
                ...
                return false;
            } elseif ($hmac != $this->hmac_check->hash(
                pack(
                    'NNCa*', $this->get_seq_no, $packet_length, $padding_length,
                    $payload . $padding
                )
            )
            ) {
        ...
        if (defined('NET_SSH2_LOGGING')) {
            ...
            $message_number = '<- ' . $message_number .
                ' (since last: ' . round($current - $this->last_packet, 4) . ', network: ' . round(
                    $stop - $start,
                    4
                ) . 's)';
            ...
            case NET_SSH2_MSG_CHANNEL_REQUEST:
                if (strlen($payload) == 31) {
                    ...
                    if (substr(
                        $payload, 9,
                        $length
                    ) == 'keepalive@openssh.com' && isset($this->server_channels[$channel])
                    ) {
                        if (ord(substr($payload, 9 + $length))) { // want reply
                            $this->_send_binary_packet(
                                pack(
                                    'CN', NET_SSH2_MSG_CHANNEL_SUCCESS,
        ...
        while (true) {
            ...
            if (isset($channel) && isset($this->channel_status[$channel]) && isset($this->window_size_server_to_client[$channel])) {
                ...
                if ($this->window_size_server_to_client[$channel] < 0) {
                    ...
                    $packet = pack(
                        'CNN', NET_SSH2_MSG_CHANNEL_WINDOW_ADJUST, $this->server_channels[$channel],
                        $this->window_resize
                    );
                    if (!$this->_send_binary_packet($packet)) {
                        return false;
                ...
                case NET_SSH2_MSG_CHANNEL_REQUEST:
                    ...
                    case 'exit-signal':
                        ...
                        extract(unpack('Nlength', $this->_string_shift($response, 4)));
                        $this->errors[] = 'SSH_MSG_CHANNEL_REQUEST (exit-signal): ' . $this->_string_shift(
                            $response,
                            $length
                        );
                        $this->_string_shift($response, 1);
                        ...
                        $this->_send_binary_packet(
                            pack(
                                'CN', NET_SSH2_MSG_CHANNEL_EOF,
                                $this->server_channels[$client_channel]
                            )
                        );
                        $this->_send_binary_packet(
                            pack(
                                'CN', NET_SSH2_MSG_CHANNEL_CLOSE,
                                $this->server_channels[$channel]
                            )
                        );
                ...
                case NET_SSH2_MSG_CHANNEL_OPEN:
                    ...
                    case NET_SSH2_MSG_CHANNEL_OPEN_CONFIRMATION:
                        ...
                        $result = $client_channel == $channel ? true : $this->_get_channel_packet(
                            $client_channel,
                            $skip_extended
                        );
                ...
                case NET_SSH2_MSG_CHANNEL_REQUEST:
                    ...
                    case NET_SSH2_MSG_CHANNEL_CLOSE:
                    return $type == NET_SSH2_MSG_CHANNEL_CLOSE ? true : $this->_get_channel_packet(
                        $client_channel,
                        $skip_extended
                    );
                }
    ...
    function getServerPublicHostKey()
    {
        ...
        case 'rsa-sha2-512':
            ...
            */
            ...
            case 'rsa-sha2-512':
                $h = pack(
                    'N5a*', 0x00305130, 0x0D060960, 0x86480165, 0x03040203, 0x05000440,
                    $hashObj->hash($this->exchange_hash)
                );
                break;
            case 'rsa-sha2-256':
                $h = pack(
                    'N5a*', 0x00303130, 0x0D060960, 0x86480165, 0x03040201, 0x05000420,
                    $hashObj->hash($this->exchange_hash)
                );
                break;
                //case 'ssh-rsa':
            default:
                $hash = 'sha1';
                $h = pack(
                    'N4a*', 0x00302130, 0x0906052B, 0x0E03021A, 0x05000414,
                    $hashObj->hash($this->exchange_hash)
                );
            }
