...
* #@+
        ...
        if ($this->engine === self::ENGINE_OPENSSL) {
            ...
            case self::MODE_ECB:
                $result = @openssl_encrypt(
                    $plaintext, $this->cipher_name_openssl, $this->key,
            ...
            case self::MODE_CBC:
                $result = openssl_encrypt(
                    $plaintext, $this->cipher_name_openssl, $this->key,
                    $this->openssl_options, $this->encryptIV
                );
            ...
            case self::MODE_CFB:
                ...
                if ($overflow) {
                    $ciphertext .= openssl_encrypt(
                        substr($plaintext, 0, -$overflow) . str_repeat(
                            "\0",
                            $this->block_size
            ...
            case self::MODE_CFB8:
                ...
                if ($this->continuousBuffer) {
                    if (($len = strlen($ciphertext)) >= $this->block_size) {
                        ...
                        $this->encryptIV = substr($this->encryptIV, $len - $this->block_size) . substr(
                            $ciphertext,
                            -$len
                        );
                    }
            ...
            case self::MODE_OFB8:
                ...
                for ($i = 0; $i < $len; ++$i) {
                    $xor = openssl_encrypt(
                        $iv, $this->cipher_name_openssl_ecb, $this->key, $this->openssl_options,
                        $this->decryptIV
                    );
        ...
        if ($this->engine === self::ENGINE_OPENSSL) {
            ...
            case self::MODE_STREAM:
                $plaintext = openssl_decrypt(
                    $ciphertext, $this->cipher_name_openssl, $this->key,
                    $this->openssl_options
                );
                break;
            case self::MODE_ECB:
            ...
            case self::MODE_CBC:
                if (!defined('OPENSSL_RAW_DATA')) {
                    ...
                    $ciphertext .= substr(
                        @openssl_encrypt(
                            $padding, $this->cipher_name_openssl_ecb, $this->key,
                            true
                        ), 0, $this->block_size
                    );
            ...
            case self::MODE_CFB:
                ...
                if ($overflow) {
                    $plaintext .= openssl_decrypt(
                        substr($ciphertext, 0, -$overflow), $this->cipher_name_openssl,
                    ...
                    $iv = openssl_encrypt(
                        str_repeat("\0", $this->block_size), $this->cipher_name_openssl,
                        $this->key, $this->openssl_options, $iv
                    );
            ...
            case self::MODE_CFB8:
                ...
                if ($this->continuousBuffer) {
                    if (($len = strlen($ciphertext)) >= $this->block_size) {
                        ...
                        $this->decryptIV = substr($this->decryptIV, $len - $this->block_size) . substr(
                            $ciphertext,
                            -$len
                        );
                    }
    ...
    function _openssl_ofb_process($plaintext, &$encryptIV, &$buffer)
    {
        ...
        if (strlen($plaintext)) {
            if ($overflow) {
                $ciphertext .= openssl_encrypt(
                    substr($plaintext, 0, -$overflow) . str_repeat("\0", $block_size),
                    $this->cipher_name_openssl, $key, $this->openssl_options, $encryptIV
                );
                ...
                $ciphertext = openssl_encrypt(
                    $plaintext, $this->cipher_name_openssl, $key, $this->openssl_options,
                    $encryptIV
                );
                if ($this->continuousBuffer) {
