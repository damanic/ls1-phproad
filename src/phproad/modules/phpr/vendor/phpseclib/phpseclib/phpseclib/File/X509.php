...
*/
    ...
    function loadX509($cert, $mode = self::FORMAT_AUTO_DETECT)
    {
        ...
        $this->signatureSubject = substr(
            $cert, $decoded[0]['content'][0]['start'],
        ...
        case isset($this->currentCert['tbsCertificate']):
            ...
            if (!empty($this->CAs)) {
                for ($i = 0; $i < count($this->CAs); $i++) {
                    ...
                    case defined('FILE_X509_IGNORE_TYPE') && $this->getDN(
                        self::DN_STRING,
    ...
    function getSubjectDNProp($propName, $withType = false)
    {
        ...
        case isset($this->currentCert['certificationRequestInfo']):
            return $this->getDNProp(
                $propName, $this->currentCert['certificationRequestInfo']['subject'],
                $withType
            );
        }
    ...
    function loadCSR($csr, $mode = self::FORMAT_AUTO_DETECT)
    {
        ...
        $this->signatureSubject = substr(
            $orig, $decoded[0]['content'][0]['start'],
            $decoded[0]['content'][0]['length']
        );
        ...
        if ($this->caFlag) {
            ...
            if (!isset($subject->currentKeyIdentifier)) {
                $this->setExtension(
                    ...
                    base64_encode($this->computeKeyIdentifier($this->currentCert)), false, false
                );
            }
        }
    ...
    function signSPKAC($signatureAlgorithm = 'sha1WithRSAEncryption')
    {
        ...
        if (isset($this->currentCert) && is_array($this->currentCert) && isset($this->currentCert['publicKeyAndChallenge'])) {
            ...
            if (!empty($this->challenge)) {
                ...
                $this->currentCert['publicKeyAndChallenge']['challenge'] = $this->challenge & str_repeat(
                    "\x7F",
    ...
    function computeKeyIdentifier($key = null, $method = 1)
    {
        ...
        case is_array($key) && isset($key['tbsCertificate']['subjectPublicKeyInfo']['subjectPublicKey']):
            return $this->computeKeyIdentifier(
