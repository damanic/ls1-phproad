...
class Phpr_Cron
{
    ...
    private static function execute_cronjobs()
    {
        ...
        //
        $jobs = Db_DbHelper::objectArray('select * from phpr_cron_jobs order by created_at asc limit 5');

        foreach ($jobs as $job) {
            Db_DbHelper::query('delete from phpr_cron_jobs where id=:id limit 1', array('id' => $job->id));
