 ...
 * PHPR Cron class
 *
 * Used for managing scheduled events and housekeeping
...
class Cron
{
    ...
    public static function update_interval($code, $now = null)
    {
        $bind = array(
            'record_code' => $code,
            'now' => $now ? $now : PhprDateTime::now()->toSqlDateTime()
        );
        DbHelper::query(
            'INSERT INTO phpr_cron_table (record_code, updated_at) 
                  VALUES (:record_code, :now) 
    ...
    public static function get_interval($code)
    {
        $interval = DbHelper::scalar(
            'select GREATEST(COALESCE(updated_at, 0),COALESCE(postpone_until, 0)) AS run_check 
                  from phpr_cron_table 
                  where record_code =:record_code',
            array('record_code' => $code)
        );
        if (!$interval) {
            self::update_interval($code);
            $interval = PhprDateTime::now()->toSqlDateTime();
        }
     ...
     * @param PhprDateTime $datetime The datetime the crontab task should be deferred until
     * @return void
     */
    public static function postpone_until($code, $datetime = null)
    {
        $bind = array(
            'record_code' => $code,
            'datetime' => $datetime ? $datetime : PhprDateTime::now()
        );
        DbHelper::query(
            'insert into phpr_cron_table (record_code, postpone_until) 
                  values (:record_code, :datetime) 
                  on duplicate key update postpone_until =:datetime',
            $bind
        );
    }
     ...
     * @param mixed $process_job_que Set to TRUE to process jobs in all ques OR FALSE to skip jobs on this run.  
     *              OR pass a que name (string) to process jobs in a named que.
     * @return void
     */
    ...
    public static function execute_crontabs()
    {
        ...
        foreach ($modules as $module) {
            if (!method_exists($module, 'subscribeCrontab')) {
                continue;
            }
            ...
            if (!is_array($cronItems)) {
                continue;
            }

            foreach ($cronItems as $code => $options) {
                ...
                $last_exec = PhprDateTime::parse(self::get_interval($code), PhprDateTime::universalDateTimeFormat);
                ...
                try {
                    ...
                    Phpr::$events->fire_event('phpr:on_execute_crontab_exception', $ex, $code);
                }
            }
        }
    }
     ...
     * <pre>
     *  <?
     *       Cron::queue_job('User_Model::static_method', array('param1', 'param2', 'param3'));
     *  ?>
     * </pre>
     * Executes:
    ...
    public static function queue_job(
        ...
        if ($available_at) {
            if (is_a($available_at, 'Phpr\DateTime')) {
        ...
        if (!$allow_duplicate) {
            $check_sql = "SELECT id FROM phpr_cron_jobs
				WHERE handler_name = :handler_name 
				AND param_data = :param_data LIMIT 1";
            $exists_id = DbHelper::scalar($check_sql, $bind);
            if ($exists_id) {
                if ($available_at) {
                    ...
                    DbHelper::query(
                        "UPDATE phpr_cron_jobs SET available_at = :available_at WHERE id = :exists_id",
                        $bind
                    );
                }
        ...
        $insert_sql = "INSERT INTO phpr_cron_jobs 
        ...
        DbHelper::query($insert_sql, $bind);
    }
    ...
    public static function execute_cronjobs($limit = 5, $que_name = null, $shuffle = false)
    {
        ...
        $sql = "SELECT * FROM phpr_cron_jobs 
				$where
				ORDER BY attempts, id ASC
				LIMIT $limit";
        $jobs = DbHelper::objectArray($sql, $que_name);
        ...
        foreach ($jobs as $job) {
            ...
            $processing_sql = "UPDATE phpr_cron_jobs 
            ...
            if ($can_process) {
                ...
                try {
                    $executable = self::getJobExecutable($job);
                    if ($executable) {
                        register_shutdown_function(array('Phpr\Cron', 'internalOnJobShutDown'), $job);
                        $result = call_user_func_array(
                            array($executable['class'], $executable['method']),
                            $executable['params']
                        );
                    } else {
                        $retry = false; //not executable no point retrying
                        throw new ApplicationException('Cron Job is not executable');
                    }
                } catch (\Exception $ex) {
                    if (self::$tracelog_events) {
                        traceLog('Cronjob Exception [' . $job->handler_name . '] - ' . $ex->getMessage());
                    }
                    Phpr::$events->fire_event('phpr:on_execute_cronjob_exception', $ex, $job);
                }
                ...
                if ($retry && $result === false) {
                    ...
                    $processing_sql = "UPDATE phpr_cron_jobs 
											       SET started_at=NULL 
											       WHERE id = :id";
                    DbHelper::query($processing_sql, $bind);
                    continue;
                }

                //no further processing required for this job -> delete
                self::deleteJobVersion($job);
            }
        }
    }
    ...
    public static function execute_cronjobs_in_background($limit = 5, $que_name = null)
    {
        ...
        if ($que_name) {
            $has_jobs = DbHelper::query('SELECT * FROM phpr_cron_jobs WHERE que_name = ?', $que_name);
            if (!$has_jobs) {
    ...
    protected static function cronHasTime()
    {
    ...
    protected static function getJobExecutable($job)
    {
    ...
    protected static function reconcileCronjobs()
    {
        ...
        if ($max_job_duration && is_numeric($max_job_duration)) {
            $jobs_sql = "SELECT id FROM phpr_cron_jobs 
						 WHERE started_at IS NOT NULL 
						 AND started_at < NOW() - INTERVAL ? SECOND";
            $jobs = DbHelper::objectArray($jobs_sql, $max_job_duration);
            if ($jobs) {
                shuffle($jobs);
                foreach ($jobs as $job) {
                    self::handleTimeoutCronjob($job);
                }
            }
        }
    }
    ...
    public static function internalOnJobShutDown($job)
    {
        if ($job && $job->id) {
            ...
            if ($error) {
                ...
                $job_processing = DbHelper::scalar('SELECT id FROM phpr_cron_jobs 
													WHERE started_at IS NOT NULL
													AND id = :id
                ...
                Phpr::$events->fire_event('phpr:on_cronjob_shutdown', $job, $error);
            }
        }
    }

    protected static function handleTimeoutCronjob($job)
    {
        if ($job->attempts >= self::$cronjob_timeout_retries) {
            self::deleteJobVersion($job);
        } else {
            DbHelper::query('UPDATE phpr_cron_jobs SET started_at = NULL WHERE id = ?', $job->id); //allow retry
        }
        if (self::$tracelog_events) {
            traceLog('Cronjob timeout on [' . $job->handler_name . ']');
        }
        Phpr::$events->fire_event('phpr:on_cronjob_exceeded_max_duration', $job);
    }

    protected static function deleteJobVersion($job)
    {
        $bind = array(
            'id' => $job->id,
            'version' => $job->version
        );
        DbHelper::query('DELETE FROM phpr_cron_jobs WHERE id = :id AND version = :version', $bind);
    }


    /**
     * Triggered when an exception occurs on execute_cron()
     * @event phpr:on_execute_cron_exception
     ...
     * @event phpr:on_execute_cronjob_exception
     * @param Exception $ex The exception returned
     ...
     * @event phpr:on_execute_crontab_exception
     * @param Exception $ex The exception returned
     ...
     * @event phpr:on_cronjob_shutdown
     ...
     * @event phpr:on_cronjob_exceeded_max_duration
     * @param object $job The job that caused the shutdown
     * return void
     * @author Matt Manning (github:damanic)
     * @package phpr.events
