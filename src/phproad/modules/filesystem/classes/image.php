...
class Image
{
    ...
    public static function makeThumb(
        ...
        elseif ($width == 'auto' && $height != 'auto') {
            ...
            $optimal_width = $width = self::getSizeByFixedHeight($src_width, $src_height, $optimal_height);
        } // Landscape
        elseif ($height == 'auto' && $width != 'auto') {
            ...
            $optimal_height = $height = self::getSizeByFixedWidth($src_width, $src_height, $optimal_width);
        } // Exact
        else {
            switch ($mode) {
                case 'keep_ratio':
                    $option_array = self::getOptimalRatio($src_width, $src_height, $width, $height);
                ...
                case 'crop':
                    $option_array = self::getOptimalCrop($src_width, $src_height, $width, $height);
                    $optimal_height = $option_array['optimal_height'];
                    $optimal_width = $option_array['optimal_width'];
                    break;
        ...
        if (!Phpr::$config->get('IMAGEMAGICK_ENABLED') || $force_gd) {
            ...
            $image = self::createImage($extension, $source_path);
            if ($image == null) {
            ...
            if ($mode == "crop") {
                self::cropImage($image_p, $optimal_width, $optimal_height, $width, $height);
            }
    ...
    private static function imResample(
        $source_path,
        $destination_path,
        $width,
        $height,
        ...
        try {
            ...
            @chmod($tmp_dir . $current_dir, Directory::getPermissions());
            ...
            if (!$file1_exists && !$file2_exists) {
                throw new ApplicationException(
                    "Error converting image with ImageMagick. IM commands: \n\n" . implode(
                        $convert_strings,
                        "\n\n"
                    ) . "\n\n"
                );
            ...
            if (file_exists($destination_path)) {
                @chmod($destination_path, File::getPermissions());
            }
     ...
     * @param string $path Specifies a source image path.
     */
    public static function deleteImageThumbs($path)
    {
    ...
    private static function cropImage(&$image_resized, $optimal_width, $optimal_height, $new_width, $new_height)
    {
        // Find center
    ...
    private static function getSizeByFixedHeight($width, $height, $new_height)
    {
        $ratio = $width / $height;
    ...
    private static function getSizeByFixedWidth($width, $height, $new_width)
    {
        $ratio = $height / $width;
    ...
    private static function getOptimalCrop($width, $height, $new_width, $new_height)
    {
    ...
    private static function getOptimalRatio($width, $height, $new_width, $new_height)
    {
