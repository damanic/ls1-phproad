...
class Upload
{
    ...
    public static function validateUploadedFile($file_info)
    {
        switch ($file_info['error']) {
            case UPLOAD_ERR_INI_SIZE :
                $max_size_allowed = File::sizeFromBytes(Upload::maxUploadSize());
    ...
    public static function extractMultiFileInfo($multi_file_info)
    {
        $result = array();
    ...
    public static function validateXhrInfo($file_info)
    {
        $max_size = Upload::maxUploadSize();
        $max_size_text = File::sizeFromBytes($max_size);
    ...
    public static function saveXhrFile($file_info, $dest_path)
    {
        // Read file
