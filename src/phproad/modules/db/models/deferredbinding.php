...
class DeferredBinding extends ActiveRecord
{
    ...
    public static function cancelDeferredActionsSubSession($masterClassName, $sessionKey)
    {
    ...
    {
        $thisDate = PhprDateTime::now();

        $records = self::create()->where(
            'ADDDATE(created_at, INTERVAL :days DAY) < :thisDate',
            array('days' => $days, 'thisDate' => $thisDate)
        )->find_all();
        foreach ($records as $record) {
            $record->delete_cancel();
        }
    ...
    protected function delete_detail_record()
    {
        ...
        try {
            ...
            $options = $master_object->get_relation_options(
                $type,
                $this->master_relation_name,
                $has_primary_key,
                $has_foreign_key
            );
