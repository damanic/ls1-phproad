...
class Db_GridWidget extends Db_FormWidget
{
    ...
    protected function load_resources()
    {
        ...
        $this->controller->addJavaScript(
            $this->map_resource_file('javascript/jquery.grid.editors.js?' . module_build('core'))
        );
     ...
     */
    public static function get_cell_error_data(
        $model,
        $db_column_name,
        $grid_field_name,
        $row_index,
    ...
    protected function prepare_data_fields($page_index = 0)
    {
        ...
        if (!$this->use_data_source) {
            ...
            $grid_data = $data_source->get_data_page(
                $pagination,
                $page_index,
                post('phpr_grid_search'),
                post('phpr_grid_search_updated_records')
            );
    ...
    public function get_editor_object($model, $column_info)
    {
        if (!isset($column['editor_class'])) {
            throw new Phpr_ApplicationException(
                sprintf('Editor class is not defined for %s column', $column_info['title'])
            );
        }
    ...
    public function get_data_source()
    {
        ...
        return $this->data_source = new Db_GridWidgetDataSource(
            $this,
            $this->controller->formGetEditSessionKey(),
            $this->data_source_id
        );
    }
    ...
    {
        $result = $model->get_grid_autocomplete_values(
            $field,
            post('autocomplete_column'),
            post('autocomplete_term'),
            post('autocomplete_row_data', array())
        );
        if (!is_array($result)) {
            $result = array();
        }
