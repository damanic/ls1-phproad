...
class Db_ListBehavior extends Phpr\ControllerBehavior
{
    ...
    public function __construct($controller)
    {
        ...
        if (!Phpr::$request->isRemoteEvent()) {
            $this->_controller->addCss(
                '/phproad/modules/db/behaviors/db_listbehavior/resources/css/list.css?' . module_build('core')
            );
        }
    }
     ...
     *                               The options allow to override
     *                               list option properties defined
     *                               in the controller. Note that
     *                               option values specified in the
     *                               method are not persistent and
     *                               will not be used in the list
     *                               AJAX calls (sorting, search,
     *                               etc.).
     *
    ...
    public function listRenderPartial($view, $params = array(), $throwNotFound = true)
    {
        ...
        $controller_class = (isset($this->_controller->list_ignore_native_controller) && $this->_controller->list_ignore_native_controller) ? get_class(
            $this->_controller
        ) : $model->native_controller;
    ...
    public function listGetRecordChildrenCount($record)
    {
        if ($this->_children_number_cache === null) {
            ...
            $cache_data = Db_DbHelper::queryArray(
                strtr(
                    $query, array(
    ...
    public function listGetPrevLevelParentId($model, $current_parent_id)
    {
        return Db_DbHelper::scalar(
            strtr(
                'select {parent_field} from {table_name} where {primary_key_field}=:parent_id', array(
                '{table_name}' => $model->table_name,
                '{parent_field}' => $model->act_as_tree_parent_key,
                '{primary_key_field}' => $model->primary_key
                )
                ...
                try {
                    if (!is_writable(PATH_APP . '/temp/')) {
                        throw new Phpr_SystemException(
                            'There is no writing permissions for the directory: ' . PATH_APP . '/temp'
                        );
                    }
                    ...
                    if (!$model_object->isExtendedWith('Core_ModelCsv')) {
                        throw new Phpr_SystemException(
                            "The mode class {$modelClass} should be extended wit the Core_ModelCsv extension."
                        );
                    }
    ...
    */
    ...
    {
        return Db_UserParameters::get(
            $this->listGetName() . '_treenodestatus_' . $node->id,
            null,
            $this->_controller->list_node_expanded_default
        );
    }
    ...
    protected function evalListColumns($onlyVisible = true)
    {
        ...
        if (!$visibleFound) {
            throw new Phpr_ApplicationException(
                'Error rendering list: there are no visible columns defined in the model.'
            );
        }
    ...
    protected function prepareRenderData($no_pagination = false, $noColumnInfoInit = false)
    {
        ...
        $this->viewData['list_sorting_column'] = $sortingColumn = $this->_controller->listOverrideSortingColumn(
            $this->evalSortingColumn()
        );
    ...
    protected function configureSlidingListData($model)
    {
        ...
        if ($current_parent_id === null || !strlen($current_parent_id)) {
            ...
            $parent_exists = Db_DbHelper::scalar(
                'select count(*) from `' . $model->table_name . '` where `' . $model->primary_key . '`=:id',
                array('id' => $current_parent_id)
            );
    ...
    protected function createModelObject()
    {
        ...
        if (!strlen($this->_controller->list_model_class)) {
            throw new Phpr_SystemException(
                'Data model class is not specified for List Behavior. Use the list_model_class public field to set it.'
            );
        }
    ...
    protected function loadData()
    {
        ...
        if ($this->_controller->list_search_enabled) {
            if (!$this->_controller->list_search_fields) {
                throw new Phpr_ApplicationException(
                    'List search is enabled, but search fields are not specified in the list settings. Please use $list_search_fields public controller field to define an array of fields to search in.'
                );
            }
