...
class Db_Sortable extends Phpr_Extension
{
    ...
    public function set_order_id()
    {
        ...
        Db_DbHelper::query(
            'update `' . $this->model->table_name . '` set sort_order=:new_id where id=:new_id', array(
            'new_id' => $new_id
            )
        );
    }
    ...
    public function set_item_orders($item_ids, $item_orders)
    {
        ...
        if (count($item_ids) != count($item_orders)) {
            throw new Phpr_ApplicationException(
                'Invalid set_item_orders call - count of item_ids does not match a count of item_orders'
            );
        }
        ...
        foreach ($item_ids as $index => $id) {
            ...
            Db_DbHelper::query(
                'update `' . $this->model->table_name . '` set sort_order=:sort_order where id=:id',
                array(
                    'sort_order' => $order,
                    'id' => $id
                )
