...
class ActiveRecord extends Sql implements IteratorAggregate
{
    ...
    /**
     * @var          array Contains a list of methods that can be accessed directly via Db\ActiveRecordProxy
     * @documentable
     */
     ...
     * {@link Db\ActiveRecord::auto_timestamps auto_timestamps} property value is TRUE.
     *
     * @documentable
     */
     ...
     * By default columns specified in {@link Db\ActiveRecord::auto_create_timestamps auto_create_timestamps} and
     * {@link Db\ActiveRecord::auto_update_timestamps auto_update_timestamps} properties are set automatically when the model
    ...
    /**
     * @var          Phpr\Validation Contains the model's validation object.
     ...
     * @return       Db\ActiveRecord Returns a model object corresponding to the found record or NULL.
     */
     ...
     * @documentable
     * @param        integer $id Specifies the primary key value.
     * @return       Db\ActiverecordProxy Returns a model object corresponding to the found record or NULL.
     */
    public function find_proxy($id = null)
    {
        ...
        $result = DbHelper::queryArray($this->build_sql());
        if ($result) {
            $data = $result[0];
            return new ActiverecordProxy($data['id'], get_class($this), $data, $this->strict);
        }
        return null;
    }
    ...
    /**
     * Finds all records and returns the {@link Db\DataCollection} object, containing a list of models.
     *
     * @documentable
     * @param        integer $id           Specifies the primary key value.
     * @param        array   $include      This parameter is reserved.
     * @param        string  $form_context Allows to set the form execution context for the loaded records.
     * @return       Db\DataCollection Returns a collection of found records.
     * @see          Db\DataCollection
     */
    ...
    /**
     * Finds all records and returns the {@link Db\DataCollection} object, containing a list of proxy data models.
     *
     * @documentable
     * @return       Db\DataCollection Returns a collection of Db\ActiverecordProxy data models.
     * @see          Db\DataCollection
     */
                ...
                if (is_string($definition)) {
                    ...
                    if (!isset($definition['sql'])) {
                        throw new SystemException(
                            'Invalid calculated column definition - no SQL clause for ' . $name . ' column in class ' . $this->className
                        );
                    }
     ...
     * @param        array  $include This parameter is reserved.
     * @return       Db\ActiveRecord Returns the model object corresponding to the found record or NULL.
     */
     ...
     * @return       Db\DataCollection Returns the data collection object.
     */
    ...
    public function get_related_records_deferred_obj($name, $deferred_session_key)
    {
        if (!isset($this->has_models[$name])) {
            throw new SystemException("Relation $name is not found in the model " . $this->className);
        }

        $type = $this->has_models[$name];
        if ($type != 'has_many' && $type != 'has_and_belongs_to_many') {
            throw new SystemException(
                'list_related_records_deferred supports only has_many and has_and_belongs_to_many relations'
            );
        }
     ...
     * @return       Db\ActiveRecord Returns the saved model object
     */
    public function save($values = null, $deferred_session_key = null)
    {
        ...
        foreach ($reflection->getProperties() as $property) {
            ...
            if (in_array($property->name, $this->encrypted_columns)) {
                $val = base64_encode(SecurityFramework::create()->tagged_encrypt($val));
            }
            ...
            if (count($record) > 0 || count($this->changed_relations)) {
                ...
                if (count($record) > 0) {
                    $this->sql_update(
                        $this->table_name,
                        $record,
                        Db::where($this->primary_key . ' = ?', $this->{$key})
                    );
                }
                $dataUpdated = true;
            }
        ...
        foreach ($row as $name => $val) {
            ...
            if ($this->modelState != self::stateSaving) {
                if (in_array($name, $this->encrypted_columns) && strlen($val)) {
                    $security_framework = SecurityFramework::create();
                    try {
     ...
     *
     * @documentable
     * @param        string $deferred_session_key Specifies the deferred session key passed to the {@link Db\ActiveRecord::save() save()} method.
     */
     ...
     *     throw new Phpr\ApplicationException("Error deleting customer. There are $order_num order(s)
    ...
    public function fields()
    {
        ...
        return $this->fields_cache = self::$describe_cache[$this->table_name] = $this->describe_table(
            $this->table_name
        );
    }
    ...
    protected function update_footprints(&$new_values)
    {
        ...
        if ($this->auto_timestamps) {
            ...
            foreach ($this->auto_update_timestamps as $field) {
                if (in_array($field, $fields)) {
                    $new_values[$field] = $this->{$field} = Phpr\DateTime::now();
                }
            }
        }
    ...
    public function columns()
    {
        ...
        foreach ($fields as $info) {
            $columns[] = new ActiveRecordColumn($info);
        }
        ...
        return $this->_columns_def = new DataCollection($columns);
    }
    ...
    public function get_primary_key_value()
    {
        ...
        throw new SystemException('A primary key field has not been set for : ' . get_class($this));
    }
    ...
    /**
     * Allows to limit the result of the {@link Db\ActiveRecord::find_all() find_all()} method with the specified number of records.
     ...
     * @return       Db\ActiveRecord Returns the model object.
     */
    ...
    /**
     * Returns a number of rows which would be returned with {@link Db\ActiveRecord::find_all() find_all()} method.
     * Use this method to obtain the number of records which is going to be returned by a subsequent {@link Db\ActiveRecord::find_all() find_all()}
     * method call.
     *
     * @documentable
    ...
    public function requestRowCount()
    {
        ...
        if (!$obj->has_group()) {
            ...
            return Sql::create()->fetchOne($sql);
        } else {
            $obj->use_calc_rows();
            $sql = $obj->build_sql();
            DbHelper::query($sql);
            return DbHelper::scalar('SELECT FOUND_ROWS()');
        }
    }
     ...
     *
     * @return   Db\ActiveRecordIterator
     * @internal For internal use only
     */
    public function getIterator()
    {
        return new ActiveRecordIterator($this);
    }
        ...
        if (!$this->__locked) {
            ...
            if ($is_object && ($value instanceof \Db\ActiveRecord)) {
                ...
                if (!is_null($this->belongs_to) && array_key_exists($name, $this->belongs_to)) {
                    ...
                    if (!$has_foreign_key) {
                        $options['foreign_key'] = Inflector::foreignKey($value->table_name, $this->primary_key);
                    }
    ...
    public function get_related($relation)
    {
        ...
        $class_name = (is_array(
            $relation
        ) && isset($relation['class_name'])) ? $relation['class_name'] : Inflector::classify($relation);
        return $class_name;
    }
    ...
    public function get_relation_options($type, $name, &$has_primary_key, &$has_foreign_key)
    {
        $default_options = array(
            ...
            'primary_key' => $this->primary_key,
            'foreign_key' => Inflector::foreignKey($name, $this->primary_key),
            'conditions' => null,
            'order' => null,
            'limit' => null,
            'finder_sql' => null
    ...
    {
        DeferredBinding::cancelDeferredActions($this->className, $deferred_session_key);
        return $this;
    }
    ...
    public function serialize($serialize_relations = true)
    {
        ...
        if (($serialize_relations === true) || is_array($serialize_relations)) {
            foreach ($this->has_models as $name => $relation) {
                ...
                if (is_array($serialize_relations) && !in_array($name, $serialize_relations)) {
                    continue;
                }
    ...
    public function unserialize($data)
    {
        ...
        if (isset($data['relations'])) {
            foreach ($data['relations'] as $key => $value) {
                ...
                $this->$key = new DataCollection($childs);
            }
        }
    ...
    /**
     * Allows to limit the result of the {@link Db\ActiveRecord::find_all() find_all()} method with a single page of records.
     * Call this method before the {@link Db\ActiveRecord::find_all() find_all()} method call.
     ...
     * @return       Phpr\Pagination Returns the pagination object.
     * @see          Phpr\Pagination
     */
     ...
     * @return       Db\ColumnDefinition Returns the column definition object.
     */
    public function define_column($dbName, $displayName)
    {
        ...
        if (!$this->is_column_cache_disabled()) {
            ...
            if (!array_key_exists($dbName, self::$cached_column_definitions[$this->className])) {
                return self::$cached_column_definitions[$this->className][$dbName] = new ColumnDefinition(
                    $this,
                    $dbName,
                    $displayName
                );
            ...
            return $this->column_definitions[$dbName] = new ColumnDefinition($this, $dbName, $displayName);
        }
    }
     ...
     * @param        string $relationName    Specifies the relation name (should be declared as {@link Db\ActiveRecord::has_many has_many},
     *                                       {@link Db\ActiveRecord::has_and_belongs_to_many has_and_belongs_to_many}, {@link
     *                                       Db\ActiveRecord::belongs_to belongs_to})
     *
     * @param  string $displayName     Specifies a name to display in lists and forms
     * @param  string $type            Specifies a display value type (see <em>db_xxx</em> constants in the description of {@link Db\ActiveRecord} class)
    ...
    public function define_relation_column($columnName, $relationName, $displayName, $type, $valueExpression)
    {
        ...
        if (!$this->is_column_cache_disabled()) {
            ...
            if (!array_key_exists($columnName, self::$cached_column_definitions[$this->className])) {
                return self::$cached_column_definitions[$this->className][$columnName] = new ColumnDefinition(
                    $this,
                    $columnName,
                    $displayName,
                    $type,
            ...
            return $this->column_definitions[$columnName] = new ColumnDefinition(
                $this,
                $columnName,
                $displayName,
                $type,
     ...
     * The column should be defined with {@link Db\ActiveRecord::define_column() define_column()},
     * {@link Db\ActiveRecord::define_relation_column() define_relation_column()} or
     * {@link Db\ActiveRecord::define_multi_relation_column() define_multi_relation_column()} method before this method is called.
     * The method returns the {@link Db\FormFieldDefinition form field definition} object which can be used for further configuration of the field. Example:
     ...
     * @return       Db\FormFieldDefinition Returns the form field definition object.
     */
    public function add_form_field($dbName, $side = 'full')
    {
        ...
        return $this->form_elements[] = new FormFieldDefinition($this, $dbName, $side);
    }
     ...
     * @return       Db\FormSection Returns the form section object.
     */
    public function add_form_section($description, $title = null, $html_id = null)
    {
        ...
        return $this->form_elements[] = new FormSection($title, $description, $html_id);
    }
     ...
     * @return       Db\FormCustomArea Returns the form custom area object.
     */
    public function add_form_custom_area($id, $location = null)
    {
        ...
        return $this->form_elements[] = new FormCustomArea($id, $location);
    }
     ...
     * @return       Db\FormPartial Returns the form partial object.
     */
    public function add_form_partial($path)
    {
        ...
        return $this->form_elements[] = new FormPartial($path);
    }
     ...
     * The form field should be defined with {@link Db\ActiveRecord::define_form_field() define_form_field()} method.
     ...
     * @return       Db\FormFieldDefinition Returns the form field definition object or NULL if the field was not found.
     */
     ...
     * Columns are defined in {@link Db\ActiveRecord::define_columns() define_columns()} method.
     *
     * @documentable
     * @param        string $columnName Specifies the column name.
     * @return       Db\ColumnDefinition Returns the column definition object or NULL if the column was not found in the model.
     */
     ...
     * {@link Db\ColumnDefinition} class, which has a number of properties determining how
     * the column is displayed and validated. You can call model's {@link Db\ActiveRecord::define_column() define_column()},
     * {@link Db\ActiveRecord::define_relation_column() define_relation_column()} and
     * {@link Db\ActiveRecord::define_multi_relation_column() define_multi_relation_column()}
     ...
     * should be defined with {@link Db\ActiveRecord::define_columns() define_columns()} method.
     * Use {@link Db\ActiveRecord::add_form_field() add_form_field()},
     * {@link Db\ActiveRecord::add_form_custom_area() add_form_custom_area()},
     * {@link Db\ActiveRecord::add_form_partial() add_form_partial()},
     * and {@link Db\ActiveRecird::add_form_section() add_form_section()}
     * methods inside this method. Example:
     * <pre>
    ...
    /**
     * Returns a formatted column value. The field should be defined with {@link Db\ActiveRecord::define_column() define_column()},
     * {@link Db\ActiveRecord::define_relation_column() define_relation_column()} or
     * {@link Db\ActiveRecord::define_multi_relation_column() define_multi_relation_column()} method.
     *
     ...
     * by calling {@link Db\ColumnDefinition::dateAsIs() dateAsIs()} of the column definition object.</span>
     *
     * @documentable
     * @param        string $dbName Specifies the column name.
    ...
    public function displayField($dbName, $media = 'form')
    {
        ...
        if (!array_key_exists($dbName, $column_definitions)) {
            throw new SystemException(
                'Cannot execute method "displayField" for field ' . $dbName . ' - the field is not defined in column definition list.'
            );
        }
    ...
    /**
     * Alias for the {@link Db\ActiveRecord::displayField() displayField()} method
     *
     * @documentable
     * @param        string $dbName Specifies the column name.
     * @return       string Returns the formatted column value.
    ...
    /**
     * Executes the find_all() method and returns a {@link Db\DataCollection collection}.
     ...
     * {
     *   Phpr::$events->addEvent('core:onBeforeDatabaseQuery', $this, 'on_before_query');
     *   Phpr::$events->addEvent('core:onAfterDatabaseQuery', $this, 'on_after_query');
     * }
     *
     * public function on_before_query($sql)
     * {
     *   // Start timing for the query
     *   Phpr\DebugHelper::start_timing($sql);
     * }
     *
     * public function on_after_query($sql, $result)
     * {
     *   // Stop timing and write the result to the trace log (logs/info.txt)
     *   Phpr\DebugHelper::end_timing($sql);
     * }
     * </pre>
     *
