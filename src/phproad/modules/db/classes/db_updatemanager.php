...
class Db_UpdateManager
{
    ...
    public static function getDbVersion($module_id)
    {
        if (self::$_versions === null) {
            $versions = Db_DbHelper::queryArray(
                'select moduleId, version_str as version from core_versions order by id'
            );
            ...
            if ($update_type == '@') {
                ...
                if ($pos === false) {
                    throw new Phpr_ApplicationException(
                        'Error parsing version file (' . $filePath . '). Version string should have a description: ' . $update_content
                    );
                }
    ...
    protected static function registerAppliedModuleUpdate($module_id, $update_id)
    {
        ...
        Db_DbHelper::query(
            'insert into core_applied_updates(module_id, update_id, created_at) values(:module_id, :update_id, :created_at)',
            array(
                'module_id' => $module_id,
                'update_id' => $update_id,
