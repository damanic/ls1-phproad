...
class Db_ActiveRecord extends Db_SqlBase implements IteratorAggregate
{
            ...
            foreach ($relation as $name => $info) {
                ...
                case 'has_many':
                    ...
                    if (is_array($info['relation'])) {
                        ...
                        $options = array_merge(
                            $defaults,
                            array('class_name' => Phpr_Inflector::classify($info['relation']))
                        );
                    }
                    ...
                    if ($action == 'bind') {
                        ...
                        if (count($info['values'])) {
                            $this->sql_insert(
                                ...
                                Phpr_Util::pairs($this->{$this->primary_key}, $info['values'])
                            );
                            $result = true;
                        }
                    } elseif ($action == 'unbind') {
    ...
    public function define_column($dbName, $displayName)
    {
        ...
        if (!$this->is_column_cache_disabled()) {
            ...
            if (!array_key_exists($dbName, self::$cached_column_definitions[$this->className])) {
                return self::$cached_column_definitions[$this->className][$dbName] = new Db_ColumnDefinition(
                    $this,
                    $dbName,
                    $displayName
                );
     ...
     *                                       {@link Db_ActiveRecord::has_and_belongs_to_many has_and_belongs_to_many}, {@link
     *                                       Db_ActiveRecord::belongs_to belongs_to})
     *
     * @param  string $displayName     Specifies a name to display in lists and forms
    ...
    public function define_relation_column($columnName, $relationName, $displayName, $type, $valueExpression)
    {
        ...
        if (!$this->is_column_cache_disabled()) {
            ...
            if (!array_key_exists($columnName, self::$cached_column_definitions[$this->className])) {
                return self::$cached_column_definitions[$this->className][$columnName] = new Db_ColumnDefinition(
                    $this,
                    $columnName,
                    $displayName,
                    $type,
            ...
            return $this->column_definitions[$columnName] = new Db_ColumnDefinition(
                $this,
                $columnName,
                $displayName,
                $type,
