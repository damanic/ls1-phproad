...
class Db_GridImagesEditor extends Db_GridEditor
{
    ...
    public function render_popup_contents($column_info, $controller, $field_name)
    {
        ...
        $images = $this->get_image_list(
            $column_info,
            post('phpr_grid_row_index'),
            $controller->formGetEditSessionKey()
        );
        $controller->renderPartial(
            PATH_APP . '/phproad/modules/db/partials/_images_editor_content.htm', array(
            'images' => $images,
    ...
    protected function get_image_list($column_info, $row_index, $session_key)
    {
        ...
        return $row_model->list_related_records_deferred(
            $column_info['images_field'],
            $this->get_record_session_key($session_key, $row_index)
        );
    }
    ...
    protected function on_update_image_list($field, $model, $column_info, $controller, $column_name)
    {
        ...
        $controller->renderPartial(
            PATH_APP . '/phproad/modules/db/partials/_images_editor_image_list.htm', array(
            'images' => $images,
    ...
    protected function on_delete_image($field, $model, $column_info, $controller, $column_name)
    {
        ...
        if ($file = Db_File::create()->find(post('file_id'))) {
            $row_model->{$column_name}->delete(
                $file,
                $this->get_record_session_key($controller->formGetEditSessionKey(), $row_index)
            );
        }
