...
class Db_AutoFootprints extends Phpr_Extension
{
    ...
    public function autoFootprintsColumnsDefined()
    {
        ...
        $field = $this->_model->define_column('created_at', $this->_model->auto_footprints_created_at_name)->dateFormat(
            $this->_model->auto_footprints_date_format
        );
        ...
        $field = $this->_model->define_column(
            'created_user_name',
            $this->_model->auto_footprints_created_user_name
        ...
        if ($hasUpdateFields) {
            $field = $this->_model->define_column(
                'updated_at',
                $this->_model->auto_footprints_updated_at_name
            )->dateFormat($this->_model->auto_footprints_date_format);
            if (!$this->_model->auto_footprints_visible) {
                $field->invisible();
            }
            if ($this->_model->auto_footprints_default_invisible) {
        ...
        if ($hasUpdateFields) {
            $field = $this->_model->define_column(
                'updated_user_name',
                $this->_model->auto_footprints_updated_user_name
            )->noLog();
            if (!$this->_model->auto_footprints_visible) {
                $field->invisible();
            }
