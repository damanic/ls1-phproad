...
class EmailParams extends ActiveRecord
{
    ...
    {
        $this->define_column('sender_name', 'Sender Name')
            ->validation()->fn('trim')
            ->required('Please specify sender name.');

        $this->define_column('sender_email', 'Sender Email')
            ->validation()->fn('trim')
            ->required('Please specify sender email address.')
            ->email();

        $this->define_column('send_mode', 'Email Method');

        $this->define_column('smtp_address', 'SMTP Address')
            ->validation()
            ->fn('trim')
            ->method('validate_smtp_address');
    ...
    public function define_form_fields($context = null)
    {
        ...
        $this->add_form_field('templating_engine')
            ->comment(
                'Please specify a templating engine to use for parsing compound email variables and email layouts.',
                'above'
            )
        ...
        if (
            class_exists('Shop\CustomerPreferences')
            && method_exists('Shop\CustomerPreferences', 'get_customer_preference')
        ) {
            $this->add_form_field('allow_recipient_blocking')
                ->tab('General Parameters')
                ->comment(
                    'Whilst switched on you can selectively display unsubscribe links in your email template content. 
                    When switched off, any recipient preferences will be ignored and any email variables 
                    that generate unsubscribe links will be excluded from email content',
                    'above'
                )
        ...
        $this->add_form_field('smtp_authorization')->tab('SMTP')
            ->comment('Use this checkbox if your SMTP server requires authorization.');

        $this->add_form_field('smtp_user', 'left')
            ->tab('SMTP')
            ->comment('SMTP user name', 'above')
            ->renderAs(frm_text);

        $this->add_form_field('smtp_password', 'right')
            ->tab('SMTP')
            ->renderAs(frm_password)
            ->comment('SMTP user password', 'above');

        $this->add_form_field('smtp_port')->tab('SMTP');
        $this->add_form_field('smtp_ssl')->tab('SMTP');

        $this->add_form_field('sendmail_path')
            ->tab('Sendmail')
            ->comment(
                'Please specify the path of the sendmail program. 
                Leave this field empty to use the default path: /usr/sbin/sendmail',
                'above'
            );
    ...
    {
        if ($value == 'php' && !Configuration::is_php_allowed()) {
            $this->validation->setError(
                'The application configuration doesn\'t allow PHP in compound email variables and layouts.',
                $name,
                true
            );
        }
    ...
    public static function get_templating_engine()
    {
        ...
        if ($engine == 'php' && !Configuration::is_php_allowed()) {
