...
class HtmlEditorConfig extends ActiveRecord
{
    ...
    {
        $obj = HtmlEditorConfig::create()->where('code=?', $code)->where('module=?', $module)->find();
        if (!$obj) {
        ...
        $obj = HtmlEditorConfig::create()->where('code=?', $code)->where('module=?', $module)->find();
        if (!$obj) {
            throw new ApplicationException('HTML editor configuration '.$code.' not found.');
        }
    ...
    public function define_form_fields($context = null)
    {
        ...
        $this->add_form_field('default_height')
            ->tab('Customize')
            ->comment(
                'Enter a number to set the editors starting height in pixels, the editor can be resized by the user',
                'above',
                true
            );

        $this->add_form_field('content_css')
            ->renderAs(frm_text)
            ->tab('Customize')
            ->comment(
                'Specify an URL of a custom CSS file to use inside the editable area. 
                 By default the following file used:<br/> /modules/cms/resources/css/htmlcontent.css',
                'above',
                true
            );

        $this->add_form_field('block_formats')
            ->renderAs(frm_text)
            ->tab('Customize')
            ->comment(
                'A comma separated list of formats that will be available in the Format Selector list.
                 The default value of this option is:<br/>p, address, pre, h1, h2, h3, h4, h5, h6', 
                'above', 
                true
            );

        $this->add_form_field('custom_styles', 'left')
            ->tab('Customize')
            ->comment(
                'A list of class titles and class names separated by =, one style per line. 
                The titles will be presented to the user in the Style Selector list 
                and the class names will be inserted. 
                If this option is not defined, the editor imports the classes from the Content CSS file. 
                Example: My style=my_css_class',
                'above'
            )
            ->size('small');
        
        $this->add_form_field('font_sizes', 'right')
            ->tab('Customize')
            ->comment(
                'A list of font sizes, one size per line. 
                      The list of font sizes will be shown in the Font Size Selector list. 
                      You can specify sizes in CSS size values, or with CSS classes. Example of usage:
                      <br/>Big text=30px<br/>My Text Size=.mytextsize',
                'above',
                true
            )
            ->size('small');
        
        $this->add_form_field('font_colors', 'left')
            ->tab('Customize')
            ->comment(
                'A comma-separated list of colors shown in the palette of colors displayed by the text color button. 
                      Example:<br/> FF00FF, FFFF00, 000000',
                'above',
                true
            );
        
        $this->add_form_field('background_colors', 'right')
            ->tab('Customize')
            ->comment(
                'A comma-separated list of colors shown in the palette of colors displayed by the background color button. 
                      Example:<br/> FF00FF, FFFF00, 000000',
                'above',
                true
            );
        
        $this->add_form_field('allow_more_colors')
            ->tab('Customize')
            ->comment(
        ...
        $this->add_form_field('valid_elements')
            ->tab('Clean Up')
            ->comment(
                'The valid_elements option defines which elements will remain in the edited text when the editor saves. 
                      You can find more information about this option in 
                      <a href="http://wiki.moxiecode.com/index.php/TinyMCE:Configuration/valid_elements" target="_blank">
        ...
        $this->add_form_field('valid_child_elements')
            ->tab('Clean Up')
            ->comment(
                'This option gives you the ability to specify what elements are valid inside different parent elements. 
                      You can find more information about this option in 
                      <a href="http://wiki.moxiecode.com/index.php/TinyMCE:Configuration/valid_child_elements" target="_blank">
    ...
    public function get_added_field_options($db_name, $current_key_value = -1)
    {
        $result = Backend::$events->fireEvent(
            'system:onExtendHtmlEditorConfigFieldOptions', 
            $db_name, 
            $current_key_value
        );
    ...
    public function list_plugins()
    {
        ...
        $all_row_buttons = array_merge(
            explode(',', $this->controls_row_1),
    ...
    public function output_editor_config()
    {
        ...
        $result['theme_advanced_blockformats'] = '"'.Strings::jsEncode($this->block_formats).'"';
        $result['theme_advanced_styles'] = '"'.Strings::jsEncode($this->custom_styles).'"';

        $custom_styles = $this->list_custom_styles();
        $result['theme_advanced_font_sizes'] = '"'.Strings::jsEncode($custom_styles).'"';
        $result['theme_advanced_font_sizes'] = '"'.Strings::jsEncode($this->list_font_sizes()).'"';

        if ($this->font_colors) {
            $result['theme_advanced_text_colors'] = '"'.Strings::jsEncode(str_replace(' ', '', $this->font_colors)).'"';
        }

        if ($this->background_colors) {
            $value = str_replace(' ', '', $this->background_colors);
            $result['theme_advanced_background_colors'] = '"'.Strings::jsEncode($value).'"';
        }

        $result['theme_advanced_more_colors'] = $this->allow_more_colors ? 'true' : 'false';

        if ($this->valid_elements) {
            $result['valid_elements'] = '"'.Strings::jsEncode($this->valid_elements).'"';
        }

        if ($this->valid_child_elements) {
            $result['valid_child_elements'] = '"'.Strings::jsEncode($this->valid_child_elements).'"';
        }
    ...
    {
        $configurations = ModuleManager::listHtmlEditorConfigs();

        foreach ($configurations as $module => $configs) {
            foreach ($configs as $code => $description) {
                $obj = HtmlEditorConfig::create()->where('code=?', $code)->where('module=?', $module)->find();
                if (!$obj) {
                    HtmlEditorConfig::create()->init_config($code, $module, $description);
                }
            }
        }
    }
