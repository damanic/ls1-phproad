...
class EmailLayout extends ActiveRecord
{
    ...
    public function define_columns($context = null)
    {
        $this->define_column('code', 'Code')
            ->validation()
            ->fn('trim')
            ->required('Please specify the layout code.')
        ...
        $this->define_column('name', 'Name')
            ->validation()
            ->fn('trim')
            ->required('Please specify the layout name.');

        $this->define_column('content', 'Content')
            ->invisible()
            ->validation()
            ->required('Please fill out the layout content.');

        $this->define_column('css', 'CSS')
            ->invisible()
            ->validation();
    }
    ...
    public function define_form_fields($context = null)
    {
        $this->add_form_field('content')
            ...
            ->size('huge')
            ->comment('Please provide the HTML/PHP code for the email layout', 'above');

        $this->add_form_field('css')
            ->tab('CSS')
            ->cssClasses('code')
            ->renderAs(frm_code_editor)
            ->size('huge')
            ->comment('This CSS document will be included to the layout', 'above')
            ->language('css');
    }
    ...
    public static function find_by_code($code)
    {
        ...
        if (!strlen($code)) {
            throw new ApplicationException('The specified layout is not found');
        }
    ...
    public function format($message, $params = array())
    {
        ...
        try {
            ...
            if ($engine == 'php') {
                ...
                $params = array_merge($params, array('message'=>$message, 'this'=>$this));
                $result = Twig::get()->parse(
                    $this->content,
                    $params,
                    'Email layout "'.$this->name.'"'
                );
            }
        } catch (\Exception $ex) {
            if ($engine == 'php') {
