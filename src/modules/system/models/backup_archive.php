 ...
 */
class Backup_Archive extends ActiveRecord
{
    ...
    public function define_form_fields($context = null)
    {
        ...
        if (strlen($this->comment) || $this->is_new_record()) {
            $this->add_form_field('comment')
                ->size('small')
                ->comment('Optionally you may comment the archive.', 'above')
                ->previewComment('Archive comment');
        }

        if ($this->is_new_record()) {
            $this->add_form_field('archive_uploaded_dir')
                ->comment(
                    'Enable this option if you want to include uploaded files (website resource files 
                    - images, CSS, JavaScript files, product and category images, etc.) to the archive. 
                    Enabling this option can significantly increase the backup file size. 
                    When this option is disabled LSAPP archives only the database content.'
                );
        }
    ...
    {
        $nowDate = Date::userDate(PhprDateTime::now())->format('%Y-%m-%d-%H-%M-%S');
        $params = Backup_Params::get();
    ...
    public static function backup($throwOnError = false, $force = false)
    {
        try {
            ...
            Backend::$events->fireEvent('system:onBeforeArchiveCreate');
            if (!Backup_Params::isConfigured()) {
                return;
            }
            
            $sql = "select count(*) from system_backup_archives 
                    where date_add(created_at, interval 5 MINUTE) > :curentTime 
                    and status_id=1";
            
            $backupInProgress = DbHelper::scalar(
                $sql,
                array( 'curentTime' => PhprDateTime::now()->toSqlDateTime() )
            );
            ...
            if ((!$lastRecord && $params->backup_on_login) || $force) {
                ...
                $interval = PhprDateTime::now()->substractDateTime($lastRecord->created_at);
            ...
            Backend::$events->fireEvent('system:onAfterArchiveCreate');
        } catch (\Exception $ex) {
            if ($throwOnError) {
                throw $ex;
            }
    ...
    protected static function createBackup($throwOnError = false)
    {
        ...
        try {
            ...
            if ($params && $params->notify_administrators) {
                $administrators = User::listAdministrators();
                $viewData = array('error'=>$ex->getMessage());
                CoreEmail::sendToList(
                    'system', 
                    'backup_failed', 
                    $viewData, 
                    'Error creating backup archive.', 
                    $administrators
                );
            }
    ...
    protected static function createDumpArchive($archivePath, $options = array())
    {
        ...
        try {
            ...
            DbHelper::exportSqlToFile($sqlPath, array(
                'separator'=>'^;^'
            ));
            ...
            if ($archive_files) {
                ...
                Directory::copy(PATH_APP.'/resources', $tmpDirPath.'/resources', $options);
                Directory::copy(PATH_APP.'/uploaded', $tmpDirPath.'/uploaded', $options);
            }
    ...
    public static function restoreData($archiveFile)
    {
         ...
         * Create archive
         */
        Backend::$events->fireEvent('system:onBeforeArchiveRestore');
        ...
        try {
            ...
            throw new ApplicationException('Error creating backup archive. '.$ex->getMessage());
        }
        ...
        try {
            ...
            Zip::unzip($tmpDirPath, $archiveFile, true, true);
            ...
            foreach ($modules as $module) {
                ...
                if (!UpdateManager::moduleVersionExists($moduleId, $version)) {
                    throw new ApplicationException(
                        'Error restoring data: the archive file was created in a  system newer than this one. 
                        Version of module "'.$moduleId.'" in the archive ('.$version.') is higher than existing version 
                        of the module installed in this system: '.$systemVersions[$moduleId].'. 
                        Operation aborted.'
                    );
                }
            }
            ...
            foreach ($directories as $directory) {
                ...
                if (file_exists($dirPath)) {
                    Directory::copy($dirPath, PATH_APP.'/'.$directory);
                }
            }
            ...
            if (!file_exists($sqlPath)) {
                throw new ApplicationException('Archive file is corrupted - database dump is not found.');
            }
            ...
            DbHelper::executeSqlFromFile($sqlPath, '^;^');
            Directory::deleteRecursive($tmpDirPath);
    ...
    /**
     * Triggered before LSAPP
     * {@link https://lsdomainexpired.mjman.net/docs/configuring_and_using_the_backup_system archive is created.
     * Example:
     * <pre>
     * public function subscribeEvents()
     * {
     *   Backend::$events->addEvent('system:onBeforeArchiveCreate', $this, 'before_archive_create');
     * }
     *
     ...
     * }
     * </pre>
     * @event system:onBeforeArchiveCreate
     * @see https://lsdomainexpired.mjman.net/docs/configuring_and_using_the_backup_system
     * @see system:onBeforeArchiveRestore
     * @see system:onAfterArchiveCreate
     * @package core.events
     * @author LSAPP - MJMAN
     */
    ...
    /**
     * Triggered after LSAPP
     * {@link https://lsdomainexpired.mjman.net/docs/configuring_and_using_the_backup_system archive} is created.
     * Example:
     * <pre>
     * public function subscribeEvents()
     * {
     *   Backend::$events->addEvent('system:onAfterArchiveCreate', $this, 'after_archive_create');
     * }
     *
     ...
     * }
     * </pre>
     * @event system:onAfterArchiveCreate
     * @see https://lsdomainexpired.mjman.net/docs/configuring_and_using_the_backup_system
     * @see system:onBeforeArchiveRestore
     * @see system:onBeforeArchiveCreate
     * @package core.events
     * @author LSAPP - MJMAN
     */
    ...
    /**
     * Triggered before LSAPP
     * {@link https://lsdomainexpired.mjman.net/docs/configuring_and_using_the_backup_system archive} is restored.
     * Example:
     * <pre>
     * public function subscribeEvents()
     * {
     *   Backend::$events->addEvent('system:onBeforeArchiveRestore', $this, 'before_archive_restore');
     * }
     *
     ...
     * }
     * </pre>
     * @event system:onBeforeArchiveRestore
     * @see https://lsdomainexpired.mjman.net/docs/configuring_and_using_the_backup_system
     * @see system:onBeforeArchiveCreate
     * @see system:onAfterArchiveCreate
     * @package core.events
