...
class ColorThemeParams extends ActiveRecord
{
    ...
    public function define_form_fields($context = null)
    {
        ...
        $this->add_form_field('theme_id')
            ->tab('Theme')
            ->renderAs('theme_selector')
            ->comment('Please choose a theme for Administration Area.', 'above');
            
        $this->add_form_field('footer_text')
            ->tab('Footer')
            ->renderAs(frm_html)
            ->htmlButtons1(
                "cut,copy,paste,separator,undo,redo,separator,link,unlink,separator,bold,italic,underline,code"
            );
    }
