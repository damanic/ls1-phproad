...
class Email_Templates extends SettingsController
{
    public $implement = 'Db_ListBehavior, Db_FormBehavior';
    public $list_model_class = 'System\EmailTemplate';
    public $list_record_url = null;

    public $form_model_class = 'System\EmailTemplate';
    ...
    public function formBeforeFileUploadSave($model, $dbName, $dbFile, $sessionKey)
    {
        if (is_a($model, 'System_EmailTemplate') && $dbName == 'file_attachments') {
            ...
            if (!$fileExtension || in_array($fileExtension, $this->blocked_file_types)) {
                $dbFile->delete();
                throw new ApplicationException('The file type '.strtoupper($fileExtension).' is not permitted for '.$dbName);
            }
        }
    }
}
