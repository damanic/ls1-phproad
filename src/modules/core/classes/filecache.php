...
class FileCache extends CacheBase
{
    ...
    public function __construct($params = array())
    {
        ...
        if (!$dir_path) {
            throw new SystemException(
                'Caching directory for the file-based caching is not specified in the config.php file'
            );
        }

        if (!file_exists($dir_path) || !is_dir($dir_path)) {
            throw new SystemException('Caching directory does not exist: ' . $dir_path);
        }

        if (!is_writable($dir_path)) {
            throw new SystemException('Caching directory is not writable: ' . $dir_path);
        }
    ...
    protected function set_value($key, $value, $ttl = null)
    {
        ...
        if (!$fp) {
            throw new SystemException('Error writing to the cache file');
        }
