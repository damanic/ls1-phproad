...
class ConfigSecurity extends PhprSecurity
{
    public $userClassName = "Core\ConfigUser";
    ...
    public function findUser($UserId)
    {
        ...
        return self::$UserCache[$UserId] = new ConfigUser();
    }
    ...
    protected function afterLogin($User)
    {
        ...
        $log_record = PhprDateTime::now()->getInteger() . '|' . Phpr::$request->getUserIp();
    ...
    protected function log_login_attempt($user_name)
    {
        ...
        $login_log = array_key_exists('unsuccessful_login_log',
            $config_content) ? $config_content['unsuccessful_login_log'] : array();

        $log_record = PhprDateTime::now()->getInteger() . '|' . Phpr::$request->getUserIp() . '|' . $user_name;
        $login_log[] = $log_record;
