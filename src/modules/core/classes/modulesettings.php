...
class ModuleSettings extends ConfigurationRecord
{
    protected $module_id;
    protected $form_code;
    ...
    {
        $record_code = self::get_record_code($module_id, $form_code);
        ActiveRecord::disable_column_cache();
    ...
    protected function get_module_obj()
    {
        ...
        if (!$this->module_obj) {
            throw new SystemException(sprintf('Module %s not found', $this->module_id));
        }
    ...
    {
        $result = ModuleManager::findById($module_id);
        if (!$result) {
            throw new SystemException(sprintf('Module %s not found', $module_id));
        }
