...
use Cms\Controller as CmsController;
use Shop\Module as ShopModule;
use Cms\Module as CmsModule;
 ...
 *
 * $recache = false;
 * $key = CacheBase::create_key('some_item', $recache, array('url'), array('cms','catalog'));
 * $value = $cache->get($key);
 *
...
abstract class CacheBase
{
     ...
     * FileCache, MemCache or ApcCache class. All these classes has same
     * methods.
     * @documentable
    ...
    public static function create()
    {
        ...
        $params = array();
        $class_name = 'Core\DummyCache';

        if ($caching_config) {
            $class_name = isset($caching_config['CLASS_NAME']) ? $caching_config['CLASS_NAME'] : null;
            if (!$class_name) {
                throw new SystemException('Caching class name is not specified in the caching configuration.');
            }
     ...
     *
     * $recache = false;
     * $key = CacheBase::create_key('some_item', $recache);
     *
     ...
     *
     * $recache = false;
     * $key = CacheBase::create_key('some_item', $recache, 'url', 'cms');
     *
     ...
     *
     * $recache = false;
     * $key = CacheBase::create_key('some_item', $recache, array('url', 'customer_group'), array('cms','catalog'));
     *
     ...
     * $recache = false;
     * $sort_order = 'name asc';
     * $key = CacheBase::create_key('some_item', $recache, array('sort_order'=>$sort_order));
     *
    ...
    public static function create_key($prefix, &$recache, $vary_by = array(), $version = array())
    {
        ...
        $vary_by = Util::splat($vary_by);
        $version = Util::splat($version);

        $obj = self::create();

        $controller = CmsController::get_instance();
            ...
            if (is_int($key)) {
                switch ($param_name) {
                    case 'url' :
                        ...
                        $reset_cache_key = array_key_exists('RESET_PAGE_CACHE_KEY',
                            $caching_params) ? $caching_params['RESET_PAGE_CACHE_KEY'] : null;
                    ...
                    case 'customer_presence' :
                        ...
                        break;
                    default:
                        throw new SystemException('Unknown cache vary-by parameter: ' . $param_name);
                        break;
                }
            } else {
                $result .= '-' . $key . ':';
        ...
        foreach ($version as $param_name) {
            ...
            switch ($param_name) {
                case 'catalog' :
                    $prev_version_value = ($key_versions && array_key_exists('catalog',
                            $key_versions)) ? $key_versions['catalog'] : 0;
                    $new_version_value = ShopModule::get_catalog_version();
                    ...
                    break;
                case 'cms' :
                    $prev_version_value = ($key_versions && array_key_exists('cms',
                            $key_versions)) ? $key_versions['cms'] : 0;
                    $new_version_value = CmsModule::get_cms_content_version();
                    $recache = $recache || ($prev_version_value != $new_version_value);
                    if ($recache) {
                    ...
                    break;
                case 'blog' :
                    $prev_version_value = ($key_versions && array_key_exists('blog',
                            $key_versions)) ? $key_versions['blog'] : 0;
                    $new_version_value = BlogModule::get_blog_content_version();
                    $recache = $recache || ($prev_version_value != $new_version_value);
                    if ($recache) {
                    ...
                    break;
                default:
                    throw new SystemException('Unknown cache version parameter: ' . $param_name);
                    break;
            }
