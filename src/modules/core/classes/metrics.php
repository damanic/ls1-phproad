 ...
 *
 *            Communication of these metric are no longer sent to servers
...
class Metrics
{
    public static function update_metrics()
    {
        ...
        try {
            ...
            DbHelper::query('update core_metrics set update_lock=NOW()');
...
//                  $ch = curl_init('https://v1.lasappexpire.mjman.net/ls_process_usage_stats/');
...
//                  $success = $result == 'thanks';
            ...
            if ($success) {
                DbHelper::query('update core_metrics set total_amount=0, total_order_num=0, page_views=0, update_lock=null, updated=NOW()');
            } else {
                DbHelper::query('update core_metrics set update_lock=null');
            }
        } catch (\Exception $ex) {
        }
    }
    ...
    public static function log_pageview()
    {
        try {
            ...
            DbHelper::query('update core_metrics set page_views = page_views+1');
        } catch (\Exception $ex) {
        }
    }
    ...
    {
        $shipping_module_records = DbHelper::objectArray('select 
    ...
    {
        $payment_module_records = DbHelper::objectArray('select 
