...
class TwigExtension extends TwigAbstractExtension
{
    ...
    public function method()
    {
        ...
        if (count($args) < 2) {
            throw new RuntimeError('The method() function should have at least 2 arguments - the class name and the method name.');
        }
                
        $class = trim(array_shift($args));
        $method = trim(array_shift($args));
            
        if (!Configuration::is_php_allowed()) {
                ...
                foreach ($prohibited_class_map as $prohibited_class_name => $prohibited_methods) {
                    if (strtoupper($prohibited_class_name) == strtoupper($class) || $class instanceof $prohibited_class_name) {
                        if ($prohibited_methods == '*') {
                            throw new RuntimeError(sprintf('Using the %s class in Twig templates is prohibited.', $class));
                        }
                        ...
                        foreach ($prohibited_methods as $prohibited_method) {
                            ...
                            if ($prohibited_method == strtoupper($method)) {
                                throw new RuntimeError(sprintf('Using the %s::%s() method in Twig templates is prohibited.', $class, $method));
                            }
                        }
                    }
                }
