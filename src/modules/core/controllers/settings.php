...
class Settings extends Controller
{
    ...
    {
        Phpr::$events->fireEvent('core:onConfigureModuleSettingsController', $this, $form_id);
        ...
        try {
            ...
            $this->viewData['form_model'] = ModuleSettings::create($module_id, $form_id);
    ...
    {
        $module = ModuleManager::findById($module_id);
        if (!$module) {
            throw new ApplicationException('Module with the specified code is not found');
        }
                
        $settings_forms = $module->listSettingsForms();
        if (!array_key_exists($form_id, $settings_forms)) {
            throw new ApplicationException('Settings form with the specified code is not found');
        }

        $form_params = $settings_forms[$form_id];
        $is_personal = isset($form_params['personal']) && $form_params['personal'];
        if (!$is_personal && !$this->currentUser->is_administrator()) {
            throw new ApplicationException('You have no rights to access this page');
        }
    ...
    protected function edit_onSave($module_id, $form_id)
    {
        try {
            ...
            $obj = ModuleSettings::create($module_id, $form_id);
