...
class CartProductFixed_Action extends CartRuleActionBase
{
    ...
    public function build_config_form($host_obj)
    {
        $host_obj->add_field('discount_amount', 'Discount amount', 'full', db_float)
            ->comment(
                'Please specify an amount to subtract from cart item unit price. ',
                'above'
            )
            ->validation()
    ...
    public function eval_discount(
        ...
        if (!array_key_exists('cart_items', $params)) {
            throw new ApplicationException(
                'Error applying the "Discount each cart item unit price by fixed amount" price rule action: 
