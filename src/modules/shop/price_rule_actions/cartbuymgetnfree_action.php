...
class CartBuyMGetNFree_Action extends CartRuleActionBase
{
    ...
    public function build_config_form($host_obj)
    {
        $host_obj->add_field('m_value', 'M value (Buy)', 'left', db_number)
            ->comment(
                'How many units of the product should be added to the cart in order to activate the action. ',
                'above'
            )
            ->validation()
        ...
        $host_obj->add_field('n_value', 'N value (Get)', 'right', db_number)
            ->comment(
                'How many units of the product to discount in case if M products has been added to the shopping cart. ',
                'above'
            )
            ->validation()
            ->required('Please specify the N value');

        $host_obj->add_field('multiples', 'Work with multiples', 'full', db_float)
    ...
    public function eval_discount(
        ...
        if (!array_key_exists('cart_items', $params)) {
            throw new ApplicationException(
                'Error applying the "Buy M get N free" 
                price rule action: the cart_items element is not found in the action parameters.'
            );
        }
        ...
        foreach ($cart_items as $item) {
            ...
            if ($active) {
                ...
                if (!$use_multiples) {
                    // if ($item->quantity >= ($m_value+1)) - if we need to activate the action on the M+1th items
                    if ($item->quantity >= $m_value) {
                        $total_discount_incl_tax = $discount_value = $current_product_price * $n_value / $item->quantity;
                    }
                } else {
                    // floor(($item->quantity-1)/$m_value); - if we need to activate the action on the M+1th items
                    $factor = floor($item->quantity / $m_value);
                    $discount_value = $current_product_price * $n_value * $factor / $item->quantity;
                    $total_discount_incl_tax = $discount_value;
                }

                if ($include_tax) {
                    /*
                     * If taxes are included to discounts,
                     * we need to extract the real discount value from the discount total
                     */
