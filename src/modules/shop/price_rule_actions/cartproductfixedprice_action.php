...
class CartProductFixedPrice_Action extends CartRuleActionBase
{
    ...
    {
        $host_obj->add_field('discount_amount', 'New cart item price', 'full', db_float)
            ->comment('Please specify a fixed price for the cart item.', 'above')
            ->validation()
     ...
     * @param array $item_discount_tax_incl_map
     *  A list of cart item identifiers and corresponding discounts with tax included.
     * @param RuleConditionBase $product_conditions
     *  Specifies product conditions to filter the products the discount should be applied to
     * @return float Returns discount value (for cart-wide actions),
     *  or a sum of discounts applied to products (for per-product actions) without tax applied
     */
    public function eval_discount(
        ...
        if (!array_key_exists('cart_items', $params)) {
            throw new ApplicationException(
                'Error applying the "Discount each cart item unit price to a fixed price" price rule action: 
                the cart_items element is not found in the action parameters.'
            );
        }
        ...
        foreach ($cart_items as $item) {
            $original_product_price = $item->get_de_cache_item('total_single_price');
            if ($original_product_price === false) {
                $original_product_price = $item->set_de_cache_item('total_single_price', $item->total_single_price());
            }
            ...
            if ($item_current_price === false) {
                $priceValue = $item->single_price_no_tax(false) - $item->get_sale_reduction();
                $item_current_price = $item->set_de_cache_item('current_price', $priceValue);
            }
                
            $rule_params['current_price'] = $item_current_price;
            ...
            if ($active) {
                ...
                if ($include_tax) {
                    $discount_amount = TaxClass::get_subtotal($item->get_tax_class_id(), $discount_amount);
                }
                ...
                if ($current_product_price <= $discount_amount) {
                    ...
                    if ($include_tax) {
                        $discount_value = TaxClass::get_subtotal($item->get_tax_class_id(), $discount_value);
                        $taxValue = TaxClass::get_total_tax($item->get_tax_class_id(), $discount_value);
                        $total_discount_incl_tax = $taxValue + $discount_value;
                    }
