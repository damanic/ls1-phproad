...
class ProductByPercentage_Action extends RuleActionBase
{
    ...
    public function build_config_form($host_obj)
    {
        $host_obj->add_field('discount_amount', 'Discount amount', 'full', db_float)
            ->comment('Please specify a percentage value to subtract from the product original price. ', 'above')
            ->validation()
    ...
    public function eval_amount(&$params, $host_obj)
    {
        if (!array_key_exists('current_price', $params)) {
            throw new ApplicationException(
                'Error applying the "By percentage of the original price" 
                price rule action: the current_price element is not found in the action parameters.'
            );
        }
