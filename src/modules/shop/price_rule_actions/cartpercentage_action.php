...
class CartPercentage_Action extends CartRuleActionBase
{
    ...
    public function build_config_form($host_obj)
    {
        $host_obj->add_field('discount_amount', 'Discount amount', 'full', db_float)
            ->comment(
                'Please specify a percentage value to subtract from the shopping cart original subtotals. ',
                'above'
            )
            ->validation()
    ...
    public function eval_discount(
        ...
        if (!array_key_exists('current_subtotal', $params)) {
            throw new ApplicationException(
                'Error applying the "Discount the shopping cart subtotal by X percents" price rule action: 
                the current_subtotal element is not found in the action parameters.'
            );
        }
        ...
        foreach ($cart_items as $item) {
            ...
            if ($include_tax) {
                $taxAmount = TaxClass::get_total_tax($item->get_tax_class_id(), $discount_value);
                $total_discount_incl_tax = $taxAmount + $discount_value;
            }
