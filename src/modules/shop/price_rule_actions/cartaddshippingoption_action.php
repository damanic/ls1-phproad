...
class CartAddShippingOption_Action extends CartRuleActionBase
{
    ...
    {
        $host_obj->add_field('shipping_option', 'Shipping Option', 'full', db_number)
            ->renderAs(frm_dropdown)
            ->comment(
                'The options listed are normally hidden from customers, 
                selecting the one you would like to present to the customer',
                'above'
            )
            ->validation()
            ->required('Please specify shipping option');
    }

    public function get_shipping_option_options($host_obj)
    {
        $shipping_options = ShippingOption::create()->where('backend_enabled = 1 AND enabled IS NULL')->find_all();
    ...
    public function eval_discount(
        ...
        if ($host_obj->shipping_option) {
            ...
            $shippingOptionId = $host_obj->shipping_option;
            if (is_object($shippingOptionId)) {
                $shippingOptionId = $shippingOptionId->id;
            }
            $selected_shipping_option = CheckoutData::get_shipping_method();
            if ($selected_shipping_option && $selected_shipping_option->id == $shippingOptionId) {
