...
class Taxes_Report extends GenericReport
{
    ...
    public $filter_filters = [
        ...
        'shipping_state' => [
            'name' => 'Shipping State',
            'class_name' => 'Shop\CustomerShippingStateFilter',
            'prompt' =>
                'Please choose shipping states you want to include to the list. 
                Orders with other shipping states will be hidden.',
            'added_list_title' => 'Added States'
        ],
        'shipping_zone' => [
            'name' => 'Shipping Zone',
    ...
    public function getChartData()
    {
        ...
        if ($chartType == ChartController::rt_column || $chartType == ChartController::rt_pie) {
            ...
            $query = "
				...
				";
            $orders = DbHelper::queryArray($query);
            if (count($orders)) {
                $taxes = array();
                ...
                if (count($taxes)) {
                    ...
                    foreach ($taxes as $tax_name => $tax_value) {
                        $new = array(
                            ...
                            'series_value' => 'serie',
                            'graph_name' => $tax_name,
                            'record_value' => $tax_value['value']
                        );
                        array_push($data, (object)$new);
                    }
                }
            }
        } elseif ($chartType == ChartController::rt_line) {
            ...
            $series_query = "
					...
					order by report_date
				";
            $series = DbHelper::queryArray($series_query);
            $orders = DbHelper::queryArray($query);
            if (count($orders) && count($series)) {
                ...
                foreach ($orders as $order) {
                    if ($order['shipping_tax_name_1']) {
                        $taxes = $this->add_to_tax_day(
                            $taxes,
                            $order['shipping_tax_name_1'],
                            $order['shipping_tax_1'],
                            $order['report_date']
                        );
                    }
                    if ($order['shipping_tax_name_2']) {
                        $taxes = $this->add_to_tax_day(
                            $taxes,
                            $order['shipping_tax_name_2'],
                            $order['shipping_tax_2'],
                            $order['report_date']
                        );
                    }
                    $sales_taxes = unserialize($order['sales_taxes']);
