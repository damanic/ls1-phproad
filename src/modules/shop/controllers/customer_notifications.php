...
class Customer_Notifications extends Controller
{
    public $implement = 'Db_FormBehavior';

    public $form_preview_title = 'Notification';
    public $form_create_title = 'Send Customer Message';
    public $form_model_class = 'Shop\CustomerNotification';
    ...
    public function create_formBeforeRender($model)
    {
        ...
        if ($models['notification']) { //copy
            ...
            if (!$customer && is_numeric($src_message->customer_id)) {
                $customer = Customer::create()->find($src_message->customer_id);
            }
    ...
    protected function create_onSave()
    {
        try {
            ...
            $customer = $models['customer'];
            if (!$order && !$customer) {
                throw new ApplicationException('Cannot send notification: Missing Order/Customer association');
            }
            if (!$customer) {
                $customer = $order->customer;
            }
    ...
    protected function apply_order_variables($message, $order)
    {
        ...
        $message = ModuleManager::applyEmailVariables($message, $order, $order->customer);
    ...
    protected function get_models()
    {
        ...
        if (is_numeric($template_id)) {
            $template = EmailTemplate::create()->find($template_id);
            if ($template) {
