...
class Orders_Report extends GenericReport
{
    ...
    public $filter_filters = [
        ...
        'products' => [
            'name' => 'Product',
            'class_name' => 'Shop_OrderProductFilter',
            'prompt' =>
                'Please choose products you want to include to the report. 
                Orders that don\'t contain the selected products will be excluded.',
    ...
    protected $timeline_charts = [ChartController::rt_line];
    ...
    protected function getChartData()
    {
        ...
        $result = array(
            ...
            'series' => DbHelper::objectArray($series_query, $bind)
        );
        return $result;
    }
    ...
    protected function renderReportTotals()
    {
        ...
        $query_str = "from shop_orders, shop_order_statuses, shop_customers
			where shop_customers.id=customer_id 
        ...
        $query = "
				...
				(select sum(
                    (total - goods_tax - shipping_tax - shipping_quote - ifnull(total_cost, 0)
                    )* shop_orders.shop_currency_rate) $query_str
                 ) as revenue,
    ...
    public static function get_totals_chart_data($start, $end)
    {
        ...
        if ($settings->dashboard_paid_only) {
            $status_filter = "and (exists (select shop_order_status_log_records.id 
            from shop_order_status_log_records, shop_order_statuses 
            where shop_order_status_log_records.order_id = shop_orders.id 
            and shop_order_statuses.id=shop_order_status_log_records.status_id 
            and shop_order_statuses.code='paid'))";
        }
        ...
        $query = "
				select
					...
					backend_report_dates.report_date as series_value,
					sum(shop_orders.total * shop_orders.shop_currency_rate) as record_value
				from 
					backend_report_dates
				left join shop_orders on report_date = shop_orders.order_date 
				                             and shop_orders.deleted_at is null $status_filter
    ...
    public static function evalTotalsStatistics($start, $end)
    {
        ...
        return DbHelper::object('
				select 
				(select sum(total * shop_orders.shop_currency_rate) 
				    from shop_orders 
				    where shop_orders.order_date >= :current_start 
				    and shop_orders.order_date <= :current_end 
				    and deleted_at is null) as totals_current,
				(select sum(total * shop_orders.shop_currency_rate) 
				    from shop_orders 
				    where shop_orders.order_date >= :prev_start 
				    and shop_orders.order_date <= :prev_end 
				    and deleted_at is null) as totals_prev
			', [
            'current_start' => $start,
            'current_end' => $end,
    ...
    public static function evalPaidOrdersStatistics($start, $end)
    {
        ...
        $status_filter = "and (exists (
            select shop_order_status_log_records.id 
            from shop_order_status_log_records, shop_order_statuses 
            where shop_order_status_log_records.order_id = shop_orders.id 
            and shop_order_statuses.id=shop_order_status_log_records.status_id 
            and shop_order_statuses.code='paid'))";
        ...
        $result = DbHelper::object('
				...
				(select sum(total * shop_orders.shop_currency_rate) 
				    ...
				    where shop_orders.order_date >= :current_start 
				    and shop_orders.order_date <= :current_end 
				    and deleted_at is null ' . $status_filter . ') as totals_current,
				(select sum(total * shop_orders.shop_currency_rate) 
				    from shop_orders 
				    where shop_orders.order_date >= :prev_start 
				    and shop_orders.order_date <= :prev_end 
				    and deleted_at is null ' . $status_filter . ') as totals_prev
			', [
            'current_start' => $start,
            'current_end' => $end,
            'prev_start' => $prevStart,
