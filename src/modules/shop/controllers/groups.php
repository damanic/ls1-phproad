...
class Groups extends Controller
{
    ...
    public $list_model_class = 'Shop\CustomGroup';
    ...
    public $form_model_class = 'Shop\CustomGroup';
    ...
    public function __construct()
    {
        ...
        if (post('add_product_mode')) {
            ...
            $partialsDir = PATH_APP . '/phproad/modules/db/behaviors/db_listbehavior/partials';
            $this->list_custom_body_cells = $partialsDir . '/_list_body_cb.htm';
            $this->list_custom_head_cells = $partialsDir . '/_list_head_cb.htm';
    ...
    public function prepare_product_list()
    {
        ...
        $products = DbHelper::objectArray($product_obj->build_sql());
    ...
    protected function onSortAlphabetically($id = null)
    {
        try {
            ...
            $products = DbHelper::objectArray(
                'select * from shop_products where id in (?)',
                [$productIds]
            );
            usort($products, array('Shop\Groups', 'cmp_products'));
    ...
    protected function onAddProducts($id = null)
    {
        try {
            ...
            $products = Product::create()->where('id in (?)', array($id_list));
            if (count($added_ids)) {
    ...
    protected function onRemoveSelectedProducts($id = null)
    {
        try {
            ...
            $id_list = post('list_ids', array());
            if (count($id_list)) {
                $products = Product::create()->where('id in (?)', array($id_list))->find_all();
    ...
    protected function get_product_list($form_model)
    {
        ...
        uasort($product_list, array('Shop\Groups', 'sortProducts'));
    ...
    {
        return strlen($id) ? CustomGroup::create()->find($id) : CustomGroup::create();
    }
}
