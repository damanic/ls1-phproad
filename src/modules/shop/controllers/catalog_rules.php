...
class Catalog_Rules extends Controller
{
    ...
    public $form_model_class = 'Shop\CatalogPriceRule';
    ...
    protected function onUpdateAction($rule_id)
    {
        try {
            if (strlen($rule_id)) {
                $rule_obj = CatalogPriceRule::create()->find($rule_id);
                if (!$rule_obj) {
                    throw new ApplicationException('Rule not found');
                }
            } else {
                $rule_obj = new CatalogPriceRule();
            }

            $params = post(get_class_id('Shop\CatalogPriceRule'), array());
    ...
    {
        $data = post(get_class_id('Shop\CatalogPriceRule'), array());
