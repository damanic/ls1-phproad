...
class Customers extends Controller
{
    ...
    public $list_model_class = 'Shop\Customer';
    ...
    public $form_model_class = 'Shop\Customer';
    ...
    public $csv_import_db_columns_header = 'LSAPP Customer Columns';
    public $csv_import_data_model_class = 'Shop\Customer';
    public $csv_import_config_model_class = 'Shop\CustomerCsvImportModel';
    ...
    public $filter_filters = [
        'billing_country' => [
            ...
            'class_name' => 'Shop\CustomerBillingCountryFilter',
            'prompt' =>
                'Please choose billing countries you want to include to the list. 
                Customers with other billing countries will be hidden.',
            'added_list_title' => 'Added Countries'
        ],
        'billing_state' => [
            'name' => 'Billing State',
            'class_name' => 'Shop\CustomerBillingStateFilter',
            'prompt' =>
                'Please choose billing states you want to include to the list. 
                Customers with other billing states will be hidden.',
            'added_list_title' => 'Added States'
        ],
        'customer_group' => [
            'name' => 'Customer Group',
        ...
        'customer_deleted_status' => [
            'name' => 'Status',
            'class_name' => 'Shop\DeletedFilter',
            'prompt' => 'Please choose whether you want to see only deleted or only active customers.',
    ...
    protected function index_onLoadMergeCustomersForm()
    {
        try {
            ...
            if (count($customer_ids) < 2) {
                throw new ApplicationException('Please select at least 2 customers to merge.');
            }

            $model = new MergeCustomersModel();
    ...
    protected function preview_onShowConvertForm($customer_id)
    {
        try {
            ...
            if (Customer::find_registered_by_email($obj->email)) {
                throw new ApplicationException("Registered customer with email {$obj->email} already exists.");
            }

            $this->viewData['groups'] = CustomerGroup::create()->order('name')->where(
    ...
    protected function preview_onResetPassword($customer_id)
    {
        try {
            ...
            if (!$customer || $customer->guest) {
                throw new ApplicationException('Cannot reset customers password, the account is not registered.');
            }

            Customer::reset_password($customer->email);
    ...
    {
        return Customer::create()->requestRowCount();
    }

    protected function evalGuestCustomerNum()
    {
        return Customer::create()
            ->where('shop_customers.guest is not null and shop_customers.guest=1')
            ->requestRowCount();
    }

    protected function evalRegisteredCustomerNum()
    {
        return Customer::create()->where('(shop_customers.guest is null or shop_customers.guest=0)')->requestRowCount();
    }
    ...
    protected function index_onDeleteSelected()
    {
        ...
        foreach ($customer_ids as $customer_id) {
            ...
            try {
                ...
                if (!$customer) {
                    throw new ApplicationException('Customer with identifier ' . $customer_id . ' not found.');
                }
                ...
                if (!$customer) {
                    ...
                    $msg = 'Error deleting customer "' . $customer->get_display_name() . '": ' . $ex->getMessage();
        ...
        foreach ($customer_ids as $customer_id) {
            ...
            try {
                ...
                if (!$customer) {
                    ...
                    $msg = 'Error restoring customer "' . $customer->get_display_name() . '": ' . $ex->getMessage();
                    Phpr::$session->flash['error'] = $msg;
                }
    ...
    protected function onUpdateStatesList()
    {
        ...
        $data = post(get_class_id('Shop\Customer'));
    ...
    public function edit_onDelete($recordId)
    {
        try {
            ...
            if ($this->enable_concurrency_locking && ($lock = RecordLock::lock_exists($obj))) {
            ...
            if ($this->enable_concurrency_locking && !RecordLock::lock_exists($obj)) {
                RecordLock::unlock_record($obj);
            }
