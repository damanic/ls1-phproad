...
class Cart_Rules extends Controller
{
    ...
    public $form_model_class = 'Shop\CartPriceRule';
    ...
    public $rules_model_class = 'Shop\CartPriceRule';
    public $rules_edit_url = 'shop/cart_rules/edit';
    public $conditions_rule_type = PriceRuleCondition::type_cart;
    ...
    {
        $data = post(get_class_id('Shop\CartPriceRule'), array());
        $action_class_name = $model->action_class_name;
        $model->action_class_name = $data['action_class_name'] ?? $action_class_name;
    }
    ...
    protected function onUpdateAction($rule_id)
    {
        try {
            if (strlen($rule_id)) {
                $rule_obj = CartPriceRule::create()->find($rule_id);
                if (!$rule_obj) {
                    throw new ApplicationException('Rule not found');
                }
            } else {
                $rule_obj = new CartPriceRule();
            }
                
            $params = post(get_class_id('Shop\CartPriceRule'), array());
    ...
    public function conditions_get_new_rule_type($condition)
    {
        if ($condition->host_rule_set != 'products_conditions') {
            ...
            return PriceRuleCondition::type_cart_products;
        }
    }
    ...
    protected function onAddCoupon($id)
    {
        try {
            ...
            $coupon->define_form_fields();
            $coupon->save(post(get_class_id('Shop\Coupon')));

            $form_model = $this->formCreateModelObject();

            $form_model->coupon_id = $coupon->id;
            $classId = get_class_id('Shop\CartPriceRule');
