...
class Categories extends Controller
{
    ...
    public $list_model_class = 'Shop\Category';
    ...
    public $form_model_class = 'Shop\Category';
    ...
    public function __construct()
    {
        ...
        if (Phpr::$router->action == 'index') {
            $partialDir = PATH_APP . '/modules/shop/controllers/categories';
        ...
        if (Phpr::$router->action == 'manage_top_products') {
            ...
            $partialDir = PATH_APP . '/modules/shop/controllers/categories';
            $this->list_custom_body_cells = $partialDir . '/_top_product_body_cells.htm';
            $this->list_custom_head_cells = $partialDir . '/_top_product_head_cells.htm';
    ...
    protected function init_add_top_produsts_list()
    {
        ...
        $partialDir = PATH_APP . '/phproad/modules/db/behaviors/db_listbehavior/partials';
        $noDataMsg = 'There are no products in this category or all products were already added to the top list.';
        $this->list_custom_body_cells = $partialDir . '/_list_body_cb.htm';
        $this->list_custom_head_cells = $partialDir . '/_list_head_cb.htm';
        ...
        $this->list_no_data_message = $noDataMsg;
    ...
    protected function index_onDeleteSelected()
    {
        ...
        foreach ($item_ids as $item_id) {
            $item = null;
            try {
                $item = Category::create()->find($item_id);
                if (!$item) {
                    throw new ApplicationException('Category with identifier ' . $item_id . ' not found.');
                }

                $category_children_id_count = count(Category::get_children_ids($item_id));
                ...
                if (!$item) {
                    ...
                    $msg = 'Error deleting category "' . $item->name . '": ' . $ex->getMessage();
    ...
    public function getPreviewUrl()
    {
        ...
        $product_page = Page::create()->find_by_action_reference('shop:category');
        if (!$product_page) {
    ...
    public function prepare_top_product_list($model, $options)
    {
        ...
        $model->where(
            ' exists (
                select * from shop_products_categories where shop_product_id=shop_products.id 
                and shop_category_id=? and product_category_sort_order is not null
                )',
            $category_id
        );
        $model->order(
            '(select product_category_sort_order from shop_products_categories 
                where shop_product_id=shop_products.id 
                and shop_category_id=' . $category_id . ' 
                and product_category_sort_order is not null)'
        );
    ...
    protected function get_top_product_sort_order($product_id)
    {
        ...
        if ($this->top_product_order_cache == null) {
            $this->top_product_order_cache = Category::get_top_products_orders($category_id);
        }
    ...
    {
        try {
            Category::set_top_orders($category_id, post('item_ids'), post('sort_orders'));
        } catch (\Exception $ex) {
    ...
    protected function manage_top_products_onRemoveSelected($category_id)
    {
        try {
            ...
            if (preg_match('/^[0-9]+$/', trim($category_id))) {
                $id_list = post('list_ids', array());
                if (!count($id_list)) {
                    throw new ApplicationException('Please select product(s) to remove.');
                }
    ...
    protected function setup_reorder_categories_list()
    {
        ...
        $this->list_no_setup_link = true;
        $this->list_search_enabled = false;
        $this->list_custom_head_cells = $partialDir . '/_categories_handle_head_col.htm';
        $this->list_custom_body_cells = $partialDir . '/_categories_handle_body_col.htm';
    }

    protected function reorder_categories_onSetOrders()
    {
        try {
            Category::set_orders(post('item_ids'), post('sort_orders'));
        } catch (\Exception $ex) {
            Phpr::$response->ajaxReportException($ex, true, true);
        }
    }
