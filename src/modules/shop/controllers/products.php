...
class Products extends Controller
{
    ...
    public $list_model_class = 'Shop\Product';
    public $list_record_url = null;
    public $list_options = [];
    public $list_name = null;
    public $list_data_context = null;
    public $list_cell_individual_partial = [
    ...
    public $form_model_class = 'Shop\Product';
    ...
    public $list_search_fields = [
        'shop_products.name',
        'shop_products.sku',
        '(select group_concat(sku) from shop_products sku_list where sku_list.product_id is not null and sku_list.product_id=shop_products.id)',
    ...
    public $csv_import_db_columns_header = 'LSAPP Product Columns';
    public $csv_import_data_model_class = 'Shop\Product';
    public $csv_import_config_model_class = 'Shop\ProductCsvImportModel';
    ...
    public $filter_filters = [
        ...
        'manufacturers' => [
            ...
            'prompt' =>
                ...
                Products of other manufacturers will be excluded.',
            'added_list_title' => 'Added Manufacturers'
        ]
    ];

    public $filter_switchers = [
        'hide_disabled' => [
            'name' => 'Hide disabled products',
            'class_name' => 'Shop\HideDisabledProductsSwitcher'
        ],
        'show_withbundleoffers' => [
            'name' => 'Products with bundle offers',
            'class_name' => 'Shop\DisplayProductBundleOfferSwitcher'
        ],
    ];
    ...
    protected $required_permissions = ['shop:manage_products'];

    public function __construct()
    {
        $this->filebrowser_dirs['resources']['path'] = '/' . SettingsManager::get()->resources_dir_path;

        $importCsvFlag = post('import_csv_flag');
        $importCsvAction = in_array(Phpr::$router->action, ['import_csv', 'import_csv_get_config']);
        if ($importCsvAction || $importCsvFlag) {
    ...
    public function listGetRowClass($model)
    {
        ...
        if ($model instanceof Product) {
            ...
            $class = $result . ($model->enabled ? null : 'disabled');
        }
        return $class;
    }
    ...
    {
        return Product::create()->where('grouped is null')->requestRowCount();
    }
    ...
    {
        Product::update_page_reference($model);
        ...
        foreach ($in_stock_values as $product_id => $value) {
            ...
            $value = trim($value);
            if (preg_match('/^[0-9]+$/', $value)) {
                Product::set_product_units_in_stock($product_id, $value);
            }
        }
            ...
            foreach ($properties as $property) {
                if ($property->get_property_set_property()) {
                    try {
                        ...
                        $msg = 'WARNING: The property value given for "' . $property->name . '" is not valid! ';
    ...
    {
        $this->viewData['form_model'] = Product::create()->find($model->id);
    ...
    public function getPreviewUrl()
    {
        ...
        $product_page = Page::create()->find_by_action_reference('shop:product');
        if (!$product_page) {
    ...
    protected function index_onDuplicateSelected()
    {
        ...
        foreach ($product_ids as $product_id) {
            ...
            try {
                ...
                if (!$product) {
                    throw new ApplicationException('Product with identifier ' . $product_id . ' not found.');
                }
                ...
                if (!$product) {
                    ...
                    $msg = 'Error duplicating product "' . $product->name . '": ';
                    Phpr::$session->flash['error'] = $msg . $ex->getMessage();
                }
        ...
        foreach ($product_ids as $product_id) {
            ...
            try {
                ...
                if (!$product) {
                    ...
                    $msg = 'Error deleting product "' . $product->name . '": ';
                    Phpr::$session->flash['error'] = $msg . $ex->getMessage();
                }
    ...
    protected function index_onLoadEnableDisableProductsForm()
    {
        try {
            ...
            if (!count($product_ids)) {
                throw new ApplicationException('Please select product(s) to enable or disable.');
            }
        ...
        foreach ($product_ids as $product_id) {
            ...
            try {
                ...
                if (!$product) {
                    ...
                    $msg = 'Error updating product "' . $product->name . '": ';
                    Phpr::$session->flash['error'] = $msg . $ex->getMessage();
                }
    ...
    public function index_onLoadGroupProductsForm()
    {
        try {
            ...
            if (count($product_ids) < 2) {
                throw new ApplicationException('Please select at least 2 products to group.');
            }

            $model = new GroupProductsModel();
    ...
    public function formCreateModelObject()
    {
        ...
        if ($context != 'grouped') {
            ...
            $obj->tax_class_id = TaxClass::get_default_class_id();
        } else {
    ...
    public function formFindModelObject($id, $copy_relations = false)
    {
        ...
        if (!$obj) {
            throw new ApplicationException($this->form_not_found_message);
        }
    ...
    protected function initGroupedProduct($parent_id, $copy_relations = false)
    {
        ...
        if ($grouped_id) {
            $obj = Product::create()->find($grouped_id);
            ...
            /*
                 * Copy regular fields
             */
            $data = post(get_class_id('Shop\Product'), []);
            ...
            foreach ($data as $field => $value) {
                ...
                if ($column_info && $column_info->type == db_date) {
                    ...
                    $obj->$field = PhprDateTime::parse(trim($value), $column_obj->getDateFormat(), $timeZoneObj);
                } elseif ($column_info && $column_info->type == db_datetime) {
                    $column_obj = $obj->find_column_definition($field);
                    $obj->$field = PhprDateTime::parse(
                        trim($value),
                        $column_obj->getDateFormat() . ' ' . $column_obj->getTimeFormat(),
                        $timeZoneObj
                    );
                } else {
                    $obj->$field = $value;
    ...
    {
        try {
            $new_product_order = Product::set_orders(post('item_ids'), post('sort_orders'));
    ...
    protected function onAddGroupedProduct($parentId = null)
    {
        try {
            ...
            $grouped_product = Product::create();
            if ($grouped_id) {
                $grouped_product = $grouped_product->find($grouped_id);
                if (!$grouped_product) {
                    throw new ApplicationException('Grouped product not found');
                }
            }
    ...
    protected function onUpdateGroupedProductList($parentId = null)
    {
        try {
            ...
            $properties = array_key_exists(
                'properties',
                $copy_properties_values
            ) ? $copy_properties_values['properties'] : array();

            if (is_array($properties) && in_array('in_stock', $properties)) {
                $updated_product_ids = array_key_exists(
                    'list_ids',
                    $copy_properties_values
                ) ? $copy_properties_values['list_ids'] : array();
                $in_stock_values = post('in_stock_values', array());
    ...
    protected function onDeleteGroupedProduct($parentId = null)
    {
        try {
            ...
            $this->form_unique_prefix = 'grouped';
            $grouped_product = Product::create()->find(post('grouped_product_id'));
            if ($grouped_product) {
                $grouped_product->init_columns_info();
                $grouped_product->define_form_fields('grouped');
    ...
    protected function onCopyProductFields($id = null)
    {
        try {
            ...
            $product_ids = post('list_ids', array());
            $product->set_data(post(get_class_id('Shop\Product'), []));

            if (!$product_ids) {
                throw new ApplicationException('Please select products to copy fields to.');
            }

            if (!$fields) {
                throw new ApplicationException('Please select fields to copy.');
            }

            $product->copy_fields_to_grouped(post('edit_session_key'), $product_ids, $fields, $_POST);
        } catch (\Exception $ex) {
            Phpr::$response->ajaxReportException($ex, true, true);
        }
    }
    ...
    protected function onLoadCustomAttributeForm()
    {
        try {
            ...
            $attribute = $id ? CustomAttribute::create()->find($id) : CustomAttribute::create();
            if (!$attribute) {
    ...
    protected function onAddCustomAttribute($parentId = null)
    {
        try {
            ...
            $postData = post(get_class_id('Shop\CustomAttribute'));
            $attribute->save($postData, $this->formGetEditSessionKey());
            ...
            if ($attribute) {
                if (!$attribute->can_delete()) {
                    throw new ApplicationException(
                        'Cannot delete product option because there are orders referring to it.'
                    );
                }
    ...
    protected function onLoadSaveOptionsForm()
    {
        try {
            $this->viewData['session_key'] = post('edit_session_key');
            $this->viewData['trackTab'] = false;
            $obj = $this->viewData['obj'] = new OptionSet();
            $obj->init_columns_info();
            $obj->define_form_fields();
        } catch (\Exception $ex) {
            $this->handlePageError($ex);
    ...
    protected function onSaveOptionSet($parentId = null)
    {
        try {
            ...
            $data = post(get_class_id('Shop\OptionSet'), []);

            $name = trim($data['name']);
            if (!strlen($name) && !strlen(trim($data['existing_id']))) {
                $obj->validation->setError(
                    'Please specify the new option set name or select existing option set.',
                    'name',
                    true
                );
            }
    ...
    protected function onDeleteOptionSet()
    {
        try {
            ...
            $obj = OptionSet::create()->find($data['existing_id']);
            if ($obj) {
                $obj->delete();
            }
        } catch (\Exception $ex) {
    ...
    protected function onLoadOptionSet($parentId = null)
    {
        try {
            ...
            if (!strlen(trim($data['existing_id']))) {
                throw new ApplicationException('Please select an option set.');
            }
            ...
            foreach ($obj->options as $option) {
                ...
                $new_option = CustomAttribute::create();
    ...
    {
        CustomAttribute::create()->set_item_orders(post('item_ids'), post('sort_orders'));
    }
    ...
    protected function onLoadExtraOptionForm()
    {
        try {
            ...
            $option = $id ? ExtraOption::create()->find($id) : ExtraOption::create();
            if (!$option) {
                throw new ApplicationException('Option not found');
            }
    ...
    protected function onAddExtraOption($parentId = null)
    {
        try {
            ...
            $option->define_form_fields();
            $option->save(post(get_class_id('Shop\ExtraOption')), $this->formGetEditSessionKey());
    ...
    {
        try {
            ExtraOption::set_orders(post('item_ids'), post('sort_orders'));
        } catch (\Exception $ex) {
            Phpr::$response->ajaxReportException($ex, true, true);
        }
    }
    ...
    protected function onLoadLoadExtraOptionsForm($parent_id)
    {
        try {
            $this->viewData['session_key'] = post('edit_session_key');
            $this->viewData['trackTab'] = false;
            $obj = $this->viewData['obj'] = new ExtraOptionSet();
            $obj->init_columns_info();
            $obj->define_form_fields('load');
        } catch (\Exception $ex) {
            $this->handlePageError($ex);
    ...
    {
        try {
            $data = post(get_class_id('Shop\ExtraOptionSet'), []);
            if (!strlen(trim($data['existing_id']))) {
                throw new ApplicationException('Please select an option set.');
            }
            ...
            $obj = ExtraOptionSet::create()->find($data['existing_id']);
            if (!$obj) {
    ...
    protected function onSetExtraGroupName()
    {
        try {
            ...
            if (!strlen($group_name)) {
                throw new ApplicationException('Please specify the group name');
            }

            $form_model = ExtraOption::create();
            $form_model->define_form_fields();
            $form_model->group_name = $group_name;

            $this->form_unique_prefix = post('form_prefix');
            $classId = get_class_id('Shop\ExtraOption');
            $this->preparePartialRender(post('form_prefix') . 'form_field_container_group_name' . $classId);
    ...
    protected function onLoadPriceTierForm()
    {
        try {
            ...
            $tier = $id ? PriceTier::create()->find($id) : PriceTier::create();
            if (!$tier) {
                throw new ApplicationException('Price tier not found');
            }
    ...
    protected function onUpdatePriceTier($parentId = null)
    {
        try {
            ...
            $tier->define_form_fields();
            $tier->save(post(get_class_id('Shop\PriceTier')), $this->formGetEditSessionKey());
    ...
    protected function onAddRelatedProducts($parentId = null)
    {
        try {
            ...
            if (!count($related)) {
                throw new ApplicationException('Please select product(s) to add.');
            }

            $product = $this->getProductObj($parentId);
            $related_products = Product::create()->where('id in (?)', array($related))->find_all();
    ...
    protected function onRemoveRelatedProduct($parentId = null)
    {
        try {
            ...
            $related_product = Product::create()->find($id);
            if ($related_product) {
    ...
    public function prepare_related_product_list()
    {
        ...
        $related_products = $obj->list_related_records_deferred(
            'related_products_all',
            post('edit_session_key', $this->formGetEditSessionKey())
        );
    ...
    protected function onLoadPropertyForm()
    {
        try {
            $id = post('property_id');
            $property = $id ? ProductProperty::create()->find($id) : ProductProperty::create();
            if (!$property) {
    ...
    protected function onAddProperty($parentId = null)
    {
        try {
            $id = post('property_id');
            $post_data = post(get_class_id('Shop\ProductProperty'));
            ...
            if ($property->api_code) {
                $set_property = PropertySetProperty::create()->find_by_api_code($property->api_code);
                if ($set_property) {
    ...
    {
        ProductProperty::create()->set_item_orders(post('item_ids'), post('sort_orders'));
    }
    ...
    protected function onLoadSavePropertiesForm()
    {
        try {
            $this->viewData['session_key'] = post('edit_session_key');
            $this->viewData['trackTab'] = false;
            $obj = $this->viewData['obj'] = new PropertySet();
            $obj->init_columns_info();
            $obj->define_form_fields();
        } catch (\Exception $ex) {
            $this->handlePageError($ex);
    ...
    protected function onSavePropSet($parentId = null)
    {
        try {
            ...
            $data = post(get_class_id('Shop\PropertySet'), []);

            $name = trim($data['name']);
            if (!strlen($name) && !strlen(trim($data['existing_id']))) {
                $obj->validation->setError(
                    'Please specify the new property set name or select existing property set.',
                    'name',
                    true
                );
            }
    ...
    protected function onLoadPropSet($parentId = null)
    {
        try {
            $data = post(get_class_id('Shop\PropertySet'), []);
            if (!strlen(trim($data['existing_id']))) {
                throw new ApplicationException('Please select a property set.');
            }

            $product = $this->getProductObj($parentId);
            $properties = $product->list_related_records_deferred('properties', post('product_session_key'));

            $property_set = PropertySet::create()->find($data['existing_id']);
            if (!$property_set) {
            ...
            foreach ($property_set->properties as $property_set_property) {
                foreach ($properties as $existing_property) {
                    ...
                    } elseif ($existing_property->api_code) {
                    ...
                    if ($prop_from_set) {
                        ...
                        continue 2;
                    } elseif ($existing_property->name == $property_set_property->name) {
                        continue 2;
                    }
                }

                $new_property = ProductProperty::create();
    ...
    protected function onDeletePropertySet()
    {
        try {
            ...
            $obj = PropertySet::create()->find($data['existing_id']);
            if ($obj) {
                $obj->delete();
            }
        } catch (\Exception $ex) {
    ...
    protected function onUpdatePropertyValues()
    {
        ...
        $property->define_columns('form');
        $property->define_form_fields();
        $data = post(get_class_id('Shop\ProductProperty'), []);
    ...
    {
        try {
            $this->form_model_class = 'Shop\Manufacturer';
            $this->resetFormEditSessionKey();
    ...
    {
        $data = post(get_class_id('Shop\Manufacturer'));

        $form_model = Manufacturer::create();
        $form_model->define_form_fields();
    ...
    protected function onAddManufacturer($id = null)
    {
        try {
            ...
            $form_model->save(post(get_class_id('Shop\Manufacturer')), $this->formGetEditSessionKey());
            UserParameters::set('manufacturer_def_country', $form_model->country_id);
            UserParameters::set('manufacturer_def_state', $form_model->state_id);

            $product = $this->getProductObj($id);

            $product->manufacturer_id = $form_model->id;
            $classId = get_class_id('Shop\Product');
            echo ">>form_field_container_manufacturer_id$classId<<";
    ...
    {
        $data = ProductStatisticsData::sales_chart_data($product_id);
        return array(
    ...
    public function preview_grouped_chart_data($product_id)
    {
        ...
        $this->viewData['chart_data'] = ProductStatisticsData::grouped_chart_data($product_id);
    }
    ...
    protected function preview_onApproveSelectedReviews($product_id)
    {
        try {
            ...
            foreach ($ids as $id) {
                $review = ProductReview::create()->find($id);
                if (!$review) {
                    throw new ApplicationException('Review with identifier ' . $id . ' not found.');
                }
