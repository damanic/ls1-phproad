...
class Customers_Report extends GenericReport
{
    ...
    public $filter_filters = [
        'customers_billing_country' => [
            'name' => 'Customers billing country',
            'class_name' => 'Shop\CustomerBillingCountryFilter',
            'prompt' =>
                'Please choose countries you want to include to the list. 
                Customers who do not have these countries saved as their current billing address will be hidden.',
            'added_list_title' => 'Added Countries'
        ],
        'customers_shipping_country' => [
            'name' => 'Customers shipping country',
            'class_name' => 'Shop\CustomerShippingCountryFilter',
            'prompt' =>
                'Please choose countries you want to include to the list. 
                Customers who do not have these countries saved as their current shipping address will be hidden.',
            'added_list_title' => 'Added Countries'
        ],
    ];
    ...
    {
        parent::__construct();
        $dir = PATH_APP . '/modules/shop/controllers/customers_report';
        $this->list_control_panel_partial = $dir . '/_reports_export_buttons.htm';
    }
    ...
    protected function add_calculated_columns(&$model)
    {
        ...
        $model->calculated_columns['report_orders_count'] = array(
            'sql' => "count(DISTINCT(shop_orders.id))",
            'type' => db_number
        );
        $model->calculated_columns['report_average_order_value'] = array(
            'sql' => "ROUND(AVG(shop_orders.total * shop_orders.shop_currency_rate),2)",
            'type' => db_number
        );

        if (!$this->is_interval_context_orders()) {
            $sql = "SELECT COUNT(roc.id) 
                    FROM shop_orders AS roc 
                    WHERE (" . $this->get_interval_field_query_string('roc.order_datetime') . ") 
                    AND roc.customer_id = shop_customers.id";
            $model->calculated_columns['report_orders_count'] = [
                'sql' => $sql,
                'type' => db_number
            ];

            $sql = "SELECT ROUND(AVG(raov.total * raov.shop_currency_rate),2) 
                     FROM shop_orders AS raov 
                     WHERE (" . $this->get_interval_field_query_string('raov.order_datetime') . ") 
                     AND raov.customer_id = shop_customers.id";
            $model->calculated_columns['report_average_order_value'] = [
                'sql' => $sql,
                'type' => db_number
            ];
        }
    ...
    protected function renderReportTotals()
    {
        ...
        if (in_array($this->get_interval_context(), $order_total_contexts)) {
            ...
            $totals_data = DbHelper::object($query);
        } else {
            $customer_ids = array();
            ...
            $result = DbHelper::object($query, array($customer_ids));
    ...
    protected function getOrderPaidStatusFilter($paid = true)
    {
        if (!$this->paid_order_status_id) {
            $this->paid_order_status_id = OrderStatus::get_status_paid()->id;
        }
        ...
        return "($qualifier (SELECT id FROM shop_order_status_log_records 
        WHERE order_id = shop_orders.id AND status_id=" . $this->paid_order_status_id . "))";
    }
    ...
    protected function get_customer_ids_with_no_orders_in_interval($paid = false)
    {
        ...
        return DbHelper::scalarArray($sql);
    }
}
