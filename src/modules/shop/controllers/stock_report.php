...
class Stock_Report extends GenericReport
{
    ...
    public $list_columns = ['stock_name', 'stock_sku', 'items_ordered', 'stock_in_stock'];
    ...
    public $filter_filters = [
        ...
        'product_skus' => [
            'name' => 'Product SKU',
            'class_name' => 'Shop\OrderSKUFilter',
            'prompt' =>
                'Please choose product SKUs you want to include in the report. 
                Orders that don\'t contain a product with the selected SKU reference will be excluded. 
                This filter includes SKUs assigned to option matrix selections.',
    ...
    protected $grouped_name_sql =
        "if (shop_products.grouped = 1, 
        concat(shop_products.name, ' (', shop_products.grouped_option_desc,')'), shop_products.name)";

    protected $stock_name_sql =
        "if (shop_option_matrix_records.sku = '' OR shop_option_matrix_records.sku IS NULL, :grouped_name , 
        CONCAT(:grouped_name, ' (' ,shop_option_matrix_options.option_value, ')' ))";
    ...
    public function __construct()
    {
        ...
        $path = PATH_APP . '/modules/shop/controllers/shop_stock_report/';
        $this->list_control_panel_partial = $path . '_reports_export_buttons.htm';
    }
    ...
    protected function onBeforeChartRender()
    {
        ...
        if ($chartType != ChartController::rt_column) {
            return;
        }
    }
    ...
    {
        $obj = Product::create();
        $obj->calculated_columns['items_ordered'] = [
            'sql' => "sum(shop_order_items.quantity)",
            'type' => db_number
        ];
        $obj->calculated_columns['stock_sku'] = [
            'sql' => "COALESCE(shop_option_matrix_records.sku, shop_products.sku)",
            'type' => db_varchar
        ];
        $obj->calculated_columns['stock_in_stock'] = [
            'sql' => "COALESCE(shop_option_matrix_records.in_stock, shop_products.in_stock)",
            'type' => db_number
        ];
        $obj->calculated_columns['stock_name'] = ['sql' => $this->getStockNameSql(), 'type' => db_varchar];
        ...
        $obj->join(
            ...
            'shop_order_items.option_matrix_record_id = shop_option_matrix_records.id'
        );
        $obj->join(
    ...
    public function listExtendModelObject($model)
    {
        if ($model instanceof Product) {
            $this->list_data_context = 'stock_report_data';
            $model->calculated_columns['items_ordered'] = array(
                'sql' => "sum(shop_order_items.quantity)",
                'type' => db_number
            );

            $model->define_column('stock_sku', 'SKU')->invisible();
            $model->calculated_columns['stock_sku'] = [
                'sql' => "COALESCE(shop_option_matrix_records.sku, shop_products.sku)",
                'type' => db_varchar
            ];

            $model->define_column('stock_name', 'SKU')->invisible();
            $model->calculated_columns['stock_name'] = [
                'sql' => $this->getStockNameSql(),
                'type' => db_varchar
            ];

            $model->define_column('stock_in_stock', 'Units In Stock')->invisible();
            $model->calculated_columns['stock_in_stock'] = [
                'sql' => "COALESCE(shop_option_matrix_records.in_stock, shop_products.in_stock)",
                'type' => db_number
            ];
        }
    ...
    public function getChartData()
    {
        ...
        if ($displayType == 'amount') {
            $amountField =
                'sum(((shop_order_items.price+shop_order_items.extras_price-shop_order_items.discount)
        ...
        $data_query = "
			select 
						...
						CONCAT(
						    COALESCE(shop_option_matrix_records.sku, shop_products.sku) , 
						    ' | ', 
						    " . $this->getStockNameSql() . "
						) AS graph_name, 
						'serie' as series_id, 
			...
			from 
				...
				LEFT JOIN shop_option_matrix_options 
				ON shop_option_matrix_options.matrix_record_id = shop_option_matrix_records.id
        ...
        $bind = array();
        $data = DbHelper::objectArray($data_query, $bind);
        return array(
            'data' => $data,
            'series' => $series
        );
    ...
    protected function renderReportTotals()
    {
        ...
        $query = "
				...
				(select sum(((shop_order_items.price+shop_order_items.extras_price-shop_order_items.discount)
				*shop_order_items.quantity) 
    ...
    protected function getStockNameSql()
    {
