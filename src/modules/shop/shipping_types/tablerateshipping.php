...
class TableRateShipping extends ShippingType
{
    ...
    public function get_info()
    {
        return array(
            'name' => 'Table Rate',
            'description' =>
                'Allows to configure shipping quotes depending on customer location. 
                You do not need any shipping service accounts in order to use this method.'
        );
    }
    ...
    protected function get_state_list($country_code, $term)
    {
        ...
        $states = DbHelper::objectArray(
            'select shop_states.code as state_code, shop_states.name
    ...
    public function validate_config_on_save($host_obj)
    {
        ...
        foreach ($input_rates as $row_index => &$rates) {
            ...
            if (!strlen($country)) {
                $host_obj->field_error('rates', 'Please specify country code. Available codes are: ' . implode(
                        ', ',
                        $country_codes
            ...
            if (!array_key_exists($country, $countries) && $country != '*') {
                $host_obj->field_error('rates', 'Invalid country code. Available codes are: ' . implode(
                        ', ',
                        $country_codes
                    ) . '. Line: ' . $line_number, $row_index, 'country');
            }
            ...
            if ($country != '*') {
                ...
                $stateCodeString = implode(', ', $state_codes);
                $state = $rates['state'] = trim(mb_strtoupper($rates['state']));
                if (!strlen($state)) {
                    $stateCodeString = implode(', ', $state_codes);
                    $host_obj->field_error(
                        'rates',
                        'Please specify state code. State codes, available for ' . $country_obj->name . ' are: '
                        . $stateCodeString . '. Line: ' . $line_number,
                        $row_index,
                        'state'
                    );
                }
                ...
                if (!in_array($state, $state_codes) && $state != '*') {
                    $host_obj->field_error(
                        'rates',
                        'Invalid state code. State codes, available for ' . $country_obj->name . ' are: '
                        . $stateCodeString . '. Line: ' . $line_number,
                        $row_index,
                        'state'
                    );
                ...
                if (!strlen($state) || $state != '*') {
                    $host_obj->field_error(
                        'rates',
                        'Please specify state code as wildcard (*) to indicate "Any state in any country" condition. 
                        Line: ' . $line_number,
                        $row_index,
                        'state'
                    );
            ...
            foreach ($numeric_fields as $code => $name) {
                ...
                if ($value_specified) {
                    if (!Number::isValidFloat($value)) {
                        $host_obj->field_error(
                            'rates',
                            'Invalid numeric value in column ' . $name . '. Line: ' . $line_number,
                            $row_index,
                            $code
                        );
                    }
                ...
                if ($index % 2) {
                    if ($value_specified && !strlen($prev_value)) {
                        $host_obj->field_error(
                            'rates',
                            'Please specify both minimum and maximum value. Line: ' . $line_number,
                            $row_index,
                            $prev_code
                        );
                    }
                    ...
                    if ($prev_value > $value) {
                        $host_obj->field_error(
                            'rates',
                            'Minimum value must be less than maximum value. Line: ' . $line_number,
                            $row_index,
                            $code
                        );
                    }
            ...
            if (!strlen($price)) {
                $host_obj->field_error(
                    'rates',
                    'Please specify shipping rate. Line: ' . $line_number,
                    $row_index,
                    'price'
                );
            }
    ...
    public function get_quote($parameters)
    {
        ...
        foreach ($p['host_obj']->rates as $row) {
            ...
            $row_city = isset($row['city']) && strlen($row['city']) ? str_replace(
                '-',
                '',
                str_replace(' ', '', mb_strtoupper($row['city']))
            ) : '*';
