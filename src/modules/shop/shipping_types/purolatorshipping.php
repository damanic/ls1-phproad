...
class PurolatorShipping extends ShippingType
{
    ...
    public function get_info()
    {
        return array(
            'name' => 'Purolator',
            'description' =>
                'This shipping method allows to request quotes and shipping options from the Purolator (E-Ship). 
                You must have a Purolator account in order to use this method.'
        );
    }
    ...
    public function build_config_ui($host, $context = null)
    {
        ...
        $this->host->add_field('test_mode', 'Developer Mode')
            ...
            ->comment('Use the Purolator E-Ship Development Environment to try out API requests.', 'above', true);

        $this->host->add_field('billing_account_number', 'Billing Account Number', 'left')
            ->tab('API Credentials')
            ->comment('Required by Purolator, but no billing methods are used yet.', 'above')
            ->renderAs(frm_text)
            ->validation()
            ->required('Please specify your Billing Account Number');

        if ($context !== 'preview') {
            $this->host->add_field('api_key', 'Key', 'left')
                ->tab('API Credentials')
                ->comment('Purolator key.', 'above')
                ->renderAs(frm_text)
                ->validation()
                ->required('Please specify API user name');

            $this->host->add_field('api_key_password', 'Key Password', 'right')
                ->tab('API Credentials')
                ->comment('Purolator key password.', 'above')
        ...
        $this->host->add_field('min_weight', 'Minimum Item Weight')
            ->tab('Shipping Parameters')
            ->comment('Minimum weight for one package. Purolator requires a minimum weight of 1 lb per item.', 'above')
            ->renderAs(frm_text)
            ->validation()
            ->fn('trim')
        ...
        $this->host->add_field('max_weight', 'Maximum Item Weight')
            ->tab('Shipping Parameters')
            ->comment(
                'Maximum weight for one package. Purolator requires a maximum weight of 150 lb per item.',
                'above'
            )
            ->renderAs(frm_text)
            ->validation()
    ...
    public function get_quote($parameters)
    {
        ...
        $wdslFolder = $this->host->test_mode ? 'Development' : 'Production';

        $client = new \SoapClient(
            PATH_APP . "/modules/shop/shipping_types/purolatorshipping/wsdl/'.$wdslFolder.'/EstimatingService.wsdl",
            [
                'trace' => true,
        ...
        for ($i = 0, $l = ceil($total_weight / $this->host->max_weight); $i < $l; ++$i) {
            ...
            if (!$response) {
                throw new SystemException('Invalid response received from Purolator.');
            }
            ...
            if (isset($response->ResponseInformation->Errors) && isset($response->ResponseInformation->Errors->Error)) {
                if (is_array($response->ResponseInformation->Errors->Error)) {
                    ...
                    throw new SystemException(
                        'Received error codes from Purolator: ' . implode('/', $codes) . '. 
                        Message: ' . implode(' Message: ', $messages)
                    );
                } else {
                    throw new SystemException(
                        'Received error code ' . $response->ResponseInformation->Errors->Error->Code .
                        ' from Purolator: ' . $response->ResponseInformation->Errors->Error->Description
                    );
                }
            }

            $estimates = $response->ShipmentEstimates;
            $estimates = is_array($estimates->ShipmentEstimate) ? $estimates->ShipmentEstimate : $estimates;
            foreach ($estimates as $service) {
