...
class CanadaPostShipping extends ShippingType
{
    private $host;
    ...
    public function get_info()
    {
        return array(
            'name' => 'Canada Post',
            'description' =>
                'This shipping method allows to request quotes and shipping options from the Canada Post. 
                You must have a Canada Post eParcel account in order to use this method.'
        );
    }
    ...
    public function build_config_ui($host, $context = null)
    {
        ...
        if ($context !== 'preview') {
            $host->add_field('cpcid', 'CPCID', 'left')
                ->tab('General Settings')
                ->comment(
                    'Your Canada Post 
                     <a onclick="return false" href="#" class="tooltip nolink" 
                     title="How to get your Canada Post Retailer ID<br/>
                     1. Sign up for a VentureOne or Commercial account at 
                     https://www.canadapost.ca/cpid/apps/signup<br/>
                     2. Email the Sell Online Help Desk at sellonline@canadapost.ca. 
                     Canada Post will send back a form to fill out your information<br/>
                     3. Fill out and send back the form.<br/>
                     4. When approved, use the retailer ID provided by Canada Post. 
                     Follow the instructions in the email to setup your shipping profile at Canada Post.">
                     Retailer ID</a>. 
                     More information about Sell Online can be found on 
                     <a href="http://sellonline.canadapost.ca/FAQ.html" target="_blank">this page</a>.',
                    'above',
                    true
                )
                ->renderAs(frm_text)
                ->validation()
                ->required('Please specify your CPCID');
        }

        $host->add_field('turn_around_time', 'Turn Around Time', 'left', db_number)
            ->tab('General Settings')
            ->comment(
                'The estimated hours between an order being placed and then being shipped. 
                Basically, adds hours to and re-calculates the estimated delivery date. 
                Overrides your default shipping profile setting at Canada Post. ',
                'above'
            )->renderAs(frm_text)
            ->validation();

        $host->add_field('allowed_methods', 'Allowed methods')
            ->tab('Shipping Parameters')
            ->renderAs(frm_checkboxlist)
            ->validation();

        $host->add_field('free_shipping_enabled', 'Enable free shipping option')
            ->tab('Free Shipping')
            ->renderAs(frm_checkbox)->validation();

        $host->add_field('free_shipping_option', 'Free shipping method')
            ->tab('Free Shipping')
            ->renderAs(frm_dropdown)
            ->validation();

        $host->add_field('free_shipping_min_amount', 'Minimum order amount for free shipping', 'full', db_number)
            ->tab('Free Shipping')
            ->renderAs(frm_text)
            ->validation();
    }
    ...
    public function get_quote($parameters)
    {
        ...
        $state = strlen($state_id) ? CountryState::create()->find($state_id) : null;
        ...
        if (curl_errno($ch)) {
            throw new SystemException('An error occurred communicating with the Canada Post server.');
        }
        ...
        if (!$response_code->length) {
            throw new SystemException('Invalid response received from Canada Post.');
        }
        ...
        if ($response_code->item(0)->nodeValue != 1) {
            ...
            throw new SystemException(
                'Received error code ' . $response_code->item(0)->nodeValue . ' from Canada Post: ' .
                $response_message->item(0)->nodeValue
            );
        }
