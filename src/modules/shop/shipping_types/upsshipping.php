...
class UpsShipping extends ShippingType
{
    private $host_obj;
    ...
    public function get_info()
    {
        return array(
            'name'=>'UPS',
            'description'=>
                'This shipping method allows to request quotes and 
                shipping options from the United Parcel Service of America (UPS). 
                You must have a UPS account in order to use this method.'
        );
    }
    ...
    public function build_config_ui($host_obj, $context = null)
    {
        ...
        $host_obj->add_field('test_mode', 'Developer Mode')
            ->tab('API Credentials')
            ->renderAs(frm_onoffswitcher)
            ->comment('Use the UPS Test Environment to try out API requests.', 'above', true);
            
        if ($context !== 'preview') {
            $host_obj->add_field('api_user', 'User Name', 'left')
                ->tab('API Credentials')
                ->comment('UPS API user name (case sensitive).', 'above')
                ->renderAs(frm_text)->validation()
                ->required('Please specify API user name');

            $host_obj->add_field('api_password', 'User Password', 'right')
                ->tab('API Credentials')
                ->comment('UPS API password.', 'above')
                ->renderAs(frm_password)
                ->validation();

            $host_obj->add_field('access_key', 'API Access Key')
                ->tab('API Credentials')
                ->comment('UPS API access key.', 'above')
                ->renderAs(frm_text)
                ->validation()
                ->required('Please specify API access key');
        }

        $host_obj->add_field('container', 'Container Type', 'left')
            ->tab('Shipping Parameters')
            ->renderAs(frm_dropdown)
            ->validation()
            ->required('Please specify container type');

        $host_obj->add_field('pickup_type', 'Pickup Type', 'right')
            ->tab('Shipping Parameters')
            ->renderAs(frm_dropdown)
            ->validation()
            ->required('Please specify pickup type');

        $host_obj->add_field('allowed_methods', ' Allowed methods')
            ->tab('Shipping Parameters')
            ->renderAs(frm_checkboxlist)
            ->validation();

        $host_obj->add_field('min_weight', 'Minimum Item Weight')
            ->tab('Shipping Parameters')
            ->comment(
                'The minimum weight for one package you are going to deliver. 
                Orders will be forced to have this minimum weight. 
                UPS requires a minimum weight of 1 lb.',
                'above'
            )
            ->renderAs(frm_text)
            ->validation()
        ...
        $host_obj->add_field('max_weight', 'Maximum Item Weight')
            ->tab('Shipping Parameters')
            ->comment(
                'The maximum weight for one package you are going to deliver. 
                Orders with weight exceeding this value will be split into smaller packages. 
                UPS requires a maximum weight of 150 lb.',
                'above'
            )
            ->renderAs(frm_text)
            ->validation()
        ...
        $host_obj->add_field('use_negotiated_rates', 'Use negotiated rates')
            ->comment(
                'Please specify your shipper number in the field below if you want to use the negotiated rates feature.'
            )
            ->tab('Shipping Parameters')
            ->renderAs(frm_checkbox);

        $host_obj->add_field('shipper_number', 'Shipper number')
            ->tab('Shipping Parameters')
            ->renderAs(frm_text)
            ->comment(
                'The Shipper Number is required in order to receive negotiated rates from UPS. 
                It is case sensitive and usually the same as user name.',
                'above'
            );

        $host_obj->add_field('add_insured_value', 'Add insured value')
            ->tab('Shipping Parameters')
            ->renderAs(frm_checkbox);
            
        $host_obj->add_field('free_shipping_enabled', ' Enable free shipping option')
            ->tab('Free Shipping')
            ->renderAs(frm_checkbox)
            ->validation();

        $host_obj->add_field('free_shipping_option', ' Free shipping method')
            ->tab('Free Shipping')
            ->renderAs(frm_dropdown)
            ->validation();
    ...
    public function get_quote($parameters)
    {
        ...
        $state = null;
        if (strlen($state_id)) {
            $state = CountryState::create()->find($state_id);
        }
        ...
        $rootUrl = $host_obj->test_mode ? "https://wwwcie.ups.com" : "https://www.ups.com";
        $url = $rootUrl.'/ups.app/xml/Rate';
        $ch = curl_init();
...
//          curl_setopt($ch, CURLOPT_FOLLOWLOCATION, 1);
        ...
        for ($i = 0, $l = ceil($total_weight / $this->host_obj->max_weight); $i < $l; ++$i) {
            if ($i == $l - 1) {
                ...
                $weight = ceil($total_weight % $this->host_obj->max_weight);
            } else {
            ...
            if (curl_errno($ch)) {
                throw new SystemException('An error occurred communicating with the UPS server.');
            }
            ...
            if ($response_code->item(0)->nodeValue != 1) {
                if ($response_code->item(0)->nodeValue == 0) {
                    ...
                    if ($error_text->item(0)->nodeValue) {
                        throw new ApplicationException($error_text->item(0)->nodeValue);
                    }
                }
            ...
            foreach ($rates as $rate) {
                ...
                $hasNegotiatedRate = null;
                $negotiatedRateKey = 'NegotiatedRates/NetSummaryCharges/GrandTotal/MonetaryValue';
                if ($host_obj->use_negotiated_rates) {
                    $hasNegotiatedRate = $xPath->evaluate('count('.$negotiatedRateKey.')', $rate);
                }
                    
                if ($hasNegotiatedRate) {
                    $negotiatedRateCurrencyKey = 'NegotiatedRates/NetSummaryCharges/GrandTotal/CurrencyCode';
                    $total = $xPath->query($negotiatedRateKey, $rate)->item(0)->nodeValue;
                    $currency_code = $xPath->query($negotiatedRateCurrencyKey, $rate)->item(0)->nodeValue;
                } else {
