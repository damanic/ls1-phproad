...
class UspsShipping extends ShippingType
{
    private $host_obj;
    ...
    */
    protected $domesticServiceList = [
        ...
        'EXPRESS_HOLD_FOR_PICKUP' => [
            'Express Mail Hold for Pickup',
            2,
            false
        ],
        'EXPRESS_PO_ADDRESSEE' => [
        ...
        'EXPRESS_FLAT_ENV_SUN_HOLIDAY' => [
            'Express Mail Flat-Rate Envelope Sunday/Holiday',
            25,
            false
        ],
        'EXPRESS_FLAT_ENV_PICKUP' => [
    ...
    public function get_info()
    {
        return array(
            'name' => 'USPS',
            'description' =>
                'This shipping method allows to request shipping options and quotes from USPS 
                - The United States Postal Service (U.S. Postal Service). 
                You must have a USPS account in order to use this method.'
        );
    }
    ...
    public function build_config_ui($host_obj, $context = null)
    {
        ...
        $host_obj->add_field('use_test_server', 'Use Test Server')
            ->tab('General Settings')
            ->renderAs(frm_onoffswitcher)
            ->comment(
                'Connect to USPS test server. Use this option of you have test USPS account.',
                'above'
            );

        if ($context !== 'preview') {
            $host_obj->add_field('api_user_id', 'USPS User Id')
                ->tab('General Settings')
                ->comment('This attribute specifies your Web Tools ID.', 'above')
                ->renderAs(frm_text)
                ->validation()
                ->required('Please specify USPS User Id');
        }

        $host_obj->add_field('machinable', 'All packages are machinable')
            ->tab('General Settings')
            ->renderAs(frm_checkbox);

        $host_obj->add_field('commercial_rates', 'Use commercial rates')
            ->tab('General Settings')
            ->renderAs(frm_checkbox);

        $host_obj->add_field('domestic_allowed_services', 'Allowed domestic shipping services')
            ->tab('Domestic Shipping')
            ->renderAs(frm_checkboxlist)
            ->validation();

        $host_obj->add_field('package_size', 'Package size')
            ->tab('Domestic Shipping')
            ->renderAs(frm_radio)
            ->validation()
            ->required('Please specify the package size');

        $host_obj->add_form_section(
            'Please specify the package dimensions, in inches. Dimensions are optional for the Regular package size.',
            'Package dimensions'
        )
            ->tab('Domestic Shipping');

        $host_obj->add_field('package_width', 'Width', 'left')
            ->tab('Domestic Shipping')
            ->renderAs(frm_text)
            ->validation()
            ->float();

        $host_obj->add_field('package_height', 'Height', 'right')
            ->tab('Domestic Shipping')
            ->renderAs(frm_text)
            ->validation()
            ->float();

        $host_obj->add_field('package_length', 'Length', 'left')
            ->tab('Domestic Shipping')
            ->renderAs(frm_text)
            ->validation()->float();

        $host_obj->add_field('package_girth', 'Girth', 'right')
            ->tab('Domestic Shipping')
            ->renderAs(frm_text)
            ->validation()
            ->float();

        $host_obj->add_field('intl_display_transit_time', 'Display transit time')
            ->tab('International Shipping')
            ->renderAs(frm_checkbox)
            ->comment(
                'Use this checkbox if you want the transit times returned by the
                 USPS web service to be displayed on the front-end store.'
            );

        $host_obj->add_field('international_allowed_services', 'Allowed international shipping services')
            ->tab('International Shipping')
            ->renderAs(frm_checkboxlist)
            ->validation();

        $host_obj->add_field('intl_container', 'Container shape', 'left')
            ->tab('International Shipping')
            ->renderAs(frm_radio);
    }
    ...
    {
        return is_array($this->host_obj->domestic_allowed_services) && in_array(
                $value,
                $this->host_obj->domestic_allowed_services
            );
    }
    ...
    {
        $options = array();
        foreach ($this->domesticServiceList as $id => $option) {
            $options[$id] = $option[0];
        }
    ...
    {
        $options = [
            'REGULAR' => ['Regular' => 'Package length plus girth is 84 inches or less'],
            'LARGE' => ['Large' => 'Length plus girth measure more than 84 inches but not more than 108 inches']
        ];
    ...
    {
        $options = [
            'RECTANGULAR' => ['Rectangular' => 'Package is rectangular in shape'],
            'NONRECTANGULAR' => ['Nonrectangular' => 'Package is not rectangular in shape']
        ];
    ...
    {
        return is_array($this->host_obj->international_allowed_services) && in_array(
                $value,
                $this->host_obj->international_allowed_services
            );
    }
    ...
    public function list_enabled_options($host_obj)
    {
        ...
        foreach ($domestic_options as $option_id => $option_name) {
            if ($this->get_domestic_allowed_services_option_state($option_id)) {
                $result[] = ['id' => $option_id, 'name' => $option_name . ' (domestic)'];
            }
        }
        ...
        foreach ($intl_options as $option_id => $option_name) {
            if ($this->get_international_allowed_services_option_state($option_id)) {
                $result[] = ['id' => $option_id, 'name' => $option_name . ' (intl)'];
            }
        }
    ...
    public function get_quote($parameters)
    {
        ...
        if ($shipping_params->weight_unit == 'KGS') {
            $total_weight = Number::kg_to_lb($total_weight);
        }

        $pounds = floor($total_weight);
        $ounces = round(($total_weight - $pounds) * 16, 2);

        $use_domestic = CountryLookup::get_usps_domestic($country->code);
        if ($country->code == 'US' || $use_domestic) {
            ...
            if (!$host_obj->use_test_server) {
                $service_url = "https://production.shippingapis.com/ShippingAPI.dll?API=RateV4&XML=";
            } else {
                $service_url = "https://stg-production.shippingapis.com/ShippingAPI.dll?API=RateV4&XML=";
            }
            $service_url .= urlencode($request_doc);
        } else {
            /*
                 * Process international shipping
             */

            $usps_country_name = CountryLookup::get_usps_name($country->code);
            ...
            if (!$host_obj->use_test_server) {
                $service_url = "https://production.shippingapis.com/ShippingAPI.dll?API=IntlRateV2&XML=";
            } else {
                $service_url = "https://stg-production.shippingapis.com/ShippingAPI.dll?API=IntlRateV2&XML=";
            }
            $service_url .= urlencode($request_doc);
        }
...
//          curl_setopt($ch, CURLOPT_FOLLOWLOCATION, 1);
        ...
        if (curl_errno($ch)) {
            throw new SystemException('An error occurred communicating with the USPS server.');
        } else {
            curl_close($ch);
        }
        ...
        if ($country->code == 'US' || $use_domestic) {
            ...
            if (is_array($this->host_obj->domestic_allowed_services)) {
                $allowed_services = $this->host_obj->domestic_allowed_services;
            }
            $allowed_services_classes = array();
            $allowed_service_ids = array();
            foreach ($allowed_services as $allowed_service_id) {
                if (array_key_exists($allowed_service_id, $this->domesticServiceList)) {
                    //some services only return commercial rates, only include them if commercial rates are used
                    $allowed = !$this->domesticServiceList[$allowed_service_id][2];
                    if ($host_obj->commercial_rates || (!$host_obj->commercial_rates && $allowed)) {
                        $class_id = $allowed_services_classes[] = $this->domesticServiceList[$allowed_service_id][1];
            ...
            if ($is_error->length) {
                ...
                throw new SystemException('Error requesting USPS rates. ' . $error_text);
            }
            ...
            $allowed_services_classes = array();
            if (is_array($this->host_obj->international_allowed_services)) {
                $allowed_services_classes = $this->host_obj->international_allowed_services;
            }
    ...
    protected function post_label_request($api, $document_name, $parameters)
    {
        if (!$this->host_obj->use_test_server) {
            $url = 'httpss://secure.shippingapis.com/ShippingAPI.dll?API=' . $api;
        } else {
            $url = 'httpss://stg-secure.shippingapis.com/ShippingAPI.dll?API=' . $api;
        }
...
//          curl_setopt($ch, CURLOPT_FOLLOWLOCATION, 1);
        ...
        $response = curl_exec($ch);
        if (curl_errno($ch)) {
            throw new SystemException('Error connecting to the USPS server: ' . curl_error($ch));
        } else {
            curl_close($ch);
        }

        if (!strlen($response)) {
            throw new SystemException('Error connecting to the USPS server.');
        }
        ...
        try {
            ...
            throw new SystemException('Invalid server response.');
        }
        ...
        if ($is_error->length) {
            $error_text = $xPath->query('Description', $is_error->item(0))->item(0)->nodeValue;
            throw new SystemException('USPS server returned error. ' . $error_text);
        }
        return $doc;
    }
     ...
     * @param $host_obj ActiveRecord object containing configuration fields values.
     * @param Order $order The order to generate the label form.
     * @param array $parameters Optional list of the shipping method specific parameters.
     * @return array Returns an array of ShippingLabel objects representing the shipping labels.
     */
    public function generate_shipping_labels($host_obj, $order, $parameters = array())
    {
        ...
        if (strlen($total_weight)) {
            if (!Number::isValidFloat($total_weight)) {
        ...
        if (strlen($postage)) {
            if (!Number::isValidFloat($postage)) {
        ...
        $insured_amount = array_key_exists(
            'label_insured_amount',
            $parameters
        ) ? trim($parameters['label_insured_amount']) : null;
        if (strlen($insured_amount)) {
            if (!Number::isValidFloat($insured_amount)) {
        ...
        if (strlen($label_date)) {
            if (!($label_date = PhprDateTime::parse($label_date, '%x'))) {
        ...
        $image_layout = array_key_exists(
            'label_image_layout',
            $parameters
        ) ? trim($parameters['label_image_layout']) : 'ONEPERFILE';
        $container_type = array_key_exists(
            'label_container_type',
            $parameters
        ) ? trim($parameters['label_container_type']) : 'RECTANGULAR';
        ...
        $is_machinable = array_key_exists(
            'label_is_machinable',
            $parameters
        ) ? trim($parameters['label_is_machinable']) : $host_obj->machinable;
        $is_machinable = $is_machinable ? 'true' : 'false';

        $separate_receipt = array_key_exists(
            'separate_receipt',
            $parameters
        ) ? trim($parameters['separate_receipt']) : false;
        ...
        $use_domestic = CountryLookup::get_usps_domestic($order->shipping_country->code);
        ...
        if ($order->shipping_country->code_iso_numeric == 840 || $use_domestic) {
            ...
            $response_fields = Xml::toPlainArray($doc, true);
            $tracking_num = $response_fields['DeliveryConfirmationNumber'];
            OrderTrackingCode::set_code($order, $host_obj, $tracking_num);

            $image_data = base64_decode($response_fields['DeliveryConfirmationLabel']);
            $labels[] = new ShippingLabel($image_data, strtolower($image_type), $order, $host_obj);
            if ($separate_receipt) {
                $receipt_data = base64_decode($response_fields['DeliveryConfirmationReceipt']);
                $labels[] = new ShippingLabel($receipt_data, strtolower($image_type), $order, $host_obj, 'Receipt');
            }
            return $labels;
        }
        ...
        foreach ($image_fields as $image_field) {
            if (isset($response_fields[$image_field]) && strlen($response_fields[$image_field])) {
                ...
                $result[] = new ShippingLabel($image_data, strtolower($image_type), $order, $host_obj);
            }
        }
    ...
    public function init_order_label_parameters($host_obj, $order)
    {
        ...
        $params = OrderShippingLabelParams::get_recent_order_params($host_obj);
        if ($params) {
    ...
    public function build_print_label_ui($host_obj, $order)
    {
        ...
        $host_obj->add_field('label_date', 'Label date')->renderAs(frm_date)->tab('Label')->comment(
            'Date the mail will enter the mail stream. No more than 3 days in the future. 
            Leave the field empty to use the current date.',
            'above'
        );

        $host_obj->add_field('label_image_type', 'Image type')
            ->renderAs(frm_dropdown)
            ->tab('Label');

        if (!$domestic) {
            $host_obj->add_field('label_image_layout', 'Image layout')
                ->renderAs(frm_dropdown)
                ->tab('Label');
        }

        if (!$domestic) {
            $field = $host_obj->add_field('label_comments', 'Comments')
                ->renderAs(frm_text)
                ->tab('Label')
                ->comment(
                    'Ignored when Container specified is a flat rate envelope. Maximum length is 76 symbols.',
                    'above'
                );
        ...
        if ($intl_type == 'first') {
            $host_obj->add_field('label_is_machinable', 'Package is machinable')
                ->renderAs(frm_checkbox)
                ->tab('Package');
        }
        ...
        $host_obj->add_field('label_weight', 'Weight')
            ->renderAs(frm_text)
            ->tab('Package')
            ->comment(
                'Use this field to override the package weight. 
                 The calculated package weight for this order is <strong>' . $weight_str . '</strong>.',
                'above',
                true
            );
        ...
        if (!$domestic) {
            $host_obj->add_field('label_postage', 'Postage')
                ->renderAs(frm_text)
                ->tab('Package')
                ->comment(
                    'Use this field for entering a postage amount, if known. 
                     If the field is empty, the postage will be automatically calculated. 
                     The calculated shipping cost for this order is 
                     <strong>' . $order->format_currency($order->shipping_quote) . '</strong>.',
                    'above',
                    true
                );

            $host_obj->add_field('label_container_type', 'Container type')
                ->renderAs(frm_dropdown)
                ->tab('Package');
        }
        ...
        if (!$domestic && ($intl_type == 'priority' || $intl_type == 'express')) {
            if ($intl_type == 'priority') {
                $host_obj->add_field('label_insured', 'Insured')
                    ->renderAs(frm_checkbox)
                    ->tab('Insurance');
            }

            $host_obj->add_field('label_insured_amount', 'Insured amount')
                ->renderAs(frm_text)
                ->tab('Insurance')
                ->comment(
                    'Use this tag for entering an insurance amount, if known. The value is ignored when container type 
					specified is a flat rate envelope or small flat rate box variation.',
                    'above'
                );
        }

        if ($domestic) {
            $host_obj->add_field('separate_receipt', 'Receipt in separate file')
                ->renderAs(frm_checkbox)
                ->tab('Label');
        }
    }
