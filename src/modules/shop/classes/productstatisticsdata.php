...
class ProductStatisticsData
{
     ...
     * @var int $product_id Specifies the product identifier.
     ...
     * @var int $product_id Specifies the product identifier.
     * @var int $month_limit The amount of past months to report from current date
    ...
    public static function sales_chart_data($product_id, $months = 12)
    {
        ...
        $last_sold_date = DbHelper::scalar(
            '
				select 
					order_datetime 
				from 
        ...
        $end_datetime = PhprDateTime::parse($last_sold_date);
        $end_date = Date::lastMonthDate($end_datetime)->getDate();
        $start_datetime = $end_datetime->addMonths(-$months);
        $start_date = Date::firstMonthDate($start_datetime)->getDate();
        ...
        $query = "
				...
				left join shop_orders 
				    on EXTRACT( YEAR_MONTH FROM report_date) = EXTRACT( YEAR_MONTH FROM shop_orders.order_datetime)
        ...
        $result->chart_data = DbHelper::objectArray($query, $bind);
        $result->chart_series = DbHelper::objectArray($series_query, $bind);
     ...
     * @var int $product_id Specifies the master product identifier.
    ...
    public static function grouped_chart_data($product_id)
    {
        ...
        return DbHelper::objectArray($query, array('product_id'=>$grouped_ids));
    }
    ...
    protected static function get_grouped_ids($product_id)
    {
        ...
        $grouped_ids = DbHelper::scalarArray(
            '
