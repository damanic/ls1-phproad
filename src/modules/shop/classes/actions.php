...
class Actions extends ActionScope
{
    ...
    /**
     * Base action for the {@link https://lsdomainexpired.mjman.net/docs/category_pag Product Category} page.
     * The action loads a category object by a category URL name specified in the page URL.
     *
     * @action shop:category
     *
     * @output Category $category A category object.
     ...
     * @see https://lsdomainexpired.mjman.net/docs/category_page Creating the Category Page
     */
    public function category()
    {
        if (!ConfigurationRecord::get()->nested_category_urls) {
            $category_url_name = $this->request_param(0);
        } else {
            $category_url_name = Router::remove_page_segments(strtolower(Phpr::$request->getCurrentUri()));
        }
        ...
        $params = array();
        $category = Category::find_by_url($category_url_name, $params);
        ...
        foreach ($metaMap as $pageField => $categoryField) {
            if (strlen($category->$categoryField)) {
                $this->page->$pageField = $category->$categoryField;
            }
        }
    }
    ...
    /**
     * Base action for the {@link https://lsdomainexpired.mjman.net/docs/product_page Product Details} page.
     ...
     *
     * @output Product $product A product object.
     * This variable can be NULL if a requested product was not found.
     * Always check whether the variable is NULL and display
     ...
     * @author LSAPP - MJMAN
     * @package shop.actions
     * @see https://lsdomainexpired.mjman.net/docs/product_page Product Details page
    ...
    public function product()
    {
        ...
        if (!strlen($product_id)) {
            $product = Product::create()
                ->where(
                    '(shop_products.grouped is null 
                    or 
                    (shop_products.grouped is not null and shop_products.product_id is not null))'
                )
                ->find_by_url_name($product_url_name);
        ...
        if ($product
            && strlen($product->product_id)
            && $product->master_grouped_product
            && $product->master_grouped_product->disable_completely
        ) {
            $this->data['product'] = null;
            return null;
        }
        ...
        foreach ($metaMap as $pageField => $productField) {
            if (strlen($product->$productField)) {
                $this->page->$pageField = $product->$productField;
            }
        }
        ...
        if (array_key_exists('product_file', $_FILES)) {
            $file_data = Upload::extractMultiFileInfo($_FILES['product_file']);
     ...
     * This AJAX handler can be used on the
     * {@link https://lsdomainexpired.mjman.net/docs/product_page Product Details}
     ...
     *
     * @input integer $product_id Specifies the {@link Product product} identifier.
     * This field is not required if the handler used on the
     ...
     * Possible values: 0, 1. By default the handler puts the message to the flash storage.
     * The message can be displayed with {@link flash_message()} function.
     * @input string $message The confirmation message string.
     * Note that you should use the <em>%s</em> symbol in the message,
     * to indicate a place where the product quantity should be inserted. Example:
     * <pre>
     * <input onclick="return $(this).getForm().sendRequest(
     *   'shop:on_addToCart',
     *   {
     ...
     * LSAPP support multiple shopping cart. By default the cart named <em>main</em> used.
     * Example:
     * <pre>
     * <input onclick="return $(this).getForm().sendRequest(
     *   'shop:on_addToCart',
    ...
    public function on_addToCart($ajax_mode = true)
    {
        ...
        if (!strlen($quantity) || !preg_match('/^[0-9]+$/', $quantity)) {
            throw new CmsException('Invalid quantity value.');
        }

        if (!isset($this->data['product'])) {
            $product_id = post('product_id');
            if (!$product_id) {
                throw new CmsException('Product not found.');
            }
        ...
        Cart::add_cart_item($this->data['product'], array(
            'quantity'=>$quantity,
     ...
     * {@link https://lsdomainexpired.mjman.net/docs/displaying_product_rating_and_reviews
     ...
     * The rating should be a number (1-5) or empty value.
     * If you do not need to implement the rating feature, you can skip
     ...
     * @input string $product_id Specifies the {@link Product product} identifier.
     * This field is optional if the review form is implemented on the
     * {@link https://lsdomainexpired.mjman.net/docs/product_page Product Details}
     * page.
     ...
     *
     * @see https://lsdomainexpired.mjman.net/docs/displaying_product_rating_and_reviews
     *  Displaying product rating and reviews
     * @see https://lsdomainexpired.mjman.net/docs/product_page Product Details page
     * @see action@shop:product
     * @author LSAPP - MJMAN
    ...
    public function on_addProductReview($ajax_mode = true)
    {
        ...
        ProductReview::create_review($this->data['product'], $this->customer, $_POST);
        $this->data['product'] = Product::create()->find_by_id($this->data['product']->id);
    ...
    /**
     * Base action for the {@link https://lsdomainexpired.mjman.net/docs/cart_page Cart} page.
     * This action can handle the shopping cart content management requests like removing items from the cart,
     * moving items to the Postponed list and changing item quantity.
     *
     * <h3>Managing content of a specific shopping cart</h3>
     * LSAPP supports multiple shopping carts. Using the shop:cart action you can manage a content of any specific
     * shopping cart. All handlers and actions which work with shopping cart accept the <em>$cart_name</em> parameter,
     * which specifies a name of a cart. By default LSAPP works with a shopping cart named <em>main</em>.
     *
     ...
     *   </li>
     * </ol>
     * Also the <em>cart_name</em> parameter should be passed to the checkout actions if you want LSAPP to
     * create an order from a specific shopping cart items.
     *
     * @action shop:cart
     *
     * @output Db\DataCollection $countries
     *  A collection of countries for populating the country list for the Estimate Shipping Cost feature.
     * Each element in the collection is an object of the {@link Country} class.
     * @output Db\DataCollection $states A list of states for a currently selected country.
     * Each element in the collection is an object of the {@link CountryState} class.
     * @output CheckoutAddressInfo $shipping_info A customer's shipping location.
     * For new customers all fields in the object are empty.
     * @output float $discount A discount value, calculated using the price rules,
     * defined on the {@link https://lsdomainexpired.mjman.net/docs/shopping_cart_price_rules/ Shop/Discounts page}
     * in the Administration Area.
     * The discount value can be not accurate on the Cart page,
     * because price rules can refer to customer details like the shipping
     ...
     * <em>rule</em> ({@link CartPriceRule} object) and <em>discount</em>.
     ...
     * If the {@link https://lsdomainexpired.mjman.net/docs/configuring_lemonstand_for_tax_inclusive_environments/
     * Display catalog/cart prices including tax} feature
     ...
     *
     * @input array $item_postponed A checkbox-type type INPUT element,
     * responsible for moving an item to or from the postponed items list.
     * Example:
     * <pre>
     *  <input type="hidden" name="item_postponed[<?= $item->key ?>]" value="0"/>
     *  <input type="checkbox" <?= checkbox_state($item->postponed) ?>
     *         name="item_postponed[<?= $item->key ?>]" value="1"/>
     * </pre>
     * The hidden element used here to provide the default value 0 for cases when the checkbox is not checked.
     * By default browsers do not send checkbox values if they are not checked.
     * Please always use this method with the item_postponed checkbox.
     ...
     * If specified, LSAPP will apply the coupon code and re-evaluate the <em>$discount</em> variable
     * @ajax on_action Applies any changes on the Cart page.
     * The handler applies item quantities, removes items marked for removing,
     * applies a coupon code, updates items postpone status.
     * This handler is equal to the regular form POST method, but works through AJAX.
     * Use this request to create the <em>Apply Changes</em>
    ...
    public function cart()
    {
        ...
        foreach ($delete_items as $key) {
            Cart::remove_item($key, $cart_name);
        }

        $postpone_items = post('item_postponed', array());
        Cart::change_postpone_status($postpone_items, $cart_name);

        $shipping_info = CheckoutData::get_shipping_info();
        $countries = $this->data['countries'] = Country::get_list($shipping_info->country);
        $shipping_country = $shipping_info->country ? $shipping_info->country : $countries[0]->id;

        $this->data['states'] = CountryState::find_by_country_id($shipping_country);
    ...
    {
        $discount_info = CheckoutData::eval_discounts($cart_name);

        if (!CheckoutData::display_prices_incl_tax()) {
            $this->data['discount'] = $discount_info->cart_discount;
        } else {
            $this->data['discount'] = $discount_info->cart_discount_incl_tax;
        }

        $this->data['subtotal'] = Cart::total_price($cart_name, true);
        $this->data['subtotal_no_discounts'] = Cart::total_price($cart_name, false);
        ...
        $this->data['cart_total_tax_incl'] = Cart::total_price($cart_name, true, null, true);
        $this->data['cart_tax'] = Cart::total_tax($cart_name);
        $tax_context = array(
            'cart_name' => $cart_name
        );
        $cart_taxes_details = TaxClass::calculate_taxes(
            Cart::list_active_items($cart_name),
            CheckoutData::get_shipping_info(),
            $tax_context
        );
        $this->data['cart_taxes'] = $cart_taxes_details->taxes;
        $this->data['estimated_total'] = max(0, $cart_total);
        $this->data['coupon_code'] = CheckoutData::get_coupon_code();
    }

    /**
     * Allows to add the <em>Estimate shipping cost</em> feature to the
     * {@link https://lsdomainexpired.mjman.net/docs/cart_page Cart page}.
     * Usually this AJAX handler used on the
     * {@link https://lsdomainexpired.mjman.net/docs/cart_page Cart page}
     * together with {@link action@shop:cart} action.
     *
     * Please read the
     * {@link https://lsdomainexpired.mjman.net/docs/implementing_the_shipping_cost_estimator_feature
     * Implementing the Shipping Cost Estimator Feature article}
     * for more information.
     *
     * @input integer $country Specifies a {@link Country country} identifier.
     * @input integer $state Specifies a {@link CountryState state} identifier.
     ...
     * Each element in the array is an object of the {@link ShippingOption} class.
     * @output array $shipping_options_flat A flat list of shipping options.
     * In the flat list all options, including options returned by multi-option shipping methods,
     * like FedEx, are presented in a single list.
     * Each element in the array is an object of the {@link ShippingOption} class.
     *
     * @ajax shop:on_evalShippingRate
     * @see https://lsdomainexpired.mjman.net/docs/implementing_the_shipping_cost_estimator_feature
     *  Implementing the Shipping Cost Estimator Feature
     * @see https://lsdomainexpired.mjman.net/docs/cart_page Cart page
     * @see action@shop:cart
     * @package shop.ajax handlers
     * @author LSAPP - MJMAN
     */
    public function on_evalShippingRate()
    {
        ...
        $zip = trim(post('zip'));
        if (!strlen($zip)) {
            throw new CmsException('Please specify a ZIP code.');
        }

        $total_price = Cart::total_price_no_tax($cart_name);
        $total_volume = Cart::total_volume($cart_name);
        $total_weight = Cart::total_weight($cart_name);
        $total_item_num = Cart::get_item_total_num($cart_name);

        CheckoutData::set_shipping_location(post('country'), post('state', null), $zip);
        $info = CheckoutData::get_shipping_info();
        $info->is_business = post('is_business');
        CheckoutData::set_shipping_info($info);

        $available_options = CheckoutData::list_available_shipping_options($this->customer, $cart_name);

        $this->data['shipping_options'] = $available_options;
        $this->data['shipping_options_flat'] = CheckoutData::flatten_shipping_options($available_options);
    }
     ...
     * {@link https://lsdomainexpired.mjman.net/docs/cart_page Cart page}
     * together with {@link action@shop:cart} action.
     * You can use this handler for creating the <em>Remove Item</em>
     * links for individual cart items. The following example creates the <em>Remove Item</em>
     * link which sends the shop:on_deleteCartItem AJAX request to LSAPP and updates the cart page content on success.
     * <pre>
     ...
     * </pre>
     *
     * @output Db\DataCollection $countries A collection of countries
     * for populating the country list for the Estimate Shipping Cost feature.
     * Each element in the collection is an object of the {@link Country} class.
     * @output Db\DataCollection $states A list of states for a currently selected country.
     * Each element in the collection is an object of the {@link CountryState} class.
     * @output CheckoutAddressInfo $shipping_info A customer's shipping location.
     ...
     * @output float $discount A discount value, calculated using the price rules,
     * defined on the
     * {@link https://lsdomainexpired.mjman.net/docs/shopping_cart_price_rules/
     * Shop/Discounts page} in the Administration Area.
     * The discount value can be not accurate on the Cart page,
     * because price rules can refer to customer details like the shipping
     * location which are not known on the Cart page and will become available only during the checkout process.
     * @output array $applied_discount_rules A list of discount rules applied to the cart products.
     ...
     * @output string $coupon_code A coupon code provided by the customer.
     *
     * @input string $key Specifies a key of a {@link CartItem shopping cart item} to remove, required.
    ...
    public function on_deleteCartItem()
    {
        ...
        $this->data['countries'] = Country::get_list();
        Cart::remove_item(post('key'), $cart_name);
    ...
    private function cart_applyQuantity($cart_name)
    {
        ...
        foreach ($quantities as $key => $quantity) {
            ...
            if (!preg_match('/^[0-9]+$/', $quantity)) {
                $item = Cart::find_item($key, $cart_name);
                if ($item) {
                    throw new CmsException('Invalid quantity value for '.$item->product->name.' product.');
                }
            }
        ...
        foreach ($filtered_list as $key => $quantity) {
            Cart::set_quantity($key, $quantity, $cart_name);
        }
    ...
    private function cart_apply_custom_data($cart_name)
    {
        ...
        foreach ($custom_data as $key => $data) {
            Cart::set_custom_data($key, $data, $cart_name);
        }
    }
    ...
    /**
     * Base action for the {@link https://lsdomainexpired.mjman.net/docs/customer_login_and_logout Login} page.
     ...
     * @input string $login {Login form} Specifies a name of a submit button element for the
     *  <em>POST</em> (non AJAX) form submit method.
     * @input string $redirect {Login and Sign up forms} An optional URL string for redirecting
     * a visitor's browser after the successful logging in.
     * Use {@link root_url()} function if your copy of LSAPP is installed in a subdirectory.
     * To redirect to the originally
     ...
     * @input string $signup {Sign up form} Specifies a name of a submit button element for the
     * <em>POST</em> (non AJAX) form submit method.
     ...
     * @author LSAPP - MJMAN
     * @package shop.actions
     * @see https://lsdomainexpired.mjman.net/docs/customer_login_and_logout Customer Login and Logout
     * @see https://lsdomainexpired.mjman.net/docs/customer_registration_page Customer registration page
     */
    ...
    /**
     * Base action for the
     * {@link https://lsdomainexpired.mjman.net/docs/customer_registration_page Customer Signup} page.
     * The action processes the signup form data,
     * creates a customer account and sends an email notification to the customer.
     *
     * @action shop:signup
     *
     ...
     * @input integer $billing_state_id An identifier of the customer billing {@link CountryState state}.
     * @input integer $billing_country_id An identifier of the customer billing {@link Country country}.
     ...
     * Note that if your copy of LSAPP is installed in a subdirectory, you need to use the {@link root_url()} function
     ...
     * @author LSAPP - MJMAN
     * @package shop.actions
     * @see https://lsdomainexpired.mjman.net/docs/customer_registration_page Customer Signup page
    ...
    public function on_login()
    {
        ...
        $customer = Customer::create()->findUser(post('email'), post('password'), true);
    ...
    /**
     * Allows to create a text field for entering a coupon code and a button
     * for processing the code and redirecting the browser to a specific page.
     * Usually this AJAX handler used on the
     * {@link https://lsdomainexpired.mjman.net/docs/cart_page Cart page}
     * together with {@link action@shop:cart} action.
     ...
     *          onclick="return $(this).getForm().sendRequest('shop:on_setCouponCode')"/>
    ...
    public function on_setCouponCode($allow_redirect = true)
    {
        ...
        foreach ($return as $changed_code) {
            if ($changed_code === false) {
                throw new CmsException('The entered coupon cannot be used.');
            } elseif ($changed_code) {
        ...
        if (strlen($coupon_code)) {
            ...
            if (!$coupon) {
                throw new CmsException('A coupon with the specified code is not found');
            }
            $validation_result = CartPriceRule::validate_coupon_code($coupon_code, $this->customer);
            if ($validation_result !== true) {
                throw new CmsException($validation_result);
            }
        }

        CheckoutData::set_coupon_code($coupon_code);
    ...
    /**
     * Base action for the {@link https://lsdomainexpired.mjman.net/docs/password_restore_page Password Restore} page.
     ...
     * @input string $redirect An optional field containing a
     *  URL for redirecting the browser after the successful password reset.
     * Use {@link root_url()} function if your copy of LSAPP is installed in a subdirectory.
     ...
     * @author LSAPP - MJMAN
     * @package shop.actions
     * @see https://lsdomainexpired.mjman.net/docs/password_restore_page Password Restore page
     */
    ...
    public function on_passwordRestore()
    {
        ...
        try {
            Customer::reset_password($validation->fieldValues['email']);
    ...
    /**
     * Base action for the {@link https://lsdomainexpired.mjman.net/docs/change_password_page Change Password} page.
     * On this page a customer can change a password used for logging into the store.
     * Note that the action requires a logged in customer,
     ...
     * the successful password update.
     * Note that you should use the {@link root_url()}
     * function if your copy of LSAPP is installed in a subdirectory.
     * @input string $flash An optional message to display after the redirection.
     * Use the {@link flash_message()} function the target page to display the message.
     ...
     *
     * @output Customer $customer A customer object, representing a currently logged in customer.
     *
     * @author LSAPP - MJMAN
     * @package shop.actions
     * @see https://lsdomainexpired.mjman.net/docs/change_password_page Creating the Change Password page
     ...
     * Usually this handler used on the
     * {@link https://lsdomainexpired.mjman.net/docs/change_password_page Change Password} page,
     ...
     * @input string $password_confirm Specifies the password confirmation. Required.
     * @input string $redirect An optional field containing an URL for redirecting a visitor's browser after
     * the successful password update. Note that you should use the
     * {@link root_url()} function if your copy of LSAPP is installed in a subdirectory.
     * @input string $flash An optional message to display after the redirection.
     * Use the {@link flash_message()} function the target page to display the message.
     *
    ...
    public function on_changePassword()
    {
        ...
        $validation->add('old_password', 'Old Password')
            ->fn('trim')
            ->required("Please specify the old password");
        $validation->add('password', 'Password')
            ->fn('trim')
            ->required("Please specify new password");
        $validation->add('password_confirm', 'Password Confirmation')
            ->fn('trim')
            ->matches('password', 'Password and confirmation password do not match.');
        ...
        try {
            $customer = Customer::create()->where('id=?', $this->customer->id)->find(null, array(), 'front_end');
    ...
    public function on_changeEmail()
    {
        ...
        if (!$this->customer) {
            throw new CmsException('You must be logged in with your customer account to use this feature');
        }

        $validation = new Validation();
        $validation->add('password', 'Password')
            ->fn('trim')->required("Please enter your account password to authorise this action");
        $validation->add('email', 'Email')
            ->fn('trim')
        ...
        try {
            ...
            if (post('flash')) {
                Phpr::$session->flash['success'] = post(
                    'flash',
                    'Your account email address has been updated to '.$customer->email
                );
            }
     ...
     * The checkout process is split to several steps.
     * A current step name is always available through the <em>$checkout_step</em> field.
     ...
     * </ul>
     * A set of form fields required and supported by the action depends on a current checkout step.
     * The <em>$checkout_step</em> field is required on each step.
     *
     * @input string $checkout_step {Any checkout step} Specifies a current checkout step. Required.
     * Depending on a value of this field, LSAPP validates corresponding form fields and switches the checkout
     ...
     * @input string $auto_skip_shipping {Any checkout step} Allows to skip the Shipping Method
     *  step if the shopping cart contains only non-shippable items.
     * Please read
     * {@link
     * https://lsdomainexpired.mjman.net/docs/skipping_the_shipping_method_step_for_downloadable_products_or_services
     * Skipping the Shipping Method step for downloadable products or services} article
     * to learn how to configure LSAPP to skip the shipping method checkout step.
     * @input string $empty_cart {Any checkout step} Allows to leave the cart content after the order is placed.
     * This allows your customers to return to previous checkout step even if they already placed the order.
     * The default value is 0 (false).
     * @input string $cart_name {Any checkout step} Optional field, the shopping cart name.
     * Use it if you want to the checkout to work with a specific shopping cart.
     * Please refer to the {@link action@shop:cart} action documentation for the
     * multiple shopping cart feature description.
     *
     * @input string $first_name {Billing and Shipping information steps} S
     * pecifies the customer billing first name. Required.
     * @input string $last_name {Billing and Shipping information steps}
     * Specifies the customer billing last name. Required.
     * @input string $email {Billing and Shipping information steps}
     * Specifies the customer email address. Required.
     * @input string $company {Billing and Shipping information steps}
     * Specifies the customer company name. Optional.
     * @input string $phone {Billing and Shipping information steps}
     * Specifies the customer phone number. Optional.
     * @input string $street_address {Billing and Shipping information steps}
     * Specifies the customer billing street address. Required.
     * @input string $city {Billing and Shipping information steps}
     * Specifies the customer billing city. Required.
     * @input string $zip {Billing and Shipping information steps}
     * Specifies the customer billing ZIP code. Required.
     * @input string $country {Billing and Shipping information steps}
     * Specifies the customer {@link Country billing country} identifier. Required.
     * @input string $state {Billing and Shipping information steps}
     * Specifies the customer {@link CountryState billing state} identifier. Required.
     * @input boolean $is_business {Shipping information step}
     * Determines whether the shipping location is a business address. Optional.
     * Accepted values are 0, 1.
     *
     * @input boolean $register_customer {Billing information step}
     * Use this field if you want a customer to be automatically registered on checkout.
     * Accepted values are: 0, 1.
     * If this field is not declared or its value is 0 (default), the following fields have no effect:
     * <ul>
     ...
     * </ul>
     * Please refer to the
     * {@link https://lsdomainexpired.mjman.net/docs/automatic_customer_registration Automatic customer registration}
     * article for the implementation details.
     * @input boolean $allow_empty_password {Billing information step}
     * Determines whether customers should enter a password to register.
     * Accepted values are: 0, 1. If the field value is 0, the password is not registered.
     * In this case LSAPP will generate a random password.
     * The default value is 0. You can set the field value to 1 and omit the customer password fields
     * (<em>customer_password</em>, <em>customer_password_confirm</em>).
     ...
     * The field has no effect if the <em>register_customer</em> field value is 0 (default).
     * @input string $customer_password_confirm {Billing and Shipping information steps}
     * Specifies the customer password confirmation.
     * Use this field along with the <em>customer_password</em> field.
     * The field has no effect if the <em>register_customer</em> field value is 0 (default).
     * @input string $no_password_error {Billing information step}
     * Specifies an error message to display in case if the customer did not specify the password.
     * Default value is "Please enter your password.".
     * The field has no effect if the <em>register_customer</em> field value is 0 (default).
     * @input string $passwords_match_error {Billing information step}
     * Specifies an error message to display in case if the password and its confirmation do not match.
     * Default value is "Password and confirmation password do not match.".
     * The field has no effect if the <em>register_customer</em> field value is 0 (default).
     * @input string $customer_exists_error {Billing information step}
     * Specifies an error to display in case if a customer with the specified email address already exists.
     * Default value is "A customer with the specified email is already registered.
     * Please log in or use another email.".
     * The field has no effect if the <em>register_customer</em> field value is 0 (default).
     * @input boolean $customer_auto_login {Review step}
     * Determines whether the new customer should be automatically logged in customer in after placing the order.
     * Accepted values are: 0, 1.
     * The field has no effect if the <em>register_customer</em> field value is 0 (default).
     * @input boolean $customer_registration_notification {Review step}
     * Determines whether a registration notification email message should be sent to the new customer.
     * LSAPP uses an email template with the <em>shop:registration_confirmation</em>
     * code for the registration notification.
     * Accepted values are: 0, 1.
     * The field has no effect if the <em>register_customer</em> field value is 0 (default).
     *
     * @input mixed $shipping_option {Shipping method step}
     * Specifies an identifier of selected {@link ShippingOption shipping method}.
     * @input mixed $payment_method {Payment method step}
     * Specifies an identifier of selected {@link PaymentMethod payment method}.
     *
     * @output string $checkout_step {Any checkout step} Specifies a current checkout step.
     * @output CheckoutAddressInfo $billing_info {Any checkout step}
     * An object of the {@link CheckoutAddressInfo} class containing the customer's billing name and address.
     * @output CheckoutAddressInfo $shipping_info {Any checkout step}
     * An object of the {@link CheckoutAddressInfo} class containing the customer's shipping name and address.
     * @output object $shipping_method {Any checkout step} A PHP object representing a selected shipping method.
     * The object has the following fields:
     * <ul>
     *   <li><em>id</em> - an identifier of the shipping method in LSAPP database.</li>
     ...
     * The object has the following fields:
     * <ul>
     *   <li><em>id</em> - an identifier of the payment method in LSAPP database.</li>
     ...
     * @output array $applied_discount_rules {Any checkout step}
     * Contains a list of discount rules applied to the cart products.
     * Each element in the array is an object with two fields:
     * <ul>
     *   <li><em>rule</em> - {@link CartPriceRule} Discount rule object.</li>
     ...
     * This value is calculated on each step of the checkout process,
     * taking into account price rules and known information about the customer.
     ...
     * {@link https://lsdomainexpired.mjman.net/docs/shopping_cart_price_rules/ Shop/Discounts page} page) and
     * known information about the customer. The tax value includes the goods tax and shipping tax.
     * @output boolean $shipping_required {Any checkout step}
     * Determines whether the shopping cart contains only non-shippable items.
     * This variable is FALSE if the shopping cart contains only non-shippable items
     * (downloadable  products or services).
     *
     * @output Db\DataCollection $countries {Billing and Shipping information steps}
     * A collection of countries for populating the Country list.
     * Each element in the collection is an object of the {@link Country} class.
     * @output Db\DataCollection $states {Billing and Shipping information steps}
     * A collection of states for populating the State list.
     * Each element in the collection is an object of the {@link CountryState} class
     * @output Db\DataCollection $shipping_states {Billing information step}
     * A collection of states for populating the Shipping State list.
     * Each element in the collection is an object of the {@link CountryState} class
     * @output array $shipping_options {Shipping method step}
     * A list of available shipping options.
     * Each element in the array is an object of the {@link ShippingOption} class.
     * @output array $shipping_options_flat {Shipping method step}
     ...
     * Each element in the array is an object of the {@link PaymentMethod} class.
     *
     ...
     * </ul>
     *
     * @ajax on_action Posts a current step form data to the server,
     * validates form data and switches the checkout process to a next step.
     ...
     * </pre>
     * @ajax shop:on_copyBillingInfo Copies billing information
     * (a customer name and address) to the shipping information checkout step.
     ...
     * @author LSAPP - MJMAN
     * @package shop.actions
     * @see https://lsdomainexpired.mjman.net/docs/checkout_page Creating the Checkout Page
    ...
    public function checkout()
    {
        ...
        $shipping_required = $this->data['shipping_required'] = CheckoutData::shipping_required();
        $skip_shipping_step = false;

        if (!$shipping_required) {
                $no_shipping_option = ShippingOption::find_by_api_code('no_shipping_required');
            if ($no_shipping_option) {
                CheckoutData::set_shipping_method($no_shipping_option->id, post('cart_name', 'main'));
            ...
            if ($no_shipping_option && $shipping_method && $shipping_method->id == $no_shipping_option->id) {
                CheckoutData::reset_shipping_method();
            }
        }
        ...
        if ($skip_to && !$move_to) {
            ...
            switch ($checkout_step) {
                case 'billing_info':
                        CheckoutData::set_billing_info($this->customer);
                    break;
                case 'shipping_info':
                        CheckoutData::set_shipping_info();
                    break;
                case 'shipping_method':
                    if (!$skip_shipping_step) {
                        CheckoutData::set_shipping_method(null, post('cart_name', 'main'));
                    }
                    break;
                case 'payment_method':
                        CheckoutData::set_payment_method();
                    break;
            }
        ...
        if (post('checkout_step')) {
            $cart_content_id = CheckoutData::get_cart_id();
            $new_content_id = Cart::get_content_id($cart_name);

            if ($new_content_id != $cart_content_id) {
                CheckoutData::reset_data();
                Phpr::$response->redirect(root_url($this->page->url).'/?'.uniqid());
            }
        } else {
            CheckoutData::set_cart_id(Cart::get_content_id($cart_name));
        }
        ...
        if (array_key_exists('customer_notes', $_POST)) {
            CheckoutData::set_customer_notes(post('customer_notes'));
        }
        ...
        if (!$checkout_step) {
            ...
            $billing_countries = Country::get_list($billing_info->country);
            $this->data['countries'] = $billing_countries;

            $billing_country = $billing_info->country ? $billing_info->country : $billing_countries[0]->id;
            $this->data['states'] = CountryState::find_by_country_id($billing_country);

            $shipping_country = $shipping_info->country ? $shipping_info->country : $billing_countries[0]->id;
            $this->data['shipping_states'] = CountryState::find_by_country_id($shipping_country);
        } elseif ($checkout_step == 'billing_info') {
            if (!$skip_data) {
                CheckoutData::set_billing_info($this->customer);
            }

            $shipping_countries = Country::get_list($shipping_info->country);
            ...
            $tax_info = TaxClass::calculate_taxes(
                Cart::list_active_items($cart_name),
                CheckoutData::get_shipping_info(),
                $tax_context
            );
            ...
            $total += $this->data['subtotal'] = Cart::total_price_no_tax($cart_name);
            ...
            $this->data['shipping_taxes'] = $shipping_taxes;
            $total += $this->data['shipping_tax'] = TaxClass::eval_total_tax($shipping_taxes);

            $payment_methods = PaymentMethod::list_checkout_applicable($cart_name, $total)->as_array();
            ...
            $dpiTax = CheckoutData::display_prices_incl_tax();
            $totals = CheckoutData::calculate_totals($cart_name);

            $this->data['discount'] = $dpiTax ? $totals->discount_tax_incl : $totals->discount;
            $this->data['goods_tax'] = $totals->goods_tax;
            $this->data['subtotal_no_discounts'] = $totals->subtotal;
            $this->data['subtotal'] = $dpiTax ? $totals->subtotal_tax_incl : $totals->subtotal_discounts;
            $this->data['shipping_taxes'] = $totals->shipping_taxes;
            $this->data['shipping_tax'] = $totals->shipping_tax;
            $this->data['shipping_quote'] = $dpiTax ? $totals->shipping_quote_tax_incl : $totals->shipping_quote;
            ...
            $order = CheckoutData::place_order(
                $this->customer,
                post('register_customer', false),
                post('cart_name', 'main'),
                post('empty_cart', true)
            );
            ...
            if (!$pay_page) {
                throw new CmsException('The Pay page is not found.');
            }
    ...
    {
        $display_prices_including_tax = CheckoutData::display_prices_incl_tax();
        $discount_info = CheckoutData::eval_discounts($cart_name);
        ...
        $this->data['cart_total'] = $cart_total = Cart::total_price($cart_name, true);
        $shipping_tax = 0;
     ...
     * Usually this AJAX handler used on the
     * {@link https://lsdomainexpired.mjman.net/docs/checkout_page Checkout page}
     * together with {@link action@shop:checkout} action,
     ...
     * })">
     * </pre>
     * To make the state list updatable, place it into a separate partial.
     * In the code example above the partial is called <em>shop:state_selector</em>.
     * The parameters control_name, control_id,
     * current_state are passed into the state list partial by LSAPP when it handles
     * the shop:on_updateStateList request.
     * You can use these parameters in the state list partial highlighting a currently selected state.
     *
     * @input integer $country Specifies the selected {@link Country country} identifier. Required.
     ...
     * @input integer $current_state Specifies the selected {@link CountryState state} identifier.
     *
     ...
     * @output integer $current_state Specifies the selected {@link CountryState state} identifier.
     *
     * @ajax shop:on_updateStateList
     * @author LSAPP - MJMAN
     * @package shop.ajax handlers
     * @link action@shop:checkout
     * @link https://lsdomainexpired.mjman.net/docs/checkout_page Creating the Checkout page
     */
    public function on_updateStateList()
    {
        $this->data['states'] = CountryState::find_by_country_id(post('country'));
    ...
    /**
     * Base action for the {@link https://lsdomainexpired.mjman.net/docs/payment_receipt_page Payment Receipt} page.
     ...
     *
     * @output Order $order An order object, loaded from the database.
     ...
     * @output Db\DataCollection $items A list of order items.
     * Each element of the collection is an object of the {@link OrderItem} class.
    ...
    public function payment_receipt()
    {
        ...
        $order = Order::create()->find_by_order_hash($order_hash);
        if (!$order) {
            return;
        }
     ...
     * This action is a part of the
     * {@link https://lsdomainexpired.mjman.net/docs/implementing_the_step_by_step_checkout step-by-step
     * checkout process}.
     ...
     * Besides the supported form fields described below this action
     * supports fields for the automatic customer registration.
     * Please refer to the
     * {@link https://lsdomainexpired.mjman.net/docs/automatic_customer_registration/
     ...
     * @input integer $country Specifies the customer billing {@link Country country} identifier. Required.
     * @input integer $state Specifies the customer billing {@link CountryState state} identifier. Required.
     ...
     * LSAPP support multiple shopping cart. By default the cart named <em>main</em> used.
     *
     * @output CheckoutAddressInfo $billing_info An address information object representing
     * a customer's billing name and address.
     * @output Db\DataCollection $countries A collection of countries for populating the Country list.
     * Each element in the collection is an object of the {@link Country} class.
     * @output Db\DataCollection $states A list of states for populating the State list.
     * Each element in the collection is an object of the {@link CountryState} class.
     ...
     * {@link https://lsdomainexpired.mjman.net/docs/shopping_cart_price_rules/ price rules}
     ...
     * Use this handler for creating a button, or link,
     * for sending the form data to the server and redirecting the browser
     ...
     * @author LSAPP - MJMAN
     * @package shop.actions
     * @see https://lsdomainexpired.mjman.net/docs/implementing_the_step_by_step_checkout
     * Implementing the Step-By-Step Checkout
     * @see  https://lsdomainexpired.mjman.net/docs/automatic_customer_registration/ Automatic customer registration
     * @see action@shop:checkout
    ...
    {
        CheckoutData::reset_data();
        CheckoutData::set_cart_id(Cart::get_content_id(post('cart_name', 'main')));
     ...
     * @input integer $country Specifies the customer shipping {@link Country country} identifier. Required.
     * @input integer $state Specifies the customer shipping {@link CountryState state} identifier. Required.
     * @input string $redirect Specifies an ULR of a page to redirect the browser after processing the form data.
     * Use a hidden field for this value. If you are implementing a conventional checkout process, a value of
     ...
     *
     * @output CheckoutAddressInfo $shipping_info An object representing a customer's shipping name and address.
     * @output Db\DataCollection $countries A collection of countries for populating the Country list.
     * Each element in the collection is an object of the {@link Country} class.
     * @output Db\DataCollection $states A list of states for populating the State list.
     * Each element in the collection is an object of the {@link CountryState} class.
     ...
     * @ajax shop:on_copyBillingInfo Copies billing information
     * (a customer name and address) to the shipping information checkout step.
     * Use this for creating a link “<em>Copy billing information</em>”
     * on the shipping information checkout page. Example:
     * <pre>
     * <a
     *   href="#"
     *   onclick="return $(this).getForm().sendRequest(
     ...
     * Each element in the array is an object of the {@link ShippingOption} class.
     * @output object $shipping_method A PHP object representing a shipping method
     * previously selected by the customer, if any.
     * The object has the following fields:
     * <ul>
     *   <li><em>id</em> - an identifier of the shipping method in LSAPP database.</li>
     *   <li><em>quote</em> - specifies the shipping quote.</li>
     ...
     * Use this handler for creating a button, or link,
     * for sending the form data to the server and redirecting the browser to a next
    ...
    public function checkout_shipping_method()
    {
        ...
        $this->data['shipping_options'] = CheckoutData::list_available_shipping_options($this->customer, $cart_name);
        $this->data['shipping_method'] = CheckoutData::get_shipping_method();
     ...
     * {@include ../docs/step-by-step-checkout}
     *
     * @input integer $payment_method An identifier of selected {@link PaymentMethod payment method}. Required.
     ...
     * Use this handler for creating a button, or link,
     * for sending the form data to the server and redirecting the browser to
    ...
    public function checkout_payment_method()
    {
        ...
        $this->data['payment_methods'] = $payment_methods;
        $this->data['payment_method'] = CheckoutData::get_payment_method();
        $this->data['applied_discount_rules'] = $discount_info->applied_rules_info;
        $this->data['active_discount_rules'] = $discount_info->active_rules_info;
     ...
     * This action is a part of the
     * {@link https://lsdomainexpired.mjman.net/docs/implementing_the_step_by_step_checkout
     * step-by-step checkout process}.
     * The Order Review page is the last step of the conventional step-by-step checkout process.
     *
     * {@include ../docs/step-by-step-checkout}
     *
     * @input boolean $customer_auto_login Determines whether the
     * customer should be automatically logged in after placing the order.
     * Accepted values are: 0, 1. See also the
     * {@link https://lsdomainexpired.mjman.net/docs/automatic_customer_registration
     * Automatic customer registration} article.
     * @input boolean $customer_registration_notification
     * Determines whether the registration notification email message should be sent to the customer.
     * LSAPP uses an email template with the <em>shop:registration_confirmation</em>
     * code for the registration notification. Accepted values are: 0, 1.
     ...
     * By default the browser is automatically redirected to the
     * {@link https://lsdomainexpired.mjman.net/docs/pay_page payment page} after the order is placed.
     * Accepted values are: 0, 1.
     * @input string $cart_name Specifies the shopping cart name.
     * LSAPP support multiple shopping cart. By default the cart named <em>main</em> used.
     *
     ...
     *   <li><em>total</em> - total tax amount.</li>
     * </ul>
     * @output CheckoutAddressInfo $billing_info An object of the {@link CheckoutAddressInfo} class
     * containing the customer's billing name and address.
     * @output CheckoutAddressInfo $shipping_info An object of the {@link CheckoutAddressInfo} class
     * containing the customer's shipping name and address.
     ...
     * and redirects the browser to the {@link https://lsdomainexpired.mjman.net/docs/pay_page payment page}.
    ...
    public function checkout_order_review()
    {
        ...
        $totals = CheckoutData::calculate_totals(post('cart_name', 'main'));
        $this->data['discount'] = $dpiTax ? $totals->discount_tax_incl : $totals->discount;
        $this->data['goods_tax'] = $totals->goods_tax;
        $this->data['subtotal_no_discounts'] = $totals->subtotal;
        $this->data['subtotal'] = $dpiTax ? $totals->subtotal_tax_incl : $totals->subtotal_discounts;
        ...
        $this->data['billing_info'] = CheckoutData::get_billing_info();
        $this->data['shipping_info'] = CheckoutData::get_shipping_info();
        $this->data['shipping_method'] = CheckoutData::get_shipping_method();
        $this->data['payment_method'] = CheckoutData::get_payment_method();
    ...
    /**
     * Base action for the {@link https://lsdomainexpired.mjman.net/docs/pay_page Payment} page.
     * The action loads an order by an order hash string, specified in the page URL and prepares all required
     * PHP variables.
     *
     * @action shop:pay
     ...
     * @output Order $order An order object, loaded from the database.
     * This variable can be NULL of the order is not found.
     * @output PaymentMethod $payment_method A payment method, selected by the customer.
     * @output PaymentType $payment_method_obj An instance of a specific payment type class,
     * for example the PayPal_Pro_Payment.
     * @output $Db\DataCollection $payment_methods A list of applicable payment methods.
     * Each element of the collection is an object of the {@link PaymentMethod} class.
     *
    ...
    public function pay()
    {
        ...
        $this->data['payment_methods'] = PaymentMethod::list_order_applicable($order, $params)->as_array();
    ...
    public function on_updatePaymentMethod()
    {
        ...
        $payment_method = PaymentMethod::create()->find($method_id);
     ...
     * {@link https://lsdomainexpired.mjman.net/docs/creating_the_bank_transfer_and_other_similar_payment_methods
     ...
     * @author LSAPP - MJMAN
     * @package shop.actions
     * @see https://lsdomainexpired.mjman.net/docs/creating_the_bank_transfer_and_other_similar_payment_methods
     * Bank transfer payment method
     * @see action@shop:pay
     */
    ...
    public function on_pay($order = null)
    {
        ...
        if (!post('pay_from_profile') || post('pay_from_profile') != 1) {
            ...
            $redirect = true;
            if (!$this->customer) {
                throw new ApplicationException('Please log in to pay using the stored credit card.');
            }

            if ($this->customer->id != $order->customer_id) {
                throw new ApplicationException('The order does not belong to your customer account.');
            }
     ...
     * The <em>Save</em> submit button should have name <em>submit_profile</em>
     * if you are implementing a regular POST form.
     ...
     * The <em>Delete</em> submit button should have name <em>delete_profile</em>
     * if you are implementing a regular POST form.
     * Alternatively you can use the {@link ajax@shop:on_deletePaymentProfile} handler.
     *
     * @output PaymentMethod $payment_method The payment method object.
     * This variable can be empty if the payment method not found.
     * @output PaymentType $payment_method_obj An instance of a specific payment method class.
     * Call the <em>render_payment_profile_form()</em> method of this object to render the payment profile form.
     * @output CustomerPaymentProfile $payment_profile A payment profile object
     ...
     * @author LSAPP - MJMAN
     * @package shop.actions
     * @see https://lsdomainexpired.mjman.net/docs/implementing_customer_payment_profiles Payment Profiles
    ...
    public function on_updatePaymentProfile($payment_method = null)
    {
        ...
        $return_page = Page::create()->find_by_action_reference('shop:payment_profiles');
        if (!$return_page) {
            throw new CmsException('The Payment Profiles page is not found.');
        }
    ...
    /**
     * Base action for the
     * {@link https://lsdomainexpired.mjman.net/docs/implementing_customer_payment_profiles Payment Profiles}
     * page.
     *
     * @action shop:payment_profiles
     *
     * @output array $payment_methods A list of payment methods which support payment profiles.
     * Each element in the collection is an object of {@link PaymentMethod} class.
     *
     * @author LSAPP - MJMAN
     * @package shop.actions
     * @see https://lsdomainexpired.mjman.net/docs/implementing_customer_payment_profiles Payment Profiles
    ...
    public function payment_profiles()
    {
        ...
        $methods = PaymentMethod::list_applicable($this->customer->billing_country_id, 1);
    ...
    protected function payment_profile_find_method()
    {
        ...
        $obj = PaymentMethod::create()->find($method_id);
        if ($obj) {
    ...
    /**
     * Base action for the {@link https://lsdomainexpired.mjman.net/docs/customer_orders_page customer order list} page.
     ...
     *
     * @action shop:orders
     * @output Db\DataCollection $orders A collection of customer orders.
     * Each element in the collection is an object of the {@link Order} class. This variable can be NULL
     * in case if there is no currently logged in customer.
     * Always check whether the variable is not empty before displaying a list of orders.
     * @author LSAPP - MJMAN
     * @package shop.actions
     * @see https://lsdomainexpired.mjman.net/docs/customer_orders_page Creating the Customer Orders page
    ...
    /**
     * Base action for the {@link https://lsdomainexpired.mjman.net/docs/order_details_page Order Details} page.
     * The action loads an order object with an identifier specified in the page URL.
     * Please note that the action requires a logged in
     ...
     * @output Db\DataCollection $items A list of the order items.
     * Each element of the collection is an object of the {@link OrderItem} class.
    ...
    /**
     * Base action for the {@link https://lsdomainexpired.mjman.net/docs/creating_the_search_page Search} page.
     * The action finds products in the database,
     * according to a search query and other search parameters passed to the search
     * page in the URL. Please read the
     * {@link https://lsdomainexpired.mjman.net/docs/creating_the_search_page Creating the Search page}
     * article
     * for action usage examples.
     *
     * The action automatically loads GET parameters from the URL and performs the search.
     * Example of a correct search URL:
     ...
     * You can define any number of the <em>option_names</em> and <em>option_values</em> fields.
     * On the form you should have an equal number of
     * the <em>option_names[]</em> and <em>option_values[]</em> fields.
     * LSAPP matches names and values specified in fields
     * with equal index. The first value from the <em>option_values[]</em>
     * field will correspond the first <em>option_names[]</em> field.
     * For example, if you want to organize a search in the Color option (and only),
     * you can define two fields (one of them is hidden):
     * <pre>
     * <form method="get" action="/search">
     *   <input name="query" type="text" value="<?= isset($query) ? $query : null ?>"/>
     *   <input type="submit" value="Find Products"/>
     ...
     * </form>
     * </pre>
     * The <em>attribute_names</em> and <em>attribute_values</em>
     * fields work in the same way. You can find more examples in
     * the {@link https://lsdomainexpired.mjman.net/docs/creating_the_search_page Creating the Search page} article.
     *
     ...
     * You can pass this string between pagination pages.
     * In the default store implementation this value can be passed to the
     ...
     * @output Db\DataCollection $products A collection of found products.
     * Each element in the collection is an object of the {@link Product} class.
     * @output Phpr\Pagination $pagination A pagination object.
     *
     * @action shop:search
     *
     ...
     * @input array $custom_groups A list of {@link CustomGroup custom product group}
     * identifiers the products should belong to.
     ...
     * Use this parameter in combination with the <em>option_names</em>
     * parameter to specify values you want to search in product options.
     ...
     * want to search in product properties. This parameter should be an array,
     * so you need to use the <em>attribute_values[]</em> name for your form controls.
     *
     * @author LSAPP - MJMAN
     * @package shop.actions
    ...
    public function search()
    {
        ...
        $this->data['query'] = Html::encode($request);

        /*
             * Load categories
         */

        $categories = Phpr::$request->getValueArray('categories');
        ...
        $manufacturers = Phpr::$request->getValueArray('manufacturers');
        ...
        $option_names = Phpr::$request->getValueArray('option_names');
        if (!is_array($option_names)) {
            $option_names = array();
        }

        $option_values = Phpr::$request->getValueArray('option_values');
        ...
        $attribute_names = Phpr::$request->getValueArray('attribute_names');
        if (!is_array($attribute_names)) {
            $attribute_names = array();
        }

        $attribute_values = Phpr::$request->getValueArray('attribute_values');
        ...
        $custom_groups = Phpr::$request->getValueArray('custom_groups');
        ...
        $pagination = new Pagination($records);
        if (!$no_query) {
            $options = array();
            ...
            $this->data['products'] = Product::find_products($request, $pagination, $page, $options);
        } else {
            $this->data['products'] = Product::create()->where('shop_products.id<>shop_products.id');
        }
     ...
     * Read the
     * {@link https://lsdomainexpired.mjman.net/docs/implementing_the_compare_products_feature
     ...
     * });">Add to compare</a>
     * </pre>
     * @input integer $product_id Specifies the {@link Product product} identifier. Required.
     * @ajax shop:on_addToCompare
     * @see https://lsdomainexpired.mjman.net/docs/implementing_the_compare_products_feature Compare Products Page
    ...
    public function on_addToCompare()
    {
        ...
        ComparisonList::add_product($product_id);
    }
    ...
    public function on_removeFromCompare()
    {
        ...
        ComparisonList::remove_product($product_id);
        $this->compare();
    }
     ...
     * The action loads a list of products, added to the product comparison list,
     * and generates corresponding PHP variables.
     *
     * @action shop:compare
     * @output Db\DataCollection $products A list of products added to the comparison list.
     * Each element in the collection is an object of the {@link Product} class.
    ...
    {
        $products = $this->data['products'] = ComparisonList::list_products();
    ...
    /**
     * Base action for the
     * {@link https://lsdomainexpired.mjman.net/docs/manufacturer_list_page/
     * Creating the Manufacturer List} page.
     * The action doesn't return disabled manufacturers.
     *
     * @action shop:manufacturers
     * @output Db\DataCollection $manufacturers A collection of manufacturers.
     * Each element in the collection is an instance of the {@link Manufacturer} class.
     * @author LSAPP - MJMAN
     * @package shop.actions
     * @see https://lsdomainexpired.mjman.net/docs/manufacturer_list_page/ Creating the Manufacturer List Page
     * @see action@shop:manufacturer
     */
    public function manufacturers()
    {
        $this->data['manufacturers'] = Manufacturer::create()
            ->order('name')
    ...
    /**
     * Base action for the
     * {@link https://lsdomainexpired.mjman.net/docs/manufacturer_list_page/ Manufacturer Details} page.
     ...
     * @output Manufacturer $manufacturer A manufacturer object loaded from the database.
     * The variable value can be NULL if the manufacturer specified in the page URL is not found in the database.
     * @author LSAPP - MJMAN
     * @package shop.actions
     * @see https://lsdomainexpired.mjman.net/docs/manufacturer_details_page/ Creating the Manufacturer Page
    ...
    /**
     * Base action for the {@link https://lsdomainexpired.mjman.net/docs/customer_profile_page/ Customer Profile} page.
     *
     ...
     *
     * @output Db\DataCollection $countries A collection of countries for populating
     * the Billing Country and Shipping Country lists.
     * Each element in the collection is an object of the {@link Country} class.
     * @output Db\DataCollection $billing_states A collection of states for populating the Billing State list.
     * Each element in the collection is an object of the {@link CountryState} class
     * @output Db\DataCollection $shipping_states A collection of states for populating the Shipping State list.
     * Each element in the collection is an object of the {@link CountryState} class
     *
     * @input string $first_name Specifies the customer first name. Required.
     * @input string $last_name Specifies the customer last name. Required.
     ...
     * @input integer $shipping_state_id An identifier of the customer shipping {@link CountryState state}.
     * @input integer $shipping_country_id An identifier of the customer shipping {@link Country country}.
     ...
     *
     * @author LSAPP - MJMAN
     * @see https://lsdomainexpired.mjman.net/docs/customer_profile_page/ Creating the Customer Profile page
     * @package shop.actions
     */
    public function customer_profile()
    {
        $billing_countries = Country::get_list();
        $this->data['countries'] = $billing_countries;

        $this->data['states'] = CountryState::find_by_country_id(
            post('billing_country_id', $this->customer->billing_country_id)
        );
        $this->data['shipping_states'] = CountryState::find_by_country_id(
            post('shipping_country_id', $this->customer->shipping_country_id)
        );
     ...
     *   <? endif ?>
     *       Please enter your email below, we will send you an email with
     *        a link to enter a new password for your account.<br>
     ...
     *            value="Your password was successfully changed, login with your new password.">
     ...
     *
     * @input string $customer_email Specifies the customer's email address.
     * Required when requesting a password restore.
     ...
     * @input string $redirect An optional field containing an URL for redirecting
     * the browser after password restore request has been submitted.
     * Use {@link root_url()} function if your copy of LSAPP is installed in a subdirectory.
     * @input string $success_message An optional message to display after the redirection.
     * Use the {@link flash_message()} function on the target page to display the message.
     * @output string $action_mode Set to 'password_update' when the page
     * url contains the password restore hash (customer is expected to enter
     * a new password) and 'restore_request' when it
     * does not (customer will enter their email to request a password restore).
     * @output boolean $success Set to true if a password was successfully reset or the password restore email was sent.
     * @output boolean $invalid_hash Set to true if the password
     * restore link is invalid (invalid hash), was previously used or
     * is expired (hash was generated more than 24 hours ago).
     *
     * @ajax shop:on_passwordRestore Processes the password restore form.
     * Use this handler for creating a Submit link or button on the password restore page. Example:
     * <pre>
     *  <a href="#"
     *     onclick="return $(this).getForm().sendRequest('shop:on_passwordRestoreRequest')">Submit</a>
     * </pre>
     *
     * @author LSAPP - MJMAN
     * @package shop.actions
    ...
    public function password_restore_request()
    {
        ...
        } elseif (strlen($hash) && !Customer::get_from_password_reset_hash($hash)) {
    ...
    public function on_passwordRestoreRequest()
    {
        ...
        if (strlen($hash)) {
            $this->data['action_mode'] = 'password_update';
            $validation = new Validation();
            $validation->add('new_password', 'Password')
                ->fn('trim')
                ->required("Please specify new password");

            $validation->add('confirm_password', 'Password Confirmation')
                ->fn('trim')
                ->matches('new_password', 'Password and confirmation password do not match.');

            $customer = Customer::get_from_password_reset_hash($hash);
            if (!$customer) {
                $this->data['invalid_hash'] = true;
            ...
            $this->data['action_mode'] = 'restore_request';
            $validation = new Validation();
            $validation->add('customer_email', 'Email')
                ->fn('trim')
                ->required('Please specify your email address')
                ->email()
                ->fn('mb_strtolower');
            ...
            try {
                if (Customer::send_password_restore($validation->fieldValues['customer_email'])) {
                    if (post('success_message')) {
                        Phpr::$session->flash['success'] = post('success_message');
                    }
                    $this->data['success'] = true;
                    ...
                    throw new CmsException('Email not sent.');
                }
            } catch (\Exception $ex) {
                throw new CmsException($ex->getMessage());
            }
