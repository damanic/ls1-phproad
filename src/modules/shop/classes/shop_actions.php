...
class Actions extends ActionScope
{
    ...
    public function product()
    {
        ...
        if (!strlen($product_id)) {
            $product = Product::create()->where('(shop_products.grouped is null or (shop_products.grouped is not null and shop_products.product_id is not null))')->find_by_url_name($product_url_name);
     ...
     * This AJAX handler can be used on the {@link https://lsdomainexpired.mjman.net/docs/product_page Product Details} page or on any other page.
     ...
     * This field is not required if the handler used on the {@link https://lsdomainexpired.mjman.net/docs/product_page Product Details} page.
     * Example:
     * <pre>
     * <input onclick="return $(this).getForm().sendRequest(
     *   'shop:on_addToCart',
     ...
     * {@link https://lsdomainexpired.mjman.net/docs/displaying_product_rating_and_reviews Displaying product rating and reviews}
     ...
     * This field is optional if the review form is implemented on the {@link https://lsdomainexpired.mjman.net/docs/product_page Product Details}
     * page.
     ...
     *
     * @see https://lsdomainexpired.mjman.net/docs/displaying_product_rating_and_reviews Displaying product rating and reviews
     * @see https://lsdomainexpired.mjman.net/docs/product_page Product Details page
     * @see action@shop:product
     * @author LSAPP - MJMAN
     * @package shop.ajax handlers
     ...
     * @output float $discount A discount value, calculated using the price rules, defined on the {@link https://lsdomainexpired.mjman.net/docs/shopping_cart_price_rules/ Shop/Discounts page} in the Administration Area.
     ...
     * Each element in the array is an object with two fields: <em>rule</em> ({@link CartPriceRule} object) and <em>discount</em>.
     ...
     * If the {@link https://lsdomainexpired.mjman.net/docs/configuring_lemonstand_for_tax_inclusive_environments/ Display catalog/cart prices including tax} feature
    ...
    protected function eval_cart_variables($cart_name)
    {
        ...
        $cart_taxes_details = TaxClass::calculate_taxes(Cart::list_active_items($cart_name), CheckoutData::get_shipping_info(), $tax_context);
    ...
    /**
     * Allows to add the <em>Estimate shipping cost</em> feature to the {@link https://lsdomainexpired.mjman.net/docs/cart_page Cart page}.
     * Usually this AJAX handler used on the {@link https://lsdomainexpired.mjman.net/docs/cart_page Cart page} together with {@link action@shop:cart} action.
     *
     * Please read the {@link https://lsdomainexpired.mjman.net/docs/implementing_the_shipping_cost_estimator_feature Implementing the Shipping Cost Estimator Feature article}
     ...
     * @see https://lsdomainexpired.mjman.net/docs/implementing_the_shipping_cost_estimator_feature Implementing the Shipping Cost Estimator Feature
     ...
     * Use {@link root_url()} function if your copy of LSAPP is installed in a subdirectory. To redirect to the originally
    ...
    /**
     * Base action for the {@link https://lsdomainexpired.mjman.net/docs/customer_registration_page Customer Signup} page.
     ...
     * the successful password update. Note that you should use the {@link root_url()} function if your copy of LSAPP is installed in a subdirectory.
     * @input string $flash An optional message to display after the redirection.
     * Use the {@link flash_message()} function the target page to display the message.
     ...
     * Usually this handler used on the {@link https://lsdomainexpired.mjman.net/docs/change_password_page Change Password} page,
     ...
     * Please read {@link https://lsdomainexpired.mjman.net/docs/skipping_the_shipping_method_step_for_downloadable_products_or_services Skipping the Shipping Method step for downloadable products or services} article
     ...
     * @input string $country {Billing and Shipping information steps} Specifies the customer {@link Country billing country} identifier. Required.
     * @input string $state {Billing and Shipping information steps} Specifies the customer {@link CountryState billing state} identifier. Required.
     ...
     * </ul>
     * Please refer to the {@link https://lsdomainexpired.mjman.net/docs/automatic_customer_registration Automatic customer registration} article for the implementation details.
     ...
     * Accepted values are: 0, 1. If the field value is 0, the password is not registered. In this case LSAPP will generate a random password.
     ...
     * LSAPP uses an email template with the <em>shop:registration_confirmation</em> code for the registration notification. Accepted values are: 0, 1.
     * The field has no effect if the <em>register_customer</em> field value is 0 (default).
     *
     * @input mixed $shipping_option {Shipping method step} Specifies an identifier of selected {@link ShippingOption shipping method}.
     * @input mixed $payment_method {Payment method step} Specifies an identifier of selected {@link PaymentMethod payment method}.
     *
     * @output string $checkout_step {Any checkout step} Specifies a current checkout step.
     * @output CheckoutAddressInfo $billing_info {Any checkout step} An object of the {@link CheckoutAddressInfo} class containing the customer's billing name and address.
     * @output CheckoutAddressInfo $shipping_info {Any checkout step} An object of the {@link CheckoutAddressInfo} class containing the customer's shipping name and address.
    ...
    public function checkout()
    {
        ...
        if (!$checkout_step) {
            ...
            $available_options = $this->data['shipping_options'] = CheckoutData::list_available_shipping_options($this->customer, $cart_name);
            $this->data['shipping_options_flat'] = CheckoutData::flatten_shipping_options($available_options);
            ...
            $tax_info = TaxClass::calculate_taxes(Cart::list_active_items($cart_name), CheckoutData::get_shipping_info(), $tax_context);
            ...
            $shipping_taxes = $this->data['shipping_taxes'] = CheckoutData::get_shipping_taxes($shipping_method, $cart_name);
            ...
            $order = CheckoutData::place_order($this->customer, post('register_customer', false), post('cart_name', 'main'), post('empty_cart', true));
     ...
     * Usually this AJAX handler used on the {@link https://lsdomainexpired.mjman.net/docs/checkout_page Checkout page} together with {@link action@shop:checkout} action,
     ...
     * The parameters control_name, control_id, current_state are passed into the state list partial by LSAPP when it handles
     ...
     * This action is a part of the {@link https://lsdomainexpired.mjman.net/docs/implementing_the_step_by_step_checkout step-by-step checkout process}.
     ...
     * Please refer to the {@link https://lsdomainexpired.mjman.net/docs/automatic_customer_registration/ Automatic Customer Registration}
     * article for details.
     *
     ...
     * LSAPP support multiple shopping cart. By default the cart named <em>main</em> used.
     *
     * @output CheckoutAddressInfo $billing_info An address information object representing a customer's billing name and address.
     ...
     * @author LSAPP - MJMAN
     * @package shop.actions
     * @see https://lsdomainexpired.mjman.net/docs/implementing_the_step_by_step_checkout Implementing the Step-By-Step Checkout
     ...
     * Accepted values are: 0, 1. See also the {@link https://lsdomainexpired.mjman.net/docs/automatic_customer_registration Automatic customer registration} article.
     ...
     * By default the browser is automatically redirected to the {@link https://lsdomainexpired.mjman.net/docs/pay_page payment page} after the order is placed.
     * Accepted values are: 0, 1.
     * @input string $cart_name Specifies the shopping cart name.
     * LSAPP support multiple shopping cart. By default the cart named <em>main</em> used.
     *
     ...
     *   <li><em>total</em> - total tax amount.</li>
     * </ul>
     * @output CheckoutAddressInfo $billing_info An object of the {@link CheckoutAddressInfo} class containing the customer's billing name and address.
     * @output CheckoutAddressInfo $shipping_info An object of the {@link CheckoutAddressInfo} class containing the customer's shipping name and address.
     ...
     * @output PaymentType $payment_method_obj An instance of a specific payment type class, for example the PayPal_Pro_Payment.
     ...
     * {@link https://lsdomainexpired.mjman.net/docs/creating_the_bank_transfer_and_other_similar_payment_methods bank transfer payment method}.
     *
     ...
     * @author LSAPP - MJMAN
     * @package shop.actions
     * @see https://lsdomainexpired.mjman.net/docs/creating_the_bank_transfer_and_other_similar_payment_methods Bank transfer payment method
     * @see action@shop:pay
     */
    ...
    /**
     * Base action for the {@link https://lsdomainexpired.mjman.net/docs/implementing_customer_payment_profiles Payment Profile} page.
    ...
    /**
     * Base action for the {@link https://lsdomainexpired.mjman.net/docs/implementing_customer_payment_profiles Payment Profiles} page.
     *
    ...
    public function orders()
    {
        ...
        $orders = Order::create()->where('deleted_at is null')->where('customer_id=?', $customer->id)->order('order_datetime desc')->find_all();
     ...
     * page in the URL. Please read the {@link https://lsdomainexpired.mjman.net/docs/creating_the_search_page Creating the Search page} article
     ...
     * the <em>option_names[]</em> and <em>option_values[]</em> fields. LSAPP matches names and values specified in fields
     ...
     * @input array $custom_groups A list of {@link CustomGroup custom product group} identifiers the products should belong to.
     ...
     * Read the {@link https://lsdomainexpired.mjman.net/docs/implementing_the_compare_products_feature Compare Products Page} article for the
    ...
    /**
     * Base action for the {@link https://lsdomainexpired.mjman.net/docs/implementing_the_compare_products_feature Compare Products} page.
    ...
    /**
     * Base action for the {@link https://lsdomainexpired.mjman.net/docs/manufacturer_list_page/ Creating the Manufacturer List} page.
    ...
    {
        $this->data['manufacturers'] = Manufacturer::create()->order('name')->where('(is_disabled is null or is_disabled=0)')->find_all();
    }

    /**
     * Base action for the {@link https://lsdomainexpired.mjman.net/docs/manufacturer_list_page/ Manufacturer Details} page.
    ...
    public function manufacturer()
    {
        ...
        $manufacturer = Manufacturer::create()->where('(is_disabled is null or is_disabled=0)')->find_by_url_name($url_name);
        if (!$manufacturer) {
            return;
        }
    ...
    public function customer_profile()
    {
        ...
        $this->data['states'] = CountryState::find_by_country_id(post('billing_country_id', $this->customer->billing_country_id));
        $this->data['shipping_states'] = CountryState::find_by_country_id(post('shipping_country_id', $this->customer->shipping_country_id));
