 ...
 */
abstract class GenericReport extends ChartController
{
    ...
    public function export_orders_and_products($format = null)
    {
        ...
        $this->listExportCsv('orders.csv', $options, null, true, array(
            'headerCallback' => array('Shop_Order', 'export_orders_and_products_header'),
            'rowCallback' => array('Shop_Order', 'export_orders_and_products_row')
        ));
    }
     ...
     * {@link https://lsdomainexpired.mjman.net/docs/list_filters/ List filters}
     * article for details about back-end list filters.
     ...
     * {
     *   if( !($controller instanceof Orders_Report) )
     *     return;
     *
     *   $controller->filter_filters['dealer'] = array(
     *     'name'=>'Dealer',
     *     'class_name'=>'DealerFilter_Filter',
     *     'prompt'=>'Please choose dealers statuses you want to include to the report.',
     ...
     *   {
     *     $model->where('customer_calculated_join.created_user_id in (?)', array($keys));
     *   }
     *
     *   public function asString($keys, $context = null)
     *   {
     *     return 'and shop_customers.created_user_id in '.$this->keysToStr($keys);
     *   }
     * }
     * </pre>
     *
     * @event shop:onExtendReportFilters
     * @see https://lsdomainexpired.mjman.net/docs/list_filters/ List filters
     * @package shop.events
     * @author LSAPP - MJMAN
     * @param GenericReport $controller The back-end controller object.
     */
