...
class ManufacturerFilter extends DataFilter
{
    public $model_class_name = 'Shop\Manufacturer';
    ...
    public function applyToModel($model, $keys, $context = null)
    {
        if ($context != 'product_list') {
            ...
            $model->where(
                '(exists 
                    (select * from shop_manufacturers where shop_manufacturers.id=(
    ...
    public function asString($keys, $context = null)
    {
        return 'and (if(
					...
					(select manufacturer_id 
					 from shop_products as inner_list 
					 where inner_list.id = shop_products.product_id)
					 )) 
					 in '.$this->keysToStr($keys);
    }
}
