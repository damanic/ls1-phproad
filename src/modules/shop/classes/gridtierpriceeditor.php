...
class GridTierPriceEditor extends GridEditor
{
    ...
    public function render_popup_contents($column_info, $controller, $field_name)
    {
        try {
            ...
            $controller->renderPartial(
                PATH_APP.'/modules/shop/behaviors/shop_optionmatrixbehavior/partials/_tier_price_editor.htm',
                array(
    ...
    protected function on_get_cell_internal_value($field, $model, $column_info, $controller, $column_name)
    {
        try {
            ...
            if (!$validation->validate(array('price'=>$base_price))) {
                throw new ApplicationException('Invalid value in the Base Price field');
            }
            ...
            foreach ($tier_price as $tier_id => &$price) {
                ...
                if (!$validation->validate(array('price'=>$price))) {
                    throw new ApplicationException('Invalid value in the tier price configuration: '.$price);
                }
            }
