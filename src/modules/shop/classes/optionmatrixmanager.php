...
use Core\Number;
use Db\Sql_Base as SqlBase;
use Db\File;
use Db\Helper as DbHelper;

/**
 * Provides methods for creating and updating
 * {@link https://lsdomainexpired.mjman.net/docs/understanding_option_matrix/ Option Matrix} records.
 * @documentable
 * @see https://lsdomainexpired.mjman.net/docs/understanding_option_matrix/ Understanding Option Matrix
 * @see OptionMatrixRecord
 * @package shop.classes
 * @author LSAPP - MJMAN
 */
class OptionMatrixManager
{
    const STATUS_OK = 'OK';
    ...
    protected $optionMatrixColumns;
    protected $optionMatrixModel;
    ...
    {
        $this->optionMatrixModel = OptionMatrixRecord::create();
        $this->optionMatrixModel->init_columns_info();
        $this->optionMatrixModel->define_form_fields();

        $grid_data_field = $this->optionMatrixModel->find_form_field('grid_data');
        $this->optionMatrixColumns = $grid_data_field->renderOptions['columns'];
    }
        
    /**
     * Returns data field values for Option Matrix record identified with product options and product.
     * @documentable
     * @param mixed $product Specifies either product identifier or product object
     *  ({@link Product}) the record belongs to.
    ...
    public function get_record($product, $options)
    {
        ...
        if (!count($product)) {
            throw new ApplicationException('Please specify product or product identifier.');
        }
            
        if (!count($options)) {
            throw new ApplicationException('Please specify product options.');
        }

        $filters = array();
            
        $hash = OptionMatrixRecord::generate_options_hash($options, false);
            
        $result = DbHelper::object(
            'select * from shop_option_matrix_records where product_id=:product_id and options_hash=:hash',
            array('product_id'=>$product, 'hash'=>$hash)
        );
        if (!$result) {
            return null;
        }
     ...
     * <ul>
     *   <li><em>disabled</em> - determines whether the record is disabled.
     *       String value, accepts <em>yes</em> and <em>no</em> values.</li>
     ...
     *   <li><em>tier_price</em> - an array of tier prices in the following format:
     *       [['group_id'=>10, 'quantity'=>4, 'price'=>10], ...].
     *       Use NULL for <em>group_id</em> element to indicate "Any customer group".</li>
     *   <li><em>cost</em> - specifies the product cost.</li>
     *   <li><em>on_sale</em> - determines whether the product is on sale.</li>
     *   <li><em>sale_price_or_discount</em> - specifies the sale price or discount in
     *       the following format: 10, -10, 10%.</li>
     ...
     * Usage example:
     * <pre>
     * $manager = new OptionMatrixManager();
     *
     * $options = array(
     *   'Color'=>'Red',
     *   'Size'=>'Large'
     ...
     * </pre>
     * @documentable
     * @param Product $product Specifies a product the record belongs to.
     * @param array $options A list of the product options and option values in
     *  the following format: ['Color'=>'Red', 'Size'=>'M'].
     * @param array $data Data values to assign to the record.
     * @param bool $skip_existing Skip the operation if a record with specified
     *  options already exists (do not update it).
     * @return mixed Returns an object with the following fields:
     *  <em>status</em>, <em>operation</em>, <em>warnings</em>, <em>id</em>.
     */
    public function add_or_update($product, $options, $data, $skip_existing = false)
    {
        ...
        foreach ($options as $option_name => $option_value) {
            if (!array_key_exists($option_name, $product_options)) {
                throw new ApplicationException(sprintf('Option %s does not exist in the product.', $option_name));
            }
        }
        ...
        foreach ($data as $field_name => &$value) {
            ...
            if (array_key_exists($field_name, $this->optionMatrixColumns)) {
                $column_options = $this->optionMatrixColumns[$field_name];
                    ...
                    case db_float:
                        if (!Number::isValidFloat($value)) {
                            throw new ApplicationException(
                                sprintf('Invalid numeric value in %s column: %s', $field_name, $column_title)
                            );
                        }
                        break;
                    case db_number:
                        if (!Number::isValidInt($value)) {
                            throw new ApplicationException(
                                sprintf('Invalid integer value in %s column: %s', $field_name, $column_title)
                            );
                        }
                        break;
                    case db_date:
                        if (!preg_match('/[0-9]{4}\-[0-9]{2}\-[0-9]{2}/', $value)) {
                            throw new ApplicationException(
                                sprintf('Invalid date value in %s column: %s', $field_name, $column_title)
                            );
                        }
                        break;
                    case 'discount':
                        ...
                        if ($field_name == 'sale_price_or_discount') {
                            ...
                            if (!strlen($value) && $on_sale_value) {
                                throw new ApplicationException(
                                    'Please specify a sale price or discount or disable the "On Sale" feature'
                                );
                            }
                        }

                        if ($error = Product::is_sale_price_or_discount_invalid($value, $base_price_value)) {
        ...
        foreach ($data as $data_key => $data_value) {
            if (array_key_exists($data_key, $this->optionMatrixColumns)) {
        ...
        if (array_key_exists('images', $original_data)) {
            ...
            if (!is_array($original_data['images'])) {
                throw new ApplicationException('The images parameter value should be an array');
            }
        ...
        if (!$existing_record) {
            ...
            $this->optionMatrixModel->sql_insert('shop_option_matrix_records', $data);
            $record_id = $this->optionMatrixModel->last_insert_id('shop_option_matrix_records', 'id');
            ...
            $sqlEntries = '';
            foreach ($options as $option_name => $option_value) {
                $sqlEntries .= $sqlBase->prepare(
                    '(:record_id, :option_id, :option_value)',
                    array(
                            'record_id'=>$record_id,
            ...
            if ($sqlEntries) {
                $sqlEntries = substr($sqlEntries, 0, -1);
                $sql = 'INSERT INTO shop_option_matrix_options (matrix_record_id, option_id, option_value)';
                $sql .= ' VALUES '.$sqlEntries;
                DbHelper::query($sql);
            }
            ...
            $this->optionMatrixModel->sql_update('shop_option_matrix_records', $data, 'id='.$existing_record->id);
        ...
        if (array_key_exists('in_stock', $data)) {
            if ($result->operation == self::OPERATION_ADD || $data['in_stock'] != $existing_record->in_stock) {
                Product::update_total_stock_value($product);
            }
        }
    ...
    {
        if (array_key_exists($product_id, self::$optionCache)) {
            return self::$optionCache[$product_id];
        }

        $option_records = DbHelper::objectArray(
            'select name, id from shop_custom_attributes where product_id=:product_id',
        ...
        return self::$optionCache[$product_id] = $result;
    }
        
    protected function parse_tier_price_data($data, $product)
    {
        if (!is_array($data)) {
            throw new ApplicationException('The tier price parameter value should be an array');
        }
        ...
        foreach ($data as $tier_data) {
            if (!array_key_exists('group_id', $tier_data)) {
                throw new ApplicationException('Index "group_id" is not found in the tier price configuration.');
            }

            if (!array_key_exists('quantity', $tier_data)) {
                throw new ApplicationException('Index "quantity" is not found in the tier price configuration.');
            }

            if (!array_key_exists('price', $tier_data)) {
                throw new ApplicationException('Index "price" is not found in the tier price configuration.');
            }
            ...
            if (!Number::isValidInt($tier_data['group_id'])) {
                throw new ApplicationException(
                    sprintf('Invalid tier price group identifier value: %s', $tier_data['group_id'])
                );
            }

            if (!Number::isValidInt($tier_data['quantity'])) {
                throw new ApplicationException(
                    sprintf('Invalid tier price quantity value: %s', $tier_data['quantity'])
                );
            }

            if (!Number::isValidFloat($tier_data['price'])) {
                throw new ApplicationException(
                    sprintf('Invalid tier price value: %s', $tier_data['price'])
                );
            }
            ...
            if (strlen($tier_data['group_id'])) {
                $group = CustomerGroup::find_by_id($tier_data['group_id']);
                if (!$group) {
                    throw new ApplicationException(
                        sprintf('Customer group with identifier %s not found.', $tier_data['group_id'])
                    );
                }
            ...
            if ($group_id !== null) {
                $tier_id = DbHelper::scalar('select 
							id 
						from 
							shop_tier_prices 
						where 
            ...
            if (!$tier_id) {
                throw new ApplicationException(
                    sprintf(
                        'Price tier for customer group "%s" and quantity "%s" not found in the parent product.',
                        $group_name,
                        $tier_data['quantity']
                    )
                );
    ...
    protected function set_record_images($record_images, $record_id)
    {
        ...
        $classId = get_class_id('Shop\OptionMatrixRecord');
        $record_files = DbHelper::queryArray(
            "select 
					* 
				from 
					db_files 
				where 
					master_object_class=:class 
        ...
        foreach ($record_images as $image_path) {
            ...
            $file->master_object_class = $classId;
            $file->field = 'images';
