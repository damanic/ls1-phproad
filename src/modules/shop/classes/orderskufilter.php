...
class OrderSKUFilter extends DataFilter
{
    ...
    public function prepareListData()
    {
        ...
        $obj->where(
            '((shop_products.grouped is null or shop_products.grouped=0) 
             or (shop_products.grouped=1 and shop_products.product_id is not null))'
        );
        return $obj;
    }
    ...
    {
        $skus = DbHelper::scalarArray("SELECT sku FROM shop_products WHERE id IN (?)", array($keys));
        ...
        return 'AND (
                    EXISTS 
                        (
                            ...
                            ON shop_order_items.option_matrix_record_id = shop_option_matrix_records.id
                            WHERE shop_orders.id = 	shop_order_items.shop_order_id
                            AND COALESCE(shop_option_matrix_records.sku, shop_products.sku) 
                            IN  '.$this->keysToStr($skus).' 
                        )
                )';
    }
}
