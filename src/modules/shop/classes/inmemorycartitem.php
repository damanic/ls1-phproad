...
class InMemoryCartItem
{
    ...
    public $bundle_offer_id = null;         // Master bundle item (ProductBundleOffer) identifier
    public $bundle_offer_item_id = null;    // Master bundle item product (ProductBundleOfferItem) identifier
    ...
    public static function gen_item_key($product_id, $options, $extras, $custom_data, $uploaded_files)
    {
        ...
        try {
            if ($uploaded_files && $uploaded_files instanceof DataCollection) {
    ...
    public function get_content_key(&$existing_items)
    {
        ...
        foreach ($existing_items as $existing_item) {
            if (!$existing_item->bundle_master_cart_key) {
                continue;
            }
            if ($existing_item->bundle_master_cart_key != $this->key) {
                continue;
            }
            if ($existing_item->key == $this->key) {
                continue;
            }
    ...
    /**
     * @deprecated
     * @see InMemoryCartItem::get_bundle_offer_item()
     */
