...
class ProductSearch
{
    ...
    protected static $sortingDirection;
    protected static $searchTestProduct;
    protected static $searchTestOmRecord;
     ...
     * <li><em>manufacturer_ids</em>
     *  - an array of {@link Manufacturer manufacturer} identifiers to limit the result
     *   with specific manufacturers.
     * </li>
     * <li><em>options</em>
     *    - an array of product options as a name-value list.
     *    Example: <em>array('color'=>'black')</em>.
     *    You can use the <em>wildcard</em> character as an option value if you want
     *    to find all products with a specific option having any value.
     * </li>
     * <li><em>properties</em>
     *    - an array of product properties as a name-value list.
     *    Example: <em>array('paper format'=>'A4')</em>.
     *    If your products can have multiple properties with a same name and you want
     *    to search by multiple property values, you
     *    can specify the property value as array:
     *    <em>array('paper format'=>array('A4', 'A5'))</em>
     *     - the method will find products
     *    with the <em>paper format</em> property having values of
     *    <em>A4</em> or <em>A5</em>. You can use the <em>wildcard</em>
     *    character for the property value if you want to find all products with
     *    a specific property having any value.
     * </li>
     ...
     * <li><em>sorting</em>
     *    - the product sorting expression, string. The following values are supported: <em>relevance</em>,
     ...
     *    product rating, and the product_rating_all value corresponds to the full
     *    product rating (approved and not approved).
     *    All options (except the relevance) support the sorting direction expression
     *    - <em>asc</em> and <em>desc</em>, so you can use values like "price desc".
     *    You can extend the list of allowed sorting columns with the
     *    {@link shop:onGetProductSearchSortColumns} event
     * </li>
     * </ul>
     * Usage example:
     * <pre>
     ...
     * </pre>
     * By default the find_products() method uses soft comparison in
     * the options and properties search. This means that if you specify
     * an option or property value <em>large</em> and if there are products
     * which have the option value <em>large frame</em>, those
     * product will be returned. You can enable strict search by adding the
     * <em>exclamation</em> sign before the option or property value:
     * <pre>
     * $options['properties'] = array('CPU'=>'!2.33');
     * $options['options'] = array('color'=>'!black');
     * </pre>
     ...
     * @return \Db\DataCollection Returns a collection of Product objects.
     */
    public static function find_products($query, $pagination, $page = 1, $options = array())
    {
        ...
        $search_in_product_names = !isset($options['search_in_product_names']) || $options['search_in_product_names'];
        ...
        if ($search_in_grouped_products) {
            $grouped_products_filter = "and ifnull(
                (
                    if (
                        grouped is null, 
                        shop_products.disable_completely, 
                        (
                            select disable_completely from shop_products parent_list 
                            where parent_list.id=shop_products.product_id
                        )
                    )
                ), 
                0
        ...
        $group_filter_field = "if (
            grouped is null, 
            enable_customer_group_filter, 
            (
                select enable_customer_group_filter 
                from shop_products parent_list 
                where parent_list.id=shop_products.product_id
            )
        )";
        $search_visibility_field = "if (
            grouped is null, 
            visibility_search, 
            (
                select visibility_search 
                from shop_products parent_list 
                where parent_list.id=shop_products.product_id
            )
        )";
        ...
        if ($configuration->search_in_option_matrix) {
            $om_records_exist_field = 'exists(
                select id from shop_option_matrix_records 
					...
					and (
						...
						(
							exists(
							    ...
							    where shop_product_id=if(grouped is null, shop_products.id, shop_products.product_id) 
        ...
        if (!$search_in_grouped_products) {
            $query_template = str_replace(
                '%CATEGORY_FILTER%',
                'shop_products_categories.shop_product_id=shop_products.id',
                $query_template
            );
        } else {
            $query_template = str_replace(
                '%CATEGORY_FILTER%',
                'shop_products_categories.shop_product_id=if(
                    grouped is null, 
                    shop_products.id, 
        ...
        $category_ids = $options['category_ids'] ?? array();
        if ($category_ids) {
            $valid_ids = array();
            ...
            if ($valid_ids) {
                ...
                $query_template = Product::set_search_query_params(
                    $query_template,
                    '%TABLES%',
                    'shop_products_categories.shop_category_id in '.$valid_ids.' and %FILTER%'
                );
            }
        }
        ...
        $manufacturer_ids = $options['manufacturer_ids'] ?? array();
        if ($manufacturer_ids) {
            $valid_ids = array();
            ...
            if ($valid_ids) {
                ...
                $query_template = Product::set_search_query_params(
                    $query_template,
                    '%TABLES%',
                    'shop_products.manufacturer_id is not null 
                     and shop_products.manufacturer_id in '.$valid_ids.' 
                     and %FILTER%'
                );
            }
        }
        ...
        if ($product_options) {
            ...
            foreach ($product_options as $name => $values) {
                ...
                foreach ($values as $value) {
                    ...
                    $name = DbHelper::escape($name);
                    $value = DbHelper::escape($value);
                    if (substr($value, 0, 1) != '!') {
                        $product_option_queries[] = "(exists(
                            select id from shop_custom_attributes 
                            where name='".$name."' 
                            and attribute_values like '%".$value."%' 
                            and product_id=shop_products.id
                        ))";
                    } else {
                        $value = substr($value, 1);
                        $product_option_queries[] = "(exists(
                            select id from shop_custom_attributes where name='".$name."' 
                            and find_in_set('".$value."', replace(attribute_values, '"."\n"."', ',')) > 0 
                            and product_id=shop_products.id
                        ))";
                    }
                }
        ...
        if ($product_properties) {
            ...
            foreach ($product_properties as $name => $values) {
                ...
                foreach ($values as $value) {
                    ...
                    $value = DbHelper::escape($value);
                    if (substr($value, 0, 1) != '!') {
                        $product_property_queries[] = "(exists(
                            select id from shop_product_properties 
                            where name='".$name."' 
        ...
        */
        ...
        if ($custom_groups) {
            ...
            foreach ($custom_groups as $custom_group) {
                if (strlen($custom_group)) {
                    $valid_groups[] = DbHelper::escape($custom_group);
                }
            }
            if (count($valid_groups)) {
                $custom_groups = "('".implode("','", $valid_groups)."')";
                $custom_group_filter = " shop_products.id in (
                    select shop_product_id from shop_products_customgroups 
                    inner join shop_custom_group 
                    on (shop_products_customgroups.shop_custom_group_id = shop_custom_group.id) 
                    where code in ".$custom_groups."
                ) ";
                $query_template = Product::set_search_query_params(
                    $query_template,
                    '%TABLES%',
                    $custom_group_filter.' and %FILTER%'
                );
            }
        }
        ...
        if ($search_in_product_names) {
            $records = DbHelper::objectArray(Product::set_search_query_params(
                $query_template,
                null,
                DbHelper::formatSearchQuery($query, 'shop_products.name', 2)
            ));
        ...
        if ($configuration->search_in_short_descriptions && $query_presented) {
            $records = DbHelper::objectArray(
                Product::set_search_query_params(
                    $query_template,
                    null,
                    DbHelper::formatSearchQuery($query, 'short_description', 2)
                )
            );
            foreach ($records as $record) {
        ...
        if ($configuration->search_in_long_descriptions && $query_presented) {
            $records = DbHelper::objectArray(
                Product::set_search_query_params(
                    $query_template,
                    null,
                    DbHelper::formatSearchQuery($query, 'pt_description', 2)
                )
            );
            foreach ($records as $record) {
                if (array_key_exists($record->id, $product_ids)) {
        ...
        if ($configuration->search_in_keywords && $query_presented) {
            $records = DbHelper::objectArray(
                Product::set_search_query_params(
                    $query_template,
                    null,
                    DbHelper::formatSearchQuery($query, 'meta_keywords', 2)
                )
            );
            foreach ($records as $record) {
                if (array_key_exists($record->id, $product_ids)) {
        ...
        if ($configuration->search_in_categories && $query_presented) {
            $category_query_template = 'select id from shop_categories 
                                        where %s 
                                        and (category_is_hidden is null 
                                        or category_is_hidden=0) 
                                        order by name';
            $category_records = DbHelper::objectArray(
                sprintf(
                    $category_query_template,
                    DbHelper::formatSearchQuery($query, 'name', 2)
                )
            );

            foreach ($category_records as $category) {
                $product_records = DbHelper::objectArray(
                    Product::set_search_query_params(
                        $query_template,
                        null,
                        'shop_products_categories.shop_category_id='.$category->id
                    )
                );
        ...
        if ($configuration->search_in_manufacturers && $query_presented) {
            $manufacturer_join = 'left join shop_manufacturers 
                                  on shop_manufacturers.id = shop_products.manufacturer_id';
            $records = DbHelper::objectArray(
                Product::set_search_query_params(
                    $query_template,
                    $manufacturer_join,
                    DbHelper::formatSearchQuery($query, 'shop_manufacturers.name', 2)
                )
            );
            foreach ($records as $record) {
                if (array_key_exists($record->id, $product_ids)) {
        ...
        if ($configuration->search_in_sku && $query_presented) {
            ...
            $filter = sprintf(
                $filter,
                DbHelper::formatSearchQuery($query, 'shop_products.sku', 2),
                DbHelper::formatSearchQuery($query, 'grouped_products.sku', 2)
            );

            $records = DbHelper::objectArray(Product::set_search_query_params($query_template, null, $filter));
            foreach ($records as $record) {
                if (array_key_exists($record->id, $product_ids)) {
                    continue;
                }
        ...
        if (strlen($min_price) || strlen($max_price)) {
            ...
            if (count($products)) {
                $grouped_products = DbHelper::objectArray("
						select
            ...
            foreach ($products as $product) {
                if (Product::check_price_range($test_product, $product, $min_price, $max_price, $test_om_record)) {
                    $filtered_products[] = $product;
                } else {
                    if (array_key_exists($product->id, $grouped_products_sorted)) {
                        foreach ($grouped_products_sorted[$product->id] as $grouped_product) {
                            $inPriceRange = Product::check_price_range(
                                $test_product,
                                $grouped_product,
                                $min_price,
                                $max_price,
        ...
        self::$sorting = $normalized_search_expr;
        self::$sortingDirection = strpos($sorting, 'desc') === false ? 1 : -1;
        self::$searchTestProduct = Product::create();
        self::$searchTestOmRecord = OptionMatrixRecord::create();

        if ($sorting != 'relevance') {
            uasort($products, array('ProductSearch', 'sort_search_result'));
        }
        ...
        if (count($products)) {
            ...
            foreach ($products as $product_data) {
                if (array_key_exists($product_data->id, $loaded_products)) {
                    ...
                    $product = Product::create()->where('shop_products.id=?', $product_data->id)->find();
                    $loaded_products[$product_data->id] = $product;
                }

                if ($product_data->om_record_id) {
                    $product->set_om_options(OptionMatrixRecord::create()->find($product_data->om_record_id));
                }
        ...
        return new DataCollection(array());
    }
    ...
    protected static function apply_om_search(&$products, $options)
    {
        ...
        if ($products_with_om) {
            ...
            $disabled_product_ids = DbHelper::scalarArray('
					select
						id
					from
						shop_products
            ...
            foreach ($product_options as $name => $value) {
                ...
                $name = DbHelper::escape($name);
                $value = DbHelper::escape($value);
                $strict = mb_substr($value, 0, 1) == '!';
                $condition = $strict ? "option_value='".mb_substr($value, 1)."'" : "option_value like '%".$value."%'";
    ...
    {
        if (self::$sorting != 'price') {
            if (self::$sorting == 'name') {
                return strcmp(mb_strtolower($product_1->name), mb_strtolower($product_2->name))*self::$sortingDirection;
            }

            $products_sorting = self::$sorting;
            ...
            if ($product_1->$products_sorting > $product_2->$products_sorting) {
                return 1*self::$sortingDirection;
            }

            return -1*self::$sortingDirection;
        } else {
            $product_1_price = Product::eval_static_product_price(self::$searchTestProduct, $product_1);
            if ($product_1->om_data_fields) {
                $product_1_price = OptionMatrixRecord::get_sale_price_static(
                    self::$searchTestProduct,
                    self::$searchTestOmRecord,
                    $product_1->om_data_fields
                );
            }

            $product_2_price = Product::eval_static_product_price(self::$searchTestProduct, $product_2);
            if ($product_2->om_data_fields) {
                $product_2_price = OptionMatrixRecord::get_sale_price_static(
                    self::$searchTestProduct,
                    self::$searchTestOmRecord,
                    $product_2->om_data_fields
                );
            }
