...
use Phpr\ApplicationException as ApplicationException;
...
class CurrencyConverter
{
    ...
    public function get_rate($from_currency, $to_currency)
    {
        ...
        $converter_params = CurrencyConversionParams::create()->get();
        if (!$converter_params) {
        ...
        if ($converter_params->enable_cron_updates) {
            ...
            */
             ...
             */
            $record = CurrencyRateRecord::create();
            $record->where('from_currency=?', $from_currency);
            $record->where('to_currency=?', $to_currency);
            $record->where('DATE_ADD(created_at, interval '.$interval.' hour) >= ?', PhprDateTime::now());
            $record = $record->find();
        ...
        try {
            ...
            if (!$rate) {
                throw new ApplicationException(
                    "Currency converter could not determine an exchange rate for $from_currency => $to_currency"
                );
            }
            ...
            traceLog(
                "Currency converter did not return an exchange rate for $from_currency => $to_currency . 
                Used last record as fallback"
            );
    ...
    public function update_rate($from_currency, $to_currency, $rate)
    {
        $result = Backend::$events->fireEvent(
            'shop:onAdjustCurrencyConverterRate',
            $from_currency,
            $to_currency,
            $rate
        );
    ...
    {
        $result = $value*$this->get_rate($from_currency, CurrencySettings::get()->code);
        return $round === null ? $result : round($result, $round);
    }

    public function update_all_rates($cron = false)
    {
        $converter = CurrencyConversionParams::create()->get();
        if (!$converter) {
            return false;
        }
        if ($cron) {
        ...
        if (!class_exists($class_name)) {
            throw new ApplicationException("Currency converter class $class_name not found.");
        }

        $converter->define_form_fields();
        $converter_obj = new $class_name();
        $interval = $converter->refresh_interval ?: 24;
        ...
        $currencies = DbHelper::queryArray($sql);
        ...
        foreach ($currencies as $data) {
            ...
            $result = DbHelper::scalar($sql, $sql_vars);
