...
class ProductExport
{
    ...
    public static function export_csv(
        ...
        $columns = Product::create()->get_csv_import_columns(false);
            
        if (is_array($columns_override)) {
            $columns_updated = array();
            foreach ($columns_override as $column_name) {
                if (!array_key_exists($column_name, $columns)) {
                    throw new ApplicationException(
                        sprintf('Column %s not found in the product column set.', $column_name)
                    );
                }
    ...
    protected static function export_images_to_zip($archivePath, $images_to_export, $base_path = 'images')
    {
        ...
        Zip::zipDirectory(PATH_APP.'/uploaded', $images_to_export, $archivePath);
    }
    ...
    protected static function get_related_skus($product)
    {
        if (self::$related_map === null) {
            ...
            $related_products = DbHelper::objectArray(
                'select
    ...
    protected static function get_global_extra_sets($product)
    {
        if (self::$extra_sets_map === null) {
            ...
            $extra_set_links = DbHelper::objectArray(
                'select code, shop_products_extra_sets.extra_product_id as product_id
    ...
    protected static function list_categories($product)
    {
        ...
        $product_categories = DbHelper::objectArray(
            "select shop_categories.id, shop_categories.name, shop_categories.category_id 
    ...
    {
        $options = DbHelper::objectArray(
            'select * from shop_extra_options 
                  where product_id=:product_id and (option_in_set is null or option_in_set=0) 
                  order by extra_option_sort_order',
        ...
        foreach ($options as $option) {
            ...
            $images = DbHelper::scalarArray(
                'select name from db_files 
                      where master_object_class=:master_object_class 
                        and master_object_id=:master_object_id 
                        and field=:field 
                      order by sort_order',
                [
                'master_object_class'=>'ExtraOption',
    ...
    protected static function get_om_price_tiers($om_record)
    {
        ...
        try {
            ...
            throw new SystemException('Error loading tier prices for Option Matrix record');
        }
    ...
    protected static function get_manufacturer($product)
    {
        if (!$product->grouped) {
            ...
            $manufacturer_id = DbHelper::scalar(
                'select manufacturer_id from shop_products where id=:id',
        ...
        return self::$manufacturer_names[$manufacturer_id] = DbHelper::scalar(
            'select name from shop_manufacturers where id=:id',
            ['id'=>$manufacturer_id]
        );
    }
    ...
    protected static function get_product_type($product)
    {
        ...
        $product_type_id = DbHelper::scalar('select product_type_id from shop_products where id=:id', array('id'=>$id));
        ...
        return self::$product_types[$product_type_id] = DbHelper::scalar(
            'select name from shop_product_types where id=:id',
            ['id'=>$product_type_id]
        );
    }
    ...
    protected static function get_property_value($product, $db_name)
    {
        ...
        $values = DbHelper::queryArray(
            'select value from shop_product_properties where product_id=:product_id and name=:name',
            array('product_id'=>$product->id, 'name'=>$prop_name)
        );
        if (count($values)) {
    ...
    protected static function get_tax_class($product)
    {
        ...
        return self::$tax_class_names[$product->tax_class_id] = DbHelper::scalar(
            'select name from shop_tax_classes where id=:id',
            ['id'=>$product->tax_class_id]
        );
    }
    ...
    protected static function list_product_groups($product)
    {
        ...
        $product_groups = DbHelper::objectArray(
            "select scg.name from shop_custom_group scg
    ...
    {
        Zip::zipDirectory($base_path, $files_to_archive, $archivePath);
    }
