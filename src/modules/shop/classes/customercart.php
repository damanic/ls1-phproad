...
class CustomerCart extends CartBase
{
    private $items = array();
    private $customer;
    ...
    public function add_item(
        ...
        /*
        $key = CustomerCartItem::gen_item_key($product_id, $options, $extra_options, $custom_data, $uploaded_files);
        */
        ...
        if ($uploaded_files && ($uploaded_files instanceof DataCollection || is_array($uploaded_files))) {
    ...
    public function list_items($cart_name)
    {
        ...
        $cartItemModel = CustomerCartItem::create()
            ->where('customer_id=?', $this->customer->id)
            ->where('cart_name=?', $cart_name)
            ->order('shop_customer_cart_items.id');

        $cartItemsArray = $cartItemModel->find_all()->as_array(null, 'item_key');
        return $this->items[$cart_name] = $cartItemsArray;
    }
    ...
    public function find_matching_item(
        $cart_name,
        $product_id,
        $options,
        $extras,
        ...
        $uploaded_files_array = Cart::uploaded_files_to_array($uploaded_files);

        $new_item_content_key = CustomerCartItem::gen_item_content_key(
            $product_id,
            $options,
            $extras,
            $custom_data,
    ...
    public function get_cart_items($cart_name)
    {
        ...
        if ($stored_items) {
            foreach ($stored_items as $key => $item) {
                ...
                $product_lookup        = Product::create()->where('id = ?', $item->product_id)->apply_visibility();
                $sql                   = $product_lookup->build_sql();
                $result = DbHelper::queryArray($sql);
                $product_data = $result[0] ?? false;
                if ($product_data) {
                    $product = new ActiverecordProxy(
                        $product_data['id'],
                        'Shop\Product',
                        $product_data
                    );
