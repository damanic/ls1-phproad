...
class AutoBilling
{
    ...
    public function process()
    {
        ...
        try {
            ...
            try {
                $template = EmailTemplate::create()->find_by_code('shop:auto_billing_report');
                if ($template) {
                    $message = str_replace(
                        '{autobilling_report}',
                        nl2br(h(self::format_result($result))),
                        $template->content
                    );
                ...
                throw new SystemException('Automated billing report sending failed. Error message: '.$ex->getMessage());
            }
        } catch (\Exception $ex) {
    ...
    protected function process_billing()
    {
        ...
        try {
            ...
            $params = AutoBillingParams::get();
            if (!$params) {
                throw new SystemException('Automated billing configuration record not found in the database.');
            }
            ...
            if (!$payment_method) {
                throw new SystemException('Automated payments configuration error - payment method not found.');
            }
                
            $current_time = PhprDateTime::now();
            ...
            $interval = new PhprDateTimeInterval($params->billing_period);
            $check_date = Date::userDate(PhprDateTime::now())->getDate()->substractInterval($interval);
                    ...
                    if ($params->payment_method->find_customer_profile($order->customer)) {
                        ...
                        try {
                            ...
                            if ($notification_failed) {
                                $msg = ' Customer notification failed: '.$notification_failed;
                                $result->invoice_data[$order->id] .= $msg;
                            }
                        } catch (\Exception $ex) {
                            /*
                        ...
                        $this->mark_fail($current_time, $order);
                        $methodName = $params->payment_method->name;
                        $msg = 'SKIPPED. Customer payment profile does not exist for '.$methodName.' payment method.';
                        $result->invoice_data[$order->id] = $msg;
                    }
                } catch (\Exception $ex) {
    ...
    {
        DbHelper::query(
            'update shop_orders set automated_billing_success=:current_time where id=:id',
            array(
    ...
    {
        DbHelper::query(
            'update shop_orders set automated_billing_fail=:current_time where id=:id',
            array(
                'current_time'=>$current_time,
                'id'=>$order->id
            )
