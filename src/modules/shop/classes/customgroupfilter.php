...
class CustomGroupFilter extends DataFilter
{
    public $model_class_name = 'Shop\CustomGroup';
    ...
    public function applyToModel($model, $keys, $context = null)
    {
        if ($context == 'product_report') {
            $model->where(
                '(exists 
                    (select * from shop_products_customgroups 
                     ...
                     and shop_products_customgroups.shop_custom_group_id in (?))
                )',
                array($keys)
            );
            return;
    ...
    public function asString($keys, $context = null)
    {
        ...
        return "and (exists 
                        (select shop_products.id from shop_products_customgroups 
                         where shop_products.id=shop_order_items.shop_product_id 
                         and shop_order_items.shop_order_id=shop_orders.id 
                         and shop_products_customgroups.shop_product_id=(
                             if(shop_products.grouped is null or shop_products.grouped=0, 
                         ...
                         and shop_products_customgroups.shop_custom_group_id in $keysStr
                     )
                 )";
    }
}
