 ...
 * dimensions and weight of the cart items. Please read the
 * {@link https://lsdomainexpired.mjman.net/docs/cart_page/ Creating the Cart page}
 * article for examples of
 * the class usage.
 *
 * All methods of this class accept the <em>$cart_name</em>
 * parameter, which default value is <em>main</em>. You can implement multiple
 * shopping carts with LSAPP. You can post the cart_name parameter to the {@link shop:on_addToCart} handler in
 * order to place an item into a specific cart.
 * By default LSAPP always work with the shopping cart named <em>main</em>.
 * @documentable
...
class Cart
{
     ...
     * @documentable
     * @deprecated Use {@link Cart::add_cart_item() add_cart_item()} method.
     * @param Product Specifies the product to add.
    ...
    public static function add_item(
        ...
        return self::add_cart_item($product, [
                'options'=>$options,
     ...
     *   <li><em>extra_options</em>
     *      - an array of product extra options in the following format: [option_key_1, option_key_2].</li>
     *   <li><em>options</em> - an array of product options in the following format: [option_key=>option_value].</li>
     *   <li><em>custom_data</em> - an array of
     * {@link https://lsdomainexpired.mjman.net/docs/allowing_customers_to_provide_order_item_specific_information/
     * custom API field} names and values.</li>
     *   <li><em>bundle_data</em>
     *      - an array containing
    ...
    public static function add_cart_item($product, $parameters)
    {
         ...
         */
        if (!isset($bundle_data_normalized)) {
            $bundle_data = self::normalize_bundle_data($bundle_data, true, true);
        } else {
            $bundle_data = $bundle_data_normalized;
        }
        ...
        $existing_item = self::get_cart()->find_matching_item(
            ...
            $custom_data,
            $uploaded_files ?? $product->list_uploaded_files(),
            array(),
            $bundle_data,
            $quantity
        );
        ...
        if (!$existing_item) {
            foreach ($bundle_data as $bundle_offer_id => $item_data) {
                foreach ($item_data as $product_data) {
                    ...
                    $product_parameters['product'] = Product::find_by_id($product_data['product_id']);
    ...
    protected static function add_product($parameters)
    {
        ...
        $event_listeners_exist = Backend::$events->listeners_exist(
            'shop:onPreProcessProductCustomData',
            'shop:onBeforeAddToCart',
            'shop:onAfterAddToCart'
        );
        if ($event_listeners_exist) {
            self::reset_cart_cache($cart_name);

            $custom_data_updated = Backend::$events->fireEvent(
                'shop:onPreProcessProductCustomData',
                $cart_name,
                $product,
                $quantity,
    ...
    {
        $allow_pre_order = OptionMatrix::get_property($options, 'allow_pre_order', $product, $option_keys);

        if (!$allow_pre_order && $product->inventory_tracking_enabled()) {
            $in_stock = OptionMatrix::get_property($options, 'in_stock', $product, $option_keys);

            if ($total_quantity > $in_stock) {
                if ($in_stock > 0) {
                    throw new CmsException(
                        'We are sorry, but only '.$in_stock.' unit(s) of the "'.$product->name.'" 
                        product are available in stock.'
                    );
                } else {
                    throw new CmsException(
                        'We are sorry, but the product "'.$product->name.'" is out of stock.'
                    );
                }
            }
        }
    ...
    protected static function check_product_availability($product, $quantity, $cart_name, $options)
    {
        if (!$product->allow_pre_order && $product->inventory_tracking_enabled()) {
            ...
            $product_om_record = OptionMatrixRecord::find_record($options, $product, true);
            ...
            foreach ($items as $item) {
                ...
                if (($product_om_record && $item_om_record && $product_om_record->id == $item_om_record->id) ||
        ...
        foreach ($bundle_data as $bundle_offer_id => $item_data) {
            foreach ($item_data as $product_data) {
                self::check_product_availability(
                    Product::find_by_id($product_data['product_id']),
                    $product_data['quantity']*$master_quantity,
                    $cart_name,
    ...
    public static function uploaded_files_to_array($files)
    {
        ...
        if ($files instanceof DataCollection) {
...
//                  $file_info['path'] = $db_file->getPath();
    ...
    public static function normalize_bundle_data(
        $bundle_data,
        $validate_quantity = false,
        $apply_grouped_products = false
    ) {
        $defaults = array(
            'extra_options'=>array(),
            'options'=>array(),
                    ...
                    if (array_key_exists('quantity', $product_data)) {
                        ...
                        if (!strlen($product_data['quantity'])) {
                            ...
                            throw new ApplicationException(
                                sprintf(
                                    'Please specify quantity for product "%s"',
                                    $product->name
                                )
                            );
                        }
                        ...
                        if (!preg_match('/^[0-9]+$/', $product_data['quantity']) || $product_data['quantity'] <= 0) {
                            ...
                            throw new CmsException(
                                sprintf(
                                    'Invalid quantity value specified for product "%s"',
                                    $product->name
                                )
                            );
                        }
                    } else {
                        $item_product = ProductBundleOfferItem::create()->find($product_data['bundle_offer_item_id']);
    ...
    protected static function check_requred_bundle_items($product, $bundle_data)
    {
        foreach ($product->bundle_offers as $bundle_offer) {
            if ($bundle_offer->is_required && !array_key_exists($bundle_offer->id, $bundle_data)) {
                throw new ApplicationException(sprintf('Please select %s.', $bundle_offer->name));
            }
        }
    }
    ...
    protected static function parse_bundle_product_id($product_id_data, &$product_id, &$bundle_offer_item_id)
    {
        ...
        if (count($parts) != 2) {
            throw new ApplicationException('Invalid bundle item product specifier');
        }
     ...
     * @documentable
     * @param string $key Specifies the item {@link CartItem::key key}.
     * @param string $cart_name Specifies the cart name.
     */
    ...
    /**
     * Finds a cart item by its {@link CartItem::key key}.
     * @documentable
     ...
     * @param string $key Specifies the cart item {@link CartItem::key key}.
    ...
    public static function list_items($cart_name = 'main', $auto_remove_items = true)
    {
        ...
        if (count($product_ids_to_load) > 0) {
            $products = new Product(null, array('no_timestamps'=>true, 'no_validation'=>true, 'no_column_init'=>true));
        ...
        $options = new CustomAttribute(null, array('no_timestamps'=>true));
    ...
    public static function move_cart()
    {
        ...
        foreach ($carts as $cart_name) {
            ...
            if ($in_memory_items) {
                $cart_behavior = ConfigurationRecord::get()->cart_login_behavior;
    ...
    private static function add_customer_cart_item($item, $existing_items)
    {
        ...
        if ($product = Product::find_by_id($item->product_id)) {
    ...
    private static function get_cart()
    {
        ...
        if (self::$customer_cart === null) {
            self::$customer_cart = new CustomerCart($customer);
        }
    ...
    private static function get_in_memory_cart()
    {
        if (self::$in_memory_cart === null) {
            self::$in_memory_cart = new InMemoryCart();
        }
     ...
     *   if ($product->sku == '123')
     *     throw new ApplicationException('You cannot add this product to the cart!');
     * }
     * </pre>
