 ...
 */
abstract class PaymentType extends XmlController
{
    /**
     ...
     * @param $host_obj ActiveRecord object containing configuration fields values
     * @return \Cms\Page Returns a page object
     */
     ...
     * @param $host_obj ActiveRecord object containing configuration fields values
     * @param \Cms\Page $page Specifies a page to be deleted
     */
     ...
     * @param $host_obj ActiveRecord object containing configuration fields values
     * @param OrderStatus $status Specifies a status to be deleted
     */
     ...
     * @param bool $back_end Determines whether the function is called from the administration area
     */
     ...
     */
    protected function log_payment_attempt(
        $order,
        $message,
        $is_successful,
        $request_array,
     ...
     * @param string $transaction_status_code Specifies a transaction payment gateway-specific status code.
    ...
    public function update_transaction_status(
        ...
        PaymentTransaction::add_transaction(
            $order,
            $host_obj->id,
            $transaction_id,
            $transactionUpdate,
    ...
    /**
     * Adds payment transaction to order history.
     * Use in place of update_transaction_status when confirming a settled or refunded payment value.
     * @param ActiveRecord $host_obj object containing configuration fields values
     * @param Order $order object
     * @param string $transaction_id Specifies a transaction identifier returned by the payment gateway. Example: kjkls
     * @param TransactionUpdate $transaction_data
     ...
     *
     * @param $order ActiveRecord object Order
    ...
    /**
     * This method should return TRUE if the payment gateway
     * supports requesting a status of a specific transaction dispute.
     ...
     * to calculate payment due and assist in refunds.
     * For example: situations where products added/removed from order change the order total after initial payment.
     */
    ...
    /**
     * @deprecated
     * Use: PaymentTransaction::get_order_balance()
     */
    public function get_total_paid($order, $include_pending = true, $in_refunds = false)
    {
        if ($in_refunds) {
            return PaymentTransaction::get_order_total_refunded($order, $include_pending);
        }
        return PaymentTransaction::get_order_total_paid($order, $include_pending);
    }

    /**
     * @deprecated
     * Use: PaymentTransaction::get_order_total_paid()
     */
    public function get_payments_total($order, $include_pending = true)
    {
        ...
        throw new SystemException('The payment gateway does not support this feature.');
    }

    /**
     * @deprecated
     * Use: PaymentTransaction::get_order_total_refunded()
     */
     ...
     * @param $host_obj ActiveRecord object containing configuration fields values
     * @param string $transaction_id Specifies a transaction identifier returned by the payment gateway. Example: kjkls
     * @param string $transaction_status_code Specifies a transaction status code returned by the payment gateway.
     */
     ...
     * The method must return an instance of the TransactionUpdate class
     * @param $host_obj ActiveRecord object containing configuration fields values
     * @param Order $order LSAPP order object the transaction is bound to
     * @param string $transaction_id Specifies a transaction identifier returned by the payment gateway. Example: kjkls
     * @param string $transaction_status_code Specifies a transaction status code returned by the payment gateway.
     * @param string $new_transaction_status_code Specifies a destination transaction status code
     * @return TransactionUpdate Transaction update information
     */
     ...
     * The method must return an instance of the TransactionDisputeUpdate class
     * @param $host_obj ActiveRecord object containing configuration fields values
     * @param string $dispute_case_id Specifies a dispute case identifier returned by the payment gateway.
     * @return TransactionDisputeUpdate Dispute update information
     */
     ...
     * The method must return an array containing instances of the TransactionUpdate class
     * @param $host_obj ActiveRecord object containing configuration fields values
     * @param string $transaction_id Specifies a transaction identifier returned by the payment gateway. Example: kjkls
     * @return array Returns an array of {@link TransactionUpdate} objects.
     */
     ...
     * The payment module must implement the update_customer_profile(),
     * delete_customer_profile() and pay_from_profile() methods if this method returns true..\
     */
     ...
     * @param \Db\ActiveRecord $host_obj ActiveRecord object containing configuration fields values
     * @param Customer $customer Customer object to create a profile for
     * @param array $data Posted payment form data
    ...
    {
        throw new SystemException('The update_customer_profile() method is not supported by the payment module.');
    }
     ...
     * @param CustomerPaymentProfile $profile Customer profile object
     */
    public function delete_customer_profile($host_obj, $customer, $profile)
    {
        throw new SystemException('The delete_customer_profile() method is not supported by the payment module.');
    }

    /**
     * Creates a payment transaction from an existing payment profile.
     * @param $host_obj ActiveRecord object containing configuration fields values
     * @param Order $order An order object to pay
     * @param bool $back_end Determines whether the function is called from the administration area
     * @param bool $redirect Determines whether the browser should be redirected
     *  to the receipt page after successful payment
     */
    public function pay_from_profile($host_obj, $order, $back_end = false, $redirect = true)
    {
        throw new SystemException('The pay_from_profile() method is not supported by the payment module.');
    }

    /**
     * Allows to extend the transaction preview popup window with transaction-specific information.
     * @param $host_obj ActiveRecord object containing configuration fields values
     * @param \Backend\Controller $controller Back-end controller object
     * @param PaymentTransaction $transaction Transaction object
     */
     ...
     * @param bool $active_only Determines if only active backend/frontend payment methods should be returned
     */
    public static function get_host_instances($active_only = true)
    {
        $instances = PaymentMethod::create();
        if ($active_only) {
            $instances->where('(enabled = 1 OR backend_enabled = 1)');
        }
        $called_class = get_called_class();
        $className = get_class_id($called_class);
        if ($called_class !== $className) {
            $instances->where('class_name = ?', $className);
        }
