...
class OrderHelper
{
    ...
    public static function recalc_order_discounts($order, $deferred_session_key = null, $save = false)
    {
        ...
        if ($order->payment_method_id) {
            $payment_method_obj = PaymentMethod::create()->find($order->payment_method_id);
        }
        if ($order->shipping_method_id) {
            $shipping_method_obj = ShippingOption::create()->find($order->shipping_method_id);
        }

        $coupon = $order->coupon_id ? Coupon::create()->find($order->coupon_id) : null;
        $coupon_code = $coupon ? $coupon->code : null;
        $customer = $order->customer_id ? Customer::create()->find($order->customer_id) : null;

        CheckoutData::override_customer($customer);
        ...
        CartPriceRule::reset_rule_cache();
        ...
        $order->free_shipping = isset($discount_info->free_shipping_options[$order->internal_shipping_suboption_id]);

        $applied_discounts_data = array();
        foreach ($cart_items as $cart_item) {
        ...
        if ($save) {
            //before save
            $items = OrderHelper::evalOrderTotals($order, null, $deferred_session_key);
            OrderHelper::apply_item_discounts($items, $applied_discounts_data, $save);
            $order->save();
    ...
    public static function evalOrderTotals(
        $order,
        $items = null,
        ...
        TaxClass::set_customer_context(self::find_customer($order, true));
        ...
        $hasShippingCreds = strlen($order->shipping_method_id) && strlen($order->shipping_country_id);
        if ($options['recalculate_shipping'] && $hasShippingCreds) {
            //recalc all
            ...
            if (array_key_exists($shipping_method_id, $methods)) {
                ...
                if (!$shipping_method->multi_option) {
                    $discount = $shipping_method->discount ?? 0;
                    $order->shipping_quote = round($quote, 2);
                    $order->shipping_discount = round($discount, 2);
        ...
        if (strlen($order->shipping_country_id)) {
            ...
            $tax_info = TaxClass::calculate_taxes($items, (object)$shipping_info, $tax_context);
            $order->goods_tax = $goods_tax = $tax_info->tax_total;
            $order->set_sales_taxes($tax_info->taxes);

            foreach ($items as $item_index => $item) {
                $item_tax_array = $tax_info->item_taxes[$item_index] ?? array();
    ...
    public static function find_customer($order, $check_order_data = false)
    {
        ...
        if (!$check_order_data) {
            ...
            $customer_id = post(
                'customer_id',
                Phpr::$request->postArray(get_class_id('Shop\Order'), 'customer_id')
            );
        }
     ...
     * @documentable
     * @deprecated Use {@link Order::list_available_shipping_options()} method instead.
     */
    ...
    public static function getAvailablePaymentMethods($order, $deferred_session_key = null)
    {
        ...
        return PaymentMethod::list_order_applicable($order, $params)->as_array();
    }
    ...
    {
        $subOptionId = $order->shipping_sub_option_id ?? null;
        $string = !empty($subOptionId) ? $subOptionId : $order->shipping_sub_option;
    ...
    public static function get_shipping_taxes($order, $deferred_session_key = null)
    {
        ...
        $shipping_taxes = TaxClass::get_shipping_tax_rates(
            ...
            $order->get_shipping_quote()
        );

        $eval_order = clone $order;
        $eval_order->items = $order->items;
        if ($deferred_session_key) {
            $eval_order->items = $order->list_related_records_deferred('items', $deferred_session_key);
        }
