...
/**
 * Provides methods which help in developing front-end pages and partials
 * for displaying and managing product bundle items.
 * @documentable
...
class BundleHelper
{
     ...
     * Use this method to determine whether a bundle item product drop-down option,
     * a radio button or a checkbox is selected.
     * Pass a {@link ProductBundleOffer bundle item object} to the first parameter
     * and {@link ProductBundleOfferItem bundle item product object}
     ...
     *     <option
     *       ...
     *       <?= option_state(BundleHelper::is_bundle_item_selected($offer, $item), true) ?>>
     *         <?= h($item->product->name) ?>
     *     </option>
     *   <? endforeach ?>
     * </select>
     ...
     * @documentable
     *
     * @param ProductBundleOffer     $bundle_offer      Specifies the bundle item object.
     * @param ProductBundleOfferItem $bundle_offer_item Specifies the bundle item product object.
     *
     * @return bool Returns TRUE if a specified bundle item product is selected. Returns FALSE otherwise.
     */
    public static function is_bundle_item_selected($bundle_offer, $bundle_offer_item)
    {
        ...
        if (($bundle_offer->control_type == ProductBundleOffer::control_dropdown ||
                $bundle_offer->control_type == ProductBundleOffer::control_radio) && $bundle_offer->is_required) {
     ...
     * Use this method to output value for the Quantity field <em>value</em> attribute.
     * Pass a {@link ProductBundleOffer bundle item object}
     * to the first parameter and {@link ProductBundleOfferItem bundle item product object}
     * to the second parameter. Example:
     * <pre>
     * <input
     *   class="text"
     *   type="text"
     *   name="<?= BundleHelper::get_product_control_name($item, $item_product, 'quantity') ?>"
     *   value="<?= BundleHelper::get_product_quantity($item, $item_product) ?>"/>
     * </pre>
     * @documentable
     *
     * @param ProductBundleOffer     $bundle_offer      Specifies the bundle item object.
     * @param ProductBundleOfferItem $bundle_offer_item Specifies the bundle item product object, optional.
     * @param int                     $product_id        Specifies Product identifier, optional.
     *
     * @return int Returns Quantity field value.
     */
     ...
     *       $is_selected = BundleHelper::is_product_option_selected($option, $value, $item, $item_product);
     *   ?>
     ...
     * @documentable
     *
     * @param CustomAttribute        $option            Specifies the option object being checked.
     * @param string                      $value             Specifies the option value to check.
     * @param ProductBundleOffer     $bundle_offer      Specifies the bundle item object.
     * @param ProductBundleOfferItem $bundle_offer_item Specifies the bundle item product object, optional.
     *
     * @return bool Returns TRUE if a specified bundle item product option is selected. Returns FALSE otherwise.
     */
    public static function is_product_option_selected($option, $value, $bundle_offer, $bundle_offer_item): bool
    {
     ...
     * The method result is suitable for passing to the {@link Product::om()}
     * and {@link ProductBundleOfferItem::get_price()} methods.
     * <pre>
     * // Load bundle item product images
     * $selected_options = BundleHelper::get_selected_options($item, $item_product);
     ...
     * Use this method in the bundle product extra options partial
     * to determine whether a specific product extra option is selected.
     * Example:
     * <pre>
     * <? foreach ($product->extra_options as $option):
     *   $is_checked = BundleHelper::is_product_extra_option_selected($option, $item, $item_product);
     * ?>
     *   <input
     *     ...
     *     <?= checkbox_state(BundleHelper::is_product_extra_option_selected($option, $item, $item_product)) ?>
     *     value="1"
     *     type="checkbox"/>
     * <? endforeach ?>
     * </pre>
     * @documentable
     *
     * @param ExtraOption            $option            Specifies the exrta option object being checked.
     * @param ProductBundleOffer     $bundle_offer      Specifies the bundle item object.
     * @param ProductBundleOfferItem $bundle_offer_item Specifies the bundle item product object, optional.
     *
     * @return bool Returns TRUE if a specified bundle item product extra option is selected. Returns FALSE otherwise.
     */
     ...
     * @param ProductBundleOfferItem $bundle_offer_item Specifies the bundle item product object, optional.
     *
     * @return Product Returns the selected product object.
     */
    public static function get_bundle_item_product($bundle_offer, $bundle_offer_item = null)
    {
        ...
        return Product::create()->find_by_id($product_id);
    }
    ...
    {
        return self::get_bundle_offer_item($bundle_offer);
    }
     ...
     *
     * @param ProductBundleOffer $bundle_offer Specifies the bundle item object.
     * @return ProductBundleOfferItem Returns the bundle item product object or NULL.
     */
     ...
     * Input element names are different for different bundle item control types
     * and this method simplifies generating the element names.
     * <pre>
     * <? if ($item->control_type == 'dropdown'): ?>
     *   <select
     *     ...
     *     name="<?= BundleHelper::get_product_selector_name($item, $selected_item_product) ?>"
     *   ...
     * <? elseif ($item->control_type == 'checkbox'): ?>
     *   <? foreach ($offer->items as $item): ?>
     *     <input
     *       type="checkbox"
     *       name="<?= BundleHelper::get_product_selector_name($offer, $item) ?>"
     *       value="<?= BundleHelper::get_product_selector_value($item) ?>"
     *       ...
     *     />
     *   ...
     * <? else: ?>
     ...
     *     <input
     *       type="radio"
     *       name="<?= BundleHelper::get_product_selector_name($offer, $selected_product) ?>"
     *       value="<?= BundleHelper::get_product_selector_value($item) ?>"
     *       ...
     *     />
     *   ...
    ...
    public static function get_product_selector_name($bundle_offer, $bundle_offer_item)
    {
        switch ($bundle_offer->control_type) {
            case ProductBundleOffer::control_checkbox:
                return 'bundle_data[' . $bundle_offer->id.']['.$bundle_offer_item->product_id.'][product_id]';
            default:
                return 'bundle_data[' . $bundle_offer->id.'][product_id]';
        }
    }
        
    /**
     * Returns a value for a bundle item product selector input element
     * (drop-down menu option, checkbox or radio button).
     * <pre>
     * <select ...>
     *   <? foreach ($offer->items as $item): ?>
     *     <option
    ...
    /**
     * Returns a name for bundle item product configuration control
     * (options, extra options or grouped product selector).
     * This is an universal method which can be used for generating names for
     * any supported bundle item product controls.
     * The <em>$control_name</em> parameter is a string representing the control name.
    ...
    public static function get_product_control_name($bundle_offer, $bundle_offer_item, $control_name)
    {
        if (!in_array($control_name, array('quantity', 'options', 'extra_options', 'grouped_product'))) {
            throw new ApplicationException(
                'Invalid control name passed to BundleHelper::get_product_control_name(). 
                Valid values are options, extra_options, grouped_product.'
            );
        }
        ...
        switch ($bundle_offer->control_type) {
            case ProductBundleOffer::control_dropdown:
     ...
     * <pre><?= BundleHelper::get_item_hidden_fields($item, $selected_item_product) ?></pre>
     * @documentable
     *
     * @param ProductBundleOffer     $bundle_offer      Specifies the bundle item object.
     * @param ProductBundleOfferItem $bundle_offer_item Specifies the bundle item product object.
    ...
    public static function get_item_hidden_fields($bundle_offer, $bundle_offer_item)
    {
        ...
        $fieldName = "bundle_data[' . $bundle_offer->id.'][post_item_product_id]";
        return '<input type="hidden" name="'. $fieldName.'" value="'.$product_id.'"/>';
    }
    ...
    protected static function get_normalized_bundle_product_data()
    {
        ...
        return self::$normalized_bundle_product_data = Cart::normalize_bundle_data(post('bundle_data', array()));
    }
    ...
    protected static function is_bundle_item_selected_internal($bundle_offer, $bundle_offer_item)
    {
        ...
        if ($bundle_offer->control_type == ProductBundleOffer::control_checkbox && $data) {
            return null;
        }
    ...
    {
        $master_product_id = self::find_bundle_data_element(
            'product_id',
            $bundle_offer,
            $bundle_offer_item,
            null
        ...
        $grouped_product_id = self::find_bundle_data_element(
            'grouped_product_id',
            $bundle_offer,
            $bundle_offer_item,
            null
        ...
        $master_product = Product::create()->find_by_id($master_product_info[1]);
