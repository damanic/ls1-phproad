<?
	...
	class Shop_SecureTrading_PaymentPages_Payment extends Shop_PaymentType
	{
		...
		public function build_config_ui($host_obj, $context = null)
		{
			...
			$host_obj->add_field('cancel_page', 'Cancel Page')->comment('Page to which the customer’s browser is redirected if payment is cancelled.<br/>You can use <strong>$lsapp_return_url</strong> and <strong>$lsapp_cancel_url</strong> variables in SecureTrading page templates to refer the Receipt and Cancel pages.', 'above', true)->tab('Configuration')->renderAs(frm_dropdown);
		}
...
//			$amount = $currency_converter->convert($order->total, $currency->code, 'USD');
			...
			if (!$backend)
			{
				$fields['lsapp_return_url'] = root_url('/ls_securetrading_return/'.$order->order_hash, true);

				$cancel_page = Cms_Page::create()->find($host_obj->cancel_page);
				if ($cancel_page)
				{
					$fields['lsapp_cancel_url'] = root_url($cancel_page->url, true);
					if ($cancel_page->action_reference == 'shop:pay')
						$fields['lsapp_cancel_url'] .= '/'.$order->order_hash;
					elseif ($cancel_page->action_reference == 'shop:order')
						$fields['lsapp_cancel_url'] .= '/'.$order->id;
				}
			} else 
			{
				$fields['lsapp_return_url'] = root_url('/ls_securetrading_return/'.$order->order_hash.'/backend', true);
				$fields['lsapp_cancel_url'] = Phpr::$request->getRootUrl().url('shop/orders/pay/'.$order->id.'?'.uniqid());
			}
