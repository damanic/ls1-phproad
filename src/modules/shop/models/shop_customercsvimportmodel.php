<?
	...
	class Shop_CustomerCsvImportModel extends Backend_CsvImportModel
	{
		...
		public function define_form_fields($context = null)
		{
			...
			$this->add_form_field('update_existing_emails')->comment('If you leave this checkbox unchecked, LSAPP will skip customers with existing emails. Otherwise existing customers will be updated using information from the CSV file and other parameters specified on this page.', 'above', true);
			...
			$this->add_form_field('auto_create_groups')->comment('You need to match the LSAPP customer <strong>Customer Group</strong> field to a CSV file column in order to use this feature. Otherwise please select a customer group for all imported customers in the list below.', 'above', true);
		...
		public function import_csv_data($data_model, $session_key, $column_map, $import_manager, $delimeter, $first_row_titles)
		{
			...
			if ($this->auto_create_groups)
			{
				if (!$import_manager->csvImportDbColumnPresented($column_map, 'group'))
					throw new Phpr_ApplicationException('Please specify a matching column for the Customer Group customer field, or uncheck the "I want LSAPP to create customer groups specified in the CSV file" checkbox.');
			} else
			{
				if (!$this->group)
