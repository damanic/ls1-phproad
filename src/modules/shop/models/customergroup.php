 ...
 * @property int $id Specifies the customer group record identifier.
 ...
 * @property bool $disable_tax_included Indicates whether taxes should not be included ib displayed product prices
...
class CustomerGroup extends ActiveRecord
{
    ...
    public $calculated_columns = [
        'customer_num' => [
            'sql' => "(select count(*) from shop_customers where customer_group_id=shop_customer_groups.id)",
            'type' => db_number
        ]
    ];
    ...
    public function define_columns($context = null)
    {
        $this->define_column('name', 'Name')
            ->order('asc')->validation()
            ->fn('trim')
            ->required("Please specify the group name");

        $this->define_column('description', 'Description')
            ->validation()
            ->fn('trim');

        $this->define_column('customer_num', 'Customers')
            ->validation()
            ->fn('trim');

        $this->define_column('disable_tax_included', 'Do not include tax into displayed product prices')
            ->listTitle('Disable Tax Inclusive');

        $this->define_column('code', 'API Code')
            ->validation()->fn('trim')
            ->fn('mb_strtolower')
            ->unique('The API Code "%s" is already in use.');
    ...
    public function define_form_fields($context = null)
    {
        ...
        $this->add_form_field('disable_tax_included')
            ->comment(
                'Use this checkbox if you want to override the global 
                "Display catalog/cart prices including tax" option 
                for customers belonging to this customer group.'
            );

        $this->add_form_field('tax_exempt')
            ->comment('Use this feature if the tax should not be applied to customers from this group.');

        $field = $this->add_form_field('code');
        $field->comment('You can use the API code for referring the customer group in the API calls.', 'above');
    ...
    {
        if ($this->code == self::guest_group || $this->code == self::registered_group) {
            throw new ApplicationException("The Guest and Registered customer groups cannot be deleted.");
        }

        if ($this->customer_num) {
            throw new ApplicationException(
                "The group cannot be deleted because {$this->customer_num} customer(s) belong to this group."
            );
        }
    }
     ...
     * @documentable
     * @return CustomerGroup Returns the Guest customer group object.
     */
    public static function get_guest_group()
    {
        ...
        if (!self::$guest_group) {
            throw new ApplicationException("The Guest customer group is not found in the database.");
        }
     ...
     * Example:
     * <pre>$groups = CustomerGroup::list_groups_by_codes(array('registered', 'reseller'));</pre>
     * @documentable
     * @param array $codes Specifies a list of customer group API codes.
     * @return DataCollection Returns a collection of customer group objects ({@link CustomerGroup}).
     */
     ...
     * @documentable
     * @return array Returns an array of {@link CustomerGroup} objects.
     */
     ...
     * @return CustomerGroup Returns the customer group object. Returns NULL if the customer group is not found.
     */
     ...
     * @param CustomerGroup $group Specifies the customer group object.
     * @author LSAPP - MJMAN
     ...
     * To add new fields to the customer group form,
     * call the {@link Db_ActiveRecord::add_form_field() add_form_field()} method of the
     ...
     * which have been added with {@link shop:onExtendCustomerGroupForm} event.
