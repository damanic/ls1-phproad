 ...
 * By default LSAPP creates a few predefined product types: <em>Downloadable</em>, <em>Goods</em>, <em>Service</em>.
 * Objects of this method are available through {@link Product::$product_type} property.
 * @documentable
 * @property string $name Specifies the product type name.
 * @property string $code Specifies the product type API code.
 * @property bool $is_default Determines whether the product type is default for new products.
 * @property bool $files Determines whether products of this type support files.
 * @property bool $inventory Determines whether products of this type support inventory tracking.
 * @property bool $shipping Determines whether products of this type support shipping.
 * @property bool $grouped Determines whether products of this type support grouped products.
 * @property bool $options Determines whether products of this type support product options.
 * @property bool $extras Determines whether products of this type support product options.
 * @see Product
 * @package shop.models
 * @author LSAPP - MJMAN
 */
class ProductType extends ActiveRecord
{
    ...
    {
        $this->define_column('name', 'Type Name')
            ->order('asc')
            ->validation()
            ->fn('trim')
            ->required('Please specify the product type name.');

        $this->define_column('code', 'API Code')
            ->validation()
            ->fn('trim')
            ->unique('API code is already in use by another product type.');

        $this->define_column('is_default', 'Is default');

        $this->define_column('files', 'Enable files')
            ->defaultInvisible();

        $this->define_column('inventory', 'Enable inventory tracking')
            ->defaultInvisible();

        $this->define_column('shipping', 'Enable shipping')
            ->defaultInvisible();

        if (!Phpr::$config->get('DISABLE_GROUPED_PRODUCTS')) {
            $this->define_column('grouped', 'Enable grouped products')
                ->defaultInvisible();
        }

        $this->define_column('options', 'Enable options')
            ->defaultInvisible();

        $this->define_column('extras', 'Enable extra options')
            ->defaultInvisible();
    ...
    public function define_form_fields($context = null)
    {
        ...
        $this->add_form_field('code', 'right')
            ->tab('Product Type');

        $this->add_form_field('is_default')
            ->tab('Product Type')
            ->renderAs('checkbox')
            ->comment('Use this checkbox if you want this product type to be applied to all new products by default.');

        $this->add_form_field('files')
            ->tab('Product Type')
            ->renderAs('checkbox')
            ->comment('Select to enable the Files tab on the product page.');

        $this->add_form_field('inventory')
            ->tab('Product Type')
            ->renderAs('checkbox')
            ->comment('Select to enable the Inventory tab on the product page.');

        $this->add_form_field('shipping')
            ->tab('Product Type')
            ->renderAs('checkbox')
            ->comment('Select to enable the Shipping tab on the product page.');
            
        if (!Phpr::$config->get('DISABLE_GROUPED_PRODUCTS')) {
            $this->add_form_field('grouped')
                ->tab('Product Type')
                ->renderAs('checkbox')
                ->comment('Select to enable the Grouped tab on the product page.');
        }
                
        $this->add_form_field('options')
            ->tab('Product Type')
            ->renderAs('checkbox')
            ->comment('Select to enable the Options tab on the product page.');

        $this->add_form_field('extras')
            ->tab('Product Type'
            )->renderAs('checkbox')
            ->comment('Select to enable the Extras tab on the product page.');
    ...
    {
        $products_num = DbHelper::scalar(
            'select count(*) from shop_products where product_type_id=:id',
            array('id'=>$this->id)
        );
        ...
        if ($products_num){
            throw new ApplicationException(
                "Cannot delete this product type. There are $products_num product(s) belonging to it."
            );
        }
        //Ensure there is at least one product type available at all times
        $count_types = DbHelper::scalar('select count(*) from shop_product_types');
        if ($count_types < 2) {
            throw new ApplicationException(
                "Product type cannot be deleted because it is the only one configured. 
                There should always be at least one product type configured."
            );
        }
    }
    ...
    public function after_save()
    {
        ...
        if ($this->is_default) {
            DbHelper::query('update shop_product_types set is_default=0 where id<>:id', array('id'=>$this->id));
        }
    }
     ...
     * The event handler should accept a single parameter - the product type object.
     * To add new columns to the product type model,
     * call the {@link Db_ActiveRecord::define_column() define_column()} method of the product type object.
     ...
     * @param ProductType $product_type Specifies the product type object.
     */
     ...
     * To add new fields to the product type form, call the
     ...
     * {@link shop:onExtendProductTypeForm} event.
