 ...
 * Objects of this class are available through the
 * {@link Product::list_reviews()} and {@link Product::list_all_reviews()} methods.
 * @documentable
 * @property string $author Specifies the review author name.
 * @property \Phpr\DateTime $created_at Specifies the review date and time.
...
class ProductReview extends ActiveRecord
{
    ...
    public static $moderation_statuses = [
        'new' => 'New',
        'approved' => 'Approved'
    ];
    ...
    public $belongs_to = [
        'customer_link' => [
            'class_name' => 'Shop\Customer',
            'foreign_key' => 'created_customer_id'
        ],
        'product_link' => [
            'class_name' => 'Shop\Product',
            'foreign_key' => 'prv_product_id'
        ]
    ];

    public $calculated_columns = [
        'review_status' => [
            'sql' => "if(prv_moderation_status = 'new', 'New', 'Approved')"
        ],
    ...
    public function define_columns($context = null)
    {
        ...
        $this->define_column('prv_moderation_status', 'Status')
            ->invisible();

        $this->define_relation_column('product_link', 'product_link', 'Product', db_varchar, '@name');

        $this->define_column('prv_rating', 'Rating')
            ->type(db_text);
        ...
        $this->define_column('customer_ip', 'Author IP')
            ->defaultInvisible();

        $this->define_column('review_title', 'Title')
            ->validation()
            ->fn('trim')
            ->required("Please specify the review title");

        $this->define_column('review_author_name', 'Author')
            ->validation()
            ->fn('trim')
            ->method('validate_author_name');

        $this->define_column('review_author_email', 'Author Email')
            ->validation()
            ->fn('trim')
            ->email(true, 'Please specify a valid email address')
            ->method('validate_author_email');

        $this->define_column('review_text', 'Review Text')
            ->invisible()
            ->validation()
            ->fn('trim')
    ...
    {
        $this->add_form_field('prv_moderation_status')
            ->renderAs(frm_dropdown)
            ->tab('Review');

        $this->add_form_field('review_title')
            ->tab('Review');

        $this->add_form_field('review_author_name', 'left')
            ->tab('Review');

        $this->add_form_field('review_author_email', 'right')
            ->tab('Review');

        $this->add_form_field('prv_rating')
            ->tab('Review')
            ->renderAs(frm_dropdown)
            ->emptyOption('<no rating specified>');

        $this->add_form_field('review_text')
            ->tab('Review');
    ...
    {
        $config = ReviewsConfiguration::create();

        if ($config->send_notifications) {
            try {
                $template = EmailTemplate::create()->find_by_code('shop:product_review_internal');
                if ($template) {
                    ...
                    $product = Product::create()->find($this->prv_product_id);
                    ...
                    $review_edit_url = $rUrl . url('shop/reviews/edit/' . $this->id . '?' . uniqid());

                    $message = $this->set_email_variables(
                        $template->content,
                        $rating_name,
                        $review_edit_url,
                        $product,
                    ...
                    $template->subject = $this->set_email_variables(
                        $template->subject,
                        $rating_name,
                        $review_edit_url,
                        $product,
                    ...
                    $users = User::list_users_having_permission('shop', 'manage_products');
    ...
    public static function create_review($product, $customer, $review_data)
    {
        ...
        $config = ReviewsConfiguration::create();
        $ipCustomerReviewExists = self::ip_customer_review_exists(
            Phpr::$request->getUserIp(),
            $customer,
            $product->id
        );
        if ($config->no_duplicate_reviews && $ipCustomerReviewExists) {
        ...
        if ($config->rating_required && !$rating) {
            throw new ApplicationException('Please specify the product rating.');
        }

        if ($rating > 5) {
            throw new ApplicationException('Product rating cannot be more than 5.');
        }
    ...
    public static function ip_customer_review_exists($ip, $customer, $product_id)
    {
        ...
        if ($customer_id) {
            return DbHelper::scalar(
                ...
                where created_customer_id=:created_customer_id and prv_product_id=:prv_product_id',
                $bind
            );
        } else {
            return DbHelper::scalar(
                'select count(*) from shop_product_reviews 
                where customer_ip=:customer_ip and prv_product_id=:prv_product_id',
                $bind
            );
        }
    }
    ...
    public function approve()
    {
        ...
        DbHelper::query('update shop_product_reviews set prv_moderation_status=:status where id=:id', $bind);
        Product::update_rating_fields($this->prv_product_id);
     ...
     * To add new columns to the review model, call the {@link Db_ActiveRecord::define_column() define_column()}
     ...
     * @param ProductReview $review Specifies the review object.
     ...
     * To add new fields to the review form, call the
     ...
     * {@link shop:onExtendProductReviewForm} event.
