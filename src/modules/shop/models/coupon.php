...
class Coupon extends ActiveRecord
{
    ...
    {
        $this->define_column('code', 'Coupon Code')
            ->order('asc')
            ->validation()
            ->fn('trim')
            ->required("Please specify a coupon code")
            ->fn('mb_strtolower')
            ->unique(
                "A coupon with the specified code already exists. 
                Please specify another code, or use the existing coupon."
            );
    }
    ...
    {
        $in_use = DbHelper::scalar('select count(*) from shop_cart_rules where coupon_id=:id', array('id'=>$this->id));
        if ($in_use) {
            throw new ApplicationException(
                "The coupon cannot be deleted because it is used in $in_use shopping cart price rule(s)."
            );
        }
    }
    ...
    public static function get_order_number($coupon_code)
    {
        ...
        if (self::$coupon_usage_map === null) {
            ...
            $coupon_usage = DbHelper::objectArray(
                'select shop_coupons.code as code, count(*) as cnt 
                      from shop_coupons, shop_orders 
                      where shop_coupons.id=shop_orders.coupon_id 
                      group by shop_orders.coupon_id'
            );
