...
use Db\ActiveRecord as DbActiveRecord;
 ...
 * Object of this class is available through the <em>$shipping_method</em> property of the {@link Order} class.
 ...
 * @property bool $taxable Determines whether tax is applicable for the shipping option.
 * @property float $quote Specifies the shipping quote.
 * This property is applicable only during the Checkout process.
 * @property bool $is_free Determines whether the shipping option is free
 * @property bool $multi_option Indicates whether the option has sub-options.
 ...
 * </ul>
 * @property bool $multi_option_name Specifies the name of the parent option for sub-options.
 * This property is set in the flat shipping option lists.
 * See {@link shop:on_evalShippingRate}, {@link action@shop:checkout}.
 ...
 * By default LSAPP does not return shipping options with errors.
 * You can enable this feature on the System/Settings/Shipping
 ...
 * @property int $id Specifies the option identifier in the database.
 * @documentable
 * @see Order
 * @see https://lsdomainexpired.mjman.net/docs/checkout_page Checkout page
 * @see https://lsdomainexpired.mjman.net/docs/developing_shipping_modules Developing shipping modules
 * @package shop.models
 * @author LSAPP - MJMAN
 */
class ShippingOption extends ActiveRecord
{
    ...
    protected static $customerGroupFilterCache = null;
    protected static $isTaxableCache = [];
    ...
    /*
     * The following field contains an error hint message
     * - for example "The postal code XXXXX is invalid for AL United States".
     */
    public $error_hint = null;


    public $custom_columns = [
        'shipping_type_name' => db_text
    ];

    public $has_and_belongs_to_many = [
        'countries' => [
            'class_name' => 'Shop\Country',
            'join_table' => 'shop_shippingoptions_countries',
            'order' => 'name'
        ],
        'customer_groups' => [
            'class_name' => 'Shop\CustomerGroup',
            'join_table' => 'shop_shippingoptions_customer_groups',
            'order' => 'name',
            'foreign_key' => 'customer_group_id',
            'primary_key' => 'shop_sh_option_id'
        ]
    ];
     ...
     * @return ShippingOption Returns the shipping option object.
     *  Returns NULL if the record with the specified API code is not found.
     */
    ...
    public function define_columns($context = null)
    {
        $this->define_column('name', 'Name')
            ...
            ->validation()
            ->fn('trim')
            ->required('Please specify the shipping option name.');
        ...
        $this->define_column('handling_fee', 'Handling Fee')
            ->currency(true)
            ->validation();
        ...
        $this->define_column('ls_api_code', 'LSAPP API Code')
            ...
            ->fn('trim')
            ->fn('mb_strtolower')
            ->unique('Shipping option with the specified LSAPP API code already exists.');

        $this->define_column('min_weight_allowed', 'Minimum Weight')
            ->validation();

        $this->define_column('max_weight_allowed', 'Maximum Weight')
            ->validation();

        $front_end = DbActiveRecord::$execution_context == 'front-end';
        if (!$front_end) {
    ...
    public function define_form_fields($context = null)
    {
        ...
        if ($context != 'print_label') {
            $this->add_form_field('enabled', 'left')
                ...
                ->comment('Make this shipping option available on the front-end website.');

            $this->add_form_field('taxable', 'right')
                ->tab('General Parameters')
                ->comment('Use tax class "Shipping" to specify tax rates for different locations.');

            $backend_enabled = $this->add_form_field('backend_enabled', 'left')
                ->tab('General Parameters')
                ->comment('Make this shipping option available in the Administration area.');
            ...
            $this->add_form_field('name')
                ->comment(
                    'Name of the shipping option. It will be displayed on the front-end website.',
                    'above'
                )
                ->tab('General Parameters');
            ...
            $this->add_form_field('handling_fee', 'left')
                ->tab('General Parameters')
                ->comment(
                    'Please specify a handling fee for this shipping method. 
                    The handling fee will be added to the shipping quote.',
                    'above'
                );

            $this->add_form_field('ls_api_code', 'right')
                ->comment(
                    'You can use the API Code for identifying the shipping method in the API calls.',
                    'above'
                )
                ->tab('General Parameters');

            $this->add_form_field('min_weight_allowed', 'left')
                ->comment(
                    'The shipping option will be ignored if the package weight is less than the specified value. 
                    Leave the field empty to cancel the minimum weight check.',
                    'above'
                )
                ->tab('General Parameters');

            $this->add_form_field('max_weight_allowed', 'right')
                ->comment(
                    'The shipping option will be ignored if the package weight is more than the specified value. 
                    Leave the field empty to cancel the maximum weight check.',
                    'above'
                )->tab('General Parameters');
            ...
            if (!$front_end) {
                $this->add_form_field('customer_groups')
                    ->tab('Visibility')
                    ->comment(
                        'Please select customer groups the shipping options should be available for. 
                        If no groups are selected, the shipping option will be available for all customer groups.',
                        'above'
                    );
            }
            ...
            if ($obj->config_countries() && !$front_end) {
                $this->add_form_field('countries')
                    ->tab('Countries')
                    ->comment(
                        'Countries the shipping method is applicable to. 
                        Uncheck all countries to make the shipping method applicable to any country.',
                        'above'
                    )
                    ->referenceSort('name');
            }
    ...
    {
        if (self::$customerGroupFilterCache === null) {
            self::$customerGroupFilterCache = array();
            $filter_records = DbHelper::objectArray('select * from shop_shippingoptions_customer_groups');
            foreach ($filter_records as $record) {
                if (!array_key_exists($record->shop_sh_option_id, self::$customerGroupFilterCache)) {
                    self::$customerGroupFilterCache[$record->shop_sh_option_id] = array();
                }

                self::$customerGroupFilterCache[$record->shop_sh_option_id][] = $record->customer_group_id;
            }
        }

        if (!array_key_exists($option_id, self::$customerGroupFilterCache)) {
            return true;
        }

        return in_array($customer_group_id, self::$customerGroupFilterCache[$option_id]);
    }
    ...
    public function before_delete($id = null)
    {
        ...
        $count = DbHelper::scalar('select count(*) from shop_orders where shipping_method_id=:id', $bind);
        if ($count) {
            throw new ApplicationException(
                'Cannot delete this shipping option because there are orders referring to it.'
            );
        }
    }
    ...
    {
        if (array_key_exists($id, self::$isTaxableCache)) {
            return self::$isTaxableCache[$id];
        }

        return self::$isTaxableCache[$id] = DbHelper::scalar(
            'select taxable from shop_shipping_options where id=:id',
            array('id' => $id)
        );
    }
    ...
    protected function load_order_label_xml_data($order)
    {
        ...
        $params = OrderShippingLabelParams::find_by_order_and_method($order, $this);
        if (!$params) {
    ...
    public function generate_shipping_labels($order, $parameters)
    {
        ...
        OrderShippingLabelParams::save_parameters($order, $this, $label_parameters);
    ...
    {
        if (isset(self::$quoteCache[$key])) {
            return self::$quoteCache[$key];
        }

        if (Phpr::$config->get('CACHE_SHIPPING_METHODS', false)) {
            $cache_data = Phpr::$session->get('shipping_options_cache');
            $cacheDataEntry = $cache_data[$this->id]['key'] ?? null;
            if ($cache_data && $cacheDataEntry == $key) {
                return self::$quoteCache[$key] = $cache_data[$this->id]['options'];
            }
        }
    ...
    protected function getQuoteInternal($params)
    {
        $default_request_params = array(
            'host_obj' => $this,
            'shipping_info' => null, //instance of AddressInfo
            'total_price' => null,
            'total_volume' => null,
            'total_weight' => null,
            'total_item_num' => null,
    ...
    protected function fetch_quote($params)
    {
        ...
        if ($shipping_type->config_countries()) {
            $country_ids = DbHelper::scalarArray(
        ...
        if (!is_array($result)) {
            ...
            $shop_currency_code = CurrencySettings::get()->code;
            ...
            foreach ($result as $name => &$option_obj) {
                ...
                if (isset($option_obj['currency']) && strlen($option_obj['currency']) == 3) {
                    if ($option_obj['currency'] !== $shop_currency_code) { //convert to shop currency
                        $option_obj['quote'] = $currency_converter->convert(
                            $option_obj['quote'],
                            $option_obj['currency'],
                            $shop_currency_code
                        );
    ...
    protected function apply_quote($quote, $params)
    {
        ...
        foreach ($active_items as $item) {
            ...
            if ($product) {
                $total_per_product_cost += $product->get_shipping_cost(
                        $params['country_id'],
                        $params['state_id'],
                        $params['zip']
                    ) * $item->quantity;
        ...
        if (!is_array($quote)) {
            ...
            $shipping_taxes = TaxClass::get_shipping_tax_rates($this->id, $params['shipping_info'], $quote);
            $this->quote_tax_incl += TaxClass::eval_total_tax($shipping_taxes);
            if ($params['display_prices_including_tax']) {
                $this->quote += TaxClass::eval_total_tax($shipping_taxes);
            }
            ...
            foreach ($quote as $name => $rate) {
                ...
                $sub_option->quote_tax_incl += TaxClass::eval_total_tax($shipping_taxes);
                if ($params['display_prices_including_tax']) {
                    $sub_option->quote += TaxClass::eval_total_tax($shipping_taxes);
                }
        ...
        } elseif (array_key_exists($this->id, $discount_info->free_shipping_options) || $this->quote == 0) {
            $this->is_free = true;
        }

        //converts quotes to active currency if specified in request parameters
        $currencyConverter = CurrencyConverter::create();
        $shopCurrencyCode = CurrencySettings::get()->code;
        $paramCurrencyCode = $params['currency_code'] ?? '';
        $paramCurrencyCodeValid = (strlen($paramCurrencyCode) == 3);
        $activeCurrencyCode = $paramCurrencyCodeValid ? $paramCurrencyCode : $shopCurrencyCode;
        if ($this->get_quote_currency() !== $activeCurrencyCode) {
            $this->convert_to_currency($activeCurrencyCode);
        }


        $event_params = array(
            'context' => (isset($params['order']) && $params['order']) ? 'order' : 'cart',
            'cart_name' => $params['cart_name'] ?? null,
            'order' => $params['order'] ?? null,
            'customer' => $params['customer'] ?? null,
            'shipping_info' => $params['shipping_info'] ?? null,
        );
     ...
     * @return bool Returns TRUE if quote applied.
     */
    public function apply_checkout_quote($cart_name = 'main')
    {
        ...
        CheckoutData::eval_discounts($cart_name, $cart_items);
        ...
        $currency_code = CheckoutData::get_currency($as_object = false);
        ...
        $quote = $this->getQuoteInternal($request_params);
        if ($quote === null) {
            throw new CmsException('Shipping method is not applicable.');
        }
     ...
     * and applies the quote to this option model
     * @documentable
     * @param Order $order Specifies the order to consider
     * @return bool Returns TRUE if quote applied.
     */
    public function apply_order_quote($order, $deferred_session_key = null)
    {
        $items = $order->items;
        $deferred_items = empty($deferred_session_key) ? false : $order->list_related_records_deferred(
            'items',
        ...
        $customer = $order->customer_id ? Customer::create()->find($order->customer_id) : null;
    ...
    protected static function filter_applicable_options($shipping_options, $params)
    {
        ...
        $params['payment_method'] = $payment_method_obj;
        ...
        foreach ($shipping_options as $option) {
            ...
            $apply_customer_group_filter = strlen($params['customer_group_id']);
            if ($apply_customer_group_filter && !self::option_visible_for_customer_group(
                    $option->id,
        ...
        $updated_options = Backend::$events->fire_event(array(
            'name' => 'shop:onUpdateShippingOptions',
            'type' => 'update_result'
        ), $result, $params);
    ...
    public function convert_to_currency($to_currency_code)
    {
        ...
        if ($this->get_quote_currency() !== $to_currency_code) {
            ...
            $this->quote_tax_incl = $currency_converter->convert(
                $this->quote_tax_incl,
                $from_currency,
                $to_currency_code
            );
            $this->quote_no_discount = $currency_converter->convert(
                $this->quote_no_discount,
                $from_currency,
                $to_currency_code
            );
            ...
            if ($this->multi_option) {
                foreach ($this->sub_options as $sub_option) {
                    $sub_option->quote = $currency_converter->convert(
                        $sub_option->quote,
                        $from_currency,
                        $to_currency_code
                    );
                    $sub_option->quote_no_tax = $currency_converter->convert(
                        $sub_option->quote_no_tax,
                        $from_currency,
                        $to_currency_code
                    );
                    $sub_option->quote_tax_incl = $currency_converter->convert(
                        $sub_option->quote_tax_incl,
                        $from_currency,
                        $to_currency_code
                    );
                    $sub_option->quote_no_discount = $currency_converter->convert(
                        $sub_option->quote_no_discount,
                        $from_currency,
                        $to_currency_code
                    );
                    $sub_option->discount = $currency_converter->convert(
                        $sub_option->discount,
                        $from_currency,
                        $to_currency_code
                    );
     ...
     * @ignore
     * @deprecated Use {@link ShippingOption::apply_checkout_quote()}
     * and  {@link ShippingOption::apply_order_quote()} instead.
     *
     */
    public function get_quote(
        ...
        traceLog('Warning: use of deprecated function - ShippingOption::get_quote()');
     ...
     * Fetches the quote from host provider
     * @ignore
     * @deprecated Use {@link ShippingOption::fetch_quote()} method instead.
     */
    protected function eval_quote(
        $country_id,
        $state_id,
        $zip,
        $city,
        ...
        traceLog('Warning: use of deprecated function - ShippingOption::eval_quote()');
        ...
        CartPriceRule::evaluate_discount(
            $payment_method_obj,
            $this,
            $cart_items,
            $shipping_info,
            CheckoutData::get_coupon_code(),
            $customer ? $customer : Controller::get_customer(),
            $total_price
        );
        ...
        if ($free_shipping_found) {
            ...
            if (!is_array($result)) {
                ...
                foreach ($result as $name => &$option_obj) {
                    if (!$updated_result) {
                        $option_obj['quote'] = 0;
                    } elseif (array_key_exists($name, $updated_result)) {
     ...
     * @documentable
     * @deprecated Use {@link ShippingOption::get_applicable_options()} method instead.
     */
    public static function list_applicable(
        ...
        traceLog('Warning: use of deprecated function - ShippingOption::list_applicable()');
        $shipping_info = new AddressInfo();
        ...
        $params = array(
            ...
            'payment_method' => CheckoutData::get_payment_method(), //out of scope
            'coupon_code' => CheckoutData::get_coupon_code(), //out of scope
        );
     ...
     * To add new columns to the shipping option model,
     * call the {@link DbActiveRecord::define_column() define_column()}
     ...
     * @param ShippingOption $shipping_option Specifies the shipping option object.
     ...
     * {@link DbActiveRecord::add_form_field() add_form_field()} method of the
     ...
     * {@link shop:onExtendShippingOptionForm} event.
     ...
     *      'shop:onGetShippingOptionFieldOptions',
     *      $this,
     *      'get_shipping_option_field_options'
     *    );
     * }
     *
     * public function extend_shipping_option_model($shipping_option, $context)
     ...
     * - the {@link ShippingOption} object and an array of shipping parameters.
     ...
     *   <li><em>option_id</em>
     *      - for multi-option shipping methods only (like USPS)
     *      - service-specific identifier of the shipping option.</li>
     *   <li><em>option_name</em>
     *      - for multi-option shipping methods only (like USPS)
     *      - service-specific name of the shipping option.</li>
     *   <li><em>shipping_option</em> - the {@link ShippingOption} object which returned the original quote.</li>
     ...
     *   <li><em>state_id</em> - {@link CountryState shipping state} identifier.</li>
     ...
     *   <li><em>cart_items</em>
     *      - a list of shopping cart items.
     *      An array of {@link CartItem} or {@link OrderItem} objects, depending on the caller context.</li>
     * </ul>
     ...
     * The event handler should accept 2 parameters - the ShippingOption object and an array of shipping parameters.
     * The event handler should return updated shipping quote.
     * The <em>$params</em> array contains the following elements:
     * <ul>
     *   <li><em>quote</em> - the original shipping quote.</li>
     *   <li><em>option_id</em> - for multi-option shipping methods only (like USPS)
     *      - service-specific identifier of the shipping option.</li>
     *   <li><em>option_name</em> - for multi-option shipping methods only (like USPS)
     *      - service-specific name of the shipping option.</li>
     *   <li><em>shipping_option</em> - the {@link ShippingOption} object which returned the original quote.</li>
     *   <li><em>handling_fee</em> - the handling fee, defined in the shipping method.</li>
     *   <li><em>country_id</em> - {@link Country shipping country} identifier.</li>
     ...
     *   <li><em>total_item_num</em> - total number of order items.</li>
     *   <li><em>cart_items</em>
     *      - a list of shopping cart items. An array of {@link CartItem} or {@link OrderItem} objects,
     *      depending on the caller context.
     *      When processing existing orders, LSAPP can automatically convert order items to cart
     *     items. In this case the reference to the original order item is
     *      stored in the {@link CartItem::$order_item $order_item}
    ...
    /**
     * Deprecated: 'shop:onFilterShippingOptions' is not a true filter event.
     *  It will only return a result from the first module to reply.
     ...
     * <ul>
     *   <li><em>options</em> - a array of shipping options. Each element is the {@link ShippingOption} object.</li>
     *   <li><em>country_id</em> - {@link Country shipping country} identifier.</li>
     *   <li><em>state_id</em> - {@link CountryState shipping state} identifier.</li>
     *   <li><em>zip</em> - shipping ZIP/Postal code.</li>
     *   <li><em>city</em> - shipping city.</li>
     ...
     *   <li><em>total_weight</em> - total weight of all order items.</li>
     *   <li><em>total_item_num</em> - total number of order items.</li>
     *   <li><em>order_items</em> - a list of order items ({@link OrderItem} or {@link CartItem} objects,
     *          depending on the caller context).</li>
     ...
     * (like USPS) you may need to update the <em>{@link ShippingOption::$sub_options $sub_options}</em> property.
     *
     * Usage example:
     * <pre>
     * public function subscribeEvents()
     ...
     *
     * The options array contains an array of {@link ShippingOption} objects
     *
     ...
     * <ul>
     *   <li><em>host_obj</em> - the shipping option {@link ShippingOption} object.</li>
     *   <li><em>country_id</em> - {@link Country shipping country} identifier.</li>
     *   <li><em>state_id</em> - {@link CountryState shipping state} identifier.</li>
     *   <li><em>zip</em> - shipping ZIP/Postal code.</li>
     *   <li><em>city</em> - shipping city.</li>
     ...
     *   <li><em>total_weight</em> - total weight of all order items.</li>
     *   <li><em>total_item_num</em> - total number of order items.</li>
     *   <li><em>order_items</em> - a list of order items ({@link OrderItem}
     *      or {@link CartItem} objects, depending on the caller context).</li>
     *   <li><em>customer_id</em> - identifier for customer {@link Customer customer}.</li>
     *
     * </ul>
     * The handler can return a string to append to the cache key.
     * This is useful when using the shop:onUpdateShippingQuote event to update
     * a quote based on external considerations.
     ...
     * The event handler should accept 2 parameters
     * - the ShippingOption object and an array of parameters that provide execution context.
     * @event shop:onAfterShippingQuoteApplied
     * @param ShippingOption $shipping_option The ShippingOption that has been updated with quotes
