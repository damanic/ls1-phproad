 ...
 * @author LSAPP - MJMAN
 */
class CountryState extends ActiveRecord
{
    public $table_name = 'shop_states';

    public $calculated_columns = [
        'country_state_name' => [
            'sql' => "concat(shop_countries.name, '/', shop_states.name)",
            'type' => db_text,
            'join' => ['shop_countries' => 'shop_countries.id=shop_states.country_id']
        ]
    ];
     ...
     * <pre>$california = CountryState::create()->find_by_code('</pre>
     * @documentable
     *
     * @param bool $no_column_info Determines whether {@link \Db\ColumnDefinition column objects}
     *  should be loaded into the memory.
     *
     * @return CountryState Returns the state object.
     */
    ...
    public function define_columns($context = null)
    {
        ...
        $this->define_column('disabled', 'Disabled');
    }
    ...
    public function define_form_fields($context = null)
    {
        ...
        if ($context != 'preview') {
            $field->comment(
                'Specify state code. You are not restricted by any rules about state codes. 
                Codes are used for referring states in your custom tax and shipping rules.',
                'above'
            );
        } else {
        ...
        $this->add_form_field('disabled')
            ->comment('Tick this box if this state should no longer be presented for selection');
    }
    ...
    public function is_in_use()
    {
        ...
        $in_use = DbHelper::scalar(
            'select count(*) from shop_customers where shipping_state_id=:id or billing_state_id=:id',
            $bind
        );
        if (!$in_use) {
            $in_use = DbHelper::scalar(
                'select count(*) from shop_orders where shipping_state_id=:id or billing_state_id=:id',
                $bind
            );
        }
    ...
    {
        if ($this->is_in_use()) {
            throw new ApplicationException("Cannot delete state because it is in use.");
        }
    }
    ...
    public function apply_visible()
    {
        $this->where('disabled IS NULL');
        return $this;
    }
     ...
     *
     * @return CountryState Returns a state object or NULL if the state is not found.
     */
    ...
    public static function find_by_country_id($country_id, $order = 'name')
    {
        $obj = self::create(true)->where('country_id = ?', $country_id)->apply_visible();
        return $obj->order($order)->find_all();
    }
}
