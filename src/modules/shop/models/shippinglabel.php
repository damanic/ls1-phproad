...
class ShippingLabel
{
    ...
    protected function save_temporary($order, $shipping_option)
    {
        ...
        if (!@file_put_contents($file_path, $this->image_data)) {
            throw new SystemException('Error saving the label file to '.$file_path);
        }
    ...
    public static function output_label($link)
    {
        $parts = explode('-', $link);
        if (count($parts) < 2) {
            throw new ApplicationException('Invalid label link format.');
        }
        ...
        $files = glob($pattern);
        if (!$files) {
            throw new ApplicationException('Label file not found.');
        }
        ...
        if (!$extension) {
            throw new ApplicationException('Unknown image label type.');
        }
