...
class ProductCsvImportModel extends CsvImportModel
{
    ...
    public $has_many = [
        'csv_file' => [
            ...
            'conditions' => "master_object_class='Shop_ProductCsvImportModel'",
            'order' => 'id',
            'delete' => true
        ],
        'config_import' => [
    ...
    public $has_and_belongs_to_many = [
        ...
        'product_groups' => [
            'class_name' => 'Shop\CustomGroup',
            'join_table' => 'shop_products_customgroups',
            'order' => 'name'
        ],
        'price_tiers' => [
            'class_name' => 'Shop\PriceTier',
            'join_table' => 'shop_tier_prices',
            'order' => 'id'
        ]
    ];
    ...
    public function __construct($values = null, $options = array())
    {
        ...
        $this->tax_class_id = TaxClass::get_default_class_id();
    }
    ...
    public function define_columns($context = null)
    {
        ...
        $this->define_column('auto_product_types', 'I want LSAPP to apply product types specified in the CSV file');

        $this->define_relation_column('product_type', 'product_type', 'Product Type ', db_varchar, '@name')
            ->defaultInvisible()
            ->listTitle('Type')
            ->validation()
            ->required();

        $this->define_column('auto_create_categories', 'I want LSAPP to create categories specified in the CSV file');
        ...
        $this->define_column(
            ...
            'I have ZIP files or a directory on the server containing product images and I want LSAPP to import them'
        );

        $this->define_multi_relation_column('images_file', 'images_file', 'Images', '@name')
            ->invisible();
        ...
        $this->define_column(
            ...
            'I have a directory on the server containing downloadable product files and I want LSAPP to import them'
        );

        $this->define_column('files_directory_path', 'Files directory on the server');

        $this->define_column('auto_tax_classes', 'I want LSAPP to create tax classes specified in the CSV file');

        $this->define_relation_column('tax_class', 'tax_class', 'Tax Class ', db_varchar, '@name');

        $this->define_column('auto_manufacturers', 'I want LSAPP to create manufacturers specified in the CSV file');

        $this->define_relation_column('manufacturer', 'manufacturer', 'Manufacturer ', db_varchar, '@name');

        $this->define_column('update_existing_sku', 'I want LSAPP to update existing products');

        $this->define_column('descriptions_html', 'The Long Description field in the CSV file contains HTML tags');

        $this->define_column(
            'short_descriptions_html',
            'The Short Description field in the CSV file contains HTML tags'
        );

        $this->define_column(
            'auto_create_product_groups',
            'I want LSAPP to create product groups specified in the CSV file'
        );

        $this->define_multi_relation_column('product_groups', 'product_groups', 'Product Groups', '@name')
            ->defaultInvisible()
            ->validation();
    }
    ...
    public function define_form_fields($context = null)
    {
        ...
        $this->add_form_field('auto_product_types')
            ->comment(
                'You need to match the LSAPP product <strong>Product Type</strong> field to a CSV file column 
                      in order to use this feature. 
                      Otherwise please select a product type for all imported products in the list below. 
                      This field is required for new products.',
                'above',
                true
            );

        $this->add_form_field('product_type')
            ->comment('Please select a product type for imported products', 'above')
            ->emptyOption('<please select>')
            ->cssClassName('expandable');

        $this->add_form_field('auto_create_categories')
            ->comment(
                'You need to match the LSAPP product <strong>Categories</strong> 
                      field to a CSV file column in order to use this feature. 
                      Otherwise please select categories for all imported products in the list below. 
                      This field is required for new products.',
                'above',
                true
            );
        ...
        $this->add_form_field('categories')
            ->comment('Please select the categories that the imported products will belong to', 'above')
            ->referenceSort('name')
            ->cssClassName('expandable');

        $this->add_form_field('images_file', 'left')
            ->renderAs(frm_file_attachments)
            ->addDocumentLabel('Upload image archive(s)')
            ->fileDownloadBaseUrl(url('ls_backend/files/get/'))
            ->noAttachmentsLabel('');

        $this->add_form_field('import_product_images')
            ->comment(
                'You need to match the LSAPP product <strong>Images</strong> 
                      field to a CSV file column in order to use this feature. 
                      The images field in the CSV file should contain paths to image files in the images archive. 
                      You can upload multiple image ZIP archives. 
                      We recommend to split large image sets to smaller archives,
                      containing not more than 20,000 files each. Size of an individual file should not exceed ' .
                File::sizeFromBytes(Upload::maxUploadSize()) . '.
                      <br/><br/>Note: you can use image ZIP archives or the images directory, or both.',
                'above',
                true
            );

        $this->add_form_field('images_directory_path')
            ->comment(
                'Use this field to specify a directory on the server, 
                which contains product images. Please use an absolute path, 
                for example /users/me/import_images. 
                The absolute path of your LSAPP directory is ' . PATH_APP,
                'above',
                true
            )
            ->renderAs(frm_text)
            ->cssClassName('offsetTop');

        $this->add_form_field('images_path')
            ->comment(
                'Use this field to specify a directory inside the ZIP archive or inside the images 
                      directory that the images column in the CSV file refers to. 
                      For example, if the images column refers to images/product.jpeg file, 
                      and the actual path to the product.jpeg file in the ZIP archive is products/images/product.jpeg, 
                      you should specify the <strong>products</strong> value in this field. 
                      Leave this field empty if the images directory is in the root of the archive.',
                'above',
                true
            )
            ->renderAs(frm_text)
            ->cssClassName('offsetTop expandable');

        $this->add_form_field('import_product_files')
            ->comment(
                'You need to match the LSAPP product <strong>Files</strong> field to a 
                      CSV file column in order to use this feature. 
                      The files field in the CSV file should contain file names separated with comma.',
                'above',
                true
            );

        $this->add_form_field('files_directory_path')
            ->comment(
                'Use this field to specify a directory on the server, 
                which contains product files. Please use an absolute path, for example /users/me/import_files. 
                The absolute path of your LSAPP directory is ' . PATH_APP,
                'above',
                true
            )
        ...
        $this->add_form_field('auto_tax_classes')
            ->comment(
                'You need to match the LSAPP product <strong>Tax Class</strong> 
                      field to a CSV file column in order to use this feature. 
                      Otherwise please select a tax class for all imported products in the field below. 
                      This field is required for new products.',
                'above',
                true
            );
        ...
        $this->add_form_field('tax_class')
            ->comment('Please select a tax class for imported products', 'above')
            ->cssClassName('expandable');

        $this->add_form_field('auto_manufacturers')
            ->comment(
                'You need to match the LSAPP product <strong>Manufacturer</strong> 
                            field to a CSV file column in order to use this feature. 
                            Otherwise please select a manufacturer for all imported products in the field below. 
                            This field is optional.',
                'above',
                true
            );

        $this->add_form_field('manufacturer')
            ->comment('Please select a manufacturer for imported products', 'above')
            ->emptyOption('<please select>')
            ->cssClassName('expandable');
        ...
        $this->add_form_field('update_existing_sku')
            ->comment(
                ...
                LSAPP will skip products with existing SKU values 
                (Option Matrix products are identified by option values). 
                Otherwise existing products will be updated using information from the CSV file 
                and other parameters specified on this page.',
                'above',
                true
            );

        $this->add_form_field('descriptions_html')
            ->comment(
                'If you leave this checkbox unchecked, 
                LSAPP will treat data in the Long Description field as plain text.',
                'above',
                true
            );

        $this->add_form_field('short_descriptions_html')
            ->comment(
                'If you leave this checkbox unchecked, 
                LSAPP will treat data in the Short Description field as plain text.',
                'above',
                true
            );

        $this->add_form_field('auto_create_product_groups')
            ->comment(
                'You need to match the LSAPP product <strong>Product Groups</strong> 
                      field to a CSV file column in order to use this feature. 
                      Otherwise please select product groups for all imported products in the list below 
                      if you would like to add the imported products to a product group.',
                'above',
                true
            );

        $this->add_form_field('product_groups')
            ->comment(
                'Please select the product groups that the imported products will belong 
                to or leave none selected to import with no product groups.',
                'above'
            )->referenceSort('name')
            ->cssClassName('expandable');
    }
    ...
    public function import_csv_data(
        ...
        if (!$config_object->update_existing_sku) {
            if (!$import_manager->csvImportDbColumnPresented($column_map, 'name')) {
                throw new ApplicationException(
                    'Please specify a matching column for the Name field or select the 
                    "I want LSAPP to update products with existing SKU" checkbox if updating products.'
                );
            }
            if (!$import_manager->csvImportDbColumnPresented($column_map, 'price')) {
                throw new ApplicationException(
                    'Please specify a matching column for the Price field or select the 
                    "I want LSAPP to update products with existing SKU" checkbox if updating products.'
                );
            }
            ...
            if (!$config_object->auto_product_types) {
                if (!$config_object->product_type) {
                    throw new ApplicationException(
                        'Please select a Product type or select the 
                    "I want LSAPP to update products with existing SKU" checkbox if updating products.'
                    );
                }
            } elseif (!$import_manager->csvImportDbColumnPresented($column_map, 'product_type')) {
                throw new ApplicationException(
                    'Please specify a matching column for the Product type field,
                     or uncheck the "I want LSAPP to apply product types specified in the CSV file" checkbox.'
                );
            }

            $hasTaxClass = $import_manager->csvImportDbColumnPresented($column_map, 'tax_class');
            if ($config_object->auto_tax_classes && !$hasTaxClass) {
                //checkbox is selected and tax class column is not specified
                throw new ApplicationException(
                    'If importing new products, please specify a matching column for the Tax Class product field 
                    or uncheck the "I want LSAPP to create tax classes specified in the CSV file" checkbox. If 
                    updating products, please select the "I want LSAPP to update products with existing SKU" checkbox.'
                );
            }

            if (!$config_object->auto_create_categories) {
                $hasNoCategories = ($config_object->categories && !$config_object->categories->count);
                if (!$config_object->categories || $hasNoCategories) {
                    throw new ApplicationException('Please select product categories.');
                }
            }
        }

        if (!$config_object->auto_tax_classes && !$config_object->tax_class) {
            throw new ApplicationException('Please select a tax class.');
        }

        if ($config_object->auto_manufacturers) {
            if (!$import_manager->csvImportDbColumnPresented($column_map, 'manufacturer_link')) {
                throw new ApplicationException(
                    'Please specify a matching column for the Manufacturer product field, 
                    or uncheck the "I want LSAPP to create manufacturers specified in the CSV file" checkbox.'
                );
            }
        }
        ...
        if ($config_object->auto_create_product_groups) {
            if (!$import_manager->csvImportDbColumnPresented($column_map, 'product_groups')) {
                throw new ApplicationException(
                    'Please specify a matching column for the Product Groups product field, 
                    or uncheck the "I want LSAPP to create product groups specified in the CSV file" checkbox.'
                );
            }
        }
        ...
        if ($import_option_matrix_records && $this->om_parent_sku_column_index === null) {
            throw new ApplicationException(
                'Please specify a matching column for Option Matrix 
                - Parent Product SKU product field or remove Option Matrix Record 
                Flag field from the column list.'
            );
        }

        if ($import_option_matrix_records && $this->om_options_column_index === null) {
            throw new ApplicationException(
                'Please specify a matching column for Options product field 
                or remove Option Matrix Record Flag field from the column list.'
            );
        }

        if ($import_option_matrix_records) {
            $this->option_matrix_manager = new OptionMatrixManager();
        }
        ...
        try {
            ...
            if ($config_object->import_product_images) {
                ...
                if (!$image_files->count && !strlen($images_server_dir)) {
                    throw new ApplicationException(
                        'Please upload an image archive or specify the images directory on the server, 
                        or uncheck the option "I have a ZIP archive(s) or a directory on the server containing 
                        product images and I want LSAPP to import them".'
                    );
                }
                ...
                foreach ($image_files as $image_archive_file) {
                    ...
                    if (!@mkdir($images_archive_dir)) {
                        throw new ApplicationException('Unable to create a temporary directory');
                    }

                    $archivePath = PATH_APP . $image_archive_file->getPath();
                    Zip::unzip($archivePath, $images_archive_dir);
                }

                if (strlen($images_server_dir)) {
                    if (!file_exists($images_server_dir)) {
                        throw new ApplicationException('Images directory not found: ' . $images_server_dir);
                    }
                ...
                if (strlen($images_root_dir)) {
                    ...
                    if (strpos($images_root_dir, '..') !== false) {
                        throw new ApplicationException(
                            'Invalid value specified in the "Path to the images directory in the ZIP file" field. 
            ...
            if ($config_object->import_product_files) {
                ...
                if (!strlen($files_server_dir)) {
                    throw new ApplicationException(
                        'Please specify the files directory on the server, or uncheck the option 
                        "I have a directory on the server containing downloadable product files 
                        and I want LSAPP to import them".'
                    );
                }

                if (!file_exists($files_server_dir)) {
                    throw new ApplicationException('Product files directory not found: ' . $files_server_dir);
                }
            ...
            while (($row = fgetcsv($csv_handle, 2000000, $delimeter)) !== false) {
                ...
                try {
                    ...
                    $isOm = $row[$om_flag_column_index] ?? false;
                    if ($import_option_matrix_records && Boolean::from($isOm)) {
                        $import_result = $this->import_om_record($column_map, $row, $warnings, $line_number);
                        if ($import_result->status != OptionMatrixManager::STATUS_SKIPPED) {
                            if ($import_result->operation == OptionMatrixManager::OPERATION_ADD) {
                                $added++;
                            } else {
                                $updated++;
                            }
                    ...
                    foreach ($column_map as $column_index => $db_names) {
                        ...
                        foreach ($db_names as $db_name) {
                            ...
                            if ($db_name == 'sku' && strlen($column_value)) {
                                $existing_product_id = DbHelper::scalar(
                                    "select id from shop_products 
                                          where lower(sku)=:sku and not (exists(
                                              select * from db_deferred_bindings
                                              where detail_class_name=:class_name 
                                                and master_relation_name='grouped_products_all' 
                                                and detail_key_value=shop_products.id))",
                                    [
                                        'sku' => mb_strtolower($column_value),
                            ...
                            if ($column_definitions[$db_name]->type == db_bool) {
                                $bind[$db_name] = Boolean::from($column_value) ? '1' : '0';
                            } else {
                                ...
                                if ($db_name == 'description') {
                                    if (!$config_object->descriptions_html) {
                                        $column_value = Html::paragraphize($column_value);
                                    }

                                    $bind['pt_description'] = html_entity_decode(
                                        strip_tags($column_value),
                                        ENT_QUOTES,
                                        'UTF-8'
                                    );
                                }
                                ...
                                if ($db_name == 'categories') {
                                    if ($config_object->auto_create_categories) {
                                        ...
                                        $product_categories = $this->create_categories(
                                            $column_value,
                                            $existing_product_id
                                        );
                                    }
                    ...
                    if (array_key_exists('csv_import_parent_sku', $bind)) {
                        ...
                        $grouped_parent_sku[$product_id] = [
                            $value,
                            $line_number,
                            $product_sku,
                            trim($bind['csv_import_parent_sku'])
                        ];
                    }
    ...
    protected function get_product_type_id($value)
    {
        if (!$this->existing_product_types) {
            ...
            $existing_product_types = DbHelper::objectArray('select id, name from shop_product_types');
        ...
        if ($value != '') {
            ...
            $existing_type = $this->existing_product_types[$key] ?? false;
            if ($existing_type) {
                return $this->existing_product_types[$key];
            } else {
                throw new ApplicationException('Specified Product type does not exist (' . $value . ').');
            }
        }
    }
    ...
    protected function create_product_groups($value)
    {
        ...
        if (!$this->existing_product_groups) {
            ...
            $existing_product_groups = DbHelper::objectArray('select id, name from shop_custom_group');
    ...
    protected function create_product_group($product_group_name)
    {
        ...
        if ($product_group_exists) {
            ...
            $product_group_obj = new CustomGroup();
    ...
    protected function prepare_product_group_code($group_name)
    {
        ...
        $orig_code = $code;
        $counter = 1;
        while (DbHelper::scalar('select count(*) from shop_custom_group where code=:code', array('code' => $code))) {
    ...
    protected function create_categories($value, $existing_product = false)
    {
        ...
        if (!$this->existing_cagetories) {
            $existing_categories = DbHelper::objectArray('select id, name, category_id from shop_categories');
        ...
        if (!$result && !$existing_product) {
            throw new ApplicationException('Category name is not specified for a new product.');
        }
    ...
    protected function create_manufacturer($value)
    {
        if (!$this->existing_manufacturers) {
            $existing_manufacturers = DbHelper::objectArray('select id, name from shop_manufacturers');
    ...
    protected function create_tax_class($value)
    {
        if (!$this->existing_tax_classes) {
            $existing_tax_classes = DbHelper::objectArray('select id, name from shop_tax_classes');
        ...
        if (!strlen($tax_class_name)) {
            throw new ApplicationException('Tax class name is not specified');
        }
    ...
    protected function prepare_category_url_name($category_name, $parent_id = null)
    {
        ...
        $url_name = $this->format_url_name($category_name, ConfigurationRecord::get()->nested_category_urls);

        $orig_url_name = $url_name;
        $counter = 1;
        $sql = 'select count(*) from shop_categories where url_name=:url_name';
        //if "Prepend parent category URL" enabled,
        //url name should be unique only between categories with the same parent
        $cr = ConfigurationRecord::get();
        if ($cr->category_urls_prepend_parent && $cr->nested_category_urls) {
            if ($parent_id) {
        ...
        while (DbHelper::scalar($sql, ['url_name' => $url_name, 'parent_id' => $parent_id])) {
    ...
    protected function prepare_product_url_name($product_name, $product_id)
    {
        ...
        while (DbHelper::scalar('select count(*) from shop_products where url_name=:url_name' . $product_filter, [
            'url_name' => $url_name,
            'id' => $product_id
        ])) {
            $url_name = $orig_url_name . $separator . $counter;
    ...
    protected function sanitize_product_url_name($name, $url_name, &$warnings, $line_number, $product_id)
    {
        ...
        if (!preg_match('/^[0-9a-z_-]$/i', $url_name)) {
            ...
            if (!strlen($new_url_name)) {
                ...
                $bind = ['url_name' => $new_url_name, 'id' => $product_id];
                $sql = 'select count(*) from shop_products 
                         where url_name=:url_name' . $product_filter;
                while (DbHelper::scalar($sql, $bind)) {
        ...
        if ($new_url_name != $url_name) {
            $this->add_warnings_line(
                $warnings,
                $line_number,
                'Product URL Name has been fixed. Original value: "' . $url_name . '", new value: "' . $new_url_name . '"'
            );
        }
    ...
    protected function prepare_manufacturer_url_name($name)
    {
        ...
        $orig_url_name = $url_name;
        $counter = 1;
        $sql = 'select count(*) from shop_manufacturers where url_name=:url_name';
        $bind = ['url_name' => $url_name];
        while (DbHelper::scalar($sql, $bind)) {
            $url_name = $orig_url_name . $separator . $counter;
            $counter++;
        }
    ...
    protected function create_product(&$bind)
    {
        ...
        if ((!array_key_exists('grouped_sort_order', $bind) || !strlen($bind['grouped_sort_order'])) &&
        ...
        $id = DbHelper::driver()->get_last_insert_id();

        if (array_key_exists('sku', $bind)) {
            $this->sku_cache[mb_strtoupper($bind['sku'])] = $id;
        }

        Product::update_total_stock_value($id);
    ...
    protected function update_product_fields($existing_product_id, $bind)
    {
        ...
        Product::update_total_stock_value($existing_product_id);
    ...
    protected function update_parent_id(
        $product_id,
        $parent_sku,
        &$warnings = null,
        $line_number = null,
        $product_sku = null,
        $original_parent_sku = null
    ) {
        $this->init_sku_cache();
        ...
        if (!strlen($parent_sku)) {
            ...
            if (!array_key_exists($parent_sku, $this->sku_cache)) {
                if (isset($warnings) && $line_number && isset($original_parent_sku)) {
                    $this->add_warnings_line(
                        ...
                        'Product was not grouped: parent SKU ' . $original_parent_sku . ' does not exist.'
                    );
                }
                return;
            }
            ...
            if ($parent_product_id == $product_id) {
                if (isset($warnings) && $line_number && isset($original_parent_sku)) {
                    $this->add_warnings_line(
                        $warnings, $line_number,
                        'Product was not grouped: SKU ' . $product_sku . ' cannot be grouped with itself.'
                    );
                }
                return;
            }
            ...
            $parents_parent_product_id = DbHelper::scalar(
                'select product_id from shop_products where id=:id',
            ...
            if ($parents_parent_product_id) {
                if (isset($warnings) && $line_number && isset($original_parent_sku)) {
                    $this->add_warnings_line(
                        $warnings,
                        $line_number,
                        'Product was not grouped: SKU ' . $original_parent_sku . ' has a parent product 
                        and cannot become a parent product itself.'
                    );
                }
                return;
            }
            //if current product has grouped products, don't allow it to get a parent
            $grouped_products = DbHelper::scalar(
                'select count(*) from shop_products where product_id=:id',
                ['id' => $product_id]
            );
            if ($grouped_products > 0) {
                if (isset($warnings) && $line_number && isset($product_sku)) {
                    $this->add_warnings_line(
                        $warnings,
                        $line_number,
                        'Product was not grouped: SKU ' . $product_sku . ' cannot be grouped, 
                        because it already contains other grouped products.'
                    );
                }
                return;
            }

            $grouped_sort_order = DbHelper::scalar(
                'select grouped_sort_order from shop_products where id=:id',
                ['id' => $product_id]
            );
            ...
            $this->update_product_fields($product_id, $bind);
            DbHelper::query(
                'delete from shop_products_categories where shop_product_id=:shop_product_id',
    ...
    protected function update_related_products($product_id, $related_sku, &$warnings, $line_number)
    {
        ...
        DbHelper::query(
            'delete from shop_related_products where master_product_id=:product_id',
        ...
        foreach ($related_sku_list as $related_product_sku) {
            ...
            if (!array_key_exists($related_product_sku, $this->sku_cache)) {
                $this->add_warnings_line(
                    $warnings,
                    $line_number,
                    'Related product with SKU "' . $related_product_sku . '" not found.'
                );
                continue;
            }
    ...
    protected function init_sku_cache()
    {
        ...
        $products = DbHelper::objectArray(
            "select id, upper(sku) as sku 
                  from shop_products 
                  where not (exists(
                      select * from db_deferred_bindings 
                      where detail_class_name=:class_name 
                        and master_relation_name='grouped_products_all' 
                        and detail_key_value=shop_products.id))",
            ['class_name' => get_class('Shop\Product')]
        );

        /*
         * Excluded not committed deferred bindings from the SKU list
         */
    ...
    protected function get_current_time()
    {
        ...
        return $this->current_time = PhprDateTime::now()->toSqlDateTime();
    }

    protected function set_product_categories($product_id, $categories)
    {
        $existing_links = DbHelper::scalarArray(
            'select shop_category_id from shop_products_categories where shop_product_id=:shop_product_id',
            ['shop_product_id' => $product_id]
        );
        ...
        foreach ($categories as $category_id) {
            if (!array_key_exists($category_id, $existing_links)) {
                $sql = 'insert into shop_products_categories(shop_product_id, shop_category_id) 
                        values (:shop_product_id, :shop_category_id)';
                $bind = ['shop_product_id' => $product_id, 'shop_category_id' => $category_id];
        ...
        foreach ($existing_links as $category_id => $val) {
            if (!in_array($category_id, $categories)) {
                $sql = 'delete from shop_products_categories 
    ...
    {
        $existing_links = DbHelper::scalarArray(
            'select shop_custom_group_id from shop_products_customgroups where shop_product_id=:shop_product_id',
            ['shop_product_id' => $product_id]
        );
        $existing_links = array_flip($existing_links);
        ...
        foreach ($product_groups as $product_group_id) {
            if (!array_key_exists($product_group_id, $existing_links)) {
                $sql = 'insert into shop_products_customgroups(shop_product_id, shop_custom_group_id) 
                        values (:shop_product_id, :shop_custom_group_id)';
                $bind = ['shop_product_id' => $product_id, 'shop_custom_group_id' => $product_group_id];
                DbHelper::query($sql, $bind
                );
            }
        }
        ...
        foreach ($existing_links as $product_group_id => $val) {
            if (!in_array($product_group_id, $product_groups)) {
                $sql = 'delete from shop_products_customgroups 
                        where shop_product_id=:shop_product_id
                          and shop_custom_group_id=:shop_custom_group_id';
                $bind = ['shop_product_id' => $product_id, 'shop_custom_group_id' => $product_group_id];
                DbHelper::query($sql, $bind);
            }
        }
    ...
    {
        $product_price_tiers = DbHelper::objectArray(
            'select * from shop_tier_prices where product_id=:product_id',
            ['product_id' => $product_id]
        );
        ...
        foreach ($product_price_tiers as $product_price_tier) {
            ...
            foreach ($price_tiers as $csv_price_tier) {
                $groupCheck = $csv_price_tier->customer_group_id == $product_price_tier->customer_group_id;
                $quantityCheck = $csv_price_tier->quantity == $product_price_tier->quantity;
                if ($groupCheck && $quantityCheck) {
                    $tier_found = true;
                    break;
                }
            }
            ...
            if (!$tier_found) {
                DbHelper::query('delete from shop_tier_prices where id=:id', array('id' => $product_price_tier->id));
            }
        }
        ...
        foreach ($price_tiers as $price_tier) {
            ...
            foreach ($product_price_tiers as $product_price_tier) {
                $groupCheck = $price_tier->customer_group_id == $product_price_tier->customer_group_id;
                $quantityCheck = $price_tier->quantity == $product_price_tier->quantity;
                if ($groupCheck && $quantityCheck) {
            ...
            if ($existing_tier_id === false) {
                $bind = [
                    'product_id' => $product_id,
                    'customer_group_id' => $price_tier->customer_group_id,
                    'quantity' => $price_tier->quantity,
                    'price' => $price_tier->price
                ];
                if ($price_tier->customer_group_id != '') {
                    $sql = 'insert into shop_tier_prices(product_id, customer_group_id, quantity, price) 
                            values (:product_id, :customer_group_id, :quantity, :price)';
                    DbHelper::query($sql, $bind);
                } else {
                    $sql = 'insert into shop_tier_prices(product_id, quantity, price) 
                            values (:product_id, :quantity, :price)';
                    DbHelper::query($sql, $bind);
                }

                $tier_id = DbHelper::driver()->get_last_insert_id();
            } else {
                DbHelper::query(
                    'update shop_tier_prices set price=:price where id=:id',
                    ['price' => $price_tier->price, 'id' => $existing_tier_id]
                );
        ...
        DbHelper::query('update shop_products set tier_price_compiled=:tier_price_compiled where id=:id', array(
    ...
    protected function set_product_attributes($product_id, $attributes)
    {
        foreach ($attributes as $attr_id => $values) {
            ...
            $bind = ['product_id' => $product_id, 'attr_name' => $attr_name];
            DbHelper::query(
                'delete from shop_product_properties where product_id=:product_id and name=:attr_name',
                $bind
            );
            ...
            foreach ($values as $value) {
                ...
                if (strlen($value)) {
                    $sql = 'insert into shop_product_properties(product_id, name, value) 
                            values (:product_id, :attr_name, :value)';
                    $bind = ['product_id' => $product_id, 'attr_name' => $attr_name, 'value' => $value];
                    DbHelper::query($sql, $bind);
                    DbHelper::query(
                        'update shop_product_properties set sort_order = LAST_INSERT_ID() where id=LAST_INSERT_ID()'
                    );
                }
            }
        }
    ...
    protected function prepare_files($files)
    {
        ...
        foreach ($files as $file) {
            ...
            if (!file_exists($full_path)) {
                throw new ApplicationException("Product file not found in the files directory: " . $full_path);
            }
    ...
    protected function set_product_images($product_id, $product_images)
    {
        ...
        $sql = "select * from db_files 
                where master_object_class=:class_name 
                  and master_object_id=:product_id 
                  and field=:field";
        ...
        $product_files = DbHelper::queryArray($sql, $bind);

        $file_obj = DbFile::create();
        ...
        foreach ($product_images as $image_path) {
            ...
            $file->master_object_class = 'Product';
            $file->field = 'images';
    ...
    protected function parse_options_string($str)
    {
        ...
        foreach ($options as $index => $option) {
            ...
            if (count($option_parts) < 2) {
                throw new ApplicationException('Invalid option value: ' . $option);
            }
    ...
    protected function check_deleted_options($product_id, $product_options)
    {
        ...
        $product_options = DbHelper::objectArray('
				select 
        ...
        foreach ($product_options as $product_option_data) {
            ...
            if (!array_key_exists($product_option_data->name, $options)) {
                if (!CustomAttribute::is_option_can_be_deleted($product_option_data->id)) {
                    throw new ApplicationException(
                        sprintf(
                            'Cannot delete product option "%s" because there are orders referring to it.',
    ...
    protected function set_product_options($product_id, $product_options)
    {
        ...
        foreach ($options_to_remove as $option_id) {
            DbHelper::query(
                'delete from shop_custom_attributes where id=:id',
                array('id' => $option_id)
            );

            CustomAttribute::cleanup_option_data($option_id);
        }
        ...
        foreach ($options as $option_name => $option_values) {
            ...
            if (array_key_exists($option_name, $options_ids_by_name)) {
                ...
                $sql = 'insert into shop_custom_attributes(name, product_id, attribute_values, option_key, sort_order) 
                        values (:name, :product_id, :attribute_values, :option_key, :sort_order)';
                DbHelper::query($sql, $bind);
            }
        }
    }
    ...
    {
        $sql = 'delete from shop_extra_options 
                where product_id=:product_id 
                  and (option_in_set is null or option_in_set=0)';
        DbHelper::query($sql, ['product_id' => $product_id]);
        ...
        foreach ($options as $option) {
            ...
            foreach ($rows as $row) {
                ...
                if (count($row_data) < 2) {
                    throw new ApplicationException(
                        'Invalid extra option value: ' . $row . '.
                         Extra option data should have at least 2 columns: option name and price.'
                    );
                }
                ...
                if (!strlen($option_name)) {
                    throw new ApplicationException(
                        'Invalid extra option value: ' . $row . '. Option name is not specified.'
                    );
                }
                ...
                if (!strlen($option_price)) {
                    throw new ApplicationException(
                        'Invalid extra option value: ' . $row . '. Option price is not specified.'
                    );
                }

                if (!preg_match('/^(\-?[0-9]*\.[0-9]+|\-?[0-9]+)$/', $option_price)) {
                    throw new ApplicationException(
                        'Invalid extra option price value: ' . $option_price . '. 
                        Please specify price as number: 0.35, 10, etc.'
                    );
                }
                ...
                $sql = 'insert into shop_extra_options(description, product_id, price, option_key, group_name) 
                        values (:description, :product_id, :price, :option_key, :group_name)';
                DbHelper::query($sql, $bind);

                $last_option_id = DbHelper::driver()->get_last_insert_id();
                DbHelper::query('update shop_extra_options set extra_option_sort_order=:sort_order where id=:id', [
                ...
                if (strlen($images) && $this->import_images) {
                    $line_number = isset($line_number) ? $line_number : null;
    ...
    {
        DbHelper::query(
            'delete from shop_products_extra_sets where extra_product_id=:product_id',
            array('product_id' => $product_id)
        );
        ...
        if ($this->extra_set_cache == null) {
            ...
            $sets = DbHelper::objectArray('select * from shop_extra_option_sets');
            foreach ($sets as $set) {
                if (strlen($set->code)) {
        ...
        $sql = 'insert into shop_products_extra_sets(extra_product_id, extra_option_set_id) 
    ...
    protected function validate_fields(&$bind, $existing_product, &$warnings, $line_number)
    {
        ...
        if (!$existing_product && (!array_key_exists('product_type_id', $bind) || !strlen($bind['product_type_id']))) {
            throw new ApplicationException('Product type is not specified for a new product.');
        ...
        if (!$existing_product && (!array_key_exists('name', $bind) || !strlen($bind['name']))) {
            throw new ApplicationException('Product name is not specified for a new product.');
        } elseif ($existing_product && array_key_exists('name', $bind) && !strlen($bind['name'])) {
            throw new ApplicationException('Product name is not specified.');
        }

        if (!$existing_product && (!array_key_exists('price', $bind) || !strlen($bind['price']))) {
            throw new ApplicationException('Product price is not specified for a new product.');
        } elseif ($existing_product && array_key_exists('price', $bind) && !strlen($bind['price'])) {
            throw new ApplicationException('Product price is not specified.');
        }

        if (!$existing_product && (!array_key_exists('tax_class_id', $bind) || !strlen($bind['tax_class_id']))) {
            throw new ApplicationException('Tax class is not specified for a new product.');
        ...
        if (!$existing_product && (!array_key_exists('categories', $bind) || !strlen($bind['categories']))) {
            throw new ApplicationException('Categories are not specified for a new product.');
        } elseif ($existing_product && array_key_exists('categories', $bind) && !strlen($bind['categories'])) {
            $this->add_warnings_line(
                $warnings,
                $line_number,
                'Product categories were not specified and were not updated.'
            );
        }

        $onSale = $bind['on_sale'] ?? false;
        $salePriceDiscount = $bind['sale_price_or_discount'] ?? false;
        if (!$existing_product && $onSale && $salePriceDiscount) {
            $bind['on_sale'] = 0;
            $this->add_warnings_line(
                $warnings,
                $line_number,
                'No sale price or discount specified, product was not imported as on sale.'
            );
        }
        if (array_key_exists('sale_price_or_discount', $bind) && strlen($bind['sale_price_or_discount'])) {
            $sale_price_error = Product::is_sale_price_or_discount_invalid($bind['sale_price_or_discount']);
            if ($sale_price_error) {
                throw new ApplicationException(
                    'Invalid sale price or discount specified: ' . $bind['sale_price_or_discount'] . '. ' . $sale_price_error
                );
            }
        }

        if (!array_key_exists('sku', $bind) || !strlen($bind['sku'])) {
            throw new ApplicationException('Product SKU is not specified');
        }
        ...
        if (array_key_exists('url_name', $bind) && strlen($bind['url_name'])) {
            $bind['url_name'] = $this->sanitize_product_url_name(
                $bind['name'],
                $bind['url_name'],
                $warnings,
                $line_number,
    ...
    protected function parse_price_tiers($price_tiers_str)
    {
        ...
        if (strlen($price_tiers_str)) {
            ...
            foreach ($rows as $row) {
                ...
                if (count($cells) != 3) {
                    throw new ApplicationException(
                        'Invalid price tier specified: ' . $row . '. 
                        Valid format for price tiers is: Customer Group|Quantity|Price (Guest|5|5.45 or *|1|23)'
                    );
                }
                ...
                if (!strlen($customer_group)) {
                    throw new ApplicationException(
                        'Missing customer group for price tier (use * to apply price tier to all customer groups).'
                    );
                }
                if ($customer_group != '*') {
                    //check that the customer group exists
                    if (!isset($this->existing_customer_groups)) {
                        $this->fetch_customer_groups();
                    }
                    if (!array_key_exists(strtoupper($customer_group), $this->existing_customer_groups)) {
                        throw new ApplicationException(
                            'Invalid customer group specified: ' . $customer_group . '. Customer group does not exist.'
                        );
                    } else {
                        $customer_group_id = $this->existing_customer_groups[strtoupper($customer_group)];
                    }
                } else {
                ...
                if (!preg_match('/^([0-9])+$/', $quantity)) {
                    throw new ApplicationException(
                        'Invalid price tier quantity: ' . $quantity . '. Please specify the quantity as an integer: 10'
                    );
                }
                ...
                if (!preg_match('/^([0-9]+\.[0-9]+|[0-9]+)$/', $price)) {
                    throw new ApplicationException(
                        'Invalid price tier price: ' . $price . '. Please specify the price as a number: 10 or 10.5'
                    );
                }
        ...
        if (strlen($shipping_cost_str)) {
            if (preg_match('/^([0-9]+\.[0-9]+|[0-9]+)$/', $shipping_cost_str)) {
                ...
                foreach ($rows as $row) {
                    ...
                    if (count($cells) < 4) {
                        throw new ApplicationException(
                            'Invalid shipping cost location specifier: ' . $row . '. 
                            Valid format for per-product shipping cost is: CA|BC|*|10'
                        );
                    }
                    ...
                    if (!preg_match('/^([0-9]+\.[0-9]+|[0-9]+)$/', $cost)) {
                        throw new ApplicationException(
                            'Invalid shipping cost value: ' . $cost . '. Please specify cost as a number: 10 or 10.5'
                        );
                    }
    ...
    protected function fetch_customer_groups()
    {
        if (!$this->existing_customer_groups) {
            ...
            $groups = DbHelper::objectArray('select id, name from shop_customer_groups');
            if (count($groups)) {
    ...
    protected function import_om_record(&$column_map, $row, &$warnings, $line_number)
    {
        ...
        if (isset($bind['enabled'])) {
            $bind['disabled'] = !Boolean::from($bind['enabled']);
        }
        ...
        if (!isset($row[$this->om_parent_sku_column_index])) {
            throw new ApplicationException('Parent product SKU is not specified for Option Matrix record');
        }
        ...
        if (!array_key_exists($sku_upper, $this->sku_cache)) {
            throw new ApplicationException(sprintf('Option Matrix parent product with SKU "%s" not found', $sku));
        }
        ...
        if ($result->status != OptionMatrixManager::STATUS_SKIPPED) {
            Backend::$events->fireEvent(
                'shop:onAfterCsvOptionMatrixImport',
                $result,
                $bind,
                $product_id,
                $record_options,
