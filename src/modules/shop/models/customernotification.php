...
class CustomerNotification extends ActiveRecord
{
    ...
    public $has_many = [
        'files' => [
            ...
            'conditions' => "(master_object_class='Shop_OrderNotification' or master_object_class='Shop_CustomerNotification') and field='files'",
            'order' => 'id',
            'delete' => true
        ]
    ];
    ...
    {
        $this->define_column('email', 'Recipient Email(s)')
            ->validation()
            ->fn('trim')
            ->required()
        ...
        $this->define_multi_relation_column('files', 'files', 'Attachments', '@name')
            ->defaultInvisible();
        ...
        $this->define_column('reply_to_email', 'Reply-To Email')
            ->validation()
            ->required('Please enter the reply-to email address.')
            ->email(false, 'Please specify a valid reply-to address');

        $this->define_column('reply_to_name', 'Reply-To Name')
            ->validation()
            ->required('Please enter the reply-to name.');
    }
    ...
    {
        $template = EmailLayout::find_by_code('external');
