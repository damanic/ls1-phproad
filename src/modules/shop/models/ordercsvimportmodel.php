...
class OrderCsvImportModel extends CsvImportModel
{
    public $table_name = 'shop_orders';
    public $shipping_method_api_codes = [];

    public $has_many = [
        'csv_file' => [
            'class_name' => 'Db\File',
            'foreign_key' => 'master_object_id',
            'conditions' => "master_object_class='Shop_OrderCsvImportModel'",
            'order' => 'id',
            'delete' => true
        ],
        'config_import' => [
                    ...
                    foreach ($column_map as $column_index => $db_names) {
                        ...
                        foreach ($db_names as $db_name) {
                            ...
                            if ($db_name == 'id' && strlen($column_value)) {
                                ...
                                $existing_order_id = DbHelper::scalar(
                                    'select id from shop_orders where id=:id',
                                    ['id' => $column_value]
                                );
                            ...
                            if ($db_name == 'shop_order_shipping_track_codes' && strlen($column_value)) {
                                $order_tracking_codes = $this->get_tracking_codes(
                                    $column_value,
                                    $warnings,
                                    $line_number
                                );
    ...
    public function update_tracking_codes($order_id, $tracking_codes)
    {
        ...
        $existing_tracking_codes = DbHelper::queryArray(
            'select id, shipping_method_id, code from shop_order_shipping_track_codes where order_id=:order_id',
            ['order_id' => $order_id]
        );
        foreach ($existing_tracking_codes as $index => $existing_code) {
            //check if it's in the array of imported codes
            // - if not, delete it; if yes, remove it from codes to add to avoid reading it
            if (array_key_exists($existing_code['shipping_method_id'], $tracking_codes)
                ...
                DbHelper::query(
                    'delete from shop_order_shipping_track_codes where id=:id',
                    ['id' => $existing_code['id']]
                );
            }
        }
        foreach ($add_tracking_codes as $shipping_method_id => $codes_to_add) {
            $shipping_method = ShippingOption::create()->find_by_id($shipping_method_id);
            foreach ($codes_to_add as $code => $k) {
                $tracking_code_obj = new OrderTrackingCode();
                $tracking_code_obj->code_shipping_method = $shipping_method;
    ...
    public function get_tracking_codes($value, &$warnings, $line_number)
    {
        ...
        if (!$this->shipping_method_api_codes) {
            $shipping_methods = DbHelper::objectArray(
                'select id, ls_api_code from shop_shipping_options where ls_api_code is not null'
            );
            foreach ($shipping_methods as $shipping_method) {
                $arrKey = trim(mb_strtolower($shipping_method->ls_api_code));
                $this->shipping_method_api_codes[$arrKey] = $shipping_method->id;
            }
        }
                ...
                if (count($data) == 2) {
                    ...
                    if (array_key_exists($shipping_api_code, $this->shipping_method_api_codes)) {
                        ...
                        $this->add_warnings_line(
                            $warnings,
                            $line_number,
                            'Shipping method with api code ' . $shipping_api_code . ' does not exist, 
                            tracking code ' . $code . ' was skipped.'
                        );
                    }
                } else {
                    $this->add_warnings_line(
                        $warnings,
                        $line_number,
                        'Invalid shipping tracking code format: ' . $tracking_code
                    );
                }
            }
        }
