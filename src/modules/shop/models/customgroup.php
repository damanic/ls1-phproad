 ...
 * There is no a special CMS action in LSAPP for loading custom groups from the database.
 * To load a custom group with a specific API code use the following code:
 * <pre>$group = CustomGroup::create()->find_by_code('featured');</pre>
 ...
 * @author LSAPP - MJMAN
 */
class CustomGroup extends ActiveRecord
{
    ...
    public $has_and_belongs_to_many = [
        'all_products'=> [
            ...
            'primary_key'=>'shop_custom_group_id',
            'foreign_key'=>'shop_product_id', 'order'=>'name'
        ],
        ...
        'products'=> [
            ...
            'order'=>'shop_products_customgroups.product_group_sort_order, shop_products.name'
        ]
    ];
        
    public $calculated_columns = [
        'product_num'=> [
            'sql'=>'select count(*) from shop_products,  shop_products_customgroups where
				shop_products.id=shop_products_customgroups.shop_product_id and
				shop_products_customgroups.shop_custom_group_id=shop_custom_group.id',
            'type'=>db_number
        ]
    ];
    ...
    public function define_form_fields($context = null)
    {
        ...
        $this->add_form_field('code')
            ->tab('Group')
            ->comment('You will use the code to refer the group to output its contents on pages.', 'above');
            
        $this->add_form_section(
            'Manage the product group contents. 
            You can manage the product order by dragging the arrow icons up and down.'
        )->tab('Products');

        if (!$front_end) {
            $this->add_form_field('all_products')
                ->tab('Products')
                ->comment('Products belonging to the group', 'above')
                ->renderAs('products')
                ->referenceSort('@name');
        }
    ...
    {
        DbHelper::query(
            'delete from shop_products_customgroups where shop_custom_group_id=:id',
            ['id'=>$this->id]
        );
    }
    ...
    public function get_products_orders()
    {
        ...
        $orders = DbHelper::objectArray(
            'select product_group_sort_order, shop_product_id 
                  from shop_products_customgroups 
                  where shop_custom_group_id=:group_id',
            ['group_id'=>$this->id]
        );
    ...
    public function set_product_orders($item_ids, $item_orders)
    {
        ...
        foreach ($item_ids as $index => $id) {
            ...
            DbHelper::query(
                'update shop_products_customgroups set product_group_sort_order=:product_group_sort_order 
                     where shop_product_id=:product_id 
                     and shop_custom_group_id=:group_id',
                [
     ...
     * The result of this function is an object of the {@link Product} class. To obtain a collection of
     * products call the {@link ActiveRecord::find_all() find_all()} method of the returned object:
    ...
    public function list_products($options = array())
    {
        ...
        foreach ($sorting as &$sorting_column) {
            ...
            if (strpos($sorting_column, 'price') !== false) {
                $customerGroupId = Controller::get_customer_group_id();
                $query = sprintf(Product::$price_sort_query, $customerGroupId);
     ...
     * To add new columns to the category model, call the {@link ActiveRecord::define_column() define_column()}
     ...
     * @param CustomGroup $group Specifies the group object.
     ...
     * which have been added with {@link shop:onExtendCustomGroupForm} event.
