...
class CustomerCartItem extends ActiveRecord
{
    ...
    public $has_many = [
        'uploaded_files'=> [
            ...
            'conditions'=>"master_object_class='Shop_CustomerCartItem' and field='uploaded_files'",
            'order'=>'id',
            'delete'=>true
        ]
    ];
    ...
    public static function gen_item_key($product_id, $options, $extras, $custom_data, $uploaded_files)
    {
        ...
        try {
            if ($uploaded_files && ($uploaded_files instanceof \Db\DataCollection || is_array($uploaded_files))) {
    ...
    public static function gen_item_content_key(
        $product_id,
        $options,
        $extras,
        $custom_data,
        $uploaded_files,
        $bundle_master_cart_key,
        $bundle_offer_id,
        $bundle_offer_item_id,
        $master_bundle_data,
        $quantity
    ) {
        $result = $product_id;
    ...
    public function get_content_key(&$existing_items)
    {
        ...
        foreach ($existing_items as $existing_item) {
            $existingItemKey = $existing_item->item_key;
            if($existingItemKey == $this->item_key){
                continue;
            }
            $existingBundleKey = $existing_item->bundle_master_cart_key;
            if (!$existingBundleKey || $existingBundleKey != $this->item_key) {
                continue;
            }
        ...
        $uploadedFiles = $this->list_related_records_deferred('uploaded_files', $this->id.session_id());
        $uploadedFilesArray = Cart::uploaded_files_to_array($uploadedFiles);
    ...
    public function mark_as_postponed($postponed)
    {
        ...
        DbHelper::query('update shop_customer_cart_items set postponed=:postponed where id=:id', [
    ...
    public function get_bundle_offer()
    {
        ...
        return ProductBundleOffer::find_by_id($this->bundle_offer_id);
    }
    ...
    public function get_bundle_offer_item()
    {
        ...
        return ProductBundleOfferItem::find_by_id($this->bundle_offer_item_id);
    }
