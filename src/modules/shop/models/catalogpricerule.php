...
class CatalogPriceRule extends PriceRuleBase
{
    ...
    public $has_and_belongs_to_many = array(
        'customer_groups'=> [
            ...
            'join_table'=>'shop_catalog_rules_customer_groups',
            'order'=>'name'
        ]
    );
    ...
    public function define_columns($context = null)
    {
        $this->define_column('name', 'Name')
            ->validation()->fn('trim')
            ->required("Please specify the price rule name.");
        
        $this->define_column('description', 'Description')
            ->validation()
            ->fn('trim')
            ->required('Please specify the rule description');
    ...
    public function define_form_fields($context = null)
    {
        ...
        $this->add_form_field('rule_conditions_field')
            ->tab('Conditions');

        $this->add_form_field('terminating')
            ->comment('Stop processing other rules for the same products, if this rule took effect.')
            ->tab('Action');

        $this->add_form_field('action_class_name')
            ->tab('Action')
    ...
    {
        $result = array();
        $actions = RuleActionBase::find_actions_by_type(RuleActionBase::type_product);
    ...
    {
        DbHelper::query('update shop_catalog_rules set sort_order=:sort_order where id=:id', [
            'sort_order'=>$this->id,
            'id'=>$this->id
        ]);
    ...
    public function is_actual_for($product, $current_product_price, $option_matrix_record = null)
    {
        $params = [
            ...
            'om_record'=>$option_matrix_record
        ];
    ...
    public static function process_products_batch($ids_str, $direct_call = false)
    {
        ...
        if (!isset($_SERVER['HTTP_LS_SUBQUERY']) && !$direct_call) {
            throw new ApplicationException('Only authorized LSAPP users can perform this operation.');
        }
    ...
    public static function process_product_om_batch($ids_str, $direct_call = false)
    {
        ...
        $records = new OptionMatrixRecord(null, array('no_column_init'=>true, 'no_validation'=>true));
    ...
    public static function apply_price_rules($product_id = null, $options = array())
    {
        ...
        try {
            ...
            if ($product_id == null) {
                $product_ids = DbHelper::scalarArray(
                    'select id from shop_products 
                          where ((grouped is null or grouped=0) or (grouped=1 and product_id is not null))'
                );
                ...
                do {
                    ...
                    if ($batch_ids) {
                        ...
                        if ($options['USE_HTTP_SUBREQUESTS']) {
                            $response = Http::sub_request(
                                '/ls_shop_process_catalog_rules_batch',
                                ['ids'=>$ids_str]
                            );
                            if (!$response) {
                                throw new ApplicationException('Unable to perform a sub-request.');
                            }
                ...
                $footprint = array(
                    PhprDateTime::gmtNow()->format(PhprDateTime::universalDateTimeFormat),
                    $user_id
                );
                ModuleParameters::set('shop', 'catalog_price_rules_footprint', $footprint);
            } else {
            ...
            throw new SystemException('Error applying catalog price rules: '.$ex->getMessage());
        }
    ...
    protected static function apply_rules_to_product_option_matrix($product)
    {
        ...
        try {
            ...
            $record_ids = DbHelper::scalarArray(
                'select id from shop_option_matrix_records where product_id=:product_id',
            ...
            do {
                ...
                if ($batch_ids) {
                    ...
                    if (!Phpr::$config->get('DISABLE_HTTP_SUBREQUESTS')) {
                        $response = Http::sub_request(
                            '/ls_shop_process_catalog_rules_om_batch',
                            ['ids'=>$ids_str]
                        );
                        if (!$response) {
                            throw new ApplicationException('Unable to perform a sub-request.');
    ...
    {
        $footprint_info = ModuleParameters::get('shop', 'catalog_price_rules_footprint');
        if (!$footprint_info) {
            return null;
        }
        ...
        $result = array();
        try {
            $time_obj = new PhprDateTime($time);
            $result['time'] = Date::userDate($time_obj);

            if ($user_id != 'system') {
                $user_obj = User::create()->find($user_id);
                if ($user_obj) {
    ...
    protected static function list_customer_groups()
    {
        ...
        return self::$customer_groups = CustomerGroup::create()->find_all()->as_mapped_array();
    }
    ...
    protected static function apply_rules_to_product_om_record($record)
    {
        ...
        $product = Product::find_by_id($record->product_id);
