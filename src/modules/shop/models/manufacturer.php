 ...
 * Objects of this class are accessible through the
 * {@link Product::manufacturer manufacturer} field of the {@link Product} class.
 *
 * You can use the following code for displaying a list of manufacturers on a page:
 * <pre>
 * <? $manufacturers = Manufacturer::create()->order('name')->find_all(); ?>
 * <ul>
 ...
 * </ul>
 * </pre>
 * Alternatively you can use the {@link action@shop:manufacturers} action for creating the
 * {@link https://lsdomainexpired.mjman.net/docs/manufacturer_details_page Manufacturer List Page}.
 *
 * @property int $id Specifies the manufacturer record identifier.
 ...
 * @property Country $country Specifies the manufacturer country.
 * @property CountryState $state Specifies the manufacturer country state.
 * @property bool $is_disabled Determines whether the manufacturer is disabled.
 * Disabled manufacturers are not displayed by the {@link action@shop:manufacturers}
 * and {@link action@shop:manufacturer} CMS actions.
 * @property \Db\DataCollection $logo A collection of the manufacturer logo images.
 * The collection always contains zero or one element - an object of {@link Db_File} class.
 * See {@link Manufacturer::logo_url() logo_url()} method.
 * @property string $url_name Specifies the manufacturer URL name.
 * @documentable
 * @see https://lsdomainexpired.mjman.net/docs/displaying_product_manufacturer_information
 ...
 * @see https://lsdomainexpired.mjman.net/docs/manufacturer_details_page Manufacturer details page
...
class Manufacturer extends ActiveRecord
{
    ...
    public $belongs_to = [
        'country'=> ['class_name'=>'Shop\Country', 'foreign_key'=>'country_id'],
        'state'=> ['class_name'=>'Shop\CountryState', 'foreign_key'=>'state_id']
    ];
    ...
    public $calculated_columns = [
        'product_num'=> [
            'sql'=>'select count(*) from shop_products where
				    shop_products.manufacturer_id=shop_manufacturers.id 
    ...
    public $has_many = [
        'logo'=> [
            ...
            'conditions'=>"master_object_class='Shop_Manufacturer' and field='logo'",
            'order'=>'id',
            'delete'=>true
        ],
        'products'=> [
    ...
    public function define_columns($context = null)
    {
        ...
        $this->define_column('url_name', 'URL Name')
            ...
            ->unique('The URL Name "%s" already in use. Please select another URL Name.')
            ->required('Please specify the manufacturer URL name.');
    ...
    public function define_form_fields($context = null)
    {
        ...
        $this->add_form_field('is_disabled')
            ->tab('Name and Description')
            ->comment('Disabled manufacturers are not displayed on the front-end store.');

        $field = $this->add_form_field('description')->tab('Name and Description')->renderAs(frm_html)->size('small');
        $editor_config = HtmlEditorConfig::get('shop', 'shop_manufacturers');
    ...
    protected function list_countries($key_value = -1)
    {
        ...
        $records = DbHelper::objectArray('select * from shop_countries order by name');
        $result = array();
    ...
    {
        $this->country_id = UserParameters::get('manufacturer_def_country');
        $this->state_id = UserParameters::get('manufacturer_def_state');
    }
    ...
    public function before_delete($id = null)
    {
        if ($this->product_num) {
            throw new ApplicationException(
                "The manufacturer cannot be deleted because {$this->product_num} products(s) refer to it."
            );
        }
    }
     ...
     * You can use exact integer values for the <em>$width</em> and <em>$height</em> parameters,
     * or word 'auto' for automatic image scaling.
     ...
     * Pass the FALSE value to the parameter to generate a PNG image.
     * The $params array allows to pass parameters to image processing modules
     ...
     * and thumbnail height is calculated by LSAPP to keep the original aspect ratio.
     ...
     * The result of this function is an object of the {@link Product} class. To obtain a collection of all
    ...
    public function list_products($options = array())
    {
        ...
        foreach ($sorting as &$sorting_column) {
            ...
            if (strpos($sorting_column, 'price') !== false) {
                $query = sprintf(Product::$price_sort_query, Controller::get_customer_group_id());
                $sorting_column = str_replace('price', $query, $sorting_column);
            }
     ...
     *   <li><em>front_end_sort_order</em> - sort the category list by the sort
     *      order defined in the Administration Area manually</li>
     ...
     * @documentable
     * @param array $options Specifies the method options.
     * @return \Db\DataCollection Returns a collection of {@link Category} objects.
     */
     ...
     * execution context string. To add new columns to the manufacturer model, call the
     ...
     * @param Manufacturer $manufacturer Specifies the manufacturer object.
     ...
     * To add new fields to the manufacturer form, call the
     ...
     * {@link shop:onExtendManufacturerForm} event.
