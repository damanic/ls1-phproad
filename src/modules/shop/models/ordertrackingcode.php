...
class OrderTrackingCode extends ActiveRecord
{
    public $table_name = 'shop_order_shipping_track_codes';

    public $belongs_to = [
        'code_shipping_method' => [
            'class_name' => 'Shop\ShippingOption',
            'foreign_key' => 'shipping_method_id'
        ],
        'shop_shipping_tracker_provider' => [
            'class_name' => 'Shop_ShippingTrackerProvider',
            'foreign_key' => 'shop_shipping_tracker_provider_id'
        ]
    ];
    ...
    public function define_columns($context = null)
    {
        ...
        $this->define_column('code', 'Tracking Code')
            ->validation()
            ->fn('trim')
            ->required('Please enter the shipping tracking number.')
            ->method('validateTrackingCode');
    }
     ...
     * @return null|ShippingTrackerInterface
     */
    ...
    protected function validateTrackingCode($name, $value)
    {
        ...
        $exists = DbHelper::scalar('select count(*) from shop_order_shipping_track_codes where code=:code and order_id=:order_id',
            $bind);
        if ($exists) {
            $this->validation->setError("The tracking code " . $value . " has already been added to order ID  " . $this->order_id,
                $name, true);
            $pass = false;
        }
        return $pass;
    ...
    {
        if (!$this->order_id) {
            throw new ApplicationException('Tracking codes must be assigned to an order ID');
        }
    }
