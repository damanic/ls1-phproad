...
class PriceRuleBase extends ActiveRecord
{
    protected $action_obj = null;
    public $action_class_name = null;
    ...
    public function get_action_obj()
    {
        ...
        if ($this->action_obj != null && get_class_id($this->action_obj) == get_class_id($class_name)) {
            return $this->action_obj;
        }

        if (!class_exists($class_name)) {
            throw new SystemException('Price rule action class "'.$class_name.'" not found.');
        }
    ...
    protected function load_xml_data()
    {
        ...
        if (strlen($this->parameters_serialized) && Controller::get_instance()) {
            try {
    ...
    public function set_orders($ids, $orders)
    {
        ...
        foreach ($ids as $index => $id) {
            ...
            $order = $orders[$index];
            DbHelper::query('update `'.$table_name.'` set sort_order=:order where id=:id', array(
                'order'=>$order,
                'id'=>$id
            ));
        }
    ...
    /**
     * @deprecated Use {@link PriceRuleBase::is_active_now()} method.
     */
     ...
     * @documentable
     * @param PhprDateTime $user_datetime Optional, the users datetime to consider, otherwise system time is used.
    ...
    public function is_active_now($user_datetime = null)
    {
        ...
        $now = PhprDateTime::now(); //system time

        if ($user_datetime && is_a($user_datetime, 'Phpr\DateTime')) {
