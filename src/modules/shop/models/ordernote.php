...
class OrderNote extends ActiveRecord
{
    ...
    {
        $this->define_column('note', 'Note text')
            ->validation()
    ...
    public function after_create()
    {
        ...
        $users = User::create()->where('id IN (?)', array($this->notification_users))->find_all_proxy();
        if ($users) {
    ...
    public function notify_users($users)
    {
        ...
        $template = EmailTemplate::create()->find_by_code('shop:order_note_internal');
        if (!$template) {
            return;
        }
    ...
    protected function set_email_variables($message)
    {
        ...
        $url =  url('shop/orders/preview/' . $this->order_id);
        $preview_url = Phpr::$request->getRootUrl() . $url . '#note_' . $this->id;
