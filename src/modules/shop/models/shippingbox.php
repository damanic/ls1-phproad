...
class ShippingBox extends ActiveRecord
{
    ...
    public function define_columns($context = null)
    {
        ...
        $this->define_column('width', 'Width')
            ->validation()
            ->fn('trim')
            ->required("Please specify the box width");

        $this->define_column('length', 'Length')
            ->validation()
            ->fn('trim')
            ->required("Please specify the box length");

        $this->define_column('depth', 'Depth')
            ->validation()
            ->fn('trim')
            ->required("Please specify the box depth");

        $this->define_column('empty_weight', 'Empty Weight');

        $this->define_column('max_weight', 'Max Weight')
            ->validation()->fn('trim')
            ->required("Please specify the maximum weight this box can carry");
