...
/**
 * Represents {@link https://lsdomainexpired.mjman.net/docs/understanding_option_matrix/ Option Matrix} record.
 * Usually you don't need to access objects of this class directly.
 * @documentable
 * @property int $id Specifies the record database identifier.
 * @property \Db\DataCollection $images A collection of images assigned to the category.
 * Each element in the collection is an object of the {@link File} class. You can use this property directly to
 * output category images, or use the {@link Category::image_url() image_url()} method. Not proxiable.
 * @property bool $disabled Determines whether the record is disabled.
 ...
 * @property bool $on_sale Determines whether the product is on sale.
 * @property string $sale_price_or_discount. Specifies the sale price or discount in the following format: 10, -10, 10%.
 * @property int $in_stock Specifies the number of items in stock.
 * @property PhprDateTime $expected_availability_date Specifies the expected availability date.
 ...
 * @author LSAPP - MJMAN
 */
class OptionMatrixRecord extends ActiveRecord implements MemoryCacheable
{
    protected static $option_matrix_record_cache = [];

    public $table_name = 'shop_option_matrix_records';

    protected static $record_cache = [];
    protected static $supported_field_cache = [];
    protected static $option_graph = [];
    protected static $cached_option_values = [];

    public $has_many = [
        'images' => [
            'class_name' => 'Db\File',
            'foreign_key' => 'master_object_id',
            'conditions' => "master_object_class='Shop_OptionMatrixRecord' and field='images'",
            'order' => 'sort_order, id',
            'delete' => true
        ],
        'record_options' => [
            'class_name' => 'Shop\OptionMatrixOption',
            'delete' => true,
            'order' => 'shop_option_matrix_options.id',
            'foreign_key' => 'matrix_record_id'
        ]
    ];
    ...
    public function define_form_fields($context = null)
    {
        ...
        $columns['disabled'] = [
            ...
            'type' => 'checkbox',
            'width' => 85,
            'checked_class' => 'status-disabled',
            'header_control' => true,
            'column_group' => 'Product'
        ];
        $columns['sku'] = [
            'title' => 'SKU',
            'type' => 'text',
        ...
        $columns['images'] = [
            'title' => 'Images',
            'type' => 'popup',
            'editor_class' => 'Db\GridImagesEditor',
            'images_field' => 'images',
            'width' => 100,
            'default_text' => '0 images',
            'column_group' => 'Product'
        ];
        ...
        $columns['base_price'] = [
            ...
            'type' => 'text',
            'align' => 'right',
            'editor_class' => 'Shop\GridTierPriceEditor',
            'width' => 100,
            'column_group' => 'Pricing'
        ];
        $columns['cost'] = [
            'title' => 'Cost',
            'type' => 'text',
        ...
        $columns['sale_price_or_discount'] = [
            'title' => 'Sale Price or Discount',
            'type' => 'text',
            'align' => 'right',
            'width' => 150,
            'validation_type' => 'discount',
            'column_group' => 'Pricing'
        ];
        ...
        $columns['in_stock'] = [
            ...
            'width' => 80,
            'align' => 'right',
            'column_group' => 'Inventory Tracking'
        ];
        $columns['expected_availability_date'] = [
            'title' => 'Expected date',
            'align' => 'right',
            'type' => 'text',
            'width' => 95,
        ...
        $columns['allow_pre_order'] = [
            'title' => 'Allow pre-order',
            'type' => 'checkbox',
            'width' => 85,
            'header_control' => true,
            'column_group' => 'Inventory Tracking'
    ...
    public function reinitialize($record_id, $skip_fields)
    {
        ...
        if ($record_id === null) {
            ...
            $row_data = DbHelper::queryArray('select * from shop_option_matrix_records where id=?', $record_id);
    ...
    private function eval_tier_price($product, $customer_group_id, $quantity)
    {
        ...
        $price = TierPrice::eval_tier_price(
            $this->tier_price_compiled,
            $customer_group_id,
            $quantity,
            $product->name,
            $this->base_price,
            $product->tier_price_compiled
        );
    ...
    public function list_group_price_tiers($product, $group_id)
    {
        ...
        return TierPrice::list_group_price_tiers(
            $this->tier_price_compiled,
            $group_id,
            $product->name,
            $base_price,
    ...
    public function set_compiled_price_rules($price_rules, $rule_map)
    {
        ...
        DbHelper::query(
            'update shop_option_matrix_records 
     ...
     * is used internally.
     */
    public static function get_sale_price_static(
        $product,
        $test_record,
        $data,
    ...
    public static function copy_records_to_product($src_product, $dest_product)
    {
        $options = DbHelper::objectArray(
            'select id, name from shop_custom_attributes where product_id=:product_id',
        ...
        $records = DbHelper::queryArray(
            'select * from shop_option_matrix_records where product_id=:product_id',
        ...
        foreach ($records as $record) {
            ...
            DbHelper::query(
                "insert into shop_option_matrix_records($record_fields_insert_str) values ($record_fields_values_str)",
                $record
            );
            $new_record_id = DbHelper::driver()->get_last_insert_id();

            /*
                 * Copy Option Matrix option records
             */

            $record_options = DbHelper::queryArray(
                '
					select 
            ...
            foreach ($record_options as $record_option) {
                ...
                DbHelper::query(
                    "insert into shop_option_matrix_options($record_option_fields_insert_str) 
                          values ($record_option_fields_values_str)",
                    $record_option
                );
            }
            ...
            $files_ids = DbHelper::scalarArray('
					select 
						id 
					from 
						db_files 
    ...
    public function set_options_hash($hash)
    {
        ...
        DbHelper::query(
            'update shop_option_matrix_records set options_hash=:hash where id=:id',
            ['id' => $this->id, 'hash' => $hash]
        );
    }
     ...
     * @param mixed $product Product object (Product) or product identifier.
     * @param bool $option_keys Indicates wether array keys in the $options parameter
     *  represent option keys (md5(name)) rather than option names.
     ...
     * @documentable
     * @param mixed $product Product object ({@link Product}) or product identifier.
     * @param int $quantity Quantity for the tier price calculations.
     * @param int $customer_group_id {@link CustomerGroup Customer group} identifier.
     * @param bool $no_tax Forces the function to not include tax into the result even if the
     * {@link https://lsdomainexpired.mjman.net/docs/configuring_lemonstand_for_tax_inclusive_environments/
     * Display catalog/cart prices including tax} option is enabled.
    ...
    public function get_price($product, $quantity = 1, $customer_group_id = null, $no_tax = false)
    {
        ...
        foreach ($override_product as $use_product) {
            if (is_a($use_product, 'Shop\Product')) {
                $product = $use_product;
                break;
            }
        }
        ...
        return TaxClass::get_total_tax($product->tax_class_id, $price) + $price;
    }
    ...
    public function get_sale_price($product, $quantity = 1, $customer_group_id = null, $no_tax = false)
    {
        ...
        $prices = Backend::$events->fireEvent(
            'shop:onOptionMatrixGetSalePrice',
            $this,
            $product,
            $quantity,
            $customer_group_id
        ...
        if (!$price && ($this->on_sale && strlen($this->sale_price_or_discount))) {
            ...
            $price = round(Product::get_set_sale_price($price, $this->sale_price_or_discount), 2);
        }

        if ($price) {
            return $no_tax ? $price : TaxClass::apply_tax_conditional($product->tax_class_id, $price);
        }
        ...
        if (!$price_rules) {
        ...
        } elseif (array_key_exists($customer_group_id, $price_rules)) {
            ...
            foreach ($price_tiers as $tier_quantity => $price) {
                if ($tier_quantity <= $quantity) {
                    $price = round($price, 2);
                    $price = $no_tax ? $price : TaxClass::apply_tax_conditional($product->tax_class_id, $price);
                    $prices = Backend::$events->fireEvent(
                        'shop:onOptionMatrixReturnCompiledPrice',
                        $this,
                        $quantity,
                        $customer_group_id,
                        $price
     ...
     * or if the product is on sale ('On Sale' checkbox).
     * @documentable
     * @param mixed $product Product object (Product) or product identifier.
     * @return bool Returns TRUE if the product is on sale.
     */
     ...
     * @documentable
     * @param mixed $product Product object ({@link Product}) or product identifier.
     * @return bool Returns TRUE if the product is out of stock.
     */
    public function is_out_of_stock($product)
    {
        ...
        $in_stock = OptionMatrix::get_property($this, 'in_stock', $product);
        ...
        if ($product->stock_alert_threshold !== null) {
            ...
            $dst = $config->default_out_of_stock_threshold;
            $os_threshold = is_numeric($dst) ? $dst : 0;
        }
    ...
    /**
     * Returns TRUE if inventory tracking for the product is enabled
     * and the product has reached the low stock threshold.
     * @documentable
     * @param mixed $product Product object ({@link Product}) or product identifier.
     * @return bool Returns TRUE if the product is out of stock.
     */
    public function is_low_stock($product)
    {
        ...
        $config = ConfigurationRecord::get();
        $lst = $config->default_low_stock_threshold;
        $low_stock_threshold = is_numeric($lst) ? $lst : 0;
    ...
    public function get_volume($product)
    {
        ...
        $width = OptionMatrix::get_property($this, 'width', $product);
        $height = OptionMatrix::get_property($this, 'height', $product);
        $depth = OptionMatrix::get_property($this, 'depth', $product);
     ...
     * @documentable
     * @param bool $option_keys Specifies whether options should be presented with option keys instead of names.
    ...
    public function before_delete($id = null)
    {
        ...
        $count = DbHelper::scalar(
            'select count(*) from shop_order_items, shop_orders 
                 where option_matrix_record_id is not null 
                   and option_matrix_record_id=:id 
     ...
     * @param CustomAttribute $option Product option object.
    ...
    protected static function get_option_graph($product)
    {
        ...
        $records = DbHelper::scalarArray("
				select 
					(select group_concat(concat('--', option_id, ':', option_value, '--') separator '') 
					 from shop_option_matrix_options where  matrix_record_id=shop_option_matrix_records.id) as options
				from
					shop_option_matrix_records
				where 
        ...
        if ($product_update_time) {
            ...
            $product_update_time = is_a(
                $product_update_time,
                'Phpr\DateTime'
            ) ? $product_update_time : new PhprDateTime($product_update_time);
    ...
    protected static function build_graph_level(&$records, $product, &$options, $level = 0, $filters = array())
    {
        ...
        foreach ($option_values as $option_value) {
            foreach ($records as $record_info) {
                ...
                if ($record_found) {
                    $result['options'][$option_value] = self::build_graph_level(
                        $records,
                        $product,
                        $options,
                        $level + 1,
     ...
     *   <li><em>type</em> - defines the column type, required. Supported values are
     *      <em>text</em>, <em>dropdown</em>, <em>checkbox</em>, <em>popup</em>
     *      (applicable for image columns only).
     *   </li>
     ...
     *   <li><em>editor_class</em> - class name for a popup editor. Required value for date columns is
     *      <em>Db_GridDateEditor</em>. It is the only supported popup editor for API fields.</li>
     *   <li><em>option_keys</em> - defines option keys for drop-down menus. Required if the column type is
     *      <em>dropdown</em>.</li>
     *   <li><em>option_values</em> - defines option values for drop-down menus. Required if the column type is
     *      <em>dropdown</em>.</li>
     *   <li><em>editor_class</em> - specifies a popup editor class. Required if the column type is <em>popup</em>.
     *      The only supported value is <em>Db_GridImagesEditor</em>.</li>
     *   <li><em>images_field</em> - specifies an images column name. Required if the editor_class is
     *      <em>Db_GridImagesEditor</em>.</li>
     * </ul>
     * API columns are supported by CSV operations and can be accessed with {@link Product::om()},
     * {@link OrderItem::om()} and {@link CartItem::om()} methods.
     *
     * In order to add an images field you should first define an images relation with the
     * {@link shop:onExtendOptionMatrixRecordModel} event. The following example
     ...
     *     'class_name'=>'File',
     *     'foreign_key'=>'master_object_id',
     *     'conditions'=>"master_object_class='Sho_OptionMatrixRecord' and field='x_extra_images'",
     *     'order'=>'sort_order, id',
     *     'delete' => true)
     *   );
     *
     *   $model->define_multi_relation_column(
     *      'x_extra_images',
     *      'x_extra_images',
     *      'Extra images',
     *      $front_end ? null : '@name');
     ...
     * @param OptionMatrixRecord $model Option Matrix record.
     ...
     *     'class_name'=>'File',
     *     'foreign_key'=>'master_object_id',
     *     'conditions'=>"master_object_class='OptionMatrixRecord' and field='x_extra_images'",
     *     'order'=>'sort_order, id',
     *     'delete' => true)
     *   );
     *
     ...
     * @param OptionMatrixRecord $model Specifies the loaded Option Matrix model
     * @param array $options Specifies product option values
     * @param mixed $product Product object (Product) or product identifier.
     * @param bool $option_keys Indicates whether array keys in the $options parameter
     *  represent option keys (md5(name)) rather than option names.
     * Otherwise $options keys are considered to be plain option name.
     * @author LSAPP - MJMAN
     * @see shop:onExtendOptionMatrix
     ...
     *
     * WARNING:
     * loading a Product object every time get_price() is called can be resource intensive
     ...
     * @param OptionMatrixRecord $model Specifies the loaded Option Matrix model
     * @param Product Specifies parent product
