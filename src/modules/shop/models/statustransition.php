...
class StatusTransition extends ActiveRecord
{
    ...
    public $calculated_columns = [
        'to_color' => [
            ...
            'join' => ['shop_order_statuses' => 'shop_order_statuses.id=to_state_id']
        ],
        'to_name' => [
            'sql' => "shop_order_statuses.name"
        ],
        'role_name' => [
            'sql' => "select name from shop_roles where shop_roles.id=role_id"
        ]
    ];
    ...
    {
        $this->define_column('to_state_id', 'Destination Status')
            ->validation()
            ->required('Please select a destination status.');

        $this->define_column('role_id', 'Role')
            ->validation()
    ...
    {
        $this->add_form_field('to_state_id')
            ->comment('Please select status an order can be transferred from current status.', 'above')
            ->renderAs(frm_dropdown)
            ->emptyOption('<please select>');

        $this->add_form_field('role_id')
            ->comment(
                'Please select a role of users who can transfer orders from the
                 current status to the status you specified above.',
                'above'
            )
            ->renderAs(frm_dropdown)
            ->emptyOption('<please select>');
    ...
    public function get_to_state_id_options()
    {
        ...
        $records = OrderStatus::create()->order('id');
    ...
    {
        return Role::create()->order('id')->find_all()->as_array('name', 'id');
    }
    ...
    public static function userTransitionsAllowed()
    {
        ...
        return DbHelper::scalar('select count(*) from shop_status_transitions where role_id=?', $role->id);
    }
}
