...
use Phpr\Html;
    ...
    /**
     * Represents a {@link Product product} property.
     * @property int $id Specifies the property record identifier.
...
class ProductProperty extends ActiveRecord
{
    ...
    protected $propertySetProperty = null;
    ...
    {
        $this->define_column('name', 'Property Name')
            ->validation()
            ->fn('trim')
            ->required();
        ...
        $this->define_column('api_code', 'API Code')
            ->invisible();
    ...
    public function define_form_fields($context = null)
    {
        ...
        $this->add_form_field('value_pickup')
            ->renderAs(frm_dropdown)
            ->emptyOption('<known property values>')
            ->comment($value_pickup_comment, 'above')
            ->cssClassName('relative');
    ...
    {
        $dep_result = Backend::$events->fireEvent(
            'shop:onGetAttributeFieldOptions',
            $db_name,
            $current_key_value
        ); //@deprecated
        
        $result = Backend::$events->fireEvent(
            'shop:onGetProductPropertyFieldOptions',
            $db_name,
            $current_key_value
        );
    ...
    public function get_value_pickup_options($key = -1)
    {
        ...
        if (empty($values)) {
            ...
            $values = DbHelper::objectArray(
                'select distinct id, value from shop_product_properties 
                      where lower(name)=:name group by value order by value',
                ['name' => $name]
            );
        }

        foreach ($values as $key => $value) {
            $value = is_object($value) ? Html::strTrim(str_replace("\n", " ", $value->value), 40) : $value;
            $result[$value] = $value;
        }
    ...
    {
        if ($this->propertySetProperty) {
            return $this->propertySetProperty;
        }
        if ($this->property_set_property_id) {
            return $this->propertySetProperty = PropertySetProperty::create()->find($this->property_set_property_id);
        }
        return null;
    }
    ...
    {
        return DbHelper::scalarArray('select distinct name from shop_product_properties');
    }
    ...
    public function load_value($property_id)
    {
        ...
        $this->value = DbHelper::scalar(
            'select value from shop_product_properties where id=:id',
            ['id'=>$property_id]
        );
    }
    ...
    public static function list_unique_values($name)
    {
        $values = DbHelper::scalarArray(
            'select distinct value from shop_product_properties where name=:name',
            ['name'=>$name]
        );
        $result = array();
     ...
     * To add new columns to the property model, call the
     ...
     * @param ProductProperty $property Specifies the product property object.
    ...
    /**
     * Allows to add new fields to the Add/Edit property section of the
     * Add/Edit Product form in the Administration Area.
     * Usually this event is used together with the {@link shop:onExtendProductPropertyModel} event.
     * To add new fields to the property form, call the
     ...
     * {@link shop:onExtendProductPropertyForm} event.
