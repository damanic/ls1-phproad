...
/**
 * Represents a {@link Product product} option.
 * Objects of this class are available through the {@link Product::$options} property.
 * @property int $id Specifies the option record identifier.
 * @documentable
 ...
 * @see https://lsdomainexpired.mjman.net/docs/displaying_product_options Displaying product options
 * @package shop.models
 * @author LSAPP - MJMAN
 */
class CustomAttribute extends ActiveRecord
{
    ...
    public function define_form_fields($context = null)
    {
        $this->add_form_field('name')
            ->comment(
                'Specify the option name, e.g. "Colors" to display near the attribute drop-down menu.',
                'above'
            );

        $this->add_form_field('attribute_values')
            ->renderAs(frm_textarea)
            ->comment('Specify option values, e.g. "Red, Green, Blue" - one value per line.', 'above');
    ...
    {
        if ($limit_to_om && $this->parent_product) {
            $configRecord = ConfigurationRecord::get();
            if ($configRecord->strict_option_values && $this->parent_product->has_om_records()) {
    ...
    {
        return OptionMatrixRecord::get_available_option_values(
            $this->parent_product,
            $this,
            ProductHelper::get_default_options($this->parent_product)
        );
    }
    ...
    {
        return DbHelper::scalarArray('select distinct name from shop_custom_attributes');
    }
    ...
    {
        $values = DbHelper::scalarArray(
            'select distinct attribute_values from shop_custom_attributes where name=:name',
            ['name'=>$name]
        );
        $result = array();
        foreach ($values as $value) {
     ...
     * @return bool Returns TRUE if a specified value exists in the option
     */
    ...
    public static function get_name_by_id($id)
    {
        if (!array_key_exists($id, self::$id_name_cache)) {
            $name =  DbHelper::scalar(
                'select name from shop_custom_attributes where id=:id',
                array('id'=>$id)
            );
            self::$id_name_cache[$id] = $name;
        }
    ...
    public static function cleanup_option_data($option_id)
    {
        ...
        $record_ids = DbHelper::scalarArray(
            'select ol.matrix_record_id as id from shop_option_matrix_options ol
				where ol.option_id = :option_id
				and not exists (
				    select * from shop_option_matrix_options il 
				    where il.matrix_record_id=ol.matrix_record_id 
        ...
        foreach ($record_ids as $id) {
            DbHelper::query('delete from shop_option_matrix_records where id=:id', array('id'=>$id));
        }

        DbHelper::query('delete from shop_option_matrix_options where option_id=:option_id', $bind);
    }
     ...
     * The event handler should accept a single parameter - the product option object.
     * To add new columns to the product option model, call the
     * {@link Db_ActiveRecord::define_column() define_column()} method of the product option object.
     * Before you add new columns to the model, you should add them to the database
     * (the <em>shop_custom_attributes</em> table).
     * <pre>
     ...
     * @param CustomAttribute $option Specifies the product option object.
     */
     ...
     * {@link shop:onExtendOptionForm} event.
