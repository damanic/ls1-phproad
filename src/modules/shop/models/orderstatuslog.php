...
class OrderStatusLog extends ActiveRecord
{
    ...
    public $calculated_columns = [
        ...
        'status_color'=> [
            'sql'=>'shop_order_statuses.color'
        ]
    ];

    public $belongs_to = [
        'status'=> [
            'class_name'=>'Shop\OrderStatus',
            'foreign_key'=>'status_id'
        ]
    ];
        
    public $custom_columns = ['send_notifications'=>db_bool];
        
    public $role_id = null;
    public $send_notifications = true;
        
    public $api_added_columns = [];
     ...
     * notifications are allowed by the
     * {@link https://lsdomainexpired.mjman.net/docs/configuring_the_order_route_and_user_roles order route}.
     *
     ...
     * @param int $status_id Specifies a new order status identifier.
     * @param Order $order An order object to change status for.
     * @param string|null $comment Specifies an optional status transition comment.
     * @param bool $send_notifications Determines whether notifications should be sent to the customer and LSAPP users.
     * @param array $api_data A list of API field values
     * @return bool Returns TRUE if the status has been successfully changed. Returns FALSE otherwise.
     */
    public static function create_record(
        ...
        $return = Backend::$events->fireEvent(
            'shop:onOrderBeforeStatusChanged',
            $order,
            $status_id,
            $prev_status,
            $comment,
        ...
        if (!OrderStatus::order_meets_status_requirements($status_id, $order)) {
            throw new ApplicationException('The order does not meet the requirements for this status transition.');
        }
        ...
        DbHelper::query(
            'update shop_orders set status_id=:status_id, status_update_datetime=:datetime where id=:id',
            [
                'status_id'=>$status_id,
                'datetime'=>Date::userDate(PhprDateTime::now()),
                'id'=>$order->id
            ]
        );
            
        $paid_status = OrderStatus::get_status_paid();
            
        if ($status_id == $paid_status->id) {
            DbHelper::query('update shop_orders set payment_processed=:payment_processed where id=:id', [
                'payment_processed'=>PhprDateTime::now(),
                'id'=>$order->id
            ]);
        }
        ...
        if ($status && $status->order_lock_action) {
            if ($order->is_order_locked() && $status->unlocks_order()) {
                ...
                OrderLockLog::add_log($order, 'Unlocked by status change');
            } elseif (!$order->is_order_locked() && $status->locks_order()) {
                $order->lock_order();
                $order->save();
                $order->status_id = $status->id;
                OrderLockLog::add_log($order, 'Locked by status change');
            }
        }
    ...
    public function get_status_options()
    {
        ...
        if (!count($this->status_ids)) {
            ...
            $end_transitions = StatusTransition::listAvailableTransitionsMulti($this->role_id, $this->status_ids);
    ...
    public function define_columns($context = null)
    {
        $this->define_relation_column('status', 'status', 'Status ', db_varchar, '@name')
            ->validation()
            ->required('Please select new order status');
    ...
    public function define_form_fields($context = null)
    {
        ...
        $statusFields[] = $this->add_form_field('status')
            ->referenceSort('name')
            ->emptyOption('<please select status>');

        $statusFields[] = $this->add_form_field('comment')
            ->comment('If configured, the comment may appear in customer email notifications.', 'above', true)
            ->commentTooltip(
                'Use the {order_status_comment} variable in email templates
                <br/>if you want to add order status comments to customer notifications.'
            );

        $statusFields[] = $this->add_form_field('send_notifications')
            ->comment(
                'Send notifications to customer(s) and LSAPP users in accordance with the order route settings.',
                'above'
            );
    ...
    public static function order_has_status_code($order, $status_codes)
    {
        ...
        foreach ($status_codes as $code) {
            $status = OrderStatus::get_by_code($code);
            if ($status) {
        ...
        $order_id = DbHelper::scalar(
            "
			 SELECT order_id
			 FROM shop_order_status_log_records
			 WHERE status_id IN (?)
			 AND order_id = '".$order->id."' LIMIT 1",
            [$status_ids]
        );
    ...
    {
        $this->send_notifications = UserParameters::get('orders_email_on_status_change', null, '1');
    }
     ...
     * The event allows to cancel the status update.
     * To cancel the update the handler should return FALSE. Event handler example:
     * <pre>
     ...
     * {
     *   $status = OrderStatus::create()->find($new_status_id);
     *
     *   //for orders over $250, set the status to "New big order" instead of "New"
     *   if($status && $status->code == 'new' && $order->subtotal >= 250)
     *   {
     *     $new_status = OrderStatus::create()->find_by_code('new_big');
     *     if($new_status)
     *     {
     *       OrderStatusLog::create_record($new_status->id, $order, $comments, $send_notifications);
     *       return false;
     *     }
     *   }
     * }
     ...
     * @see OrderStatusLog::create_record()
     * @param Order $order Specifies the order object.
     * @param int $status_id Specifies a new order status identifier.
     * @param int $prev_status_id Specifies a previous order status identifier.
     * @param string $comment Specifies an optional status transition comment.
     * @param bool $send_notifications Determines whether notifications should be sent to the customer and LSAPP users.
     * @return bool Returns FALSE if the status transition should be cancelled.
     */
    private function event_onOrderBeforeStatusChanged(
        $order,
        $status_id,
        $prev_status_id,
        $comment,
     ...
     * @see OrderStatusLog::create_record()
     * @param Order $order Specifies the order object.
     * @param OrderStatus $status Specifies a new order status object.
     * @param int $prev_status_id Specifies a previous order status identifier.
     */
    ...
    /**
     * Triggered before LSAPP updates inventory for products of a specific order.
     ...
     *   ...
     *
     *   // Return TRUE to suppress the default LSAPP inventory update
     *   return true;
     * }
     * </pre>
     ...
     * @param Order $order Specifies the order object.
     * @param OrderStatus $status Specifies a new order status object.
     * @return bool Returns TRUE if the inventory update should be cancelled.
     */
