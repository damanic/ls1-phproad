<?
	...
	class Shop_ProductCsvImportModel extends Backend_CsvImportModel
	{
		...
		public function define_columns($context = null)
		{
			...
			$this->define_column('import_product_images', 'I have a ZIP archive(s) or a directory on the server containing product images and I want LSAPP to import them');
			...
			$this->define_column('import_product_files', 'I have a directory on the server containing downloadable product files and I want LSAPP to import them');
			...
			$this->define_column('auto_create_product_groups', 'I want LSAPP to create product groups specified in the CSV file');
		...
		public function define_form_fields($context = null)
		{
			...
			$this->add_form_field('auto_product_types')->comment('You need to match the LSAPP product <strong>Product Type</strong> field to a CSV file column in order to use this feature. Otherwise please select a product type for all imported products in the list below. This field is required for new products.', 'above', true);
			...
			$this->add_form_field('auto_create_categories')->comment('You need to match the LSAPP product <strong>Categories</strong> field to a CSV file column in order to use this feature. Otherwise please select categories for all imported products in the list below. This field is required for new products.', 'above', true);
			...
			$this->add_form_field('import_product_files')->comment('You need to match the LSAPP product <strong>Files</strong> field to a CSV file column in order to use this feature. The files field in the CSV file should contain file names separated with comma.', 'above', true);
			$this->add_form_field('files_directory_path')->comment('Use this field to specify a directory on the server, which contains product files. Please use an absolute path, for example /users/me/import_files. The absolute path of your LSAPP directory is '.PATH_APP, 'above', true)->renderAs(frm_text)->cssClassName('expandable');

			$this->add_form_field('auto_tax_classes')->comment('You need to match the LSAPP product <strong>Tax Class</strong> field to a CSV file column in order to use this feature. Otherwise please select a tax class for all imported products in the field below. This field is required for new products.', 'above', true);
			$this->add_form_field('tax_class')->comment('Please select a tax class for imported products', 'above')->cssClassName('expandable');

			$this->add_form_field('auto_manufacturers')->comment('You need to match the LSAPP product <strong>Manufacturer</strong> field to a CSV file column in order to use this feature. Otherwise please select a manufacturer for all imported products in the field below. This field is optional.', 'above', true);
			...
			$this->add_form_field('update_existing_sku')->comment('If you leave this checkbox unchecked, LSAPP will skip products with existing SKU values (Option Matrix products are identified by option values). Otherwise existing products will be updated using information from the CSV file and other parameters specified on this page.', 'above', true);
			$this->add_form_field('descriptions_html')->comment('If you leave this checkbox unchecked, LSAPP will treat data in the Long Description field as plain text.', 'above', true);
			$this->add_form_field('short_descriptions_html')->comment('If you leave this checkbox unchecked, LSAPP will treat data in the Short Description field as plain text.', 'above', true);
			
			$this->add_form_field('auto_create_product_groups')->comment('You need to match the LSAPP product <strong>Product Groups</strong> field to a CSV file column in order to use this feature. Otherwise please select product groups for all imported products in the list below if you would like to add the imported products to a product group.', 'above', true);	
		...
		public function import_csv_data($data_model, $session_key, $column_map, $import_manager, $delimeter, $first_row_titles)
		{
			...
			if(!$config_object->update_existing_sku)
			{
				if (!$import_manager->csvImportDbColumnPresented($column_map, 'name'))
					throw new Phpr_ApplicationException('Please specify a matching column for the Name field or select the "I want LSAPP to update products with existing SKU" checkbox if updating products.');
				if (!$import_manager->csvImportDbColumnPresented($column_map, 'price'))
					throw new Phpr_ApplicationException('Please specify a matching column for the Price field or select the "I want LSAPP to update products with existing SKU" checkbox if updating products.');
				
				if (!$config_object->auto_product_types && !$config_object->product_type)
					throw new Phpr_ApplicationException('Please select a Product type or select the "I want LSAPP to update products with existing SKU" checkbox if updating products.');
				elseif ($config_object->auto_product_types && !$import_manager->csvImportDbColumnPresented($column_map, 'product_type'))
					throw new Phpr_ApplicationException('Please specify a matching column for the Product type field, or uncheck the "I want LSAPP to apply product types specified in the CSV file" checkbox.');
				...
				if ($config_object->auto_tax_classes && !$import_manager->csvImportDbColumnPresented($column_map, 'tax_class'))
				{
					...
					throw new Phpr_ApplicationException('If importing new products, please specify a matching column for the Tax Class product field or uncheck the "I want LSAPP to create tax classes specified in the CSV file" checkbox. 
						If updating products, please select the "I want LSAPP to update products with existing SKU" checkbox.');
				}
			...
			if ($config_object->auto_manufacturers)
			{
				if (!$import_manager->csvImportDbColumnPresented($column_map, 'manufacturer_link'))
					throw new Phpr_ApplicationException('Please specify a matching column for the Manufacturer product field, or uncheck the "I want LSAPP to create manufacturers specified in the CSV file" checkbox.');
			}
			
			if ($config_object->auto_create_product_groups)
			{
				if (!$import_manager->csvImportDbColumnPresented($column_map, 'product_groups'))
					throw new Phpr_ApplicationException('Please specify a matching column for the Product Groups product field, or uncheck the "I want LSAPP to create product groups specified in the CSV file" checkbox.');
			}
			...
			try
			{
				...
				if ($config_object->import_product_images)
				{
					...
					if (!$image_files->count && !strlen($images_server_dir))
						throw new Phpr_ApplicationException('Please upload an image archive or specify the images directory on the server, or uncheck the option "I have a ZIP archive(s) or a directory on the server containing product images and I want LSAPP to import them".');
				...
				if ($config_object->import_product_files)
				{
					...
					if (!strlen($files_server_dir))
						throw new Phpr_ApplicationException('Please specify the files directory on the server, or uncheck the option "I have a directory on the server containing downloadable product files and I want LSAPP to import them".');
