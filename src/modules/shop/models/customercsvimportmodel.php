...
class CustomerCsvImportModel extends CsvImportModel
{
    ...
    public $has_many = [
        'csv_file'=> [
            ...
            'conditions'=>"master_object_class='Shop_CustomerCsvImportModel'",
            'order'=>'id',
            'delete'=>true
        ],
        'config_import'=> [
    ...
    public $belongs_to = [
        ...
        'default_country'=> ['class_name'=>'Shop\Country', 'foreign_key'=>'billing_country_id']
    ];
    ...
    public function define_columns($context = null)
    {
        ...
        $this->define_column('update_existing_emails', 'I want LSAPP to update customers with existing emails');
        $this->define_column('auto_create_groups', 'I want LSAPP to create customer groups specified in the CSV file');
    ...
    public function define_form_fields($context = null)
    {
        ...
        $this->add_form_field('update_existing_emails')
            ->comment(
                'If you leave this checkbox unchecked, LSAPP will skip customers with existing emails. 
                Otherwise existing customers will be updated using information from the CSV file 
        ...
        $this->add_form_field('default_country', 'left')
            ->comment(
                'This country will be used for customers with no country specified in the CSV file.', 
                'above', 
                true
            )->emptyOption('<please select>');

        $this->add_form_field('auto_create_groups')
            ->comment(
                'You need to match the LSAPP customer <strong>Customer Group</strong> field 
                      to a CSV file column in order to use this feature. 
                      Otherwise please select a customer group for all imported customers in the list below.', 
                'above', 
                true
            );
        
        $this->add_form_field('group')
            ->comment('Please select a customer group for imported products', 'above')
        ...
        if ($this->auto_create_groups) {
            if (!$import_manager->csvImportDbColumnPresented($column_map, 'group')) {
                throw new ApplicationException(
                    'Please specify a matching column for the Customer Group customer field, or uncheck the 
                    "I want LSAPP to create customer groups specified in the CSV file" checkbox.'
                );
            }
        } else {
            if (!$this->group) {
                throw new ApplicationException('Please select a customer group.');
            }
        }
            
        if (!$this->default_country) {
            throw new ApplicationException('Please select a default country.');
        }
                ...
                foreach ($column_map as $column_index => $db_names) {
                    ...
                    foreach ($db_names as $db_name) {
                        ...
                        if ($column_definitions[$db_name]->type == db_bool) {
                            $bind[$db_name] = Boolean::fromString($column_value) ? '1' : '0';
                        } else {
                            if ($db_name == 'group') {
    ...
    protected function find_existing_customer_id($email)
    {
        ...
        return DbHelper::scalar(
            'select id from shop_customers where lower(email)=:email', 
    ...
    protected function create_group($value)
    {
        if (!$this->existing_groups) {
            $existing_groups = DbHelper::objectArray('select id, name from shop_customer_groups');
    ...
    protected function validate_fields(&$bind, $existing_customer = false)
    {
        if (!array_key_exists('first_name', $bind) || !strlen($bind['first_name'])) {
            throw new ApplicationException('Customer first name is not specified');
        }
                
        if (!array_key_exists('last_name', $bind) || !strlen($bind['last_name'])) {
            throw new ApplicationException('Customer last name is not specified');
        }

        if (!array_key_exists('email', $bind) || !strlen($bind['email'])) {
            throw new ApplicationException('Customer email is not specified');
        }
        ...
        if (array_key_exists('password', $bind)) {
            if (strlen($bind['password'])) {
                $bind['password'] = SecurityFramework::create()->salted_hash($bind['password']);
            } else {
                unset($bind['password']);
            }
        } elseif (!$existing_customer) {
            $bind['password'] = SecurityFramework::create()->salted_hash(uniqid('password', true));
        }
    ...
    protected function find_country($country)
    {
        ...
        $country_id = DbHelper::scalar(
            'select id from shop_countries 
                  where lower(name)=:country 
                     or lower(code)=:country 
                     or lower(code_3)=:country 
                     or code_iso_numeric=:country',
            ['country'=>$key]
        );
        if (!$country_id) {
            throw new ApplicationException('Unknown country: '.$country);
        }
    ...
    protected function find_state($country_id, $state)
    {
        ...
        $state_id = DbHelper::scalar(
            'select id from shop_states where (lower(name)=:state or lower(code)=:state) and country_id=:country_id',
            ['state'=>$key, 'country_id'=>$country_id]
        );
        if (!$state_id) {
            throw new ApplicationException('Unknown state: '.$state);
        }
