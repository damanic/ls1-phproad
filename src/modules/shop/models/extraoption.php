...
/**
 * Represents a {@link Product product} extra option.
 * @property int $id Specifies the extra option record identifier.
 ...
 * @property \Db\DataCollection $images A collection of the extra option images.
 ...
 * @author LSAPP - MJMAN
 */
class ExtraOption extends ActiveRecord
{
    ...
    public $has_many = [
        'images'=> [
            ...
            'conditions'=>"master_object_class='Shop_ExtraOption' and field='images'",
            'order'=>'sort_order, id',
            'delete'=>true
        ]
    ];
    ...
    public function define_form_fields($context = null)
    {
        $this->add_form_field('description')
            ->comment('Specify the option description, e.g. "Gift wrap".', 'above')
            ->size('small')
            ->tab('Option');
        
        $this->add_form_field('price')
            ->comment('Specify a value to be added to the product price if this option is selected.', 'above')
            ->tab('Option');
        
        $this->add_form_field('group_name')
            ->comment('You can group extras with equal group names.', 'above')
            ->tab('Option')->renderAs(frm_dropdown);
        ...
        $this->add_form_field('weight', 'left')
            ->tab('Shipping');
        
        $this->add_form_field('width', 'right')
            ->tab('Shipping');
        
        $this->add_form_field('height', 'left')
            ->tab('Shipping');
        
        $this->add_form_field('depth', 'right')
            ->comment('You can group extras with equal group names.', 'above')
            ->tab('Shipping');
     ...
     * </pre>
     * @documentable
     * @param Product $product Specifies a product the extra option belongs to.
    ...
    public function get_price($product, $force_tax = false)
    {
        ...
        $include_tax = $force_tax || CheckoutData::display_prices_incl_tax();
        if (!$include_tax) {
            return $price;
        }
     ...
     * {@link https://lsdomainexpired.mjman.net/docs/configuring_lemonstand_for_tax_inclusive_environments/ tax configuration}.
     * @documentable
     * @param Product $product Specifies a product the extra option belongs to.
     * @return float Returns the extra option price.
     */
    ...
    public static function set_orders($item_ids, $item_orders)
    {
        ...
        foreach ($item_ids as $index => $id) {
            ...
            DbHelper::query(
                'update shop_extra_options set extra_option_sort_order=:extra_option_sort_order where id=:id',
                [
    ...
    {
        return DbHelper::scalarArray('select distinct group_name from shop_extra_options order by group_name');
    }
        
    /**
     * Finds an extra option belonging to a specific product.
     * @documentable
     * @param Product Specifies the product object.
     * @param string $extra_key Specifies the extra option key.
     * @return ExtraOption Returns the extra option object. Returns NULL if the extra option is not found.
     */
     ...
     * @param ExtraOption $extra_option Specifies the extra option object.
     */
     ...
     * {@link Db_ActiveRecord::add_form_field() add_form_field()} method of the option object.
     * <pre>
     * public function subscribeEvents()
     * {
     *   Backend::$events->addEvent('shop:onExtendExtraOptionModel', $this, 'extend_extra_model');
     ...
     * {@link shop:onExtendExtraOptionForm} event.
