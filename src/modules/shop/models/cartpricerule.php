...
class CartPriceRule extends PriceRuleBase
{
    ...
    public $has_and_belongs_to_many = [
        'customer_groups'=> [
            ...
            'join_table'=>'shop_cart_rules_customer_groups',
            'order'=>'name'
        ]
    ];
    ...
    public $has_many = [
        'rule_conditions'=> [
            'class_name'=>'Shop\PriceRuleCondition',
    ...
    public $calculated_columns = [
        'coupon_code'=>'select shop_coupons.code 
                        from shop_coupons 
                        where shop_coupons.id=shop_cart_rules.coupon_id',
        'customer_group_ids_str'=>"select group_concat(shop_customer_group_id separator ',') 
                                   from shop_cart_rules_customer_groups 
                                   where shop_cart_rules_customer_groups.shop_cart_rule_id=shop_cart_rules.id"
    ];
        
    public $custom_columns = [
        'rule_conditions_field'=>db_text,
        'products_conditions_field'=>db_text
    ];
    ...
    public function define_columns($context = null)
    {
        $this->define_column('name', 'Name')
            ->validation()->fn('trim')->required("Please specify the price rule name.");
        
        $this->define_column('description', 'Description')
            ->validation()->fn('trim')->required('Please specify the rule description');
    ...
    public function after_validation($deferred_session_key = null)
    {
        if (strlen($this->date_start) && strlen($this->date_end)) {
            $start_obj = PhprDateTime::parse($this->date_start, PhprDateTime::universalDateFormat);
            $end_obj = PhprDateTime::parse($this->date_end, PhprDateTime::universalDateFormat);
    ...
    public function define_form_fields($context = null)
    {
        if ($context != 'eval_discounts') {
            ...
            $this->add_form_field('date_start', 'left')
                ->placeholder('00:00', 'time')
                ->tab('Rule Settings')
                ->comment('Optional time in 24 hour format hh:mm. Default 00:00 (midnight)');

            $this->add_form_field('date_end', 'right')
                ->placeholder('00:00', 'time')
                ->tab('Rule Settings')
                ->comment('Optional time in 24 hour format hh:mm. Default 00:00 (midnight)');
            ...
            if (!$front_end) {
                $this->add_form_field('customer_groups')
                    ->tab('Rule Settings')
                    ->comment(
                        'Please select customer groups the rule is enabled for. 
                        Do not select any group to make the rule enabled for all customer groups.',
                        'above'
                    );
            }
            ...
            $this->add_form_field('terminating')
            ...
            $this->add_custom_field('free_shipping_options', 'Free shipping options')
                ->tab('Free Shipping')
                ->renderAs(frm_checkboxlist)
                ->comment(
                    'Please select the shipping options that you want to be free, 
                    if all conditions specified on the Conditions tab are satisfied.',
                    'above'
                );
    ...
    {
        $result = array();
        $shipping_options = ShippingOption::create()->where('enabled = 1')->find_all();
    ...
    public function get_coupon_options($key = -1)
    {
        ...
        $coupons = Coupon::create()->order('code')->find_all();
    ...
    {
        $result = array();
        $actions = RuleActionBase::find_actions_by_type(RuleActionBase::type_cart);
    ...
    {
        DbHelper::query('update shop_cart_rules set sort_order=:sort_order where id=:id', array(
            'sort_order'=>$this->id,
            'id'=>$this->id
        ));
    ...
    {
        $root_rule = new PriceRuleCondition();
        $root_rule->host_rule_set = 'cart_conditions';
        $root_rule->class_name = 'Shop\RuleIfCondition';
    ...
    public function is_actual_for(
        ...
        $conditions_root = PriceRuleConditionProxy::get_root_condition($this->id, 'cart_conditions');
    ...
    public function eval_discount(
        ...
        $product_conditions_root = PriceRuleConditionProxy::get_root_condition($this->id, 'products_conditions');
        $actionObj =  $this->get_action_obj();
        return $actionObj->eval_discount(
            $params,
            $this,
            $product_price_map,
            $item_discount_tax_incl_map,
            $product_conditions_root
        );
    }
    ...
    * @param $coupon_check bool, optional (set to true to find rules for the specified coupon only)
    */
    protected static function list_active_rules($coupon, $customer, $coupon_check = false)
    {
        ...
        if ($customer) {
            ...
            $customer_group_id = CustomerGroup::get_guest_group()->id;
        }

        $result = array();
        $current_user_time = Date::userDate(PhprDateTime::now());
        foreach ($rules as $rule) {
            ...
            if ($rule->max_coupon_uses && strlen($rule->coupon_code)) {
                if (Coupon::get_order_number($rule->coupon_code) >= $rule->max_coupon_uses) {
                    continue;
                }
            }
        ...
        $updated_results = Backend::$events->fire_event(
            ['name' => 'shop:onListActiveCartPriceRules', 'type' => 'update_result'],
            $result,
            $event_params
        );
    ...
    */
    public static function validate_coupon_code($coupon_code, $customer, $validate_expiration_only = false)
    {
        ...
        $rules = CartPriceRule::create()->where('active=1');
        $rules->join('shop_coupons', 'shop_coupons.id=coupon_id');
        $rules->where('(shop_coupons.code=?)', $coupon->code);
        $rules = $rules->order('sort_order')->find_all();

        $guest_group_id = CustomerGroup::get_guest_group()->id;
        $customer_group_id = $customer ? $customer->customer_group_id : $guest_group_id;

        $current_user_time = Date::userDate(PhprDateTime::now());
        $timezone_name = ' ('.Date::getUserTimezone()->getName().')';
                ...
                if ($rule->date_start && $rule->date_end) {
                    if ($rule->date_start->compare($rule->date_end) === 0) {
                        $error = 'This coupon is not active yet. You can use it on '.
                            Date::display($rule->date_start, '%c').
                            $timezone_name.'.';
                        continue;
                    }
                    ...
                    if ($rule->date_start->compare($current_user_time) > 0) {
                        $error = 'This coupon is not active yet. You can use it between '.
                            Date::display($rule->date_start, '%c').' and '.
                            Date::display($rule->date_end, '%c').
                            $timezone_name.'.';
                        continue;
                    }
                ...
                if (strlen($rule_group_ids)) {
                    ...
                    if (!in_array($customer_group_id, $rule_groups)) {
                        ...
                        foreach ($rule_groups as $rule_group_id) {
                            $group = CustomerGroup::find_by_id($rule_group_id);
                            if ($group) {
                        ...
                        if ($customer_group_id == $guest_group_id && !in_array($customer_group_id, $rule_groups)) {
                            $error = 'This coupon code is only available to registered users. 
                                      Please log in to continue and use this coupon code.';
                        } elseif (!$group_names) {
                            $error = 'A coupon with the specified code is not found.';
                        } else {
                            $error = 'This coupon is only available to the following customer group(s): ';
                            $error .= implode($group_names);
                        }
    ...
    public function get_customer_usage_number($customer)
    {
        ...
        if (self::$customer_usage_map === null) {
            $usage = DbHelper::objectArray('select 
     ...
     * @param PaymentMethod $payment_method Specifies a payment method selected by the customer
     * @param ShippingOption $shipping_method Specifies a shipping method selected by the customer
     * @param array $cart_items Contains an array of shopping cart items (objects of the CartItem class)
     * @param CheckoutAddressInfo $shipping_address Specifies a shipping address specified by the customer
     * @param string $coupon Specified a coupon code provided by the customer
     * @param Customer $customer Specifies a current customer
     * @param float $subtotal Specifies the order subtotal, after applying the catalog price rules
     * @return DiscountData An object containing discount information
     */
    public static function evaluate_discount(
        $payment_method,
        $shipping_method,
        $cart_items,
        $shipping_address,
        ...
        foreach ($rules as $rule) {
            $ruleApplies = $rule->is_actual_for(
                $payment_method,
                $shipping_method,
                $cart_items,
                $shipping_address,
     ...
     * @package shop.events
     * @author Matt Manning (github:damanic)
     * @param CartPriceRule $price_rule Specifies the price rule obj
     * @param bool $price_rule_id Specifies the price rule id.
