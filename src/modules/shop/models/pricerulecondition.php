...
class PriceRuleCondition extends ActiveRecord
{
    ...
    {
        return $this->get_condition_obj() instanceof RuleCompoundConditionBase;
    }
    ...
    {
        $doc = new \DOMDocument('1.0', 'UTF-8');
        $root_element = Xml::createNode($doc, $doc, 'condition');
        foreach ($this->added_fields as $code => $form_field) {
            $field_element = Xml::createNode($doc, $root_element, 'field');
            Xml::createNode($doc, $field_element, 'id', $code);
            $value_element = Xml::createNode($doc, $field_element, 'value');
            Xml::createCdata($doc, $value_element, $this->$code);
        }
