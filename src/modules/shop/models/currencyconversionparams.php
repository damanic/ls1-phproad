...
class CurrencyConversionParams extends ActiveRecord
{
    ...
    {
        $this->add_form_field('class_name')
            ->renderAs(frm_dropdown)
            ->tab('Converter');

        $this->add_form_field('refresh_interval')
            ->renderAs(frm_dropdown)
            ->comment('Select update period of the internal currency rate cache.', 'above')
            ->tab('Converter');

        $this->add_form_partial(PATH_APP.'/modules/shop/currency_converters/partials/_cron.htm')
            ->tab('Converter');

        $this->add_form_field('enable_cron_updates')
        ...
        if (!$this->added_fields) {
            $this->add_form_section('The selected currency converter has no additional configuration parameters.')
                ->tab('Configuration');
        }
    ...
    public function get_converter_object()
    {
        ...
        if (strlen($this->class_name)) {
            $converters = ModuleManager::findById('shop')->listCurrencyConverters();
            foreach ($converters as $class_name) {
                if (get_class_id($this->class_name) == get_class_id($class_name)) {
    ...
    public function before_save($deferred_session_key = null)
    {
        ...
        $xml = '<?xml version="1.0"?><converter_settings></converter_settings>';
        $data = strlen($this->config_data) ? $this->config_data : $xml;
        ...
        $converter_node = Xml::createNode($doc, $doc->firstChild, $this->class_name);
        foreach ($this->added_fields as $code => $form_field) {
            Xml::createNode($doc, $converter_node, 'field', $this->$code)->setAttribute('id', $code);
        }
