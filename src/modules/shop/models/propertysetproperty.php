...
class PropertySetProperty extends ActiveRecord
{
    ...
    public function define_columns($context = null)
    {
        ...
        $this->define_column('value', 'Default Value')
            ->order('asc')
            ->validation()
            ->fn('trim');
        ...
        $this->define_column('validate', 'Validate')
            ->order('asc')
            ->validation()
            ->fn('trim');

        $this->define_column('select_values', 'Select Values')
            ->validation()
            ->fn('trim');
        ...
        $this->define_column('api_code', 'API Code')
            ...
            ->validation()
            ->fn('trim')
            ->unique('API CODE "%s" already exists.');
    ...
    public function define_form_fields($context = null)
    {
        ...
        $this->add_form_field('value', 'left')
            ->comment('If this property should always be presented with a default value, enter it here')
            ->size('small');

        $this->add_form_field('validate', 'right')
            ->renderAs(frm_dropdown)
            ->emptyOption('<none>')
            ->comment('Select a validation rule, if applicable')
            ->size('small');

        $this->add_form_field('required', 'right')
            ->comment('Tick if a value for this property is always required', 'above')
            ->size('small');

        $this->add_form_field('select_values')
            ->renderAs(frm_text)
            ->comment(
                'If the property value should be limited to a select list, 
                enter the values separated by the | character. 
                Eg: Option 1 | Option 2 | Option 3 ', 'above'
            );

        $this->add_form_field('api_code')
            ->comment('Specify a unique API code for this property', 'above')
            ->size('small');
    }
        ...
        if ($this->validate) {
            ...
            switch ($this->validate) {
                ...
                case 'db_float':
                    ...
                    break;
                case 'db_date':
                    $dt = PhprDateTime::now()->format('%x');
                    $validate_default_value->date('%x', 'All values must be a valid date format (eg. ' . $dt . ')');
                    break;
                default:
                    break;
            }
    ...
    public function get_property_set()
    {
        if ($this->property_set_id) {
            return PropertySet::create()->find($this->property_set_id);
        }
        return null;
    }
    ...
    public function validate_property_value($value)
    {
        ...
        switch ($this->validate) {
            ...
            case 'db_date':
                $dt = PhprDateTime::now()->format('%x');
                $validate_value->date('%x', 'Value must be a valid date format (eg. ' . $dt . ')');
                break;
            default:
                break;
        }
