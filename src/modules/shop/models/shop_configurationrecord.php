<?php
	...
	class Shop_ConfigurationRecord extends Db_ActiveRecord 
	{
		...
		{
			$this->add_form_field('cart_login_behavior')->tab('Cart Settings')->comment('Select a behavior LSAPP should apply when a customer logs into the store, in case if some products have been added to the guest cart before the login.', 'above')->renderAs(frm_radio);

			$this->add_form_field('display_prices_incl_tax')->tab('Tax')->comment('Enable this option if you want LSAPP to apply tax to product prices before displaying them in the catalog, cart and invoices.');
