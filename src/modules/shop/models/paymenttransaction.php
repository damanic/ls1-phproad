...
class PaymentTransaction extends ActiveRecord
{
    ...
    public $has_many = [
        'disputes' => [
            'class_name' => 'Shop\PaymentTransactionDispute',
            'foreign_key' => 'shop_payment_transaction_id',
            'order' => 'shop_payment_transaction_disputes.created_at desc, shop_payment_transaction_disputes.id desc',
            'delete' => true
        ]
    ];

    public $custom_columns = [
        'actual_user_name' => db_text
    ];
    ...
    {
        $this->define_column('created_at', 'Created/Updated At')
            ->dateFormat('%x %H:%M');

        $this->define_column('transaction_id', 'Transaction ID')
            ->order('asc')
            ->validation()
            ->fn('trim')
            ->required("Please specify transaction ID");

        $this->define_column('transaction_status_name', 'Status Name')
            ->validation()
            ->fn('trim');

        $this->define_column('transaction_status_code', 'Status Code')
            ->validation();

        $this->define_column('user_note', 'User Notes')
            ->validation();
    ...
    public static function add_transaction(
        ...
        if (!is_a($transaction_data, 'Shop\TransactionUpdate')) {
            throw new ApplicationException(
                'Invalid transaction data given, must be instance of Shop\TransactionUpdate'
            );
        }
    ...
    public function add_dispute(TransactionDisputeUpdate $dispute_update)
    {
        $bind = array(
        ...
        $dispute = PaymentTransactionDispute::create()->where(
            'api_transaction_id = :tid AND case_id = :case_id',
            $bind
        )->find();
        if (!$dispute) {
            $dispute = PaymentTransactionDispute::create();
        }
        ...
        $dispute->reason_desription = $dispute_update->reason_description;
    ...
    {
        $transactions = array();
        $transactionsFound = array();
        foreach ($order->payment_transactions as $transaction) {
            if ($transaction->payment_method_id) {
                $pmId = $transaction->payment_method_id;
                $tId = $transaction->transaction_id;
                if (!isset($transactionsFound[$pmId][$tId])) {
                    $transactionsFound[$pmId][$tId] = $transactions[] = $transaction;
                }
            }
        }
    ...
    public static function transaction_exists($payment_method_id, $transaction_id)
    {
        ...
        $exists = DbHelper::scalar('SELECT id 
    ...
    public static function get_void_transaction_ids(Order $order)
    {
        ...
        return DbHelper::scalarArray($sql, $bind);
    }
     ...
     * @param bool $include_pending If payments pending should be included (not yet settled)
     ...
     * @param $order ActiveRecord object Order
     * @param bool $include_pending If refunds pending should be included (not yet settled)
    ...
    public function list_available_transitions()
    {
        ...
        return $this->payment_method->list_available_transaction_transitions(
            $this->transaction_id,
            $this->transaction_status_code
        );
    }
    ...
    public function update_transaction_status($order, $new_transaction_status_code, $new_order_status_id, $comment)
    {
        ...
        if (!strlen($new_transaction_status_code)) {
            throw new ApplicationException('Please specify a new transaction status.');
        }
        ...
        try {
            $transaction_update_result = $this->payment_method->set_transaction_status(
                $order,
                $this->transaction_id,
                $this->transaction_status_code,
                $new_transaction_status_code
            );
            if (!$transaction_update_result || !($transaction_update_result instanceof TransactionUpdate)) {
                throw new ApplicationException('Transaction status has not been updated.');
            }
        } catch (\Exception $ex) {
            $message = $ex->getMessage();
            if (strlen($new_order_status_id)) {
                $message = Strings::finalize($message) . " The order status has not been updated.";
            }
        ...
        /*
             * Update transaction status - LSAPP database
         */
        ...
        try {
            ...
            $message = "The transaction has been successfully updated on the payment gateway";
            $message .= ", but an error occurred while updating the transaction status in the LSAPP database:";
            $message .= " " . Strings::finalize($ex->getMessage());
            throw new ApplicationException($message);
        }
        ...
        if (strlen($new_order_status_id)) {
            try {
                OrderStatusLog::create_record($new_order_status_id, $order, $comment);
            } catch (\Exception $ex) {
                $message = "The transaction has been successfully updated on the payment gateway";
                $message .= ", but an error occurred whilst attempting to update the order status:";
                $message .= " " . Strings::finalize($ex->getMessage());
                throw new ApplicationException($message);
            }
        }
    ...
    public function request_transaction_status($order)
    {
        ...
        if (!is_object($transaction_update_result) || !($transaction_update_result instanceof TransactionUpdate)) {
            throw new ApplicationException('Transaction status has not been updated.');
        }
    ...
    public function refresh_transaction_history($order)
    {
        ...
        foreach ($transaction_updates as $transaction_update) {
            if (!is_object($transaction_update) || !($transaction_update instanceof TransactionUpdate)) {
                throw new ApplicationException('Transaction status has not been updated.');
            }
        }
