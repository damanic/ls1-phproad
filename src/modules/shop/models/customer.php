 ...
 * @property int $id Specifies the customer record identifier.
 * @property PhprDateTime $created_at Specifies the date and time when the customer record was created.
 * @property PhprDateTime $updated_at Specifies the date and time when the customer record was updated last time.
 ...
 * @property CountryState $billing_state Specifies a billing state.
 * @property Country $billing_country Specifies a billing country.
 ...
 * @property CountryState $shipping_state Specifies a shipping state.
 * @property Country $shipping_country Specifies a shipping country.
 ...
 * @property bool $guest Indicates whether the customer is a guest.
 * @property \Db\DataCollection $orders A list of customer orders.
 * Each element of the collection is an object of the {@link Order} class.
 * @property string $notes Specifies the customer notes.
 * @property CustomerGroup $group A reference to a customer group the customer belongs to.
 * @property \Db\DataCollection $image The customer photo.
...
class Customer extends ActiveRecord
{
    ...
    public $calculated_columns = [
        ...
        'order_count' => [
            'sql' => "(select count(*) from shop_orders where shop_orders.customer_id=shop_customers.id AND shop_orders.deleted_at IS NULL)",
            'type' => db_number
        ]
    ];
    ...
    public $belongs_to = [
        ...
        'group' => ['class_name' => 'Shop\CustomerGroup', 'foreign_key' => 'customer_group_id'],
    ];
    ...
    public $has_many = [
        ...
        'image' => [
            ...
            'conditions' => "master_object_class='Shop_Customer' and field='image'",
            'order' => 'sort_order, id',
            'delete' => true
        ]
    ];
    ...
    {
        $this->define_column('first_name', 'First Name')
            ->order('asc')
            ->validation()
            ->fn('trim')
            ->maxWords(1)
        ...
        $this->define_column('last_name', 'Last Name')
            ->validation()
            ->fn('trim')
            ->maxWords(1)
            ->alphaLang()
        ...
        $this->define_column('company', 'Company')
            ->validation()
            ->fn('trim');

        $this->define_column('phone', 'Phone')
            ->validation()
            ->fn('trim');
        ...
        $this->define_column('order_count', 'Order Count')
            ->defaultInvisible();

        $this->define_column('preferred_currency_code', 'Preferred Currency')
            ->defaultInvisible();
    ...
    public function validate_email($name, $value)
    {
        ...
        $customer = self::create()
            ->where('(shop_customers.guest <> 1 or shop_customers.guest is null)')
            ->where('email=?', $value);
        ...
        if ($customer) {
            $this->validation->setError(
                "Email " . $value . " is already in use. Please specify another email address.",
                $name,
                true
            );
        }

        $customers_assigned = Customer::find_customers_by_email_trace($value);
        if ($customers_assigned && $customers_assigned->count) {
            if (!$this->id) {
                $this->validation->setError(
                    "Email " . $value . " has already been used. Please specify another email address.",
                    $name,
                    true
                );
            } else {
     ...
     * @return Customer Returns a customer object or NULL of the customer is not found.
     */
     ...
     * @return Customer|false Returns the customer if found or FALSE otherwise.
     */
    public static function find_by_customer_reference($customer_ref)
    {
        ...
        foreach ($lookup as $customer) {
            if ($customer && is_a($customer, 'Shop\Customer')) {
                return $customer;
            }
        }
        $customer = Customer::create()->find($customer_ref);
    ...
    public function get_group_options($key_value = -1)
    {
        if ($key_value != -1) {
            ...
            $obj = CustomerGroup::create()->find($key_value);
            return $obj ? $obj->name : null;
        }

        $options = array(null => '<please select>');
        $groupModel = CustomerGroup::create();
        $groupModel->where(
            '(shop_customer_groups.code is null or shop_customer_groups.code<>?)',
            CustomerGroup::guest_group
        );
        $groups = $groupModel->order('name')->find_all();
    ...
    {
        $currency_code = null;
        if (is_a($currency, 'Shop\CurrencySettings')) {
    ...
    public function before_save($deferred_session_key = null)
    {
        ...
        if ($this->guest) {
            if (!$this->customer_group_id) {
                $group = CustomerGroup::create()->find_by_code(CustomerGroup::guest_group);
                if ($group) {
            ...
            if (!$this->customer_group_id) {
                $group = CustomerGroup::create()->find_by_code(CustomerGroup::registered_group);
                if ($group) {
                    $this->customer_group_id = $group->id;
                }
            }
     ...
     * @documentable
     * @param Order Order Specifies an order object to copy data to.
     */
     ...
     * @documentable
     * @param Order Order Specifies an order object to copy data from.
     */
     ...
     * with {@link Customer::restore_customer() restore_customer()} method.
     * @documentable
     */
    public function delete_customer()
    {
        if ($this->orders->count) {
            ...
            DbHelper::query(
                'update shop_customers set deleted_at=:deleted_at where id=:id',
                array(
    ...
    /**
     * Restores a customer previously deleted with {@link Customer::delete_customer() delete_customer()} method.
     * @documentable
     */
    ...
    public function before_delete($id = null)
    {
        if ($order_num = $this->orders->count) {
            throw new ApplicationException(
                "Error deleting customer. There are $order_num order(s) belonging to this customer."
            );
        }
    }
    ...
    public function generate_password_restore()
    {
        ...
        $sqlHashExists = 'select count(*) from shop_customers where password_restore_hash = :hash';
        while ($count = DbHelper::scalar($sqlHashExists, ['hash' => $hash])) {
            $hash = $this->password_to_hash(rand(1, 40000));
        }
        $this->password_restore_hash = $hash;
        $this->password_restore_time = PhprDateTime::now();
        return $hash;
    }

    protected function password_to_hash($plaintext_password)
    {
        return SecurityFramework::create()->salted_hash($plaintext_password);
    }

    /**
     * Finds a customer with the specified password reset hash
     * @param string $hash The password reset hash
     * @return mixed Customer object if customer with specified hash is found and
     *  hash is still valid (generated under 24 hours ago) or null if customer not found
     */
    public static function get_from_password_reset_hash($hash)
    {
        ...
        return Customer::create()
            ->where('password_restore_hash=?', $hash)
            ->where('password_restore_time>?', $yesterday)
            ->find(null, [], 'front_end');
    }
    ...
    public static function reset_password($email)
    {
        ...
        $customer = self::create()
            ->where('(shop_customers.guest <> 1 or shop_customers.guest is null)')
            ->where('shop_customers.email=?', $email)
            ->find(null, [], 'front_end');
        if (!$customer) {
            throw new ApplicationException('Customer with specified email is not found.');
        }

        if ($customer->deleted_at) {
            throw new ApplicationException('Customer account was deleted.');
        }

        $customer->generate_password();
        $customer->save();

        $template = EmailTemplate::create()->find_by_code('shop:password_reset');
        if ($template) {
    ...
    public static function send_password_restore($email)
    {
        ...
        $customer = self::create()
            ...
            ->find(null, [], 'front_end');
        if (!$customer) {
            throw new ApplicationException('No customer found for the specified email.');
        }

        if ($customer->deleted_at) {
            throw new ApplicationException('Your customer account was deleted.');
        }

        $customer->generate_password_restore();
        $customer->save();

        $template = EmailTemplate::create()->find_by_code('shop:password_restore');
        if ($template) {
            $template->subject = $customer->set_customer_email_vars($template->subject);
            $message = $customer->set_customer_email_vars($template->content);
            $template->send_to_customer($customer, $message);
     ...
     * @param Product $product A product object to return a quantity for
     * @return int
     */
    public function get_purchased_item_quantity($product)
    {
        ...
        $sql = 'select sum(quantity) 
                from shop_order_items, shop_orders, shop_order_status_log_records, shop_order_statuses 
                where shop_order_items.shop_order_id=shop_orders.id
        ...
        return $this->product_quantity_cache[$product->id] = DbHelper::scalar($sql, [
    ...
    public function set_customer_email_vars($message_text)
    {
        ...
        $message_text = CompoundEmailVar::apply_scope_variables($message_text, 'shop:customer', $email_scope_vars);
    ...
    public function get_recent_notifications($limit = 20)
    {
        ...
        $notifications = CustomerNotification::create();
    ...
    public function merge_into($destination)
    {
        if (!$destination || !$destination->id) {
            throw new ApplicationException('Invalid destination customer.');
        }

        Backend::$events->fireEvent('shop:onBeforeCustomerMerged', $this, $destination);

        $sql = 'update shop_orders set customer_id=:destination_customer_id 
        ...
        $sql = 'update shop_customer_payment_profiles 
                set customer_id=:destination_customer_id 
                where customer_id=:source_customer_id';
        DbHelper::query($sql, [
            'destination_customer_id' => $destination->id,
            'source_customer_id' => $this->id
        ...
        $sql = 'update shop_customer_cart_items 
                set customer_id=:destination_customer_id 
                where customer_id=:source_customer_id';
        DbHelper::query($sql, [
            'destination_customer_id' => $destination->id,
    ...
    {
        $template = EmailTemplate::create()->find_by_code('shop:registration_confirmation');
        if ($template) {
    ...
    {
        if (Customer::find_registered_by_email($this->email)) {
            throw new ApplicationException("Registered customer with email {$this->email} already exists.");
        }
     ...
     *
     * @return Customer|ActiveRecord The associated customer if found
     */
    public function findUser($login_id, $password, $include_deleted = false)
    {
        ...
        $this->where('email=?', $login_id)
            ->where('shop_customers.password=?', $password_hash)
            ->where('(shop_customers.guest is null or shop_customers.guest=0)');
        if (!$include_deleted) {
     ...
     * @return \Db\DataCollection Returns a collection of Customer if found
     */
     ...
     * execution context string. To add new columns to the customer model,
     * call the {@link ActiveRecord::define_column() define_column()}
     ...
     * To add new fields to the customer form, call the
     ...
     * which have been added with {@link shop:onExtendCustomerForm} event.
     ...
     * By default when a customer submits the Login form, LSAPP checks the specified email
     ...
     * The handler method should return the customer object ({@link Customer}), NULL or FALSE.
     ...
     *     ->where('password=?', SecurityFramework::create()->salted_hash($password))
     *     ->find();
     * }
     * </pre>
     ...
     * a partial containing new buttons, and a {@link Customer customer} object.
     * <pre>
     * public function subscribeEvents()
     * {
    ...
    /**
     * Allows to load extra CSS or JavaScript files on the Customer List,
     * Customer Preview and Create/Edit Customer pages.
     ...
     * {@link shop:onExtendCustomerForm} event.
     * <pre>
     * public function subscribeEvents() {
     *   Backend::$events->addEvent('shop:onExtendCustomerModel', $this, 'extend_customer_model');
     *   Backend::$events->addEvent('shop:onExtendCustomerForm', $this, 'extend_customer_form');
     ...
     *     array('class_name'=>'Manufacturer', 'join_table'=>'test_customers_manufacturers',
     *       'primary_key'=>'customer_id', 'foreign_key'=>'manufacturer_id'
     *   ));
     *   $customer->define_multi_relation_column(
     *      'test_manufacturers',
     *      'test_manufacturers',
     ...
     * @param Customer $source Specifies the source customer object.
     * @param Customer $destination Specifies the destination customer object.
     ...
     * The event handler should return the Customer if found
     * @event shop:onCustomerFindByCustomerReference
     * @triggered /modules/shop/models/shop_customer.php
     * @param Customer , The matching customer obj
     * @author Matt Manning (github:damanic)
     * @package shop.events
     */
