...
class PriceTier extends ActiveRecord
{
    ...
    {
        $this->add_form_field('customer_group')
            ->emptyOption('Any customer')
            ->referenceSort('name');

        $this->add_form_field('quantity')
            ->comment('A minimum quantity of ordered items, starting from which the price should be applied.', 'above');
