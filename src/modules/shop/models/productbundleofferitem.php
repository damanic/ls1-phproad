 ...
 * @documentable
 * @property int $id Specifies the bundle item product identifier.
 * @property int $product_id Identifier of a product ({@link Product}) object this item is offering.
 * @property int $default_quantity Specifies the default product quantity.
 * @property bool $allow_manual_quantity Determines whether manual quantity input is allowed.
 * @property bool $is_default Determines whether this product is a default product in the bundle item.
 * @property ProductBundleOffer $bundle_offer A reference to the parent bundle offer object.
 * @property Product $product A reference to the product associated with this bundle item product record.
 ...
 * @package shop.models
 */
class ProductBundleOfferItem extends ActiveRecord
{
    ...
    public $belongs_to = array(
        'bundle_offer'=>array('class_name'=>'Shop\ProductBundleOffer', 'foreign_key'=>'item_id'),
        'product'=>array('class_name'=>'Shop\Product', 'foreign_key'=>'product_id'),
    );
    ...
    public function define_columns($context = null)
    {
        ...
        if (!$front_end) {
            ...
            $this->define_column('price_or_discount', 'Price or discount')
                ->validation()->fn('trim')
                ->method('validate_price_or_discount');

            $this->define_column('default_quantity', 'Default quantity')
                ->validation()
                ->fn('trim')
                ->required('Please specify default quantity.');
        }
    }
    ...
    public function validate_price_or_discount($name, $value)
    {
        if (!strlen($value)) {
            ...
            $type = 'price';
            $discountModes = [
                self::price_override_percentage_discount,
                self::price_override_fixed_discount
            ];
            if (in_array($this->price_override_mode, $discountModes)) {
                $type = 'discount';
            }
            $this->validation->setError('Please specify product '.$type.' in the Price or Discount field', $name, true);
        }
     ...
     * The optional <em>$product</em> parameter represents a currently
     * selected product object. If no object provided, the product assigned to the item will be used. If you use
     * {@link https://lsdomainexpired.mjman.net/docs/understanding_option_matrix/ Option Matrix}
     * feature, pass selected bundle item product's options to the second parameter.
     * Use {@link BundleHelper::get_selected_options()}
     ...
     * </pre>
     * @documentable
     * @param Product Currently selected product object.
     * If no object provided, the product assigned to the item will be used.
     * @param array $product_options Specifies selected product options
     * ({@link https://lsdomainexpired.mjman.net/docs/understanding_option_matrix/ Option Matrix} support)
     * @param bool $apply_catalog_price_rules Determines whether catalog price rules should be applied to the result.
     ...
     * the product assigned to the item will be used. If you use
     * {@link https://lsdomainexpired.mjman.net/docs/understanding_option_matrix/ Option Matrix}
     * feature, pass selected bundle item product's options to the second parameter.
     * Use {@link BundleHelper::get_selected_options()}
     * method to load the selected options. Example:
     ...
     * is not necessary in front-end code - use {@link ProductBundleOfferItem::get_price() get_price()} method instead.
     * @documentable
     * @param Product Currently selected product object.
     * If no object provided, the product assigned to the item will be used.
     * @param int $quantity Specifies the product quantity ordered.
     ...
     */
    public function get_price_no_tax(
        $product = null,
        $quantity = 1,
        $customer_group_id = null,
        $product_options = null,
        $apply_catalog_price_rules = false
    ) {
        $price = null;
        $product = $product ? $product : $this->product;
        ...
        if (is_array($product_options)) {
            ...
            if ($om_record) {
                $method = $apply_catalog_price_rules ? 'get_sale_price' : 'get_price';
                $price = $om_record->$method($product, $quantity, $customer_group_id, true);
            }
        }

        if ($price === null) {
            $method = $apply_catalog_price_rules ? 'get_sale_price_no_tax' : 'price_no_tax';
            $price = $product->$method($quantity, $customer_group_id);
        }

        return $price;
    }

    public function get_sale_price_no_tax(
        $product = null, 
        $quantity = 1, 
        $customer_group_id = null, 
        $product_options = null
     ...
     *
     * Only accepts ISO currency codes registered in CurrencySettings
     *
    ...
    public function set_currency_context($currency_code)
    {
        if ($this->currency_context !== $currency_code) {
            $currency = CurrencyHelper::get_currency_setting($currency_code);
            if (!$currency) {
                throw new ApplicationException('Currency code not found in CurrencySettings');
            }
    ...
    protected function get_override_price_value()
    {
        ...
        $shop_currency =  CurrencySettings::get();
     ...
     * @documentable
     * @param int $id Specifies the bundle item product identifier.
     * @return ProductBundleOfferItem Returns the bundle item product object. Returns NULL if the object is not found.
     */
