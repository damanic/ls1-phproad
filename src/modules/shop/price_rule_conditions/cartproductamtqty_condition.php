...
class CartProductAmtQty_Condition extends RuleIfCondition
{
    protected $operators = [
        'is' => 'is',
        'is_not' => 'is not',
    ...
    {
        return array(RuleConditionBase::type_product, RuleConditionBase::type_cart_product_attribute);
    }
    ...
    public function is_true(&$params, $host_obj)
    {
        if (!array_key_exists('cart_items', $params)) {
            throw new ApplicationException(
                'Error evaluating the cart items total quantity or total amount condition: 
        ...
        foreach ($params['cart_items'] as $item) {
            ...
            if ($item_current_price === false) {
                $item_current_price = $item->set_de_cache_item(
                    'current_price',
                    $item->single_price_no_tax() - $item->get_sale_reduction()
                );
            }
            ...
            foreach ($host_obj->children as $subcondition) {
                ...
                } elseif ($subcondition_result === $required_condition_value) {
                    $subconditions_result = true;
                    break;
                }
