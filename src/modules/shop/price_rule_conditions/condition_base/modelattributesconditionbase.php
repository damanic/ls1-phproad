...
class ModelAttributesConditionBase extends RuleConditionBase
{
    ...
    public function get_value_control_type($host_obj)
    {
        ...
        DbHelper::query(
            'update shop_price_rule_conditions set condition_control_type=:control_type where id=:id',
            [
                'control_type' => $result,
                'id' => $host_obj->id
            ]);
    ...
    {
        $classId = get_class_id('Shop\PriceRuleCondition');
        $post_data = post($classId, array());
        ...
        if (post('type') == 'attribute_change') {
            echo ">>form_field_container_operator$classId<<";
            $controller->formRenderFieldContainer($host_obj, 'operator');

            echo ">>form_field_container_value$classId<<";
    ...
    public function prepare_reference_list_info($host_obj)
    {
        ...
        if (!count($visible_columns)) {
            throw new SystemException('Order column is not defined in the ' . $model_class . ' model.');
        }
    ...
    public function onPreRender($controller, $host_obj)
    {
        ...
        $post_data = post(get_class_id('Shop\PriceRuleCondition'), array());
        ...
        $baseClassDir = PATH_APP . '/modules/shop/price_rule_conditions/condition_base/modelattributesconditionbase';
        $controller->list_custom_body_cells = $baseClassDir . '/_multiselect_body_control.htm';
        $controller->list_custom_head_cells = $baseClassDir . '/_multiselect_head_control.htm';

        $searchFields = $this->get_reference_search_fields($reference_info->reference_model, $reference_info->columns);
        $isDbTree = $reference_info->reference_model->isExtendedWith('Db_Act_As_Tree');
        ...
        $controller->list_options['list_render_as_sliding_list'] = $isDbTree;
    ...
    public function eval_is_true($model, $host_obj, $custom_value = '__no_value_provided__')
    {
        ...
        if ($control_type == 'dropdown') {
            ...
            if ($operator == 'is') {
                ...
                if ($model_value instanceof \Db\ActiveRecord) {
                ...
                if ($model_value instanceof \Db\DataCollection) {
        ...
        if ($control_type == 'multi_value') {
            ...
            $isActiveRecord = $model_value instanceof \Db\ActiveRecord;
            $isCollection = $model_value instanceof \Db\DataCollection;
            if (!$isCollection && !$isActiveRecord && !(is_array($model_value))) {
                return false;
            }
            ...
            } elseif ($model_value instanceof \Db\ActiveRecord) {
