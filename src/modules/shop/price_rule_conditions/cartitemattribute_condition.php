...
class CartItemAttribute_Condition extends ModelAttributesConditionBase
{
    protected $model_class = 'Shop\CartItemAttributeContainer';
    ...
    public function get_value_dropdown_options($host_obj, $controller)
    {
        ...
        if (in_array($attribute, array('discounted_bundle_item', 'bundle_item'))) {
            ...
            return [
                'false' => 'FALSE (product IS NOT a ' . $discounted . ' bundle item)',
                'true' => 'TRUE (product IS a ' . $discounted . ' bundle item)'
            ];
        }
    ...
    public function is_true(&$params, $host_obj)
    {
        ...
        if ($attribute == 'price') {
            if (!array_key_exists('current_price', $params)) {
                throw new ApplicationException(
                    'Error evaluating the cart item attribute condition: 
        ...
        if ($attribute == 'row_total') {
            if (!array_key_exists('row_total', $params)) {
                throw new ApplicationException(
                    'Error evaluating the cart item attribute condition: 
                    the row_total element is not found in the condition parameters.'
                );
            }
        ...
        if ($attribute == 'quantity') {
            if (!array_key_exists('quantity_in_cart', $params)) {
                throw new ApplicationException(
                    'Error evaluating the cart item attribute condition: 
                    the quantity_in_cart element is not found in the condition parameters.'
                );
            }
        ...
        if ($attribute == 'discount') {
            if (!array_key_exists('item_discount', $params)) {
                throw new ApplicationException(
                    'Error evaluating the cart item attribute condition: 
                    the item_discount element is not found in the condition parameters.'
                );
            }
        ...
        if ($attribute == 'bundle_item') {
            if (!array_key_exists('item', $params)) {
                throw new ApplicationException(
                    'Error evaluating the cart item attribute condition: 
                    the item element is not found in the condition parameters.'
                );
            }
 ...
 */
class CartItemAttributeContainer extends ActiveRecord
{
