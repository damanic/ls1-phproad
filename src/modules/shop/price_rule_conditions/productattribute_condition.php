 ...
 */
class ProductAttribute_Condition extends ModelAttributesConditionBase
{
        ...
        if ($attribute == 'product') {
            $products = DbHelper::objectArray(
                'select id, name from shop_products 
                     where grouped is null and (disable_completely is null or disable_completely=0) order by name'
            );
    ...
    public function prepare_reference_list_info($host_obj)
    {
        ...
        if ($attribute == 'product') {
            ...
            $this->reference_info['reference_model'] = new Product();
    ...
    {
        if (get_class_id($model) == get_class_id('Shop\Product')) {
    ...
    public function is_true(&$params, $host_obj)
    {
        if (!array_key_exists('product', $params)) {
            throw new ApplicationException(
                'Error evaluating the product attribute condition: 
                the product element is not found in the condition parameters.'
            );
        }
        ...
        if (!in_array($attribute, $extended_attributes)) {
            /*
             * If om_record (Option Matrix record) key exists in the parameters,
             * try to load the attribute value from it,
             * instead of loading it from the product.
             */
            $attribute_value = '__no_value_provided__';
            if (isset($params['om_record']) &&
                $params['om_record'] instanceof OptionMatrixRecord
        ...
        if ($attribute == 'on_sale') {
            $om_record = $params['om_record'] ?? null;
            $om_record = ($om_record && $om_record instanceof OptionMatrixRecord) ? $om_record : null;
            $test_object = $om_record ? $om_record : $params['product'];


            if ($test_object->on_sale) {
                $value = $test_object->sale_price_or_discount;
                if (!Product::is_sale_price_or_discount_invalid($value) && strlen($value)) {
        ...
        if ($attribute == 'current_price') {
            if (!array_key_exists('current_price', $params)) {
                throw new ApplicationException(
                    'Error evaluating the product attribute condition: 
                    the current_price element is not found in the condition parameters.'
                );
            }
        ...
        if ($attribute == 'product') {
            $test_product = new Product(null, array('no_column_init' => true, 'no_validation' => true));
