...
class CartAttribute_Condition extends ModelAttributesConditionBase
{
    protected $model_class = 'Shop\CartAttributeContainer';
    ...
    public function get_value_dropdown_options($host_obj, $controller)
    {
        ...
        if ($attribute == 'shipping_country') {
            $records = DbHelper::objectArray(
                'select * from shop_countries where enabled_in_backend=1 order by name'
            );
            $result = array();
        ...
        if ($attribute == 'shipping_state') {
            $records = DbHelper::objectArray(
                'select shop_states.id, shop_countries.name as country_name, 
    ...
    {
        if ($model instanceof CountryState) {
    ...
    public function is_true(&$params, $host_obj)
    {
        ...
        if ($attribute == 'shipping_method') {
            if (!array_key_exists('shipping_method', $params)) {
                throw new ApplicationException(
                    'Error evaluating the cart attribute condition: 
                    the shipping_method element is not found in the condition parameters.'
                );
            }
        ...
        if ($attribute == 'payment_method') {
            if (!array_key_exists('payment_method', $params)) {
                throw new ApplicationException(
                    'Error evaluating the cart attribute condition: 
                the payment_method element is not found in the condition parameters.'
                );
            }
        ...
        if ($attribute == 'shipping_country') {
            if (!array_key_exists('shipping_address', $params)) {
                throw new ApplicationException(
                    'Error evaluating the cart attribute condition: 
                    the shipping_address element is not found in the condition parameters.'
                );
            }
        ...
        if ($attribute == 'subtotal') {
            if (!array_key_exists('subtotal', $params)) {
                throw new ApplicationException(
                    'Error evaluating the cart attribute condition: 
                    the subtotal element is not found in the condition parameters.'
                );
            }
        ...
        if ($attribute == 'total_quantity') {
            if (!array_key_exists('cart_items', $params)) {
                throw new ApplicationException(
                    'Error evaluating the cart attribute condition: 
                    the cart_items element is not found in the condition parameters.'
                );
            }
        ...
        if ($attribute == 'total_discount') {
            if (!array_key_exists('cart_discount', $params)) {
                throw new ApplicationException(
                    'Error evaluating the cart attribute condition: 
                    the cart_discount element is not found in the condition parameters.'
                );
            }
 ...
 */
class CartAttributeContainer extends ActiveRecord
{
    ...
    public $belongs_to = array(
        'shipping_country' => array('class_name' => 'Shop\Country', 'foreign_key' => 'shipping_country_id'),
        'shipping_state' => array('class_name' => 'Shop\CountryState', 'foreign_key' => 'shipping_state_id'),
        'shipping_method' => array('class_name' => 'Shop\ShippingOption', 'foreign_key' => 'shipping_method_id'),
        'payment_method' => array('class_name' => 'Shop\PaymentMethod', 'foreign_key' => 'payment_method_id')
    );
