...
class Shop_RuleConditionsBehavior extends Phpr_ControllerBehavior
{
    ...
    public function __construct($controller)
    {
        ...
        $this->hideAction('conditionsRegisterViewPaths');
    ...
    public function onLoadConditionSetup()
    {
        ...
        if ($condition) {
            $this->conditionsRegisterViewPaths($condition);
        }

        $this->renderPartial('condition_settings_form');
    }
        
    public function conditionsRegisterViewPaths($condition)
    {
        $classInfo = new ReflectionClass($condition->class_name);
        $reflectionObjects = [
            $classInfo,
        ...
        foreach ($reflectionObjects as $reflectionObject) {
            $baseDir = dirname($reflectionObject->getFileName());
            $folderName = strtolower($reflectionObject->getShortName());
            $this->_controller->formRegisterViewPath($baseDir.'/'.$folderName);
        }
    }
    ...
    public function onSaveCondition()
    {
        try {
            ...
            $data = post(get_class_id($condition), array());
