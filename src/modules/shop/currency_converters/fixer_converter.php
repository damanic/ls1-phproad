 ...
 */
class Fixer_Converter extends CurrencyConverterBase
{
    ...
    public function get_info()
    {
        return array(
            'name' => 'Fixer.io',
            'description' =>
                'Fixer.io is a free API for current and historical foreign exchange rates 
    ...
    public function get_exchange_rate($host_obj, $from_currency, $to_currency)
    {
        ...
        if (!strlen($feed_content)) {
            throw new SystemException('Error loading Fixer.io currency exchange feed.');
        }

        $data = json_decode($feed_content, true);

        if (!$data || !isset($data['base'])) {
            throw new SystemException('Fixer.io currency exchange rate service has returned invalid data');
        }
