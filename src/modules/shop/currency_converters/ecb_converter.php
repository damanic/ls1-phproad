 ...
 */
class Ecb_Converter extends CurrencyConverterBase
{
    const feed_address = 'https://www.ecb.europa.eu/stats/eurofxref/eurofxref-daily.xml';
    ...
    public function get_info()
    {
        return array(
            ...
            'description' =>
                'This converter uses the free currency exchange rate feed 
                provided by European Central Bank (www.ecb.int)'
        );
    }
    ...
    public function get_exchange_rate($host_obj, $from_currency, $to_currency)
    {
        ...
        if (!strlen($feed_content)) {
            throw new SystemException('Error loading European Central Bank feed.');
        }
        ...
        if ($from_currency == 'EUR') {
            ...
            if (!$from_rate->length) {
                throw new SystemException('Currency rate for ' . $from_currency . ' not found');
            }
        ...
        if ($to_currency == 'EUR') {
            ...
            if (!$to_rate->length) {
                throw new SystemException('Currency rate for ' . $to_currency . ' not found');
            }
        ...
        if (!is_numeric($to_rate) || !is_numeric($from_rate)) {
            throw new SystemException('Invalid currency rates received');
        }
