 ...
 */
class ERAIO_Converter extends CurrencyConverterBase
{
    ...
    public function get_info()
    {
        return array(
            ...
            'description' =>
                'A free API for current and historical foreign exchange rates 
    ...
    {
        $host_obj->add_field('api_key', 'API KEY')
            ->tab('Configuration')
            ->renderAs(frm_text)
            ->comment('This service requires an API key from exchangeratesapi.io ', 'above', true);

        $host_obj->add_field('use_ssl', 'Use SSL')
            ->tab('Configuration')
            ->renderAs(frm_onoffswitcher)
            ->comment('Only switch this on if your API plan allows for HTTPS access ', 'above', true);
    }

    /**
     * Returns exchange rate for two currencies
     * This method must be implemented in a specific currency converter.
     * @param \Db\ActiveRecord $parameters_base Active Record object to read parameters from
    ...
    public function get_exchange_rate($host_obj, $from_currency, $to_currency)
    {
        ...
        if (empty(self::$api_result)) {
            ...
            if (!strlen($feed_content)) {
                throw new SystemException('Error loading exchangeratesapi.io currency exchange feed.');
            }

            $data = json_decode($feed_content, true);
            if (!$data || !isset($data['base'])) {
                throw new SystemException(
                    'exchangeratesapi.io currency exchange rate service has returned invalid data'
                );
            }
        ...
        if (!is_numeric($rate) || ($rate <= 0)) {
            throw new SystemException('exchangeratesapi.io currency exchange rate service has returned invalid data');
        }
