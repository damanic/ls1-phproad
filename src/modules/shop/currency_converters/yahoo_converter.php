 ...
 */
class Yahoo_Converter extends CurrencyConverterBase
{
    ...
    public function get_exchange_rate($host_obj, $from_currency, $to_currency)
    {
        ...
        if (!strlen($feed_content)) {
            throw new SystemException('Error loading Yahoo currency exchange feed.');
        }

        $data = explode(',', $feed_content);
        if (count($data) < 2) {
            throw new SystemException('Yahoo currency exchange rate service has returned invalid data');
        }
