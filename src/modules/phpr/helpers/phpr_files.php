...
class Phpr_Files
{
        ...
        switch ($fileInfo['error']) {
            case UPLOAD_ERR_INI_SIZE:
            {
                ...
                throw new Phpr_ApplicationException(
                    sprintf("File size exceeds maximum allowed size (%s).", $MaxSizeAllowed)
                );
            }
     ...
     * @param string $directory Specifies a path to the directory.
     * @param mixed $file_types An array of file extensions (without dots) to find.
     * @return array Returns a sorted array of full file paths.
     */
    public static function findFiles($directory, $file_types = null)
    {
        ...
        $result = self::findFilesRecursive($directory, '', $file_types);
        sort($result);
    ...
    protected static function findFilesRecursive($directory, $base, $file_types)
    {
        ...
        while (($file = readdir($handle)) !== false) {
            ...
            if (($type = pathinfo($file, PATHINFO_EXTENSION)) !== '') {
                if ($file_types && !in_array($type, $file_types)) {
                    continue;
                }
            }
            ...
            if ($is_file) {
                ...
                $result = array_merge($result, self::findFilesRecursive($path, $base . '/' . $file, $file_types));
            }
        }
