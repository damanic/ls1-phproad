...
class Phpr_DateTimeFormat
{
    ...
    (
        ...
        'C' => array(
            self::spType => self::spTypeInt,
            self::spIntMin => 0,
            self::spIntMax => 99,
            self::spMethod => self::spCustomMethod,
            self::spIntPadding => 2
        ),
        'd' => array(
            self::spType => self::spTypeInt,
            self::spIntMin => 1,
            self::spIntMax => 31,
            self::spMethod => 'GetDay',
            self::spIntPadding => 2,
            self::spParserMeaning => self::spPrMnDay
        ),
        ...
        'H' => array(
            ...
            self::spIntMin => 0,
            self::spIntMax => 23,
            self::spMethod => 'GetHour',
            self::spIntPadding => 2,
            self::spParserMeaning => self::spPrMnHour
        ),
        'I' => array(
            self::spType => self::spTypeInt,
            self::spIntMin => 1,
        ...
        'm' => array(
            ...
            self::spIntMin => 1,
            self::spIntMax => 12,
            self::spMethod => 'GetMonth',
            self::spIntPadding => 2,
            self::spParserMeaning => self::spPrMnMonth
        ),
        'M' => array(
            self::spType => self::spTypeInt,
            self::spIntMin => 0,
            self::spIntMax => 59,
            self::spMethod => 'GetMinute',
            self::spIntPadding => 2,
            self::spParserMeaning => self::spPrMnMinute
        ),
        ...
        'p' => array(
            ...
            self::spDomain => 'ampm_',
            self::spMethod => self::spCustomMethod,
            self::spParserMeaning => self::spPrMnCustom,
            self::spValueList => array('ampm_am', 'ampm_pm')
        ),
        'S' => array(
            self::spType => self::spTypeInt,
            self::spIntMin => 0,
            self::spIntMax => 59,
            self::spMethod => 'GetSecond',
            self::spIntPadding => 2,
            self::spParserMeaning => self::spPrMnSecond
        ),
        ...
        'Y' => array(
            ...
            self::spIntMin => 1,
            self::spIntMax => 9999,
            self::spMethod => 'GetYear',
            self::spParserMeaning => self::spPrMnYear
        )
    );
        ...
        foreach ($FormatSpecifiers as $MatchData) {
            ...
            if (!array_key_exists($MatchSpecifier, $processedSpecifiers)) {
                ...
                if ($specifierDesc[self::spType] == self::spTypeString) {
                    ...
                    $specifierValue = Phpr::$lang->mod(
                        'phpr',
                        $specifierDesc[self::spDomain] . $methodValue,
                        self::localizationPrefix
                    );
     ...
     * @param boolean $ValidationOnly Perform validation, but do not create Phpr_DateTime object
     * @return mixed Returns Phpr_DateTime if $ValidationOnly is false and $Str contains a valid value.
     * If $ValidationOnly is true, returns a boolean value determining whether $Str value passed the validation.
     */
    public static function parseDateTime($Str, $Format, $TimeZone = null, $ValidationOnly = false)
    {
        self::init();
