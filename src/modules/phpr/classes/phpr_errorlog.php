...
class Phpr_ErrorLog
{
    ...
    public static function get_exception_details($exception)
    {
        $error = (object)array(
            ...
            'document' => $exception instanceof Cms_ExecutionException ? $exception->document_name(
            ) : Phpr_Files::rootRelative($exception->getFile()),
            'document_type' => $exception instanceof Cms_ExecutionException ? $exception->document_type(
            ) : 'PHP document',
    ...
    protected function writeLogMessage($message, $log_to_db = true, $details = null)
    {
        ...
        if ((Phpr::$config->get(
            ...
            if (is_int($ttl) && $ttl > 0) {
                Db_DbHelper::query(
                    'delete from trace_log where log=:log and record_datetime < DATE_SUB(:date, INTERVAL :days DAY)',
                    array('date' => $record->record_datetime, 'days' => $ttl, 'log' => 'ERROR')
                );
            }
        }
