...
class SettingsManager extends SettingsRecord
{
    ...
    public function define_columns($context = null)
    {
        ...
        if (!Theme::is_theming_enabled()) {
            $this->define_column('default_templating_engine', 'Templating engine')
                ->validation()
                ->method('validate_default_templating_engine');
        }
            
        $this->define_column('enable_filebased_templates', 'Enable file-based templates');

        $this->define_column('templates_dir_path', 'Path to the templates directory')
            ->validation()
            ->fn('trim')
            ->method('validate_templates_path');

        $this->define_column('content_file_extension', 'Content file extension')
            ->validation()
            ->fn('trim');

        $this->define_column('resources_dir_path', 'Path to the website resources directory')
            ->validation()
            ->fn('trim')
            ->required('Please specify the resources directory path')
            ->method('validate_resources_directory');
    }
        
    public function define_form_fields($context = null)
    {
        if (!Theme::is_theming_enabled()) {
            $this->add_form_field('default_templating_engine')->tab('Template Engine')
                ->renderAs(frm_dropdown);
        }

        $extraFieldClass = $this->enable_filebased_templates ? 'separatedField' : null;
        $this->add_form_field('enable_filebased_templates')
            ->renderAs(frm_onoffswitcher)
            ->comment(
                'Enable file-based templates if you want LSAPP to store pages, 
                partials and templates in files instead of the database.',
                'above'
            )
        ...
        $field = $this->add_form_field('templates_dir_path')
            ->cssClassName($extraFieldClass)
            ->tab('Template Engine')
            ->comment(
                'Please specify an absolute path to the directory on the server, 
                where you want to store LSAPP templates. 
                The directory should exist and be writable for PHP.',
                'above',
                true
            );
        ...
        $this->add_form_field('content_file_extension')
            ->cssClassName($extraFieldClass)
            ->tab('Template Engine')
            ->comment(
                'Please choose a file extension to use for partial, template and page content files. 
                PHP-specific page files (pre- and post- action code, AJAX declarations, etc.) 
                always have .php extension.',
                'above'
            )
            ->renderAs(frm_dropdown);
            
        $field = $this->add_form_field('resources_dir_path')
            ->tab('Resources directory')
            ->comment(
                'Please specify path to the website resources directory relative to your LSAPP installation root. 
                The directory should exist and be writable for PHP.',
                'above',
                true
            );
    ...
    public function get_templates_dir_path($theme)
    {
        ...
        if (Theme::is_theming_enabled() && $theme) {
            $result .= '/'.$theme->code;
        }
    ...
    {
        $prev_filebased_status =
            isset($this->fetched['enable_filebased_templates']) && $this->fetched['enable_filebased_templates'];

        $prev_extension = $this->fetched['content_file_extension'] ?? null;
        ...
        if ($this->enable_filebased_templates) {
            if ($prev_extension && $prev_extension != $this->content_file_extension) {
                ...
                foreach ($themes as $theme) {
                    Partial::update_content_file_extension(
                        $this->get_templates_dir_path($theme),
                        $prev_extension,
                        $this->content_file_extension
                    );
                    Template::update_content_file_extension(
                        $this->get_templates_dir_path($theme),
                        $prev_extension,
                        $this->content_file_extension
                    );
                    Page::update_content_file_extension(
                        $this->get_templates_dir_path($theme),
                        $prev_extension,
                        $this->content_file_extension
                    );
                ...
                Partial::update_content_file_extension(
                    $this->get_templates_dir_path(null),
                    $prev_extension,
                    $this->content_file_extension
                );
                Template::update_content_file_extension(
    ...
    public function copy_templates_to_files()
    {
        ...
        $file_permissions = File::getPermissions();
        ...
        if (Theme::is_theming_enabled()) {
            $themes = Theme::list_themes();
            foreach ($themes as $theme) {
                $this->create_templates_directory(
                    $templates_directory,
                    $theme->code,
                    'The existing theme templates directory is not writable',
                    'Error creating the theme templates directory',
                    null,
                    false
                );
                $this->create_templates_directory(
        ...
        $partials = Partial::create()->find_all();
        ...
        $templates = Template::create()->find_all();
    ...
    public function create_templates_directory(
        ...
        if ($alias) {
            ...
            if (file_exists($dest_path)) {
                if (!is_writable($dest_path)) {
                    throw new ApplicationException($non_writable_message.': '.$dest_path);
                }
        ...
        if (file_exists($dest_path)) {
            ...
            if (!@mkdir($dest_path)) {
                throw new ApplicationException($mkdir_message.': '.$dest_path);
            }
            ...
            if ($protect) {
                ...
                if (!@file_put_contents($htacces_path, $data)) {
                    throw new ApplicationException('Error creating file: '.$htacces_path);
                }
