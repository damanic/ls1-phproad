...
class ThemeImportModel extends ActiveRecord
{
    ...
    public $has_many = array(
        'file' => array(
            ...
            'conditions'=>"master_object_class='Cms_ThemeImportModel'",
            'order'=>'id',
            'delete'=>true
        )
    );
    ...
    {
        $this->define_multi_relation_column('file', 'file', 'Theme file', '@name')
            ->validation()
            ->required('Please upload LCA file.');

        $this->define_column('theme_id', 'Theme')
            ->validation()
            ->fn('trim')
            ->required('Please select theme to import the file to.');
    }

    public function define_form_fields($context = null)
    {
        $this->add_form_field('file')
            ->renderAs(frm_file_attachments)
            ->renderFilesAs('single_file')
            ->addDocumentLabel('Upload file')
            ->fileDownloadBaseUrl(url('ls_backend/files/get/'))
            ->noAttachmentsLabel('')
            ->comment('Please upload LCA archive.', 'above');

        $this->add_form_field('theme_id')
            ->renderAs(frm_dropdown)
            ->emptyOption('<please select>')
            ->comment(
                'Please select theme to import the archive to. 
                 Pages, partials, layouts and file resources in the selected theme 
                 could be replaced with archive content.',
                'above'
            );
    }
    ...
    public function import($data, $session_key)
    {
        ...
        $export_manager = ExportManager::create();
