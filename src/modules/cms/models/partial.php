...
class Partial extends CmsObject
{
    ...
    public function define_columns($context = null)
    {
        $this->define_column('name', 'Name')
            ...
            ->validation()
            ->fn('trim')
            ->required("Please specify the partial name.")
            ->regexp(
                '/^[a-z_0-9:]*$/i',
                'Partial name can only contain latin characters, numbers, colons and underscores.'
            )->fn('strtolower')
            ->unique(
                'Name "%s" already used by another partial. Please use another name.',
        ...
        if ($settings_manager->enable_filebased_templates) {
            $this->define_column('file_name', 'File Name')
                ...
                ->fn('strtolower')
                ->unique(
                    'File name "%s" already used by another partial. Please use another file name.',
                    array($this, 'configure_unique_validator')
                );
        }
        $this->define_column('description', 'Description')->validation()->fn('trim');
     ...
     * Please note that the method finds partials in context of the currently active
     * {@link https://lsdomainexpired.mjman.net/docs/themes/ theme}.
     * @documentable
     * @param string $name Specifies the partial name.
     * @return object Partial Returns the partial object or NULL if the partial is not found.
     */
    ...
    public function copy_to_file($templates_dir = null)
    {
        ...
        try {
            ...
            throw new ApplicationException('Error saving partial '.$this->name.' to file. '.$ex->getMessage());
        }
    ...
    public function get_file_path($file_name)
    {
        ...
        $settings_manager = SettingsManager::get();
        $themeRelPath = '/partials/'.$file_name.'.'.self::get_content_extension();
        return $settings_manager->get_templates_dir_path($this->get_theme()).$themeRelPath;
    }
    ...
    public static function get_content($name, &$content, $file_name)
    {
        try {
            ...
            if ($settings_manager->enable_filebased_templates) {
                if (!$file_name) {
                    throw new ApplicationException('File is not specified');
                }
            ...
            throw new ApplicationException('Error rendering partial '.$name.'. '.$ex->getMessage());
        }
    }
    ...
    public function get_content_code()
    {
        ...
        if ($settings_manager->enable_filebased_templates) {
            if (!$this->file_name) {
                throw new ApplicationException('Partial file is not specified for the '.$this->name.' partial');
            }
                    
            $path = $this->get_file_path($this->file_name);
                    
            if (!file_exists($path)) {
                throw new ApplicationException('Partial file not found: '.$path);
            }
     ...
     * To add new columns to the partial model, call the
     * {@link Db\ActiveRecord::define_column() define_column()} method of
     * the partial object.
     ...
     * @author LSAPP - MJMAN
     * @param Partial $partial Specifies the partial object to extend.
     ...
     * {@link cms:onExtendPartialModel} event.
     ...
     *     throw new ApplicationException("You cannot delete this partial!");
     * }
     * </pre>
     * @event cms:onDeletePartial
     * @package cms.events
     * @author LSAPP - MJMAN
     * @param object Partial $partial Specifies the partial object to be deleted.
     */
     ...
     * <li><em>file_name</em> - path to the partial file (if
     ...
     * a configured {@link Partial} object. The following example repeats the default functionality,
     ...
     * @author LSAPP - MJMAN
     * @param Backend\Controller $controller The back-end controller object.
     * @return object Partial configured CMS Partial object.
     */
