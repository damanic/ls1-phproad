...
class GlobalContentBlock extends ActiveRecord
{
    ...
    public function define_columns($context = null)
    {
        $this->define_column('name', 'Name')
            ...
            ->validation()
            ->fn('trim')
            ->required("Please specify the block name.");

        $this->define_column('code', 'Code')
            ->validation()
            ->fn('trim')
            ->fn('strtolower')
            ->required("Please specify the block code.")
            ->regexp(
                ',^[/a-z0-9_\.:-]*$,i',
                "Block code can contain only latin characters, numbers and signs _, -, /, :, and ."
            )
            ->unique('Block with code %s already exists.');
    ...
    public function define_form_fields($context = null)
    {
        ...
        if ($type == 'html') {
            ...
            $editor_config = HtmlEditorConfig::get('cms', 'cms_global_content_block');
