...
class MaintenanceParams extends ConfigurationRecord
{
    public $record_code = 'maintenance_configuration';
        
    public static function create()
    {
        $configObj = new MaintenanceParams();
        return $configObj->load();
    }

    protected function build_form()
    {
        $this->add_field('enabled', 'Enable maintenance mode', 'full', db_number)
            ->renderAs(frm_checkbox)
            ->comment("During the maintenance session visitors will see the page which you select below.", "above");

        $this->add_field('maintenance_page', 'Maintenance notification page', 'full', db_text)
    ...
    public static function set_status($enabled)
    {
        ...
        if (!$obj->maintenance_page) {
            throw new ApplicationException('The maintenance mode is not configured');
        }
    ...
    public function get_maintenance_page()
    {
        ...
        $page_info = PageReference::get_page_info($this, 'maintenance_page', $page);
