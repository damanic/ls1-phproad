...
class Template extends CmsObject
{
    ...
    public function define_columns($context = null)
    {
        $this->define_column('name', 'Name')
            ...
            ->validation()
            ->fn('trim')
            ->required("Please specify the template name.");

        $settings_manager = SettingsManager::get();
        if ($settings_manager->enable_filebased_templates) {
            $this->define_column('file_name', 'File Name')
                ->validation()
                ->fn('trim')
                ->required("Please specify the file name.")
                ->regexp(
                    '/^[a-z_0-9-;]*$/i',
                    'File name can only contain latin characters, numbers, dashes, underscores and semi-colons.'
                )
                ->fn('strtolower')
                ->unique(
                    'File name "%s" already used by another template. Please use another file name.',
        ...
        $this->define_column('html_code', 'HTML Code')
            ->invisible()
            ->validation()
            ->required('Please specify the template HTML code.');
    ...
    public function before_delete($id = null)
    {
        ...
        if ($isInUse) {
            throw new ApplicationException("Unable to delete template: there are pages ($isInUse) which use it.");
        }
    ...
    public function get_file_path($file_name)
    {
        ...
        $settings_manager = SettingsManager::get();
        $themePath = $settings_manager->get_templates_dir_path($this->get_theme());
        $themeDir = self::get_templates_dir_name($this->get_theme());
        return $themePath.'/'.$themeDir.'/'.$file_name.'.'.self::get_content_extension();
    }
    ...
    public function create_file_name($add_extension = true)
    {
        ...
        $result = $this->generate_unique_file_name(
            self::db_name_to_file_name($this->name),
            $templates_dir.'/'.self::get_templates_dir_name($this->get_theme()).'/',
    ...
    {
        $file_name = pathinfo($file_name, PATHINFO_FILENAME);
        return Inflector::humanize($file_name);
    }
    ...
    public function get_content()
    {
        try {
            ...
            if ($settings_manager->enable_filebased_templates) {
                if (!$this->file_name) {
                    throw new ApplicationException('Template file is not specified');
                }
                        
                $path = $this->get_file_path($this->file_name);
                        
                if (!file_exists($path)) {
                    throw new ApplicationException('Template file not found: '.$path);
                }
                    
                $content = file_get_contents($path);
            }
                
            $result = Backend::$events->fire_event(
                array(
                    'name' => 'cms:onGetTemplateContent',
                    'type' => 'filter'
                ),
                array(
                    'name' => $this->name,
            ...
            throw new ApplicationException('Error rendering template '.$this->name.'. '.$ex->getMessage());
        }
    }
    ...
    public static function auto_create_from_files()
    {
		...
		$path = $settings_manager->get_templates_dir_path($current_theme);
		$folder = self::get_templates_dir_name($current_theme);
        $dir = $path.'/'.$folder;
    ...
    public function assign_file_name($file_name)
    {
        ...
        if ($in_use) {
            throw new ApplicationException('The file name is already in use.');
        }
     ...
     * To add new columns to the layout model, call the 
	 * {@link Db_ActiveRecord::define_column() define_column()} method of
     * the layout object.
     ...
     * @author LSAPP - MJMAN
     * @param Template $layout Specifies the layout object to extend.
     ...
     * The event handler should accept two parameters - the layout object and the form execution context string.
     * To add new fields to the layout form, call the 
	 * {@link Db_ActiveRecord::add_form_field() add_form_field()} method of
     * the layout object.
     * <pre>
     * public function subscribeEvents()
     * {
	 ...
	 * which have been added with {@link cms:onExtendTemplateForm} event.
     ...
     *     throw new ApplicationException("You cannot delete this layout!");
     * }
     * </pre>
     * @event cms:onDeleteTemplate
     * @package cms.events
     * @author LSAPP - MJMAN
     * @param Template $template Specifies the layout object to be deleted.
     */
     ...
     * <li><em>file_name</em> - path to the layout file (if 
	 ...
	 * <em>content</em> containing the updated content.
     * <pre>
     * public function subscribeEvents()
     * {
     ...
     * a configured {@link Template} object. The following example repeats the default functionality,
     ...
     * @author LSAPP - MJMAN
     * @param Backend\Controller $controller The back-end controller object.
     * @return Template configured CMS Layout object.
     */
