...
use Phpr\Strings;
     ...
     * Page class represents a front-end website theme.
     * @documentable
     * @property string $name Specifies the theme name.
     * @property string $code Specifies the theme code.
     * @property string $description Specifies the theme description.
     * @property string $author_name Specifies the theme author name.
     * @property string $author_website Specifies an URL of the theme author's website.
     * @property boolean $is_default Determines whether the theme is default.
     * @property boolean $is_enabled Determines whether the theme is enabled.
     * @property string $templating_engine Specifies a templating engine the theme uses.
     * Supported options are <em>php</em>, <em>twig</em>.
     * @property string $agent_detection_mode Specifies the user agent detection mode.
     * Supported options are <em>disabled</em>, <em>built-in</em>, <em>custom</em>.
     * @property string $agent_list Specifies a list of user agent for the <em>built-in</em> agent detection.
     * @property string $agent_detection_code
     *  Specifies the user agent detection PHP code for the <em>custom</em> agent detection mode.
...
class Theme extends CmsObject
{
    ...
    private static $defaultTheme = false;
    private static $editTheme = false;
    ...
    public function define_columns($context = null)
    {
        ...
        $this->define_column('code', 'Code')
            ...
            ->fn('trim')
            ->fn('strtolower')
            ->required("Please specify the theme code.")
            ->regexp(
                ',^[a-z0-9_\.-]*$,i',
                "Theme code can contain only latin characters, numbers and signs _, -, /, and ."
            )
        ...
        $this->define_column('templating_engine', 'Templating engine')
            ->validation()
            ->method('validate_templating_engine');

        $this->define_column('agent_detection_mode', 'User agent detection mode')
            ->defaultInvisible()
            ->validation()
            ->method('validate_agent_detection_mode');

        $this->define_column('agent_list', 'User agents')->invisible();
        $this->define_column('agent_detection_code', 'User agent detection code')
            ->invisible()
            ->validation()
            ->fn('trim');
    }
    ...
    public function define_form_fields($context = null)
    {
        ...
        if ($context != 'enable') {
            ...
            $this->add_form_field('agent_detection_mode')
                ->renderAs(frm_dropdown)
                ->tab('User agent/device detection')
                ->comment(
                    'The theme can be automatically activated based on the user agent/device. 
                    Please select the detection mode if you want to use this option.',
                    'above'
                );

            $this->add_form_field('agent_list')
                ->renderAs(frm_checkboxlist)
                ->tab('User agent/device detection')
                ->comment('Select user agents the theme should be active for.', 'above');

            $this->add_form_field('agent_detection_code')
                ->renderAs(frm_code_editor)
                ->size('large')
                ->language('php')
                ->comment(
                    'Please specify PHP expression for detecting the user agent. The code should return Boolean value.',
                    'above'
                )
                ->tab('User agent/device detection');
        }
    ...
    public function validate_code($name, $value)
    {
        if (in_array($value, array('pages', 'partials', 'templates', 'layouts'))) {
            $this->validation->setError(
                'Theme code cannot be "pages", "partials", "templates" and "layouts".',
                $name,
                true
            );
        }
    ...
    public function validate_agent_detection_mode($name, $value)
    {
        if (strlen($value) && $value != self::agent_detection_disabled && $this->is_default) {
            $this->validation->setError(
                'This theme is default. User agent detection is not applicable to a default theme.',
                $name,
                true
            );
        }
    ...
    {
        if ($value == 'php' && !Controller::is_php_allowed()) {
            $this->validation->setError(
                'The application configuration doesn\'t allow PHP in CMS templates.',
                $name,
                true
            );
        }
    ...
    public function before_save($deferred_session_key = null)
    {
        ...
        if (($this->is_new_record() || $old_code != $this->code) && $directory_exists) {
            throw new ApplicationException(
                'Theme directory already exists: '.$theme_path
            );
        }
        ...
        if ($settings_manager->enable_filebased_templates) {
            ...
            if (($this->is_new_record() || $old_code != $this->code) && $directory_exists) {
                throw new ApplicationException('Theme templates directory already exists: '.$theme_templates_path);
            }
        }
                
        if ($this->is_new_record()) {
            self::create_theme_directory($this->code);
        } elseif (strlen($old_code) && $old_code != $this->code) {
            if (!@rename($themes_path.'/'.$old_code, $themes_path.'/'.$this->code)) {
                throw new ApplicationException('Error renaming the theme directory: '.$themes_path.'/'.$old_code);
            }
            ...
            if ($settings_manager->enable_filebased_templates) {
                ...
                if ($old_path != $themes_path.'/'.$old_code) {
                    if (!@rename($old_path, $new_path)) {
                        throw new ApplicationException('Error renaming the theme directory: '.$old_path);
                    }
                }
            }
        }
    ...
    public function after_delete()
    {
        if (strlen($this->code)) {
            ...
            if (file_exists($theme_path)) {
                Directory::deleteRecursive($theme_path);
            }
            ...
            if ($settings_manager->enable_filebased_templates && $this->code) {
                ...
                if (file_exists($theme_templates_path)) {
                    Directory::deleteRecursive($theme_templates_path);
                }
            }
            ...
            DbHelper::query(
                'delete content_blocks from content_blocks, pages 
                     where content_blocks.page_id=pages.id 
    ...
    public function before_delete($id = null)
    {
        if ($this->is_default) {
            throw new ApplicationException(
                sprintf(
                    'Theme "%s" is default. Please select another default theme before deleting this theme.',
                    $this->name
                )
            );
        }
    ...
    public static function create_theme_directory($theme_code = null)
    {
        ...
        if (!file_exists($themes_path) || !is_dir($themes_path)) {
            if (!@mkdir($themes_path, Directory::getPermissions())) {
                throw new ApplicationException('Error creating directory: '.$themes_path);
            }
        }
            
        if ($theme_code) {
            if (!is_writable($themes_path)) {
                throw new ApplicationException('Themes directory is not writable: '.$themes_path);
            }
                
            $theme_path = $themes_path.'/'.$theme_code;
            if (!file_exists($theme_path) || !is_dir($theme_path)) {
                if (!@mkdir($theme_path, Directory::getPermissions())) {
                    throw new ApplicationException('Error creating directory: '.$theme_path);
                }
                
                $resources_path = $theme_path.'/resources';
                if (!@mkdir($resources_path, Directory::getPermissions())) {
                    throw new ApplicationException('Error creating directory: '.$resources_path);
                }
            }
            ...
            if ($settings_manager->enable_filebased_templates) {
                ...
                if (!file_exists($theme_templates_path) || !is_dir($theme_templates_path)) {
                    if (!@mkdir($theme_templates_path, Directory::getPermissions())) {
                        throw new ApplicationException('Error creating directory: '.$theme_templates_path);
                    } else {
                        if (!is_writable($theme_templates_path)) {
                            throw new ApplicationException('Themes directory is not writable: '.$theme_templates_path);
                        }
                    }
                }
                    
                $settings_manager->create_templates_directory(
                    $theme_templates_path,
                    'pages',
                    'The existing pages directory is not writable',
                    'Error creating the pages directory'
                );
                $settings_manager->create_templates_directory(
                    $theme_templates_path,
                    'layouts',
                    'The existing templates directory is not writable',
                    'Error creating the templates directory',
                    'templates'
                );
                $settings_manager->create_templates_directory(
                    $theme_templates_path,
                    'partials',
                    'The existing partials directory is not writable',
                    'Error creating the partials directory'
                );
            }
        }
    }
    ...
    {
        return ModuleParameters::get('cms', 'enable_theming');
    }
    ...
    public static function get_active_theme()
    {
        ...
        if (count($active_theme) > 0) {
            foreach ($active_theme as $theme) {
                if (is_object($theme) && $theme instanceof Theme) {
                    return self::$activeTheme = $active_theme[0];
                }
            }
        }
            
        $themes = DbHelper::objectArray(
            'select id, agent_detection_mode, agent_list, agent_detection_code, name, code 
                  from cms_themes 
                  where is_enabled is not null 
                    and is_enabled=1 order by name'
        );
                ...
                foreach ($theme_agents as $theme_agent_id) {
                    foreach ($known_agents as $agent_id => $agent_info) {
                        if ($agent_id == $theme_agent_id && strpos($agent, $agent_info['signature']) !== false) {
                            return self::$activeTheme = self::create()->where('id=?', $theme->id)->find();
                        }
                    }
                }
            }
            ...
            if (strlen($theme->agent_detection_code)) {
                try {
                    ...
                    throw new SystemException(
                        sprintf(
                            'Error evaluating the user agent detection code for theme "%s (%s)". %s',
                            $theme->name,
                            $theme->code,
                            Strings::finalize($ex->getMessage())
                        )
                    );
                }
            }
        ...
        if (count($themes)) {
            return self::$activeTheme = self::create()->where('id=?', $themes[0]->id)->find();
        }
    ...
    {
        if (self::$defaultTheme === false) {
            self::$defaultTheme = self::create()->where('is_default=1')->find();
        }
    ...
    {
        if (self::$editTheme !== false) {
            return self::$editTheme;
        }
            
        if ($theme_id = UserParameters::get('cms-edit-theme')) {
            $theme = self::get_theme_by_id($theme_id);
            if ($theme) {
                return self::$editTheme = $theme;
            }
        }
        ...
        return self::$editTheme = Theme::create()->order('name')->find();
    }
        
    public static function set_edit_theme($id)
    {
        if (!strlen($id)) {
            throw new ApplicationException('Please select theme.');
        }
        ...
        UserParameters::set('cms-edit-theme', $id);
    }
    ...
    {
        return Theme::create()->order('name')->find_all();
    }
    ...
    {
        if (!$this->is_enabled) {
            throw new ApplicationException(sprintf('Theme "%s" is disabled and cannot be default.', $this->name));
        }

        if (strlen($this->agent_detection_mode) && $this->agent_detection_mode != self::agent_detection_disabled) {
            throw new ApplicationException(
                sprintf(
                    'Theme "%s" depends on the user agent and cannot be default.',
                    $this->name
                )
            );
        }
        ...
        if (!$this->is_default) {
            Backend::$events->fire_event('cms:onSetDefaultTheme', self::get_default_theme(), $this);
        }

        $bind = array('id'=>$this->id);
        DbHelper::query('update cms_themes set is_default=1 where id=:id', $bind);
        DbHelper::query('update cms_themes set is_default=0 where id<>:id', $bind);
    }
        
    public function enable_theme()
    {
        $this->is_enabled = true;
        DbHelper::query('update cms_themes set is_enabled=1 where id=:id', array('id'=>$this->id));
    }
        
    public function disable_theme()
    {
        if ($this->is_default) {
            throw new ApplicationException(sprintf('Theme "%s" is default and cannot be disabled.', $this->name));
        }
            
        $this->is_enabled = false;
        DbHelper::query('update cms_themes set is_enabled=0 where id=:id', array('id'=>$this->id));
    }
     ...
     * @return Theme Returns new theme
     */
    public function duplicate_theme($data)
    {
        ...
        $pages = Page::create()->where('theme_id=?', $this->id)->find_all();
        $page_map = array();
        ...
        $templates = Template::create()->where('theme_id=?', $this->id)->find_all();
        $template_map = array();
        ...
        $partials = Partial::create()->where('theme_id=?', $this->id)->find_all();
        ...
        Directory::copy($old_resources_path, $new_resources_path);
     ...
     * @return Theme Returns the new theme.
     */
    public static function create_safe(
        $code,
        $name,
        $description,
        $author_name,
        ...
        $counter = 1;
        $original_code = $code;
        while (DbHelper::scalar('select count(*) from cms_themes where code=:code', array('code'=>$code))) {
            $counter++;
     ...
     * The handler should return a {@link Theme} object representing a theme which should be used.
     * <pre>
     ...
     * {
     *   return Theme::create()->find_by_code('utility');
     * }
     * </pre>
     * @event cms:onGetActiveTheme
     * @package cms.events
     * @author LSAPP - MJMAN
     * @return Theme Returns the theme object. Returns NULL if the default theme should be used.
     */
     ...
     * @package cms.events
     * @author LSAPP - MJMAN
     * @param Theme $old_theme The old default theme object.
     * @param Theme $new_theme The new default theme object.
     */
    private function event_onSetDefaultTheme($old_theme, $new_theme)
    {
    }
}
