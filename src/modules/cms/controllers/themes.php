...
class Themes extends Controller
{
    ...
    public $list_model_class = 'Cms\Theme';
    ...
    public $form_model_class = 'Cms\Theme';
    ...
    protected function index_onshow_set_default_theme_form()
    {
        try {
            ...
            $themes = Theme::create()->where('(is_default is null or is_default = 0)')->order('name')->find_all();
    ...
    protected function index_onset_default_theme()
    {
        try {
            ...
            if (!$theme_id) {
                throw new ApplicationException("Please select a default theme.");
            }
    ...
    protected function index_ondelete_selected()
    {
        ...
        foreach ($item_ids as $item_id) {
            $item = null;
            try {
                $item = Theme::create()->find($item_id);
                if (!$item) {
                    throw new ApplicationException('Theme with identifier '.$item_id.' not found.');
                }
    ...
    protected function index_onshow_duplicate_theme_form()
    {
        try {
            $ids = post('list_ids', array());
            if (count($ids) != 1) {
                throw new ApplicationException('Please select a theme to duplicate.');
            }

            $existing_theme = $this->viewData['existing_theme'] = Theme::create()->where('id=?', $ids[0])->find();
            if (!$existing_theme) {
                throw new ApplicationException('Theme not found.');
            }
                    
            $theme = $this->viewData['theme'] = Theme::create();
    ...
    protected function index_onduplicate_theme()
    {
        try {
            ...
            if (!$theme_id) {
                throw new ApplicationException("Original theme not found.");
            }
    ...
    protected function index_onexport_theme()
    {
        try {
            ...
            $theme = Theme::create()->find($model->theme_id);
            if (!$theme) {
                throw new ApplicationException("Theme not found.");
            }
    ...
    protected function index_onimport_theme()
    {
        try {
            ...
            $message = '
            Theme has been successfully imported. Pages: new - %s, updated - %s. 
            Partials: new - %s, updated - %s.
            Layouts: new - %s, updated - %s. 
            Global content blocks: new - %s.
            ';
    ...
    {
        try {
            $theme = Theme::create()->enable_theming(post('Cms_Theme', array()));
            $this->viewData['theme'] = $theme;
