...
class Content extends Controller
{
    ...
    public $list_model_class = 'Cms\GlobalContentBlock';
    ...
    public $form_model_class = 'Cms\GlobalContentBlock';
    ...
    {
        $resources_path = SettingsManager::get()->resources_dir_path;
        $this->filebrowser_dirs[$resources_path] = array(
            'path'=>'/'.$resources_path,
            'root_upload'=>true,
            'title'=>'Website resources directory'
        );
        ...
        if ($this->currentUser && $this->currentUser->isAdministrator()) {
            $partialsPath = PATH_APP.'/phproad/modules/db/behaviors/db_listbehavior/partials/';
            $this->list_custom_body_cells = $partialsPath.'/_list_body_cb.htm';
            $this->list_custom_head_cells = $partialsPath.'/_list_head_cb.htm';
        }
    }
    ...
    protected function index_onDeleteSelected()
    {
        ...
        foreach ($item_ids as $item_id) {
            $item = null;
            try {
                $item = GlobalContentBlock::create()->find($item_id);
                if (!$item) {
                    throw new ApplicationException('Block with identifier '.$item_id.' not found.');
                }
                ...
                if (!$item) {
                    ...
                    $msg = 'Error deleting global content block "'.$item->name.'": '.$ex->getMessage();
                }
