...
class Partials extends Controller
{
    ...
    {
        if (!SettingsManager::get()->enable_filebased_templates || !($model instanceof Partial)) {
            return null;
        }
    ...
    protected function index_onDeleteSelected()
    {
        ...
        foreach ($item_ids as $item_id) {
            $item = null;
            try {
                $item = Partial::create()->find($item_id);
                if (!$item) {
                    throw new ApplicationException('Partial with identifier '.$item_id.' not found.');
                }
