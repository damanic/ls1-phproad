...
class Pages extends Controller
{
    ...
    public $list_model_class = 'Cms\Page';
    ...
    public $form_model_class = 'Cms\Page';
    ...
    protected $required_permissions = array(
        'cms:manage_pages',
        'cms:manage_page_content',
        'cms:manage_static_pages',
        'cms:manage_page_order'
    );
    ...
    public function __construct()
    {
        ...
        if ($this->currentUser) {
            ...
            if (Phpr::$router->action == 'edit'
                || Phpr::$router->action == 'create'
                || Phpr::$router->action == 'content') {
        ...
        if ($this->currentUser) {
            ...
            $this->viewData['can_manage_page_order'] = $this->currentUser->get_permission('cms', 'manage_page_order');
        }
    }
    ...
    public function listGetRowClass($model)
    {
        ...
        if (!$this->viewData['can_edit_pages'] && !$model->has_contentblocks) {
            $classes .= ' disabled';
        }
    ...
    {
        return Page::eval_page_statistics();
    }
    ...
    public function index_onCreateStaticPage()
    {
        try {
            $obj = Page::create();
            $obj->url = $obj->find_available_url(Page::default_static_page_url);
            $obj->title = Page::default_static_page_name;
            $obj->protocol = 'any';
            $content = '<? content_block("content", "Page Content") ?>';
    ...
    {
        try {
            MaintenanceParams::set_status(false);
    ...
    {
        try {
            MaintenanceParams::set_status(true);
    ...
    {
        $model = $this->viewData['form_model'] = Page::create()->find($model->id);
    ...
    public function edit_formBeforeRender($model)
    {
        ...
        if (Phpr::$router->action == 'content'
            && !$this->currentUser->get_permission('cms', array('manage_page_content', 'manage_static_pages'))) {
            Phpr::$response->redirect(url('/'));
        }
    ...
    {
        try {
            Page::set_orders(post('item_ids'), post('sort_orders'));
        } catch (\Exception $ex) {
            Phpr::$response->ajaxReportException($ex, true, true);
        }
    }
    ...
    public function create_from_files()
    {
        try {
            ...
            $this->viewData['directories'] = Page::list_orphan_directories();
            $this->viewData['model'] = PageFileImportModel::init();
        } catch (\Exception $ex) {
            $this->handlePageError($ex);
        }
    }
    ...
    {
        try {
            $imported = PageFileImportModel::import(post('Cms_PageFileImportModel', array()));
            if ($imported > 1) {
    ...
    {
        try {
            SettingsManager::get()->copy_templates_to_db();
        } catch (\Exception $ex) {
            Phpr::$response->ajaxReportException($ex, true, true);
        }
    }

    private function getPageObj($id)
    {
    ...
    protected function onCustomEvent($id = null)
    {
        ...
        if (Phpr::$router->action == 'edit' || Phpr::$router->action == 'create') {
            $page = $this->getPageObj($id);
        }

        Backend::$events->fireEvent(post('custom_event_handler'), $this, $page);
    }
}
