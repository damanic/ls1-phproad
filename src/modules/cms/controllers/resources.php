...
use Db\RecordLock;
    ...
    /**
     * CMS resource manager controller.
...
class Resources extends Controller
{
    ...
    public function index()
    {
        ...
        try {
            ...
            if (!file_exists($dir) || is_file($dir)) {
                throw new ApplicationException(sprintf('Directory "%s" does not exist.', $dir));
            }
        } catch (\Exception $ex) {
    ...
    {
        $dir = post('dir');
        $expanded_folders = UserParameters::get('resource-manager-expanded-folders', null, array());
        if (post('status')) {
        ...
        UserParameters::set('resource-manager-expanded-folders', $expanded_folders, null);
    ...
    public function file_upload($ticket, $dir = null)
    {
        ...
        try {
            ...
            if (!Phpr::$security->validateTicket($ticket, true)) {
                throw new ApplicationException('Authorization error.');
            }
            ...
            @chmod($destPath, File::getPermissions());
                
            Backend::$events->fireEvent('cms:onAfterResourceFileUpload', $destPath);
    ...
    {
        try {
            $this->validation->add('file_name', 'File Name')
                ->fn('trim')
                ->required("Please specify the file name")
                ->regexp(
                    '/^[0-9a-z\.\-_]*$/i',
                    "File name contains invalid character. 
                    Allowed characters are Latin letters, numbers and following characters: '.', '-', '_'."
                );
            ...
            if (!file_exists(PATH_APP.post('file_path'))) {
                throw new ApplicationException("File {$file_path} not found.");
            }
            ...
            try {
                ...
                throw  new ApplicationException('Error renaming file: '.$ex->getMessage());
            }
        } catch (\Exception $ex) {
    ...
    protected function edit_onSave($file_path)
    {
        try {
            ...
            if ($lock = RecordLock::lock_exists(md5($file_path_str))) {
                throw new ApplicationException(
                    sprintf(
                        'User %s is editing this file. The edit session started %s. You cannot save changes.',
                        $lock->created_user_name,
                        $lock->get_age_str()
                    )
                );
            }
            ...
            if (post('redirect', 1)) {
                ...
                if (!RecordLock::lock_exists(md5($file_path_str))) {
                    RecordLock::unlock_record(md5($file_path_str));
                }

                Phpr::$response->redirect(url('cms/resources'));
            }
            {
                Phpr::$session->flash['success'] .= ' at '.Date::display(PhprDateTime::now(), '%X');
    ...
    protected function index_onNewFile()
    {
        try {
            ...
            if (!$this->isEditable($newFileName)) {
                throw new ApplicationException(
                    "You may create only editable file types: ".implode(', ', $this->getEditableTypes()).'.'
                );
            }

            $file_path = post('file_path');
            if (!file_exists(PATH_APP.post('file_path'))) {
                throw new ApplicationException("Directory {$file_path} not found.");
            }
            ...
            try {
                ...
                @chmod(PATH_APP.$newFilePath, File::getPermissions());
            } catch (\Exception $ex) {
                throw  new ApplicationException('Error creating file: '.$ex->getMessage());
            }
    ...
    protected function index_onShowMoveForm()
    {
        try {
            ...
            $files = post('file_names', array());
            if (!count($files)) {
                throw new ApplicationException('Please select file(s) to move.');
            }
    ...
    protected function index_onMoveFile()
    {
        try {
            ...
            $theming_enabled = Theme::is_theming_enabled();
            ...
            if ($destDir == $srcDir) {
                throw new ApplicationException(
                    "Destination directory matches the source directory. Please select another directory."
                );
            }
            ...
            foreach ($files as $file) {
                ...
                try {
                    ...
                    throw  new ApplicationException('Error moving file: '.$ex->getMessage());
                }
            }
        } catch (\Exception $ex) {
            Phpr::$response->ajaxReportException($ex, true, true);
    ...
    {
        try {
            $settings = UserParameters::get('cms_folders_view_settings', null, array());
            $settings[post('dir')] = post('view_type');
            UserParameters::set('cms_folders_view_settings', $settings);
    ...
    protected function index_onNewFolder()
    {
        try {
            ...
            $this->validation->add('folder_name', 'Folder Name')
                ->fn('trim')
            ...
            try {
                ...
                @chmod(PATH_APP.$newFolderPath, File::getPermissions());
            } catch (\Exception $ex) {
                throw  new ApplicationException('Error creating folder: '.$ex->getMessage());
            }
            ...
            if ($expand_folder == $resources_root_dir) {
                $expanded_folders[Resources::resources_root_folder_code] = 1;
            } elseif ($theming_enabled && $expand_folder == $theme_resources_root_dir) {
                $expanded_folders[Resources::theme_resources_root_folder_code] = 1;
            } else {
    ...
    protected function index_onShowRenameFolderForm()
    {
        try {
            ...
            if ($dir == '/'.$this->getResourcesRootDir(true)) {
                throw new ApplicationException(
                    'We are sorry, the root resources directory cannot be renamed or deleted.'
                );
            }
            ...
            if (Theme::is_theming_enabled()) {
                if ($dir == '/'.$this->getThemeResourcesRootDir(true)) {
                    throw new ApplicationException(
                        'We are sorry, the root theme resources directory cannot be renamed or deleted.'
                    );
                }
            }
    ...
    protected function index_onShowRenameFoldeForm()
    {
 // deprecated
    ...
    protected function index_onRenameFolder()
    {
        try {
            ...
            if ($dir == '/'.$this->getResourcesRootDir(true)) {
                throw new ApplicationException('We are sorry, root resources directory cannot be renamed or deleted.');
            }
                    
            if (Theme::is_theming_enabled()) {
                if ($dir == '/'.$this->getThemeResourcesRootDir(true)) {
                    throw new ApplicationException(
                        'We are sorry, root theme resources directory cannot be renamed or deleted.'
                    );
                }
            }
            ...
            $newFolderPath = $file_path.'/'.$newFolderName;
            if (file_exists(PATH_APP.$newFolderPath)) {
                throw new ApplicationException("Folder $newFolderPath already exists.");
            }

            try {
                @rename(PATH_APP.post('dir'), PATH_APP.$newFolderPath);
            } catch (\Exception $ex) {
                throw  new ApplicationException('Error renaming folder: '.$ex->getMessage());
            }
    ...
    protected function index_onShowDeleteFoldeForm()
    {
        try {
            ...
            if ($files) {
                throw new ApplicationException(
                    'Directory is not empty. Please delete all files and folders from the directory.'
                );
            }
    ...
    protected function index_onDeleteFolder()
    {
        try {
            ...
            try {
                ...
                throw  new ApplicationException('Error deleting folder: '.$ex->getMessage());
            }
    ...
    {
        $result = SettingsManager::get()->resources_dir_path;
        if (!$relative) {
    ...
    protected function getCurrentFolder()
    {
        ...
        if (file_exists($currentFolder)
            && is_dir($currentFolder)
            && (strpos($currentFolder, $resources_root_dir) === 0 || strpos($currentFolder, $theme_root_dir) === 0)) {
    ...
    protected function getFileThumb($path, $width = 60, $height = 60)
    {
        ...
        if ($isImage) {
            $thumbName = Image::createThumbName($path, $width, $height, 'keep_ratio');
            ...
            $errThumbName = Image::createThumbName($path, $width, $height, 'keep_ratio_err');
            ...
            try {
                Image::makeThumb(
                    PATH_APP.$path,
                    PATH_APP.'/uploaded/thumbnails/'.$thumbName,
                    $width,
                    $height,
                    false,
                    'keep_ratio'
     ...
     * @event cms:onAfterResourceFileUpload
     * @see backend:onAfterFileBrowserUpload
     * @package cms.events
     * @author LSAPP - MJMAN
     * @param string $dest_path Specifies the file path on the server
     */
    private function event_onAfterResourceFileUpload($dest_path)
    {
    }
}
