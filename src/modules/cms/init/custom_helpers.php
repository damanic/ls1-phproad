 ...
 * ...
 * </pre>
 * The default value for the <em>enctype</em> attribute is <em>multipart/form-data</em>
 * The default value for the <em>method</em>
...
function open_form($attributes = array())
{
    ...
    $result = Form::openTag($attributes);
 ...
 * {@link https://lsdomainexpired.mjman.net/docs/lemonstand_front_end_javascript_framework
 * LSAPP front-end JavaScript framework} article
 ...
 * @see https://lsdomainexpired.mjman.net/docs/lemonstand_front_end_javascript_framework
 *  LSAPP front-end JavaScript framework
 * @author LSAPP - MJMAN
 * @package cms.helpers
 */
function flash_message()
{
    if (array_key_exists('system', Phpr::$session->flash->flash)) {
        ...
        if (strpos($system_message, 'flash_partial') !== false
            && !array_key_exists('error', Phpr::$session->flash->flash)) {
            $partial_name = substr($system_message, 14);
            $success_message = Phpr::$session->flash->flash['success'] ?? null;

            Controller::get_instance()->render_partial($partial_name, array('message' => $success_message));
 ...
 * Use it for creating editable areas on pages.
 * Please read {@link https://lsdomainexpired.mjman.net/docs/creating_editable_blocks/ Creating editable blocks}
 ...
 * @param string $name specifies a content block name.
 * Name is used as a title for WYSIWYG editor on the Edit Page Content page.
 * @package cms.helpers
 * @author LSAPP - MJMAN
 * @see content_block()
...
{
    echo Html::content_block($code, $name);
}

/**
 * Inserts plain text content block into a page.
 * Use it for creating editable areas on pages. Please read
 * {@link https://lsdomainexpired.mjman.net/docs/creating_editable_blocks/ Creating editable blocks}
 * article for details about the function usage.
 ...
 * @param string $code specifies a content block code. This parameter used by LSAPP for identifying code blocks.
 * Content block codes can contain only digits, Latin letters and underscore characters.
 * @param string $name specifies a content block name. Name is used as a title
 *  for a text area on the Edit Page Content page.
 * @package cms.helpers
 * @author LSAPP - MJMAN
 * @see content_block()
...
{
    echo h(Html::content_block($code, $name));
}
 ...
 * @param boolean $return_content Indicates that the function should return the
 *  block content instead of outputting it to the browser.
...
{
    return Html::global_content_block($code, $return_content);
}
...
if (!function_exists('site_url')) {
     ...
     *  LSAPP's version of the function with Html helper class: Html::site_url().</span>
     * The function is similar to the {@link root_url()}
     *  function with the default TRUE value of $add_host_name_and_protocol
     * parameter.
    ...
    {
        return Html::site_url($url, $add_host_name_and_protocol);
    }
}
 ...
 * @documentable
 * @param string $src_mode Specifies whether resource files
 *  should be included in the source code mode (not minified and not combined).
 * @return string Returns HTML markup containing links to the resource files.
 * @deprecated Use {@link Controller::js_combine()} and {@link Controller::css_combine()} methods instead.
 * @see https://lsdomainexpired.mjman.net/docs/combining_and_minifying_javascript_and_css_files/
 *  Combining and minifying JavaScript and CSS resources.
...
{
    return Html::include_resources($src_mode);
}
 ...
 * Note that you can use resource combining methods of {@link Controller} class {@link Controller::js_combine()}
 * and {@link Controller::css_combine()} for creating links to CSS and JavaScript files
...
{
    return Html::theme_resource_url($path, $root_url, $add_host_name_and_protocol);
}
...
{
    return Html::resource_url($path, $root_url, $add_host_name_and_protocol);
}
