...
class ActionManager
{
    ...
    public static function execAction($name, $controller)
    {
        $parts = explode(':', $name);
        if (count($parts) != 2) {
            throw new ApplicationException("Invalid action identifier: $name");
        }
                
        $className = ucfirst($parts[0]).'_Actions';
        if (!Phpr::$classLoader->load($className)) {
            throw new ApplicationException("Actions scope class is not found: $className");
        }
    ...
    public static function execAjaxHandler($name, $controller)
    {
        $parts = explode(':', $name);
        if (count($parts) != 2) {
            throw new ApplicationException("Invalid event handler identifier: $name");
        }
        ...
        if (!$isEventHandler) {
            throw new ApplicationException("Specified method is not AJAX event handler: $method");
        }

        $obj = new $className();
        if (!method_exists($obj, $method)) {
            throw new ApplicationException("AJAX handler not found: $name");
        }
    ...
    {
        $jsPopup = "new PopupForm('onShowActionDocument', {ajaxFields: {'module': '$module', 'name': '$1'}});";
        $replacement = '<a href="#" onclick="'.$jsPopup.' return false;">';
        return preg_replace(
            '/<a\s*href="@([a-z0-9_]+)"\s*>/',
            $replacement,
            $str
