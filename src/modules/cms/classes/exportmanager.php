...
{
    /**
     * @var integer Specifies the number of new pages created by the last
     * {@link ExportManager::import() import()} method call.
     * @documentable
     */
    public $import_new_pages = 0;
        
    /**
     * @var integer Specifies the number of pages updated by the last
     * {@link ExportManager::import() import()} method call.
     * @documentable
     */
    ...
    /**
     * @var integer Specifies the number of new partials created by the last
     * {@link ExportManager::import() import()} method call.
     * @documentable
     */
    ...
    /**
     * @var integer Specifies the number of partials updated by the last
     * {@link ExportManager::import() import()} method call.
     * @documentable
     */
    ...
    /**
     * @var integer Specifies the number of new layouts created by the last
     * {@link ExportManager::import() import()} method call.
     * @documentable
     */
    ...
    /**
     * @var integer Specifies the number of layouts updated by the last
     * {@link ExportManager::import() import()} method call.
     * @documentable
     */
    ...
    /**
     * @var integer Specifies the number of files imported by the last
     * {@link ExportManager::import() import()} method call.
     * @documentable
     */
    ...
    /**
     * @var integer Specifies the number of global content blocks imported by the last
     * {@link ExportManager::import() import()} method call.
     * @documentable
     */
     ...
     * @documentable
     * @param array $object_types Specifies CMS object types to export.
     * Supported values:
     * <em>pages</em>,<em>partials</em>,<em>templates</em>,<em>global_content_blocks</em>,<em>resources</em>.
     * @param integer $theme_id Specifies a {@link Theme theme} identifier.
     * @param bool $ignore_payment_paritals Determines whether payment partials should be ignored.
     * @param bool $ignore_theme_description Determines whether the theme description should be ignored.
     * @return string Returns a name of the exported file.
     */
    public function export(
        $object_types,
        $theme_id = null,
        $ignore_payment_paritals = false,
        $ignore_theme_description = false
    ) {
        $tmpDirPath = null;
        $archivePath = null;
        @set_time_limit(3600);
        ...
        try {
            $theme = null;
            if ($theme_id) {
                $theme = Theme::create()->find($theme_id);
                if (!$theme) {
            ...
            if (!is_writable(PATH_APP.'/temp/')) {
                throw new SystemException('Insufficient writing permissions to  '.PATH_APP.'/temp.');
            }

            $tmpDirPath = PATH_APP.'/temp/'.uniqid('lca');
            if (!@mkdir($tmpDirPath)) {
                throw new SystemException('Unable to create directory '.$tmpDirPath);
            }
            ...
            $versions = UpdateManager::getVersions();
            ...
            $key = 'global_content_blocks';
            $export_global_content_blocks = array_key_exists($key, $object_types) && $object_types[$key];

            if ($export_global_content_blocks || $export_pages || $export_partials || $export_templates || $theme) {
                $data_path = $tmpDirPath.'/cms_data.xml';
                $document = new SimpleXMLExtended('<cms_data></cms_data>');
                    
                if ($theme) {
                    $this->export_theme_data($document, $theme, $ignore_theme_description);
                }
                ...
                if ($export_partials) {
                    $this->export_partials_data($document, $theme_id, $ignore_payment_paritals);
                }
            ...
            if ($export_resources) {
                ...
                if (!$theme) {
                    $src_path = PATH_APP.'/'.SettingsManager::get()->resources_dir_path;
                } else {
                    $src_path = PATH_APP.'/'.$theme->get_resources_path();
                }

                Directory::copy($src_path, $tmpDirPath.'/resources', $options);
            }

            /*
             * Create archive
             */
            Zip::zipDirectory($tmpDirPath, $archivePath);
    ...
    protected function export_pages_data($document, $theme_id)
    {
        ...
        foreach ($pages as $page) {
            ...
            for ($reset_index = $index; $reset_index <= Page::max_block_num; $reset_index++) {
                $name_field = 'page_block_name_'.$index;
                $content_field = 'page_block_content_'.$index;
            ...
            $page_element->addChild(
                'enable_page_customer_group_filter',
                $page->enable_page_customer_group_filter ? 1 : 0
            );
    ...
    protected function export_partials_data($document, $theme_id, $ignore_payment_paritals)
    {
        Partial::auto_create_from_files();
            
        $partials_root = $document->addChild('partials');
        $partials = Partial::create();
        ...
        foreach ($partials as $partial) {
            if ($ignore_payment_paritals) {
                if (substr($partial->name, 0, 7) == 'payment' || substr($partial->name, 0, 16) == 'payment_profile:') {
                    continue;
                }
            }
    ...
    protected function export_global_content_blocks($document)
    {
        ...
        $blocks = GlobalContentBlock::create()->find_all();
        ...
        foreach ($blocks as $block) {
            ...
            $block_element->addChild('type')->addCData($block->block_type);
        }
    }
    ...
    protected function export_content_blocks_data($document, $pages)
    {
        ...
        foreach ($pages as $page) {
            ...
            foreach ($content_blocks as $block) {
                ...
                $block_element->addChild('code')->addCData($block->code);
                $block_element->addChild('type')->addCData($block->type);
    ...
    protected function export_theme_data($document, $theme, $ignore_theme_description)
    {
        ...
        $theme_node->addChild('description')->addCData($ignore_theme_description ? '' : $theme->description);
     ...
     * The method throws {@link SystemException} in case of error.
     * @documentable
     * @param mixed $file_info
     *      Specifies either path to the CMS archive file or an array containing information about an
     *      {@link http://www.php.net/manual/en/features.file-upload.post-method.php uploaded file}.
     * @param integer $theme_id
     *      Specifies a {@link Theme Theme } identifier. If the parameter value is -1, a new theme will be created.
        ...
        try {
            ...
            if (is_array($fileInfo)) {
                ...
                if (!move_uploaded_file($fileInfo['tmp_name'], $filePath)) {
                    throw new SystemException('Unable to copy uploaded file to '.$filePath);
                }
            } else {
                $filePath = $fileInfo;
            }
                    
            Module::begin_content_version_update();
            $this->restore_data($filePath, $theme_id);
            Module::end_content_version_update();
    ...
    protected function restore_data($archiveFile, $theme_id)
    {
        ...
        try {
            ...
            Zip::unzip($tmpDirPath, $archiveFile);
            ...
            if (!file_exists($manifestPath)) {
                throw new ApplicationException('Archive file is corrupted - manifest file is not found.');
            }

            $systemVersions = UpdateManager::getVersions();
            ...
            foreach ($modules as $module) {
                ...
                if (!strlen($moduleId)) {
                    throw new ApplicationException('Archive file is corrupted - module identifier is not found.');
                }
                ...
                if (!UpdateManager::moduleVersionExists($moduleId, $version)) {
                    $notFoundModules[] = $moduleId.': '.$version;
                }
            }
            ...
            if ($notFoundModules) {
                throw new ApplicationException(
                    sprintf(
                        'Error restoring data. The following module versions are not present in this system: %s.',
                        implode(', ', $notFoundModules)
                    )
                );
            }
            ...
            if ($theme_id != -2) {
                if ($theme_id > 0) {
                    ...
                    $theme_author = null;
                    $theme_website = null;
                    $agent_detection_mode = Theme::agent_detection_disabled;
                    $agent_list = null;
                    ...
                    if (file_exists($dataPath)) {
                        ...
                        if ($theme_nodes->length) {
                            ...
                            $templating_engine = $this->eval_node_value(
                                $xPath,
                                $theme_node,
                                'templating_engine',
                                'php'
            ...
            if (!$theme) {
                $resources_directory = PATH_APP.'/'.SettingsManager::get()->resources_dir_path;
            } else {
            ...
            if (file_exists($dirPath)) {
                Directory::$copiedFileCount = 0;
                Directory::copy($dirPath, $resources_directory);
                $this->import_files = Directory::$copiedFileCount;
            }
    ...
    protected function import_global_content_blocks($xPath)
    {
        ...
        if ($blocks->length) {
            foreach ($blocks as $xml_block) {
                ...
                if (GlobalContentBlock::get_by_code($code)) {
                    continue;
                }

                $db_block = GlobalContentBlock::create();
                    
                $db_block->name = $name;
                $db_block->code = $code;
                $db_block->block_type = $this->eval_node_value($xPath, $xml_block, 'type', 'html');
        ...
        if ($pages->length) {
            ...
            if ($content_blocks->length) {
                foreach ($content_blocks as $xml_block) {
                    ...
                    $code = $this->eval_node_value($xPath, $xml_block, 'code');
                    $db_block = ContentBlock::get_by_page_and_code($page_id, $code);
                    if (!$db_block) {
                        $db_block = ContentBlock::create();
                        $db_block->code = $code;
                        $db_block->page_id = $page_id;
                    }
                        
                    $db_block->type = $this->eval_node_value($xPath, $xml_block, 'type', 'html');
                    $db_block->content = $this->eval_node_value($xPath, $xml_block, 'content');
                    $db_block->save();
                }
            }
    ...
    /**
     * Triggered before a CMS theme is exported.
     * In the event handler you can throw an exception if the operation should be cancelled.
     * @event cms:onBeforeDataExport
     * @package cms.events
     * @author LSAPP - MJMAN
     */
    private function event_onBeforeDataExport()
    {
    }
     ...
     * In the event handler you can throw an exception if the operation should be cancelled.
     * @event cms:onBeforeDataImport
     * @package cms.events
     * @author LSAPP - MJMAN
     * @param mixed $file_info Specifies either path to the CMS archive file or an array containing information about an
     * {@link http://www.php.net/manual/en/features.file-upload.post-method.php uploaded file}.
             */
    private function event_onBeforeDataImport($file_info)
    {
    }
}
