 ...
 * The instance of the class is created automatically by LSAPP and available in
...
class Controller
{
     ...
     * See the {@link https://lsdomainexpired.mjman.net/docs/actions Actions page} for details.
     * The contents of this field is automatically converted to PHP variables when
     * the page is rendered.
     * @documentable
     * @see https://lsdomainexpired.mjman.net/docs/actions Actions
     */
    public $data = array();

    /**
     * @var Page A reference to the current page.
     ...
     * Use {@link Controller::request_param() request_param()} method to access page parameters by index.
     * @documentable
     */
     ...
     * @return Controller Returns the controller object or NULL.
     * @documentable
     */
    ...
    public function __construct($authorize = true)
    {
        if (Phpr::$config->get('OPTIMIZE_FRONTEND_QUERIES')) {
            ActiveRecord::$execution_context = 'front-end';
        }
     ...
     * @documentable
     * @return Shop\CustomerGroup Returns a customer group object.
     */
    public static function get_customer_group()
    {
        ...
        if ($controller && $controller->customer) {
            ...
            return CustomerGroup::get_guest_group();
        }
    }
    ...
    public function open($page, &$params)
    {
        ...
        try {
            ...
            $gaSettings = Stats_Settings::getLazy();
            ...
            if (is_a($result_onBeforeDisplay, 'Cms\Page')) {
            ...
            if ($template) {
                $this->evalWithException('?>'.$template_content, Controller::cms_type_template, $template->name);
            } else {
            ...
            if (count($handlerNameParts) == 1) {
                ...
                if ($handlerName == 'on_action') {
                    ...
                    if ($php_is_allowed) { // Ignore custom AJAX handlers field if PHP is not allowed
                        try {
                            if (!self::$_ajax_handlers_loaded) {
                                ...
                                $this->evalWithException(
                                    $this->page->get_ajax_handlers_code(),
                                    'Page AJAX handlers',
                                    $this->page->label ? $this->page->label : $this->page->title,
                                    array(),
                                    true
                                );
                            }
                    ...
                    if (!function_exists($handlerName)) {
                        throw new ApplicationException('AJAX handler not found: '.$handlerName);
                    }

                    call_user_func($handlerName, $this, $this->page, $this->request_params);
                }
            } else {
                ActionManager::execAjaxHandler($handlerName, $this);
            }
    ...
    protected function evalWithException(
        $code,
        $call_stack_object_type,
        $call_stack_object_name,
        $params = array(),
        $eval_as_handler = false
    ) {
        $eval_result = null;

        try {
            $call_stack_obj = new CallStackItem($call_stack_object_name, $call_stack_object_type, $code);
            ...
            $eval_code_result = Backend::$events->fire_event(
                array('name' => 'cms:onEvaluateCode', 'type' => 'filter'),
                array(
                'page' => $this->page,
                'controller' => $this,
            ...
            if ($eval_code_result['content'] !== null) {
                ...
                if ($engine_code == 'php') {
                    if (self::is_php_allowed()) {
                        ...
                        throw new ApplicationException('PHP is not allowed in CMS templates.');
                    }
                } elseif ($engine_code == 'twig') {
                    $object_name = $call_stack_object_type.' - '.$call_stack_object_name;
                    echo $this->get_twig_parser()->parse(
                        substr($code, 2),
                        array_merge($this->data, $params, array('this'=>$this)),
                        $object_name
                    );
                } else {
                    throw new ApplicationException('Unknown templating engine: '.$engine_code);
                }
            }
            ...
            if ($error) {
                ...
                if ($pos !== false) {
                    ...
                    throw new ExecutionException($errorMessageText, $this->_cms_call_stack, $errorLine);
                } else {
                    throw new ExecutionException($errorMessage, $this->_cms_call_stack, null);
                }
            }

            array_pop($this->_cms_call_stack);

            $html_output_filter = Backend::$events->fire_event(
                array('name' => 'cms:beforeOutputEvaluatedCode', 'type' => 'filter'),
                array(
                'page' => $this->page,
                'controller' => $this,
                'params' => array_merge($this->data, $params),
            ...
            $forward_exception_classes = array(
                'Cms\ExecutionException',
                'Phpr\ValidationException',
                'Phpr\ApplicationException',
                'Cms\Exception'
            );
            ...
            if ($ex instanceof TwigError) {
                if (!$ex->getPrevious()) {
                    throw new ExecutionException($ex->getMessage(), $this->_cms_call_stack, $ex->getTemplateLine());
                } else {
            ...
            if ($this->_cms_call_stack && strpos($ex->getFile(), "eval()") !== false) {
                throw new ExecutionException($ex->getMessage(), $this->_cms_call_stack, $ex->getLine());
            }
    ...
    {
        $exception_text = $message.Strings::finalize($ex->getMessage());
    ...
    protected function reset_cache_request()
    {
        ...
        $key = 'RESET_PAGE_CACHE_KEY';
        $reset_cache_key = array_key_exists($key, $caching_params) ? $caching_params[$key] : null;
        if (!$reset_cache_key) {
            return false;
        }
    ...
    protected function eval_page_content()
    {
        ...
        if ($php_is_allowed) {
            $this->evalHandler(
                $this->page->get_pre_action_code(!$php_is_allowed),
                'CMS page PRE action code',
                $this->page->label ? $this->page->label : $this->page->title
            );
        }

        /*
         * We always execute the page action code even if
         * the PRE Action code returned -1 (CMS, Validation or Application exception).
        ...
        $api_result = Backend::$events->fire_event(
            array('name' => 'cms:onBeforeEvalPageContent', 'type' => 'filter'),
            array(
            'page'=>$this->page,
            'page_content'=>false
            )
        ...
        if (!$loaded_from_api && function_exists('get_page_caching_params')) {
            ...
            $recache = false;
            $cache_key = CacheBase::create_key($key_prefix, $recache, $vary_by, $versions);
        ...
        if (!$loaded_from_cache && !$loaded_from_api) {
            $action_result = true;
            if (strlen($this->page->action_reference) && $this->page->action_reference != Page::action_custom) {
                try {
                    ActionManager::execAction($this->page->action_reference, $this);
                } catch (ValidationException $ex) {
                    $action_result = false;
                    Phpr::$session->flash['error'] = $ex->getMessage();
                } catch (ApplicationException $ex) {
            ...
            if ($action_result && $php_is_allowed) {
                $this->evalHandler(
                    $this->page->get_post_action_code(!$php_is_allowed),
            ...
            $this->evalWithException(
                '?>'.$this->page->get_content_code(),
                Controller::cms_type_page,
                $this->page->label ? $this->page->label : $this->page->title
            );
     ...
     * Please read {@link https://lsdomainexpired.mjman.net/docs/using_the_head_field/ this article} for details.
     * Example:
     * <pre><? $this->render_head() ?></pre>
     * @documentable
     * @see https://lsdomainexpired.mjman.net/docs/using_the_head_field/ Using the page Head Declarations field
     */
    public function render_head()
    {
        ...
        $this->evalWithException(
            '?>'.$this->page->get_head_code(),
            Controller::cms_type_head,
            $this->page->label ? $this->page->label : $this->page->title
        );
     ...
     * {@link https://lsdomainexpired.mjman.net/docs/using_the_page_blocks_feature/ this article} for details. Example:
     ...
     * @see https://lsdomainexpired.mjman.net/docs/using_the_page_blocks_feature/ Using the page blocks feature
     */
    public function render_block($block_code, $default = null)
    {
        ...
        if (!array_key_exists($block_code, $this->_page_block_content)) {
            ...
            if (array_key_exists($block_code, $blocks)) {
                ob_start();
                $this->evalWithException('?>'.$blocks[$block_code], Controller::cms_type_block, $block_code);
     ...
     * This method is useful for creating the
     * {@link https://lsdomainexpired.mjman.net/docs/customer_login_and_logout customer login} page.
     ...
     * @documentable
     * @see Controller::create_redirect_url() create_redirect_url()
    ...
    /**
     * Creates a redirection URL string suitable for using with
     * {@link Controller::redirect_url() redirect_url()} method.
     ...
     * @documentable
     * @see Controller::redirect_url() redirect_url()
    ...
    public function create_redirect_url($include_query_string = false)
    {
        ...
        if ($include_query_string) {
            $query_string = Phpr::$request->getQueryString();
            if ($query_string) {
    ...
    protected function apply_security($page, $request_params)
    {
        if ($page->protocol != 'any' && $page->protocol != 'none') {
            ...
            if ($page->protocol != $protocol) {
                ...
                Phpr::$response->redirect(
                    Phpr::$request->getRootUrl($page->protocol).
                    root_url($page->url).
                    $param_str.
                    $url_params_str,
                    true
        ...
        $ticketName = Phpr::$request->getField(Phpr::$config->get('TICKET_PARAM_NAME', 'ls_frontend_ticket'));
        if ($ticketName && Phpr::$config->get('SECURITY_AUTO_REDIRECT', true)) {
        ...
        if ($this->customer) {
            CheckoutData::load_from_customer($this->customer);
        }
    }
     ...
     * When LSAPP parses an URL, it considers all URL segments following a page address as parameters.
     ...
     * {@link https://lsdomainexpired.mjman.net/docs/adding_a_template/ layouts} to output a current page content.
     * Example:
     * <pre><? $this->render_page() ?></pre>
     * @see https://lsdomainexpired.mjman.net/docs/adding_a_template/ Adding a layout
     * @documentable
     */
    ...
    /**
     * Outputs a {@link https://lsdomainexpired.mjman.net/docs/creating_a_partial/ partial} with the specified name.
     *
     ...
     * ?></pre>
     * @documentable
     * @see https://lsdomainexpired.mjman.net/docs/creating_a_partial/ Creating a partial
     * @see https://lsdomainexpired.mjman.net/docs/caching_pages_and_partials/ Caching pages and partials
     ...
     * Use this method in custom {@link https://lsdomainexpired.mjman.net/docs/ajax/ AJAX} handlers to force the controller to execute the default page action.
     ...
     * </pre>
     * @documentable
     * @see https://lsdomainexpired.mjman.net/docs/ajax/ AJAX
     */
    public function action()
    {
        ...
        if ($this->page->action_reference != Page::action_custom) {
            ActionManager::execAction($this->page->action_reference, $this);
        }
     ...
     * It allows to wrap any code around a standard LSAPP handler. The following example
     ...
     * {
     *   if (!Shop\Cart::list_active_items())
     *     throw new Exception('Your cart is empty.');
     *
     *   $controller->exec_action_handler('shop:on_checkoutSetBillingInfo');
     *
     *   Shop\CheckoutData::copy_billing_to_shipping();
     *   Shop\CheckoutData::set_payment_method(
     *     Shop\PaymentMethod::find_by_api_code('no_payment')->id
     *   );
     *   Shop\CheckoutData::set_shipping_method(
     *     Shop\ShippingOption::find_by_api_code('free_shipping')->id
     *   );
     *   Shop\CheckoutData::place_order($controller->customer);
     *
    ...
    {
        return ActionManager::execAjaxHandler($handler, $this);
    }

    public static function is_php_allowed()
    {
        return Configuration::is_php_allowed();
    }
    ...
    protected static function get_templating_engine_code($object_type)
    {
        $cms_object_types = array(
            Controller::cms_type_page,
            Controller::cms_type_template,
            Controller::cms_type_partial,
            Controller::cms_type_block,
            Controller::cms_type_head,
        );
        ...
        if (Theme::is_theming_enabled()) {
            ...
            $engine = SettingsManager::get()->default_templating_engine;
        }
    ...
    protected function get_twig_parser()
    {
        ...
        return $this->_twig_parser = new Twig($this);
    }
    ...
    protected function resource_combine($type, $files, $options, $show_tag = true)
    {
        ...
        foreach ($files as $file) {
            ...
            if (substr($file, 0, 1) == '@') {
                ...
                if ($current_theme) {
                    ...
                    $file = '/'.SettingsManager::get()->resources_dir_path.$file;
                }
            }
        ...
        $files_string = ResourceCombine::encode_param($files_array);
     ...
     * Please read
     * {@link https://lsdomainexpired.mjman.net/docs/combining_and_minifying_javascript_and_css_files/ Combining and minifying JavaScript and CSS files}
     ...
     * relative to the LSAPP root directory. Remote resources should be prefixed with http:// or https://
     * protocol specifier. Theme resources should be prefixed with <em>@</em> symbol. Alongside with the
     * file paths, the $files array accepts file aliases, which point to LSAPP built-in front-end JavaScript
     * libraries:
     * <ul>
     *   <li><em>mootools</em> - included MooTools core script</li>
     *   <li><em>ls_core_mootools</em> - includes LSAPP MooTools-specific front-end library/li>
     *   <li><em>jquery</em> - includes jQuery core script</li>
     *   <li><em>jquery_noconflict</em> - includes jQuery noConflict() call</li>
     *   <li><em>ls_core_jquery</em>  - includes LSAPP jQuery-specific front-end library</li>
     * </ul>
     * The $options parameter accepts the following parameters:
     * <ul>
     *   <li><em>reset_cache</em> - reset the file cache. LSAPP caches combined files on the server.
     *       By default it updates the cache as soon as any of the cached files is changed. However it cannot
     *       track updates of remote files. You can use this parameter for forcing LSAPP to update the file cache.</li>
     ...
     *
     * The following code adds jQuery, LSAPP jQuery library and {@link http://fancybox.net/ FancyBox} plugin to the page
     ...
     * @documentable
     * @see https://lsdomainexpired.mjman.net/docs/combining_and_minifying_javascript_and_css_files/ Combining and minifying JavaScript and CSS files
     ...
     * Please read {@link https://lsdomainexpired.mjman.net/docs/combining_and_minifying_javascript_and_css_files/ Combining and minifying JavaScript and CSS files}
     * documentation article for details about the method usage.
     * The $files parameter should contain a list of СЫЫ files to include. The files must be specified
     * relative to the LSAPP root directory. Remote resources should be prefixed with http:// or https://
     * protocol specifier. Theme resources should be prefixed with <em>@</em> symbol. Alongside with the
     * file paths, the $files array accepts file aliases, which point to LSAPP built-in front-end CSS
     * libraries:
     * <ul>
     *   <li><em>ls_styles</em> -  includes LSAPP front-end CSS styles file</li>
     * </ul>
     * The $options parameter accepts the following parameters:
     * <ul>
     *   <li><em>reset_cache</em> - reset the file cache. LSAPP caches combined files on the server.
     ...
     *   <li><em>src_mode</em> - do not minify the combined files (leave the comments, and formatting as is).</li>
     * </ul>
     * LSAPP automatically processes relative URLs in the CSS files, so you can use CSS files from different locations and it will not break images.
     * The $show_tag parameter determines whether <em>link</em> HTML tag should be returned. The default value is TRUE,
     * meaning that the tag is returned. If you want to create the tag manually pass FALSE to the parameter.
     *
     * The following code adds LSAPP front-end CSS file, styles.css file and {@link http://fancybox.net/ FancyBox} CSS file to the page
     * (the example assumes that FancyBox files have been uploaded to the /resources directory).
     * <pre>
     * <head>
     ...
     * The event allows to alter the {@link Controller::get_customer_group_id()} method result,
     ...
     * @package cms.events
     * @author LSAPP - MJMAN
     * @param Controller $controller Specifies the CMS controller object.
     ...
     * @author LSAPP - MJMAN
     * @param array $params An array with the <em>page</em> element representing a {@link Page page} object.
     ...
     * The event handler should return FALSE to stop LSAPP from injecting
     ...
     * The event handler receives the same parameters as {@link Controller::render_partial()} method.
     ...
     * The event handler can return a string which is returned from {@link Controller::js_combine()}
     * and {@link Controller::css_combine()} methods.
     ...
     *   <li><em>files</em> - a list of files passed to {@link Controller::js_combine()} or {@link Controller::css_combine()} method.</li>
     *   <li><em>options</em> - a list of options passed to {@link Controller::js_combine()} or {@link Controller::css_combine()} method.</li>
     *   <li><em>show_tag</em> - value of the $show_tag parameter passed to {@link Controller::js_combine()} or {@link Controller::css_combine()} method.</li>
     * </ul>
     ...
     * @package cms.events
     * @see Controller::js_combine()
     * @see Controller::css_combine()
     * @see https://lsdomainexpired.mjman.net/docs/combining_and_minifying_javascript_and_css_files/ Combining and minifying JavaScript and CSS files
     * @author LSAPP - MJMAN
     ...
     * The event handler should parse the requested URL string and return the page object (Page)
     ...
     * The handler can return null or false to use the default LSAPP routing process.
     * <pre>
     ...
     *   $page = Page::create()->find_by_url('/product');
     *
     *   return array(
     *     'page'=>$page,
     *     'params'=>array('ipad')
     ...
     * By default, when the browser requests a page not found in LSAPP, the CMS page with the url
     ...
     * <ul>
     *   <li><em>page</em> - specifies a page ({@link Page}) the code is being evaluated for.</li>
     *   <li><em>controller</em> - specifies the controller ({@link Controller}) object.</li>
     ...
     * </ul>
     * The handler can return an array with the single element <em>content</em>. In this case LSAPP will use the returned value instead
