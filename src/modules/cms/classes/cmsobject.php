 ...
 */
class CmsObject extends ActiveRecord
{
    /**
    ...
    protected function save_to_file($data, $dest_path)
    {
        ...
        if ($file_exists && !is_writable($dest_path)) {
            throw new ApplicationException('File is not writable: '.$dest_path);
        }
            
        if (strlen($data) || $file_exists) {
            if (@file_put_contents($dest_path, $data) === false) {
                throw new ApplicationException('Error writing file: '.$dest_path);
            }
        } else {
            if (!@touch($dest_path)) {
                throw new ApplicationException('Error creating file: '.$dest_path);
            }
        }
            
        try {
            @chmod($dest_path, File::getPermissions());
        } catch (exception $ex) {
        }
    }
    ...
    protected function validate_file_name($file_name)
    {
        ...
        if (!strlen($file_name)) {
            throw new ApplicationException('Please enter the file name');
        }

        if (!preg_match('/^[a-z_0-9-;]*$/i', $file_name)) {
            throw new ApplicationException(
                'File name can only contain latin characters, numbers, dashes and underscores.'
            );
        }
    ...
    {
        return SettingsManager::get()->content_file_extension;
    }
     ...
     * @return mixed Returns Theme if theming is enabled. Otherwise returns null.
     */
    public function get_theme()
    {
        ...
        return Theme::get_theme_by_id($this->theme_id);
    }
