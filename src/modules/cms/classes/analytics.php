...
class Analytics
{
    ...
    public static function getVisitorsChartData($start, $end)
    {
        if (self::isGoogleAnalyticsEnabled()) {
            $active_days = 7;
            $start = $end->substractInterval(new DateTimeInterval($active_days-1));
    ...
    public static function deleteStalePageviews($numberToKeep = null)
    {
        if ($numberToKeep === null) {
            $numberToKeep = Stats_Settings::get()->keep_pageviews;
        }

        $cnt = DbHelper::scalar("select count(*) from cms_page_visits");
        ...
        DbHelper::query("delete from cms_page_visits order by id limit $offset");
    }
    ...
    public static function logVisit($page, $url)
    {
        ...
        if (!Stats_Settings::get()->enable_builtin_statistics) {
            return;
        }

        $ip = Phpr::$request->getUserIp();
        if (Stats_Settings::ipIsFiltered($ip)) {
            return;
        }
        ...
        $bind['visit_date'] = Date::userDate(PhprDateTime::now())->getDate();
        $bind['url'] = $url;
            
        DbHelper::query(
            "insert into cms_page_visits(url, visit_date, ip, page_id) values (:url, :visit_date, :ip, :page_id)",
            $bind
        );
    }
    ...
    {
        return Stats_Settings::get()->ga_service_enabled;
    }
    ...
    {
        ModuleParameters::set('cms', 'analytics', null);
    }
        
    private static function initGaCache($start, $end)
    {

        $prevCache = $cache = ModuleParameters::get('cms', 'analytics');

        try {
            $startFormatted = $start->format(PhprDateTime::universalDateFormat);
            $endFormatted = $end->format(PhprDateTime::universalDateFormat);
            ...
            Dashboard::evalPrevPeriod($start, $end, $prevStart, $prevEnd);

            $chart_start = $end->substractInterval(new DateTimeInterval(30));
            ...
            $current_data = $ga->downloadReport(
                array('ga:date'),
                array('ga:visits','ga:bounces','ga:newVisits','ga:avgSessionDuration','ga:pageviews','ga:visitors'),
                $start,
                $end
            );
            $previous_data = $ga->downloadReport(
                array('ga:date'),
                array('ga:visits','ga:bounces','ga:newVisits','ga:avgSessionDuration','ga:pageviews','ga:visitors'),
                $prevStart,
                $prevEnd
            ...
            ModuleParameters::set('cms', 'analytics', $cache);
            ModuleParameters::set('cms', 'analytics_error', null);
        } catch (\Exception $ex) {
            if (!$prevCache) {
                $prevCache = array(
            ...
            ModuleParameters::set(
                'cms',
                'analytics_error',
                'Error loading Google Analytics report. Cached data used. '.$ex->getMessage()
            );
    ...
    private static function int_evalVisitorsStatistics($start, $end)
    {
        ...
        $data = DbHelper::object('select
				(select count(distinct ip) from cms_page_visits 
				where visit_date >= :current_start and visit_date <= :current_end) as unique_visitors_current,
				(select count(distinct ip) from cms_page_visits 
				where visit_date >= :prev_start and visit_date <= :prev_end) as unique_visitors_previous,
				(select count(*) from cms_page_visits 
				where visit_date >= :current_start and visit_date <= :current_end) as pageviews_current,
				(select count(*) from cms_page_visits 
				where visit_date >= :prev_start and visit_date <= :prev_end) as pageviews_previous
			', array(
    ...
    private static function int_getVisitorsChartData($start, $end)
    {
        ...
        return DbHelper::objectArray($query, array('start'=>$start, 'end'=>$end));
    }
    ...
    private static function int_evalTopPages($start, $end)
    {
        ...
        return DbHelper::objectArray(
            "select url, count(*) as cnt from cms_page_visits
