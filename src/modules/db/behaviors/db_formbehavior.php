...
class Db_FormBehavior extends Phpr_ControllerBehavior
{
    ...
    public function edit($recordId, $context = null)
    {
        try {
            ...
            $this->_controller->viewData['form_model'] = $this->viewData['form_model'] = $this->_controller->formFindModelObject(
                $recordId
            );
    ...
    public function formFieldGetOptions($dbName, $model, $keyValue = -1)
    {
        ...
        if (strlen($optionsMethod)) {
            if (!method_exists($model, $optionsMethod)) {
                throw new Phpr_SystemException(
                    "Method $optionsMethod is not found in the model class " . get_class($model)
                );
            }
        ...
        if (!$columnDefinition->isReference) {
            throw new Phpr_SystemException(
                "Error loading options for $dbName field. Please define method $methodName in the model."
            );
        }

        $has_primary_key = $has_foreign_key = false;
        $relationType = $model->has_models[$columnDefinition->relationName];
        $options = $model->get_relation_options(
            $relationType,
            $columnDefinition->relationName,
            $has_primary_key,
            $has_foreign_key
        );
        ...
        if ($relationType == 'belongs_to' || $relationType == 'has_and_belongs_to_many') {
            ...
            if ($field_definition->referenceDescriptionField !== null) {
                $object->calculated_columns['_description_calc_column'] = array(
                    'sql' => str_replace(
                        '@',
                        $object->table_name . '.',
                        $field_definition->referenceDescriptionField
                    )
                );
            }
            ...
            foreach ($records as $record) {
                if ($field_definition->referenceDescriptionField === null) {
                    if (!$isTree) {
                        ...
                        $result[$record->get_primary_key_value()] = array(
    ...
    public function formOptionState($dbName, $value, $model)
    {
        ...
        if (strlen($optionStateMethod)) {
            if (!method_exists($model, $optionStateMethod)) {
                throw new Phpr_SystemException(
                    "Method $optionStateMethod is not found in the model class " . get_class($model)
                );
            }
        ...
        if (!$columnDefinition->isReference || $columnDefinition->referenceType != 'has_and_belongs_to_many') {
            throw new Phpr_SystemException(
                "Error evaluating option state for $dbName field. Please define method $methodName in the model."
            );
        }
    ...
    public function formCreateModelObject()
    {
        ...
        if (!strlen($modelClass)) {
            throw new Phpr_SystemException(
                'Form behavior: model class is not specified. Please specify it in the controller class with form_model_class public field.'
            );
        }
    ...
    public function create_onSave()
    {
        try {
            ...
            $redirectUrl = Phpr_Util::any(
                $this->_controller->form_create_save_redirect,
                $this->_controller->form_redirect
            );
        ...
        try {
            ...
            if ($this->_controller->enable_concurrency_locking && ($lock = Db_RecordLock::lock_exists($obj))) {
                throw new Phpr_ApplicationException(
                    sprintf(
                        'User %s is editing this record. The edit session started %s. You cannot save changes.',
            ...
            if (post('redirect', 1)) {
                $redirectUrl = Phpr_Util::any(
                    $this->_controller->form_edit_save_redirect,
                    $this->_controller->form_redirect
                );
    ...
    public function edit_onDelete($recordId = null)
    {
        try {
            ...
            if ($this->_controller->enable_concurrency_locking && ($lock = Db_RecordLock::lock_exists($obj))) {
                throw new Phpr_ApplicationException(
                    sprintf(
                        'User %s is editing this record. The edit session started %s. The record cannot be deleted.',
                        $lock->created_user_name,
                        $lock->get_age_str()
                    )
                );
            ...
            $redirectUrl = Phpr_Util::any(
                $this->_controller->form_delete_redirect,
                $this->_controller->form_edit_save_redirect
            );
    ...
    public function onUpdateFileList($recordId = null)
    {
        ...
        if (!$model_class) {
            $model = strlen($recordId) ? $this->_controller->formFindModelObject(
                $recordId
            ) : $this->_controller->formCreateModelObject();
        } else {
    ...
    public function onSaveFormGridCsv($record_id)
    {
        try {
            ...
            if (!$custom_model_class) {
                $model = Phpr::$router->action == 'create' ? $this->_controller->formCreateModelObject(
                ) : $this->_controller->formFindModelObject($record_id);
            } else {
    ...
    public function onFormGridCsvUploaded($record_id)
    {
        try {
            ...
            try {
                ...
                if (!$custom_model_class) {
                    $data_model = Phpr::$router->action == 'create' ? $this->_controller->formCreateModelObject(
                    ) : $this->_controller->formFindModelObject($record_id);
                } else {
    ...
    public function formRenderFormTab($form_model, $tab_index)
    {
        ...
        $this->formRenderPartial(
            'form_tab',
            array('form_tab_elements' => $form_elements[$keys[$tab_index]], 'tab_index' => $tab_index)
        );
    }
    ...
    public function formGetRecordFinderModel($masterModel, $fieldDefinition)
    {
        ...
        $options = $masterModel->get_relation_options(
            $relationType,
            $columnDefinition->relationName,
            $has_primary_key,
            $has_foreign_key
...
//			$model = $model ? $model : $this->formCreateModelObject();
    ...
    protected function formLoadResources()
    {
        ...
        $this->_controller->addCss(
            '/phproad/modules/db/behaviors/db_formbehavior/resources/css/forms.css?' . module_build('core')
        );

        //DATEPICKER
        $this->_controller->addJavaScript(
            '/phproad/modules/db/behaviors/db_formbehavior/resources/javascript/datepicker.js?' . module_build('core')
        );
        ...
        if (!$this->_controller->form_disable_ace) {
            ...
            $this->_controller->addJavaScript(
                '/phproad/modules/db/behaviors/db_formbehavior/resources/javascript/ace_wrapper.js?' . module_build(
                    'core'
                )
            );
        }
        ...
        //GRID CONTROL WIDGET
        $this->_controller->addJavaScript(
            '/phproad/modules/db/behaviors/db_formbehavior/resources/javascript/grid_control.js?' . module_build('core')
        );
        ...
        //FILEUPLOADER
        $this->_controller->addJavaScript(
            '/phproad/modules/db/behaviors/db_formbehavior/resources/javascript/fileupload/jquery.fileupload.js?' . module_build(
                'core'
            )
        );
        $this->_controller->addJavaScript(
            '/phproad/modules/db/behaviors/db_formbehavior/resources/javascript/fileupload/jquery.iframe-transport.js?' . module_build(
                'core'
            )
        );
    }
    ...
    public function formInitWidget($db_name, $model = null)
    {
        ...
        return $this->_widgets[$db_name] = new $widget_class_name(
            $this->_controller,
            $model,
            $relation_db_name,
            $form_field->renderOptions
        );
    }
