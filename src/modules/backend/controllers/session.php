...
class Session extends Controller
{
    ...
    protected function restore_onSend()
    {
        try {
            ...
            $obj = User::create()->findUserByLogin($login);
            if (!$obj) {
            ...
            if (!Email::sendOne('backend', 'password_reset', $viewData, 'Password restore', $obj)) {
                throw new ApplicationException('Cannot send email message. Please see the error log for  details.');
            }
    ...
    public function restore_finish($hash)
    {
        ...
        try {
            $user = User::create()->find_by_password_restore_hash($hash);
            if (!$user) {
                throw new ApplicationException('We are sorry, the Password Restore link you provided is not valid.');
            }
        } catch (\Exception $ex) {
