...
use Backend;
use Phpr;
use Phpr\Controller as PhprController;
...
class Controller extends PhprController
{
    public $app_tab;
    public $app_page;
    ...
    public function __construct()
    {
        ...
        if (!$isPublicAction && !PhprController::$no_permissions_check) {
            ...
            if ($this->required_permissions) {
                ...
                foreach ($this->required_permissions as $permission) {
                    ...
                    if ($cnt != 2) {
                        throw new SystemException('Invalid permission qualifier: '. $permission);
                    }
    ...
    {
        $hidden_hints = UserParams::get('hidden_hints', null, array());
        $hidden_hints[post('name')] = 1;
        UserParams::set('hidden_hints', $hidden_hints);
    }
        
    protected function onFinishTour()
    {
        $completed_tours = UserParams::get('completed_tours', null, array());
        $completed_tours[post('name')] = 1;
        UserParams::set('completed_tours', $completed_tours);
    }
    ...
    // {
    //  RecordLock::unlock(post('lock_id'));
    // }
     ...
     * public function subscribeEvents()
     * {
     *   Backend::$events->addEvent('backend:onControllerInit', $this, 'on_Controller_init');
     * }
     *
     * public function on_Controller_init($controller)
     * {
     *   // Do something
     * }
     * </pre>
