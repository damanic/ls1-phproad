...
class Html
{
    ...
    {
        $url = Strings::normalizeUri($url);
        $backendUrl = Strings::normalizeUri(Phpr::$config->get('BACKEND_URL', 'backend'));
    ...
    public static function menuButton($caption, $items, $attributes = array())
    {
        ...
        $attrList = PhprHtml::formatAttributes($attributes, array('class'=>'button menu'));
        ...
        foreach ($items as $caption => $attributes) {
            ...
            $attrStr = PhprHtml::formatAttributes($attributes);
    ...
    {
        $attrList = PhprHtml::formatAttributes($attributes, array('class'=>'menu imageLink '.$button_class));
    ...
    public static function button(
        ...
        if (is_array($attributes)) {
            ...
            $aAttrList = PhprHtml::formatAttributes($aAttrs, array('href'=>'#'));
            $divAttrList = PhprHtml::formatAttributes($divAttrs, array('class'=>'button'));
        } else {
            $aAttrList = PhprHtml::formatAttributes($aAttrs, array('href'=>$attributes));
            $divAttrList = PhprHtml::formatAttributes($divAttrs, array('class'=>'button'));
        }
     ...
     */
    public static function ctr_button(
        $caption,
        $button_class,
        $attributes = array(),
        $ajaxHandler = null,
        ...
        if (is_array($attributes)) {
            ...
            $defaults = array(
                'href'=>$attributes,
                'class'=>$button_class.' imageLink img_noBottomPading'
            );
            $aAttrList = PhprHtml::formatAttributes($aAttrs, $defaults);
        }
     ...
     * Usually usage of this helper requires only 2 parameters - a caption and a server handler method name.
     */
    ...
    /*
     * A simplified version of the control panel button helper.
     * Outputs a button triggering AJAX request, with default parameters.
     */
    public static function ctr_ajaxButton(
        $caption,
        $button_class,
        $ajaxHandler,
        $attributes = array(),
    ...
    public static function ajaxPagination($pagination, $nextPageHandler = 'null', $prevPageHandler = 'null', $exactPageHandler = 'null')
    {
        ...
        $result .= '<p><span class="interval">Showing  <strong>'.($pagination->getFirstPageRowIndex()+1).'-'.($pagination->getLastPageRowIndex()+1).'</strong>';
        $result .= ' of </span><strong class="row_count" id="list_row_count_label">'.$pagination->getRowCount().'</strong> records. ';
        ...
        if ($pageNumber < 11) {
            for ($i = 1; $i <= $pageNumber; $i++) {
                if ($i != $curPageIndex+1) {
                    $result .= '<a href="#" onclick="return '.sprintf($exactPageHandler, $i-1).'">'.$i.'</a>';
                } else {
            ...
            for ($i = $startIndex+1; $i <= $endIndex+1; $i++) {
                if ($i != $curPageIndex+1) {
                    $pages_str .= '<a href="#" onclick="return '.sprintf($exactPageHandler, $i-1).'">'.$i.'</a>';
                } else {
                    $pages_str .= '<span class="current">'.$i.'</span>';
                }
            }
            ...
            if ($startIndex > 0) {
                ...
                if ($startIndex > 1) {
                    $start_links .= '<span class="ellipsis">&#8230;</span>';
                }
            ...
            if ($endIndex < $lastPageIndex) {
                if ($lastPageIndex - $endIndex > 1) {
                    $pages_str .= '<span class="ellipsis">&#8230;</span>';
                }
        ...
        if ($curPageIndex) {
            $result .= '<a href="#" onclick="return '.sprintf($nextPageHandler, $curPageIndex-1).'">';
        }
        ...
        if ($curPageIndex < $pageNumber-1) {
            $result .= '<a href="#" onclick="return '.sprintf($prevPageHandler, $curPageIndex+1).'">';
        }
