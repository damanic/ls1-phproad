...
class Reports
{
    ...
    {
        $localeDate = Phpr::$lang->getString('phpr.dates', 'short_date_format');
        $localeWeek = Phpr::$lang->getString('phpr.dates', 'week_abbr');
        $dateFormat = str_replace('%', null, $localeDate);

        $days = Html::loadDatesLangArr('A_weekday_', 7);
        $daysShort = Html::loadDatesLangArr('a_weekday_', 7, 7);
        $months = Html::loadDatesLangArr('n_month_', 12);
        $monthShort = Html::loadDatesLangArr('b_month_', 12);
            
        $intYears = self::formatDatesHash(ReportsData::listReportYears());
        $intMonths = self::formatDatesHash(ReportsData::listReportMonths());
    ...
    {
        $reports = ModuleManager::listReports();
        $reports_list = array();
