...
Request.Phpr = new Class({
	...
	success: function(text)
	{
		...
		var phpr_css_list = phrp_js_list = [];
		...
		if (phpr_css_list.length == 0 && phrp_js_list.length == 0)
			process();
		else 
			load_assets(phrp_js_list, phpr_css_list, process);
...
String.implement({
	phprStripScripts: function(option, script_callback){
		var scripts = '';

		var text = this.replace(/<script[^>]*>([^\b]*?)<\/script>/gi, function(){
			scripts += arguments[1] + '\n';
			
			if (script_callback !== undefined)
...
Asset.css = function(source, onLoad) {
	...
	if (("onload" in link) && !Browser.Engine.webkit) {
...
function load_javascript_in_sequence(sources, callback)
{
	...
	Asset.javascript(source, {
		onload: function(){
			if (sources.length > 0)
				load_javascript_in_sequence(sources, callback);
			else
				callback()
		}
	});
...
function load_assets(javascript, css, callback)
{
	var js_list = javascript.filter(function(item){
		return jQuery('head script[src="'+item+'"]').length == 0;
	});

	var css_list = css.filter(function(item){
		return jQuery('head link[href="'+item+'"]').length == 0;
	});
	...
	load_javascript_in_sequence(js_list, function(){
		js_loaded = true;
...
function phpr_dispatch_widget_response_data(data)
{
	jQuery(window).trigger('phpr_widget_response_data', data);
}
